					...
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li>
								...
								<a href="Inventario-Tienda.php">Informe Inventarios</a>
							</li>
							<li class="dropdown">
													...
													<ul class="dropdown-menu dropdown-info">
														<li>
				<a  href="Inventario-Tienda.php">Ver todas las tiendas</a>
			</li>
														<?php
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
    	...
    	?>
    		<li>
				<a  href="Inventario-Tienda.php?QueryCon=<?php Echo($SelectDown);?>&Mensaje=27"><?php Echo utf8_encode($NomDown); ?></a>
			</li>
    	<?php
 }
}
    	...
    	$Costo_Proyectado_Pref=$row['Costo_Proyectado_Pref'];
					 ...
					 ?>
						...
						<div class="modal-footer">
												<a href="Inventario-Tienda.php" class="btn btn-sm" data-dismiss="modal">
...
if ($TxtConsultaVen=="") {
	$sql ="SELECT DISTINCT(Ref_Completa) FROM t_inventario_ref ORDER BY Ref_Completa Asc";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
	$Lista=$Lista.$row['Ref_Completa'].",";                  
 }
}
}
...
{
	$sql ="SELECT DISTINCT(Ref_Completa) FROM t_inventario_ref  WHERE  Tienda_Id_Tienda='".$TxtConsultaVen."'  order by Ref_Completa  Asc";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
...
<form action="codigos_barra_inventario.php" method="post">
									<table style="font-size: 12px;" id=""  class="table table-responsive table-striped table-bordered table-hover" width="100%">
                                    ...
                                    <th class="success"></th>
												...
												<thead>
													...
													<tr class="warning">
														...
														<th class="tdcustom" >Ref. Antigua</th>
													...
													<tr>
														...
														<th class="tdcustom" >Costo Prod.</th>	
														<th class="tdcustom" >Costo Comercial</th>
														<th class="tdcustom" ><input type="checkbox" onclick="marcar(this);" /> Marcar/Desmarcar Todos</th>															
...
if ($TxtConsultaVen!="") {
			...
			$sql ="SELECT  D.Id_Tienda,Id_Registro_Inv,Ref_Completa,Inv_Ref,Cantidad_Inv,Fecha_Ingreso,B.Ref_Antigua,B.Detalle_Antiguo, C.Nom_Cat_Producto,D.Nom_Tienda,B.PVP_Ref FROM t_inventario_ref as A, t_referencias as B, t_categoria_producto as C, t_tiendas as D WHERE Ref_Completa='".$Cadena[$i]."' and A.Inv_Ref=B.Cod_Referencia and B.Categoria_Id_Categoria_Prod=C.Id_Cat_Producto and Tienda_Id_Tienda='".$TxtConsultaVen."' and A.Tienda_Id_Tienda=D.Id_Tienda";
			//Echo($sql);
			$result = $conexion->query($sql);
			if ($result->num_rows > 0) {
   				 while ($row = $result->fetch_assoc()) { 
...
$sql ="SELECT SUM(Costo_Insumo_Ref*Cant_Solicitada) AS CostoProduccion FROM t_insumos_ref WHERE Referencia_Cod_Referencia='".$Inv_Ref."' ";
		//Echo($sql);
		$result = $conexion->query($sql);
		if ($result->num_rows > 0) {
    		while ($row = $result->fetch_assoc()) {                
			...
			$TotalCosto=$CostoProduccion*$Cantidad_Inv;
...
else
{
		...
		$sql ="SELECT  D.Id_Tienda,Id_Registro_Inv,Ref_Completa,Inv_Ref,Cantidad_Inv,Fecha_Ingreso,B.Ref_Antigua,B.Detalle_Antiguo,C.Nom_Cat_Producto,D.Nom_Tienda,B.PVP_Ref FROM t_inventario_ref as A, t_referencias as B, t_categoria_producto as C, t_tiendas as D WHERE Ref_Completa='".$Cadena[$i]."' and A.Inv_Ref=B.Cod_Referencia and B.Categoria_Id_Categoria_Prod=C.Id_Cat_Producto and A.Tienda_Id_Tienda=D.Id_Tienda";
			//Echo($sql);
			$result = $conexion->query($sql);
			if ($result->num_rows > 0) {
   				 while ($row = $result->fetch_assoc()) { 
...
?>
													...
													<?php Echo($Id_Registro_Inv) ?>
														...
														<td>
															<?php Echo($Ref_Antigua) ?>
														</td>
														<td>
															<?php Echo($Ref_Completa) ?>
														</td>
														<td class="center">
															<?php Echo(InventarioReal($Ref_Completa,$Id_Tienda));?> Und.	
														</td>
														<td>
														...
														<td>
															 <input type="checkbox" name="codigo[]" value="<?php Echo($Id_Registro_Inv) ?>">
														</td>
													</tr>
													<?php 
									...
									<div align="center">
									<input class="" type="submit" name="enviar" value="Imprimir Codigos de Barra" />	
									</div>
											</form>
										</div>
