...
$sql ="SELECT SUM(Cant_Solicitada*Costo_Insumo_Ref) as TotalInsumo FROM t_insumos_ref WHERE Referencia_Cod_Referencia='".$Referencia."'";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
$TotalInsumo=$row['TotalInsumo'];
 }
}
...
// Campo Número de Cliente //**************************************************************************************************************************************
         ...
         $pdf-> Image('https://modasof.com/espejo/Administrator/Images/Logos/logo-blanco.png',20,20,40,20);
...
$pdf->Cell(62,6,'Insumo',1,0,'C',1);
$pdf->Cell(35,6,'Cantidad X Prenda',1,0,'C',1);
$pdf->Cell(24,6,'Costo Insumo',1,0,'C',1); 
...
$sql ="SELECT A.Cod_Insumo, Cant_Solicitada,Nom_Insumo,Unidad_Insumo,Costo_Insumo_Ref FROM t_insumos_ref AS A, t_insumos as B WHERE Referencia_Cod_Referencia='".$Cod_Referencia."' and A.Cod_Insumo=B.Cod_Insumo";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
       ...
       $ValorInsumos=$Costo_Insumo_Ref*$Cant_Solicitada;
        ...
        $pdf->Cell(62,4,utf8_encode($Nom_Insumo),1,0,'L',1);

         $pdf->SetFillColor(255,255,255);
        $pdf->SetTextColor(0,0,0);
        $pdf->SetXY(139,$EjeInicial  );//Ubicación en PDF 
        $pdf->Cell(35,4,utf8_encode($Cant_Solicitada." ".$Unidad_Insumo),1,0,'C',1);

         $pdf->SetFillColor(255,255,255);
        $pdf->SetTextColor(0,0,0);
        $pdf->SetXY(174,$EjeInicial  );//Ubicación en PDF 
        $pdf->Cell(24,4,utf8_encode("$ ".number_format($ValorInsumos)),1,0,'R',1);
 }
}
        ...
        $pdf->SetXY(150,198  );//Ubicación en PDF
        $pdf->Cell(10,3,utf8_decode('TOTAL INSUMOS'),0,1,'R');

        $pdf->SetFont('Arial','B', 8);
        $pdf->SetXY(172,197 );//Ubicación en PDF
        $pdf->Cell(26,4,utf8_encode("$ ".number_format($TotalInsumo)),1,1,'R');
        ...
        $pdf->Cell(260,2, utf8_decode(' Cr. 9 No. 7C-33 (Valledupar) Barrio Novalito Tels: 5838943 - 300-2886406 Email Modasofaute@hotmail.com '), 0, 'L');
		        ...
		        $pdf->Output("Facturas/Ficha-Tecnica2-Modasof.pdf",'F');
		...
		echo "<script language='javascript'>window.open('Facturas/Ficha-Tecnica2-Modasof.pdf','_blank','');</script>";

		echo "<script language='javascript'>window.open('VerInventario-Tienda.php?QueryCon=".$get_tienda."','_parent','');</script>";//para ver el archivo pdf generado
