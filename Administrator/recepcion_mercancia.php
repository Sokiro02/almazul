...
$id_despachos = $_GET['Despacho'];


/* REALIZA LA SUMA DEL INVENTARIO ENVIADO*/
$consulta_inventario ="SELECT  sum(valor_total) as suma,id_tienda,id_ref,img_ref,cod_ref,detalle_ref,nom_tienda,talla_id,cantidad,valor_unidad,valor_total,talla,cliente,fecha_ingreso,status_recibido FROM t_temporal_inventario WHERE id_despacho='".$id_despachos."' and status_recibido='ENVIADO'";
		...
		<div class="breadcrumbs ace-save-state" id="breadcrumbs">
			<ul class="breadcrumb">
				<li>
					...
					<a href="tiendas_recepcion.php">Atras</a>
				</li>
			</ul><!-- /.breadcrumb -->						
		</div>
...
<h5><i class="fa fa-clock-o"></i> Enviado el:  <?php Echo (fechasql($fecha_ingreso)); ?>  </h5>	
			</div>
		</div>
	</div>
</div>
...
<div class="col-sm-4 center">
	<h5>ORDEN DESPACHO: OD<?php Echo utf8_encode($id_despachos) ?></h5>
<?php 
...
<form method="post" action="recepcion_mercancia_guardar.php" id="FormOrden">
							...
							<table style="font-size: 12px;" id="dynamic-table"  class="table table-responsive table-striped table-bordered table-hover" width="100%">
												<thead>
													<tr class="warning">
														...
														<th class="tdcustom" style="width: 5%;">Cant Recibida</th>
													</tr>
												...
												<tbody>
<?php 
	$consulta_inventario ="SELECT  id,id_ref,img_ref,cod_ref,detalle_ref,talla_id,cantidad,valor_unidad,valor_total,talla,cliente FROM t_temporal_inventario WHERE id_despacho='".$id_despachos."' and status_recibido='ENVIADO'";
		...
		while ($row = $resultados->fetch_assoc()) {
			...
			?>
				<tr>
					...
					<td>
						<input name="recibido[]" id="cat" value ="<?php echo $cantidad; ?>" type="text" size="10" maxlength="4" required/> 
					</td>					
				</tr>
			<?php
		}                
...
?>
						...
						<!-- FIN TABLA -->
<div class="center">
<!-- aqui iba el formulario inicio -->
     <div class="form-group">
      <label for="comment"><b>Observaciones:</b></label>
      <textarea class="form-control required" rows="5" id="comentarios" name="comentarios"></textarea>
    </div> 
...
<?php 
		if ($status_recibido=="ENVIADO") {
			?>
			 <div class="form-group">
    	<div>
            <button type="submit" name="submit" class="btn btn-xs btn-success"><i class="fa fa-save"></i> RECIBIR MERCANCIA</button>
    	</div>
    </div>
			...
			?>
			<h1>Lo sentimos, el pedido ya se encuentra en estado RECIBIDO</h1>
        ...
        $(document).ready(function()
        {
             $("#FormOrden").validate({
                 ...
                 messages: {
                     ...
                     "comentarios": { required:"Debes incluir algún comentario de Recibido"},
                 },
