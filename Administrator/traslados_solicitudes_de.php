...
$codigo = $_GET['Traslado'];



include("Lib/seguridad.php");
$Datos="Ingreso a Detalle de traslado de referencias nro: ".$codigo;
$Paginas= $_SERVER['PHP_SELF'];
$seguridad = AgregarLog($IdUser,$Datos,$Paginas);


//DATOS DEL TRASLADO
$sql = "SELECT * FROM t_traslados WHERE id_traslado = '".$codigo."'";
$result = $conexion->query($sql) or die('Error en :'.$sql." ".mysqli_error($conexion));
$fila = $result->fetch_assoc();
    $id = $fila['id_traslado'];
    $fecha = $fila['fecha_solicitud'];
    $id_tienda_desde = $fila['id_tienda_desde'];
    $id_tienda_hasta = $fila['id_tienda_hasta'];
    $status = $fila['status'];
    $iduser1 = $fila['id_user'];
    
    //NOMBRE DE LA TIENDA HASTA
    $sql2 = "SELECT * FROM t_tiendas WHERE Id_Tienda='".$id_tienda_hasta."'";
    $result2 = $conexion->query($sql2);
    $fila2=$result2->fetch_assoc();
    $Nombre_Tienda = $fila2['Nom_Tienda'];
    
    //NOMBRE DE LA TIENDA DESDE
    $sql2 = "SELECT * FROM t_tiendas WHERE Id_Tienda='".$id_tienda_desde."'";
    $result2 = $conexion->query($sql2);
    $fila2=$result2->fetch_assoc();
    $Nombre_Tienda_Desde = $fila2['Nom_Tienda'];
    
    
    //NOMBRE DEL USUARIO
    $sql3 = "SELECT * FROM t_usuarios WHERE Id_Usuario='".$iduser1."'";
    $result3 = $conexion->query($sql3);
    $fila3 = $result3->fetch_assoc();
    $Usuario = $fila3['Nombres']." ".$fila3['Apellidos'];
    $Img_Perfil=$fila3['Img_Perfil'];
...
//FUNCIONES PARA GUARDAR O RECHAZAR LOS TRASLADOS
function Traslados(id,accion) {
    if(accion=='GUARDAR'){
        if ( confirm("Estas seguro realizar el traslado?") ) {
                $.post("GuardarTraslados.php", { Codigo: id, Accion: accion}, function(data){
                    if(data.trim() == "true"){
                        alert("Traslado Aceptado con exito, enviar prenda física e informar a la tienda que debe recibir el traslado por sistema.");
                        location.href = "tiendas_traslado.php"; 
                        //location.reload(true);
                    } else {
                        alert("Error al desaprobar el registro "+data);
                    }
                });          
            return true;
        } else {
        ...
        if ( confirm("Estas seguro de rechazar todo el traslado?") ) {
                $.post("GuardarTraslados.php", { Codigo: id, Accion: accion}, function(data){
                    if(data.trim() == "true"){
                        alert("Traslado rechazado con exito");
                        location.href = "tiendas_traslado.php"; 
                        //location.reload(true);
                    } else {
                        alert("Error al desaprobar el registro "+data);
...
function miFuncionEliminar(id) {
    if ( confirm("Estas seguro de eliminar el registro?") ) {
            $.post("traslados_no_aprobar.php", { Eliminar: id}, function(data){
                //$("#Tallas").html(data);
                if(data.trim() == "true"){
                    //alert(data.trim());
                    //refreshDivs('tabla',1,'traslados_tabla_detalle.php');
                    alert("Referencia desaprobada con exito");
                    location.reload(true);
                } else {
                    alert("Error al desaprobar el registro "+data);
                }
...
function miFuncionAprobar(id) {
    if ( confirm("Estas seguro de aprobar el registro?") ) {
            $.post("traslados_si_aprobar.php", { Eliminar: id}, function(data){
                //$("#Tallas").html(data);
                if(data.trim() == "true"){
                    //alert(data.trim());
                    //refreshDivs('tabla',1,'traslados_tabla_detalle.php');
                    alert("Referencia aprobada con exito");
                    location.reload(true);
                } else {
                    alert("Error al aprobar el registro "+data);
                }
            });          
        return true;
    } else {
...
function refreshDivs(divid,secs,url)
  ...
  $Valide=$_GET['Mensaje'];
	...
	if ($Valide==62) {
        echo "<script>jQuery(function(){swal(\"¡ Datos guardados con exito!\", \"Correctamente\", \"success\");});</script>";
    };
    if ($Valide==63) {
        	...
        	<div class="widget-box">
        		<div class="widget-body">
        			<div class="widget-main">
                        ...
                        <b><h5 id="desde"><?php echo $Nombre_Tienda_Desde;?></h5></b>
                        <h5><b>HASTA LA TIENDA:</b></h5>
                        <b><h5 id="hasta"><?php echo $Nombre_Tienda;?></h5></b>
        			</div>
        		</div>
        	</div>
        </div>  
        <div class="col-sm-4 center"> 
        	<h5>ORDEN TRASLADO: OT<?php echo $id; ?></h5>
			...
			<div class="col-sm-12">
				<div class="widget-box">
					<div class="widget-header">
						<h4 class="widget-title">APRUEBE O DESAPRUEBE LAS REFERENCIAS A TRASLADAR</h4>
    ...
    <table style="font-size: 12px;" id="dynamic-table"  class="table table-responsive table-striped table-bordered table-hover" width="100%">
    	<thead>
    		<tr class="warning">
                ...
                <th class="tdcustom" style="width: 2%;">Aprobado?</th>
        ...
        <tbody>
<?php 
    $sql="SELECT * FROM t_traslados_detalle WHERE id_traslado='".$codigo."'";
    ...
    if ($result->num_rows>0){
        while ($fil = $result->fetch_assoc()) {
            ...
            $buscarreferencia = "SELECT * FROM t_referencias WHERE Cod_Referencia='".$cod."'";
            $resultados = $conexion->query($buscarreferencia) or die ('Error: '.mysqli_error($conexion));
            $r = $resultados->fetch_assoc();
            $detalle_ref= $r['Detalle_Referencia'];
            ...
            <tr>
                ...
                <td><?php echo $detalle_ref;?> </td>
                <td><?php echo $Nom_Talla;?> </td>
                <td><?php echo $cantidad;?> </td>
                <td>
                <?php 
                if ($aprobado=='NO'){ ?>
                    <span class="badge badge-danger"><?php echo $aprobado;?> </span>    
                <?php    
                }else{ ?>
                    <span class="badge badge-success"><?php echo $aprobado;?></span>
                <?php
                }
                ?>
                </td>
                <td><a  onclick="miFuncionEliminar(<?php echo $id_detalle;?>)" id="rechazar" class="btn btn-xs btn-danger" data-rel="tooltip" data-placement="top" data-original-title="Desaprobar"><i class="ace-icon fa fa-trash-o bigger-110"></i></a>
                <a  onclick="miFuncionAprobar(<?php echo $id_detalle;?>)"  id="aprobar" class="btn btn-xs btn-success"><i class="ace-icon fa fa-check-circle  bigger-110"></i>
                </a>
                </td>
            </tr>
            <?php
...
<div class="center">
<button type="submit" onclick="Traslados(<?php echo $id;?>,'GUARDAR')" id="guardar" class="btn btn-xs btn-success"><i class="fa fa-plus-square"></i> GUARDAR TRASLADO</button>
<button type="submit" onclick="Traslados(<?php echo $id;?>,'RECHAZAR')" id="rechazar" class="btn btn-xs btn-danger"><i class="fa fa-plus-square"></i> RECHAZAR TRASLADO</button>
<a href="traslados_solicitudes.php?TxtTienda=<?php echo $id_tienda_desde; ?>"><button type="reset" id="cancelar" class="btn btn-xs btn-warning"><i class="fa fa-plus-square"></i> CANCELAR</button></a>
</div>
