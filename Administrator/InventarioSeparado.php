...
<html lang="en">
	<head>
		...
		<meta charset="utf-8" />
		<title>Inventario Separado</title>
					...
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li>
								...
								<a href="InventarioSeparado.php">Inventario Separado</a>
							</li>
						</ul><!-- /.breadcrumb -->
...
<div class="col-sm-12"><!-- Inicio Panel 12 -->
											...
											<ul class="nav nav-tabs" id="myTab">
												<li class="active">
													<a data-toggle="tab" href="#tabs-1">
														...
														Separados Tienda <?php Echo($MiTienda); ?>
													</a>
												</li>
										...
										<div class="table-header" style="background-color: #000;">
											Lista de Separados <?php Echo($MiTienda); ?>
										</div>
...
if ($MyIdTienda=="") {
	$sql ="SELECT Id_Venta FROM t_plansepare Where Estado_Venta<>'3'  order by Id_Venta  DESC";
//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
...
{
	$sql ="SELECT Id_Venta FROM t_plansepare WHERE Tienda_Id_Tienda='".$MyIdTienda."' and Estado_Venta<>'3' order by Id_Venta  DESC";
//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
                                    ...
                                    <th class="success"></th>
												...
												<thead>
													...
													<tr class="warning">
														...
														<th class="tdcustom" style="width: 10%;">Fecha Separado</th>
														...
														<th class="tdcustom" style="width: 10%;">Separado Nº</th>
...
for($i=0; $i<$min; $i++)
{
$sql ="SELECT A.Vendedor_Id_Usuario,B.Nombres,B.Apellidos,B.Img_perfil,C.Nom_Cliente,C.Ape_Cliente,A.Valor_Prenda,A.Valor_Final,D.Img_Referencia,A.Ref_Vendida,A.Fecha_Solicitud,A.Factura_Id_Factura,A.Cant_Solicitada,E.Fecha_Separe From t_plansepare as A, t_usuarios as B, t_clientes as C, t_referencias as D, t_separados as E WHERE A.Id_Venta='".$Cadena[$i]."' and A.Vendedor_Id_Usuario=B.Id_Usuario and A.Cliente_Id_Cliente=C.Id_Cliente and A.Referencia_Id_Referencia=D.Cod_Referencia and A.Factura_Id_Factura=E.Num_Separe "; 
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
       
        $Fecha_Solicitud=$row['Fecha_Separe'];
        $Nombres=$row['Nombres'];
