					...
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li>
								...
								<a href="Informe-Inventarios.php">Informe Inventarios</a>
							</li>
							<li class="dropdown">
													...
													<ul class="dropdown-menu dropdown-info">
														<li>
				<a  href="Informe-Inventarios.php">Ver todas las tiendas</a>
			</li>
														<?php
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
    	...
    	?>
    		<li>
				<a  href="Informe-Inventarios.php?QueryCon=<?php Echo($SelectDown);?>&Mensaje=27"><?php Echo utf8_encode($NomDown); ?></a>
			</li>
    	<?php
 }
}
...
$DetalleRef=$_GET['DetalleRef'];
if ($DetalleRef!="") {
$sql ="SELECT Img_Referencia,Costo_Proyectado_Pref FROM t_referencias WHERE Cod_Referencia='".$DetalleRef."'";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
    	...
    	$Costo_Proyectado_Pref=$row['Costo_Proyectado_Pref'];
					 ...
					 ?>
						...
						<div class="modal-footer">
												<a href="Informe-Inventarios.php" class="btn btn-sm" data-dismiss="modal">
								...
								<div class="col-xs-12 col-sm-3 center">
									<div class="col-xs-12 col-sm-12">
                                        <div class="white-box text-center"> <img width="200" height="200" src="<?php Echo($Img_Referencia); ?>" class="img-responsive"> 
                                        </div>
															...
															<div class="widget-box transparent">
																<div class="widget-header widget-header-small header-color-blue2">
																	<h4 class="widget-title smaller">
																		...
																		Total Activos Ref. <?php Echo($DetalleRef) ?>
																	</h4>
																</div>
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
    	...
    	$Nom_Tienda=$row['Nom_Tienda'];
											...
											<span class="percent">
												<?php Echo(formatomoneda(ArqueoInventarioRef($DetalleRef,$Id_Tienda)*$Costo_Proyectado_Pref) )?>
												...
												<?php Echo($Nom_Tienda); ?>
												<br>
												<strong class="green"><?php Echo(ArqueoInventarioRef($DetalleRef,$Id_Tienda)) ?> Und.</strong>
...
if ($TxtConsultaVen=="") {
	$sql ="SELECT DISTINCT(Inv_Ref) FROM t_inventario_ref ORDER BY Id_Registro_Inv DESC";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
	$Lista=$Lista.$row['Inv_Ref'].",";                  
 }
}
}
...
{
	$sql ="SELECT DISTINCT(Inv_Ref) FROM t_inventario_ref WHERE Tienda_Id_Tienda='".$TxtConsultaVen."' order by Id_Registro_Inv  DESC";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
...
/////// Validación si se consulta por una tienda en especifico ////////////
if ($TxtConsultaVen!="") {
	$sql ="SELECT SUM(Cantidad_Inv) AS IngresoInicial FROM t_inventario_ref WHERE Inv_Ref='".$Cadena[$i]."' and Tipo_Mov_Inv='Ingreso Inicial' and Tienda_Id_Tienda='".$TxtConsultaVen."'";
	//Echo($sql);
...
{
	$sql ="SELECT SUM(Cantidad_Inv) AS IngresoInicial FROM t_inventario_ref WHERE Inv_Ref='".$Cadena[$i]."' and Tipo_Mov_Inv='Ingreso Inicial'";
	//Echo($sql);
...
if ($TxtConsultaVen!="") {
	...
	// Ingreso de Producción 
			$sql ="SELECT SUM(Cantidad_Inv) AS IngresoProduccion FROM t_inventario_ref WHERE Inv_Ref='".$Cadena[$i]."' and Tipo_Mov_Inv='IngresoProd' and Tienda_Id_Tienda='".$TxtConsultaVen."'";
			//Echo($sql);
			$result = $conexion->query($sql);
			if ($result->num_rows > 0) {
   			 while ($row = $result->fetch_assoc()) {                
        	$IngresoProduccion=$row['IngresoProduccion'];
							}
			}
...
{
			// Ingreso de Producción 
			$sql ="SELECT SUM(Cantidad_Inv) AS IngresoProduccion FROM t_inventario_ref WHERE Inv_Ref='".$Cadena[$i]."' and Tipo_Mov_Inv='IngresoProd'";
			//Echo($sql);
			$result = $conexion->query($sql);
			if ($result->num_rows > 0) {
   			 while ($row = $result->fetch_assoc()) {                
...
if ($TxtConsultaVen!="") {
	...
	$sql ="SELECT SUM(Cantidad_Inv) AS SalidaVentas FROM t_inventario_ref WHERE Inv_Ref='".$Cadena[$i]."' and Tipo_Mov_Inv='Salida Venta' and Tienda_Id_Tienda='".$TxtConsultaVen."'";
		//Echo($sql);
		$result = $conexion->query($sql);
		if ($result->num_rows > 0) {
    		while ($row = $result->fetch_assoc()) {                
        	$SalidaVentas=$row['SalidaVentas'];
				}
			}
}
else
{
		$sql ="SELECT SUM(Cantidad_Inv) AS SalidaVentas FROM t_inventario_ref WHERE Inv_Ref='".$Cadena[$i]."' and Tipo_Mov_Inv='Salida Venta'";
		//Echo($sql);
		$result = $conexion->query($sql);
		if ($result->num_rows > 0) {
    		while ($row = $result->fetch_assoc()) {                
...
if ($TxtConsultaVen!="") {
			...
			$sql ="SELECT  Img_Referencia,Cod_Referencia,Coleccion_Nom_Coleccion, Tipo_Tela, Estado_Ref, Costo_Proyectado_Pref,Nom_Cat_Producto,Detalle_Antiguo,Nom_SubCat_Producto,V_Mano_Obra_Ref FROM t_referencias as A, t_categoria_producto as B,t_subcategoria_producto as C, t_inventario_ref as D WHERE Cod_Referencia='".$Cadena[$i]."' and A.Categoria_Id_Categoria_Prod=B.Id_Cat_Producto and A.SubCategoria_Id_Subcategoria_Prod=C.Id_SubCat_Producto and Tienda_Id_Tienda='".$TxtConsultaVen."' and A.Cod_Referencia=D.Inv_Ref";
			//Echo($sql);
			$result = $conexion->query($sql);
			if ($result->num_rows > 0) {
   				 while ($row = $result->fetch_assoc()) { 
...
$sql ="SELECT SUM(Costo_Insumo_Ref*Cant_Solicitada) AS CostoProduccion FROM Referencia_Cod_Referencia WHERE Referencia_Cod_Rerefencia='".$Cadena[$i]."' ";
		//Echo($sql);
		$result = $conexion->query($sql);
		if ($result->num_rows > 0) {
    		while ($row = $result->fetch_assoc()) {                
        	$CostoProduccion=$row['CostoProduccion']+10000;
				}
			}

			$TotalCosto=$CostoProduccion+$V_Mano_Obra_Ref;

				// Operaciones 
				$SumaIngresos=$IngresoInicial+$IngresoProduccion;
				$SumaEgresos=$SalidaVentas;
				$TotalInventarioRef=$SumaIngresos-$SumaEgresos;
				$TotalActivoRef=$TotalCosto*$TotalInventarioRef;
...
else
{
		...
		$sql ="SELECT  Img_Referencia,Cod_Referencia,Coleccion_Nom_Coleccion, Tipo_Tela, Estado_Ref, Costo_Proyectado_Pref,Nom_Cat_Producto,Detalle_Antiguo,Nom_SubCat_Producto,V_Mano_Obra_Ref FROM t_referencias as A, t_categoria_producto as B,t_subcategoria_producto as C WHERE Cod_Referencia='".$Cadena[$i]."' and A.Categoria_Id_Categoria_Prod=B.Id_Cat_Producto and A.SubCategoria_Id_Subcategoria_Prod=C.Id_SubCat_Producto";
			//Echo($sql);
			$result = $conexion->query($sql);
			if ($result->num_rows > 0) {
   				 while ($row = $result->fetch_assoc()) { 
    			$Img_Referencia=$row['Img_Referencia'];
       			...
       			$Tipo_Tela=$row['Tipo_Tela'];
        		$Cod_Referencia=$row['Cod_Referencia'];
        		$Costo_Proyectado_Pref=$row['Costo_Proyectado_Pref']+10000;
        		$Nom_Cat_Producto=$row['Nom_Cat_Producto'];
        		$Nom_SubCat_Producto=$row['Nom_SubCat_Producto'];
        		$Detalle_Antiguo=$row['Detalle_Antiguo'];
        		$V_Mano_Obra_Ref=$row['V_Mano_Obra_Ref'];
...
$sql ="SELECT SUM(Costo_Insumo_Ref*Cant_Solicitada) AS CostoProduccion FROM t_insumos_ref WHERE Referencia_Cod_Referencia='".$Cadena[$i]."' ";
		//Echo($sql);
		$result = $conexion->query($sql);
		if ($result->num_rows > 0) {
    		while ($row = $result->fetch_assoc()) {                
...
?>
														...
														?>
													<img src="<?php echo $mostrar_img; ?>" width="70px" height="70px">
														</td>
														<td class="center">
														...
														<td class="center">
															<?php Echo($Nom_Cat_Producto);?>		
														</td>
														<td>
															<?php Echo($Nom_SubCat_Producto) ?>
														</td>
														<td class="center">
															<?php Echo($Tipo_Tela);?>		
														</td>
														...
														<td class="tdcustom">
															<?php Echo ($TotalInventarioRef); ?> Und.	
														</td>
														
														
														
														<td>
															<?php Echo utf8_encode("$ ".number_format($TotalCosto));  ?>
														</td>
														
														<td>
															<?php Echo utf8_encode("$ ".number_format($TotalActivoRef));  ?>
														</td>
														
													
														 <td>
			<a data-rel="tooltip" data-placement="top" title="Detalle" href="Informe-Inventarios.php?DetalleRef=<?php Echo($Cod_Referencia) ?>">
															<i class="fa fa-search bigger-110"> Detalle Prenda</i>	
															</a>
														</td>
