...
<html lang="en">
	<head>
		...
		<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0" />
		 <!-- Notificaciones Push -->
		<script src="https://modasof.com/espejo/assets/js/push.min.js" async></script>
		...
		<script src="https://modasof.com/espejo/assets/js/ace-extra.min.js" async></script>
    ...
    <!-- Sweet Alert Script -->
    <script src="https://modasof.com/espejo/assets/js/sweetalert.min.js" async></script>
    <!--/Fin Alertas-->
											...
											<ul class="nav nav-tabs" id="myTab">
												...
												<i class="red ace-icon fa fa-gear bigger-120"></i>
													...
													<a data-toggle="tab" href="#tabs-4">
														<i class="black ace-icon fa fa-gear bigger-120"></i>
														Atributos Insumos
...
$sql ="SELECT Distinct(Cod_Insumo) FROM t_insumos order by Id_Insumo DESC";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
...
for($i=0; $i<$min; $i++)
{
$sql ="SELECT C.Nom_Color,Id_Insumo, Cod_Insumo, Nom_Insumo, Unidad_Insumo, Url_Insumo, Detalle_Insumo,Nom_CategoriaIns,Tipo_Insumo FROM t_insumos as A, t_categorias_insumos as B,t_colores as C WHERE Cod_Insumo='".$CadenaInsumos[$i]."' and A.Categoria_Id_Categoria_Insumo=B.Id_Categoria_Insumo and A.Color_Ppal=C.Id_Color"; 
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
        ...
        if ($Tb_Tipo_Insumo==1) {
         ...
         if ($Tb_Tipo_Insumo==2) {
        	$Nom_Tipo_Insumo="PRODUCTO TERMINADO";
													...
													<img src="<?php echo $mostrar_img; ?>" width="40px" height="40px">
														</td>
														<!-- <td class="tdcustom">
										...
										<?php  
										$Valledupar=(round(DisponibilidadBodega(5,$Tb_Cod_Insumo),1)." ".$Tb_Unidad_Insumo);
										//Echo($Valledupar);
										?>
										...
										<?php  
										$Barranquilla=(round(DisponibilidadBodega(6,$Tb_Cod_Insumo),1)." ".$Tb_Unidad_Insumo);
										//Echo($Barranquilla);
										?>
...
<?php 
$sql ="SELECT Id_Mov_Insumos FROM t_mov_insumos order by Id_Mov_Insumos DESC";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
											...
											<table style="font-size: 12px;" id="dynamic-table2"  class="table table-responsive table-striped table-bordered table-hover" width="100%">
												<thead>
													...
													<tr class="warning">
														...
														<th class="tdcustom" style="width: 5%;">Cod Insumo.</th>
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
        ...
        $Tb2_OrdenCompra=$row['OrdenCompra'];
         $Tb2_SolicitudPedido=$row['Solicitud_Id_Solicitud'];
        ...
        $Insumo_Cod_Insumo=$row['Insumo_Cod_Insumo'];
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
        ...
        $Tb2_Unidad_Insumo=$row['Unidad_Insumo'];
								...
								<?php Echo utf8_encode($Insumo_Cod_Insumo) ?>
														</td>
															<td class="">
							...
							<a href="Proveedor-DetalleCompra.php?NumeroOrden=<?php Echo utf8_encode($Tb2_OrdenCompra) ?>" class="tooltip-primary blue" data-rel="tooltip" data-placement="top" title="Ver Orden de Compra">
								<?php
								if ($Tb2_OrdenCompra!=0) {
									Echo utf8_encode($Tb2_OrdenCompra);
								}
								 ...
								 ?>
														...
														<td class="tdcustom">
															<?php 
															if ($Tb2_SolicitudPedido!=0) {
																Echo utf8_encode($Tb2_SolicitudPedido);
															}
...
<!-- FIN MODAL Subrubro-->
					...
					<table id="simple-table1" class="table  table-bordered table-hover" width="100%">
											<thead>
												<tr>
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
       ...
       $Inicial = substr($Tb_NomRubro, 0, 1);
       $TercerLetra = substr($Tb_NomRubro, 2, 1);
...
<form id="FormRubro" method="post" action="Insumo-ActualizarNomCategoria.php">
												<tr>
													...
													<td class="center">
														<?php Echo utf8_encode($Inicial.$TercerLetra) ?>
													</td>
													<td>
...
<?php 
 ...
 ?>
					...
					<a  data-toggle="modal" data-target="#myModalAtributo" href="#"><span class="btn btn-info pull-right"><i class="fa fa-plus-square"> Crear Atributo </i></span></a>
					

<!-- Modal Atributo-->
<div class="modal fade" id="myModalAtributo" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Crear Nuevo Atributo</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
         <form class="form-horizontal" action="Insumo-CrearAtributo.php" id="FormNuevoSubRubro" method="post" enctype="multipart/form-data">
...
?>
         	...
         	<div class="form-group">
				<label class="col-sm-3 control-label no-padding-right" for="form-field-1"> Nuevo Atributo</label>
					<div class="col-sm-9">
						<input required="true" type="text" id="form-field-1" name="txtNombreSubRubro" class="col-xs-10 col-sm-5" value="" />
					</div>
			</div>
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
$ListaCategorias=$ListaCategorias.$row['Id_Categoria_Insumo'].",";                  
 }
}
?>
<?php 
$CadenaLista2=explode(",", $ListaCategorias);
//Split al Arreglo
$longitud2 = count($CadenaLista2);
$min2=$longitud2-1;
//Recorro todos los elementos
for($i=0; $i<$min2; $i++)
      {
			$sql ="SELECT Id_Categoria_Insumo, Cod_Categoria_Insumo, Nom_CategoriaIns,CategoriaIns_Publicada FROM t_categorias_insumos WHERE Id_Categoria_Insumo='".$CadenaLista2[$i]."'";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
        $Tb_IdRubro=$row['Id_Categoria_Insumo'];
...
<form id="FormRubro" method="post" action="Insumo-ActualizarNomCategoria.php">
												<tr>
													...
													<td>
														<?php Echo utf8_encode($Tb_NomRubro) ?>
													</td>
	...
	<form method="post" action="Insumo-ActualizarAtributo.php">
													<?php 
$sql ="SELECT Id_Atributo_Categoria, Categoria_Id_Categoria_Insumo, Nom_Atributo_Categoria, Atributo_Categoria_Publicada FROM t_atributos_categoria_ins WHERE Categoria_Id_Categoria_Insumo='".$CadenaLista2[$i]."'";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
        $Tb_Id_Atributo_Categoria=$row['Id_Atributo_Categoria'];
        $Tb_Nom_Atributo_Categoria=$row['Nom_Atributo_Categoria'];
        $Tb_Atributo_Categoria_Publicada=$row['Atributo_Categoria_Publicada'];
						 ...
						 ?>
																<div class="col-xs-12 col-sm-9">
																	...
																	<div class="profile-user-info profile-user-info-striped">
																		<div class="profile-info-row">
																			<div class="profile-info-name"> <?php Echo utf8_encode($Tb_Id_Atributo_Categoria) ?> </div>

																			<div class="profile-info-value">
																			
													<input  required="true" class="input-xxlarge" type="text" name="NomUpdate-<?php Echo utf8_encode($Tb_Id_Atributo_Categoria) ?>" value="<?php Echo utf8_encode($Tb_Nom_Atributo_Categoria); ?>">
...
<!-- Inicio Modal -->
	...
	include("Lib/footer.php")
	 ?>
		...
		<!--[if !IE]> -->
		<script src="https://modasof.com/espejo/assets/js/jquery-2.1.4.min.js" async=""></script>
...
<![endif]-->
		...
		<script src="https://modasof.com/espejo/assets/js/bootstrap-datepicker.min.js" async=""></script>
		<script src="https://modasof.com/espejo/assets/js/bootstrap-timepicker.min.js" async=""></script>
		...
		<script src="https://modasof.com/espejo/assets/js/bootstrap-datetimepicker.min.js" async=""></script>
		<script src="https://modasof.com/espejo/assets/js/bootstrap-colorpicker.min.js" async=""></script>
		<script src="https://modasof.com/espejo/assets/js/jquery.knob.min.js" async=""></script>
		<script src="https://modasof.com/espejo/assets/js/autosize.min.js" async=""></script>
		<script src="https://modasof.com/espejo/assets/js/jquery.inputlimiter.min.js" async=""></script>
		<script src="https://modasof.com/espejo/assets/js/jquery.maskedinput.min.js" async=""></script>
		<script src="https://modasof.com/espejo/assets/js/bootstrap-tag.min.js" async=""></script>
		<!-- ace scripts -->
		<script src="https://modasof.com/espejo/assets/js/ace-elements.min.js" async=""></script>
		<script src="https://modasof.com/espejo/assets/js/ace.min.js" async=""></script>

		<script src="https://modasof.com/espejo/assets/js/echarts.min.js" async=""></script>
		...
		<script src="https://modasof.com/espejo/assets/js/buttons.flash.min.js" async=""></script>
		<script src="https://modasof.com/espejo/assets/js/buttons.html5.min.js" async=""></script>
		<script src="https://modasof.com/espejo/assets/js/buttons.print.min.js" async=""></script>
		<script src="https://modasof.com/espejo/assets/js/buttons.colVis.min.js" async=""></script>
		<script src="https://modasof.com/espejo/assets/js/dataTables.select.min.js"></script>
		<script src="https://modasof.com/espejo/assets/js/pdfmake.min.js" async=""></script>
		<script src="https://modasof.com/espejo/assets/js/vfs_fonts.js" async=""></script>
		<script src="https://modasof.com/espejo/assets/js/jszip.min.js" async=""></script>
		...
		<script src="https://modasof.com/espejo/assets/js/jquery.magnific-popup.min.js" async=""></script>
