...
<html lang="en">
	<head>
		...
		<meta charset="utf-8" />
		<title>Informe Facturas Tienda Modasof</title>
					...
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li>
								...
								<a href="Informe-VentasTienda.php">Informe Facturación</a>
							</li>
...
$NumeroFactura=$_GET['NumeroFactura'];
				...
				$sql ="SELECT Fecha_Factura,Factura_Paga FROM t_facturas Where Num_Factura='".$NumeroFactura."'";  
      	...
      	$Pagode="Crédito";
					...
					<form action="Clientes-EditarFactura.php" method="post" id="FormUpdateFactura">
						<input style="display: none;" type="text" name="TxtFactura" value="<?php Echo($NumeroFactura)?>">
									<div class="modal-dialog">
										<div class="modal-content">
											<div class="modal-header">
												...
												<h4 class="blue bigger">Cambiar Fecha en Facura  Nº <?php Echo($NumeroFactura);?></h4>
											</div>
							...
							<label for="form-field-select-3">Seleccione la nueva fecha de la Factura :</label>
  ...
  <div class="input-group">
                                ...
                                <select name="TxtTipoFactura" required="true" data-placeholder="Seleccionar...">
                                	...
                                	<option value="2">Crédito</option>
												...
												<a href="Informe-VentasTienda.php" class="btn btn-sm" data-dismiss="modal">
...
<div class="widget-body">
		<h4 class="widget-title lighter">
			...
			<div class="row">
				<form action="Informe-VentasTienda.php?QueryCon=<?php Echo($TxtConsultaVen) ?>" method="post" id="FormFechas" autocomplete="off">
					<div class="col-xs-12 col-sm-6">
...
if ($TxtConsultaVen=='') {
	$sql ="SELECT DISTINCT(Num_Factura) FROM t_facturas WHERE Fecha_Factura >='".$FechaStart."' and Fecha_Factura <='".$FechaEnd."' and estado_sc='0' order by Num_Factura  DESC";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
	$Lista=$Lista.$row['Num_Factura'].",";                 
 }
}
}
else
{
	$sql ="SELECT DISTINCT(Num_Factura) FROM t_facturas WHERE Fecha_Factura >='".$FechaStart."' and Fecha_Factura <='".$FechaEnd."' and  Tienda_Id_Tienda='".$TxtConsultaVen."' and estado_sc='0' order by Num_Factura  DESC";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
                                    ...
                                    <th class="success"></th>
												...
												<thead>
													...
													<tr class="warning">
														...
														<th class="tdcustom" >Total Factura</th>
...
$sql ="SELECT IFNULL(sum(Valor_Ingreso),0) as TotalAbonos FROM t_ingresos WHERE Factura_Cod_Factura='".$Cadena[$i]."' and Tienda_Id_Tienda='".$TiendaOk."'";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
...
$sql ="SELECT cliente_id_cliente FROM t_facturas WHERE Num_Factura='".$Cadena[$i]."'";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
...
?>
												...
												<ul class="dropdown-menu dropdown-info dropdown-menu-right">
													<li>
		<a  data-rel="tooltip" data-placement="top" title="Imprimir Factura" href="Pdf_Factura.php?FacturaImp=<?php Echo($Cadena[$i]); ?>&DeTienda=<?php echo($TiendaOk) ?>"><i class="fa fa-file-pdf-o bigger-100 blue"></i> Imprimir Factura</a>
													</li>
													<li>
														<a target="_blank"  data-rel="tooltip" data-placement="top" title="Abonar-Cancelar" href="Clientes-Abonos.php?AbonosFactura=<?php Echo($Cadena[$i]) ?>&&Detienda=<?php echo($TiendaOk) ?>"><i class="fa fa-dollar bigger-100 blue"></i> Ver/Crear Abonos</a>
													</li>

													<li>
														<a target="_blank"  data-rel="tooltip" data-placement="top" title="Crear Devolución" href="Clientes-Devoluciones.php?NumeroFactura=<?php Echo($Cadena[$i]) ?>"><i class="fa fa-exchange bigger-100 blue"></i> Cambio por Talla</a>
													</li>
													<li>
														<a target="_blank"  data-rel="tooltip" data-placement="top" title="Detalle Factura" href="Clientes-DetalleFactura.php?NumeroFactura=<?php Echo($Cadena[$i]) ?>&DeTienda=<?php echo($TiendaOk) ?>"><i class="fa fa-eye bigger-100 blue"></i> Detalle Factura</a>
													</li>
													...
													if ($IdRol==1) {
														...
														<li class="divider"></li>
													<li>
														<a style="position: relative;" data-rel="tooltip" data-placement="top" title="Anular Factura"  href="Clientes-AnularFactura.php?NumeroFactura=<?php Echo($Cadena[$i]) ?>"><i class="fa fa-close red "></i> Anular Factura</a>
													</li>
													<li>
											<a style="position: relative;" data-rel="tooltip" data-placement="top" title="Editar Fecha"  href="Informe-VentasTienda.php?NumeroFactura=<?php Echo($Cadena[$i]) ?>&EditTask=1"><i class="fa fa-calendar red "></i> Editar Fecha</a>
													</li>
														<?php
													}
													 ?>
