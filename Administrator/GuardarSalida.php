...
if ($Consecutivo_Salidas != $FactNum) {
...
$sql=("INSERT INTO t_remisiones (Fecha_Remision, Num_Remision, Cliente_Id_Cliente, Subtotal_Remision, Iva_Remision, Total_Remision, Estado_Remision, Marca_Temporal, Usuario_Vendedor, Tienda_Id_Tienda, Observa_Remision) VALUES 
  ('".utf8_decode($TiempoActual)."',
  '".utf8_decode($Consecutivo_Salidas)."','".utf8_decode($ClienteSel)."','".utf8_decode($ValorSubtotal)."','".utf8_decode($ValorIva)."','".utf8_decode($ValorTotal)."','".utf8_decode($EstadoRemision)."','".utf8_decode($TiempoActual)."','".utf8_decode($IdUser)."','".utf8_decode($VentaTienda)."','".$Txtobservaciones."')");
//echo($sql);
$result = $conexion->query($sql);



$sql=("UPDATE t_config_tienda SET Consecutivo_Salidas='".$FactNum."' WHERE Tienda_Id_Tienda='".$MyIdTienda."';");
//echo($sql);
$result = $conexion->query($sql);
...
$VentaConfirmada=2;
$Bodega="Bodega";
$SalidaVenta=utf8_decode("Salida Remision");
...
foreach ($_POST['TxtCadIdVenta'] as $IdVenta => $Iv) {
  ...
  $sql=("UPDATE t_salidas_remisiones SET Estado_Venta='".$VentaConfirmada."',Cliente_Id_Cliente='".$ClienteSel."',Factura_Id_Factura='".$FactNum."' WHERE Id_Venta='".$VentaId."';");
//echo($sql);
$result = $conexion->query($sql);
}
...
echo("<script type='text/javascript' language='Javascript'>window.open('https://modasof.com/espejo/Administrator/CrearRemisionTienda.php');</script>");

echo("<script type='text/javascript' language='Javascript'>window.open('Pdf_Remision.php?FacturaImp=".$FactNum."&mitienda=".$idtienda."&cliente=".$ClienteSel."');</script>");



header("location:CrearRemisionTienda.php?Mensaje=333&Factura=".$FactNum."");
