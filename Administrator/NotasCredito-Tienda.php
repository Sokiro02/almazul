...
<html lang="en">
	<head>
		...
		<meta charset="utf-8" />
		<title>Notas Crédito Modasof</title>
					...
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li>
								...
								<a href="NotasCredito-Tienda.php">Informe Notas Crédito</a>
							</li>
							<li style="display: none;" class="dropdown">
													...
													<ul class="dropdown-menu dropdown-info">
														<li>
				<a  href="NotasCredito-Tienda.php">Ver todas las tiendas</a>
			</li>
														<?php
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
    	...
    	?>
    		<li>
				<a  href="NotasCredito-Tienda.php?QueryCon=<?php Echo($SelectDown);?>&Mensaje=27"><?php Echo utf8_encode($NomDown); ?></a>
			</li>
    	<?php
 }
}
...
<div class="widget-body">
		<h4 class="widget-title lighter">
			...
			<div class="row">
				<form action="NotasCredito-Tienda.php?QueryCon=<?php Echo($TxtConsultaVen) ?>" method="post" id="FormFechas" autocomplete="off">
...
<div class="col-sm-12"><!-- Inicio Panel 12 -->
										...
										<div class="table-header" style="background-color: #000;">
											...
											<?php 
												...
												{
													Echo("Notas Crédito del ".fechaSql($FechaUno)." al ".fechaSql($FechaDos));
												}
											?> 
										</div>
...
if ($TxtConsultaVen=="") {
	$sql ="SELECT DISTINCT(notacredito_num) FROM t_devoluciones WHERE Fecha_Devolucion >='".$FechaStart."' and Fecha_Devolucion <='".$FechaEnd."'  ORDER BY Id DESC";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
...
{
	$sql ="SELECT DISTINCT(notacredito_num) FROM t_devoluciones WHERE Fecha_Devolucion >='".$FechaStart."' and Fecha_Devolucion <='".$FechaEnd."' and  Tienda_Id_Tienda='".$TxtConsultaVen."' order by notacredito_num  DESC";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
 ...
 ?>
                                    ...
                                    <th class="success"></th>
												...
												<thead>
													...
													<tr class="warning">
														<th class="tdcustom" >NC</th>
														<th class="tdcustom" >Cargada por</th>
														<th class="tdcustom" >Tienda</th>
														<th class="tdcustom" >Fecha</th>
														<th class="tdcustom" >Cliente</th>
														<th class="tdcustom" >S.Contable</th>
														<th class="tdcustom" >Valor</th>
														<th class="tdcustom" >Redimida</th>
														<th class="tdcustom" >Ref.</th>
...
if ($TxtConsultaVen!="") {
	$sql ="SELECT Id,A.Fecha_Devolucion, Num_Factura,Total_Devolucion,Observa_Usuario, B.Nombres,B.Apellidos,C.Nom_Tienda, D.Nom_Cliente,D.Ape_Cliente,A.Ref_Completa,A.notacredito_num FROM t_devoluciones as A, t_usuarios as B, t_tiendas as C, t_clientes as D WHERE A.notacredito_num='".$Cadena[$i]."' and A.Tienda_Id_Tienda='".$TxtConsultaVen."' and A.Cambio_Id_Usuario=B.Id_Usuario and A.Tienda_Id_Tienda=C.Id_Tienda and A.Cliente_Id_Cliente=D.Id_Cliente";
		//Echo($sql);
		$result = $conexion->query($sql);
}
else  
{
	$sql ="SELECT Id,A.Fecha_Devolucion, Num_Factura,Total_Devolucion,Observa_Usuario, B.Nombres,B.Apellidos,C.Nom_Tienda, D.Nom_Cliente,D.Ape_Cliente,A.Ref_Completa,A.notacredito_num FROM t_devoluciones as A, t_usuarios as B, t_tiendas as C, t_clientes as D WHERE A.notacredito_num='".$Cadena[$i]."' and A.Cambio_Id_Usuario=B.Id_Usuario and A.Tienda_Id_Tienda=C.Id_Tienda and A.Cliente_Id_Cliente=D.Id_Cliente";
		//Echo($sql);
		$result = $conexion->query($sql);
}
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {  
        ...
        $Id=$row['Id'];
        $FechaNota=$row['Fecha_Devolucion'];
...
$sql ="SELECT SUM(Total_Devolucion) as TotalDevoluciones FROM t_devoluciones WHERE notacredito_num='".$Cadena[$i]."'";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
...
$sql ="SELECT SUM(Valor_Ingreso) as TotalRedimido FROM t_ingresos WHERE Cliente_Id_Cliente='".$Cadena[$i]."' and Medio_Pago='5' and Num_Transaccion='".$notacredito_num."'";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
...
?>
														...
														<td>
															<?php Echo utf8_encode("$ ".number_format($TotalDevoluciones));  ?>
														</td>
														<td>
															<?php 
	$sql ="SELECT IFNULL(sum(Valor_Ingreso),0) as IngresoNc,Concepto_Ingreso,Fecha_Ingreso,Ingreso_id_Usuario FROM t_ingresos WHERE Num_Transaccion='".$notacredito_num."' and Medio_Pago='5'";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
...
$IngresoNc=$row['IngresoNc']; 	

	if ($IngresoNc==0) {
				// Cambiamos las notas que no están redimidas
			$conceptoredimido="Nota sin redimir";
			$Fecharedimido="";
			$usuarioredimido="";
			echo("<strong class='red'>".$conceptoredimido."<br>".$Fecharedimido."<br>".$usuarioredimido."</strong>");
	}
	else{
			//
		$conceptoredimido=$Concepto_Ingreso;
		$Fecharedimido=$Fecha_Ingreso;
		$usuarioredimido=ObtenernombreUsuario($Ingreso_id_Usuario);
		echo("<strong class='green'>".$conceptoredimido."<br>".$Fecharedimido."<br>".$usuarioredimido."</strong>");
	}
														...
														<td>
															<?php 
															$sql ="SELECT Ref_Completa FROM t_devoluciones WHERE notacredito_num='".$notacredito_num."'";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
	$Ref_Completa=$row['Ref_Completa']; 
	Echo ($Ref_Completa."<br/>");               
 }
}
