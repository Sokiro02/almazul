...
if ($Update!="") {
	$sql="UPDATE t_temporal_traslados SET Observa_Cliente='".$TxtDetalle."' WHERE Id_Temporal_Sol='".$Update."'";
	$result=$conexion->query($sql);
	header("OrdenTrasladoCliente.php?Mensaje=1");
}

$Delete=$_GET['Delete'];

if ($Delete!="") {
	$sql="DELETE FROM t_temporal_traslados  WHERE Id_Temporal_Sol='".$Delete."'";
	$result=$conexion->query($sql);
	header("OrdenTrasladoCliente.php");
}

$DeleteAll=$_GET['DeleteAll'];

if ($DeleteAll=="true") {
	$sql="DELETE FROM t_temporal_traslados  WHERE Solicitud_Id_Usuari='".$IdUser."'";
	$result=$conexion->query($sql);
	header("OrdenTrasladoCliente.php");
}
...
<html lang="en">
	<head>
		...
		<meta charset="utf-8" />
		<title>Orden de Traslado Cliente</title>
...
color:#FFF;
  ...
  $Valide=$_GET['Mensaje'];
    ...
    if ($Valide==1) {
     ...
     if ($Valide==333) {
        echo "<script>jQuery(function(){swal(\"¡ Pedido Cliente Creado!\", \"Correctamente \", \"success\");});</script>";
    };
     if ($Valide==2) {
					...
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li>
								...
								<a href="OrdenTrasladoCliente.php">Orden Traslado Cliente</a>
							</li>
						</ul><!-- /.breadcrumb -->
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
      $Cons_PedidosCl=$row['Cons_PedidosCl']+1;
                    ...
                    <h3 class="m-b-0 text-white">Orden de Traslado Nº PDC<?php Echo($Cons_PedidosCl) ?></h3> 
                            <div class="card-body">
                                <div class="table-responsive">
                          <?php
// Contador de LLamadas 
$sql ="SELECT Sum(Cant_Solicitada*Valor_Final) as SumaPedido FROM t_temporal_traslados WHERE Solicitud_Id_Usuari='".$IdUser."'";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
      $SumaPedido=$row['SumaPedido'];
                                    ...
                                    <?php 
$sql="SELECT C.Nom_Talla,B.Img_Referencia,Id_Temporal_Sol, Cant_Solicitada, Talla_Solicitada, Solicita_Id_Tienda, Referencia_Id_Referencia, Fecha_Solicitud, Solicitud_Id_Usuari, Valor_Prenda,Valor_Final, Observa_Cliente FROM t_temporal_traslados as A, t_referencias as B, t_tallas as C  WHERE A.Referencia_Id_Referencia=B.Cod_Referencia and A.Talla_Solicitada=C.Id_Talla and Solicitud_Id_Usuari='".$IdUser."' order by Id_Temporal_Sol Desc";
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
...
$VrDescuento=$CalculoDescuento*100;
                                     ?>
                                            <tr>
                                                ...
                                                <td style="padding-top: 60px;">
                                                    <form action="OrdenTrasladoCliente.php?Update=<?php Echo($Id_Temporal_Sol); ?>&Mensaje=1" method="post">
                                                ...
                                                <td style="padding-top: 70px;">
                                                	<strong><?php Echo(formatomoneda($Valor_Prenda)) ?></strong>
                                                ...
                                                <td class="center" style="padding-top: 70px;">
                                                    <strong><?php Echo(round($VrDescuento,0)); ?> %</strong>
                                                </td>
                                                ...
                                                <td style="padding-top: 70px;" align="center">
                                                	<a href="OrdenTrasladoCliente.php?Delete=<?php Echo($Id_Temporal_Sol); ?>" data-rel="tooltip" data-placement="top" title="Eliminar Referencia" href="javascript:void(0)" class="text-inverse" title="" data-toggle="tooltip" data-original-title="Delete"><i class="fa fa-trash bigger-150 red"></i></a>
                                                </td>
                                            </tr>
                                       <?php 
                                   }
                               ...
                               else
                               {
                               	...
                               	<tr>
                               		<td colspan="6">
                               			<h2><i class="fa fa-info-circle"> Seleccione un producto de la galería y de clic en el icono <i class="fa fa-cut"></i> <a class="red" href="Lista-Referencias.php"><small class="red">Aquí</small></a></i></h2>
                               		</td>
                               	</tr>
                               	<?php
                               }
                                    ...
                                    <a data-rel="tooltip" data-placement="top" title="Volver a la Galeria" href="Lista-Referencias.php" class="btn btn-dark btn-outline"><i class="fa fa-arrow-left"></i> Agregar  Referencia</a>
                                </div>
                            </div>
                        </div>
                    </div>
...
$sql ="SELECT SUM(Valor_Final*Cant_Solicitada) as TotalPedido FROM t_temporal_traslados WHERE  Solicitud_Id_Usuari='".$IdUser."'"; 
//Echo($sql); 
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
...
$sql ="SELECT SUM(Cant_Solicitada) as SumaPrendas FROM t_temporal_traslados WHERE Solicitud_Id_Usuari='".$IdUser."'";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
      $SumaPrendas=$row['SumaPrendas'];
                                ...
                                <table class="table product-overview">
                                  <tr>
                                    <td>
                                      <a class="btn btn-danger btn-sm" href="OrdenTrasladoCliente.php?DeleteAll=true"><i class="fa fa-times"></i> Cancelar Traslado</a>
                                    </td>
                                  </tr>
                                  <tr>
                     ...
                     <form action="GuardarIngresoTraslado.php" method="post" id="FormPago" autocomplete="off" >
                                <table class="table product-overview">
                                  <tr>
...
symbolPosition : 'left' // use this setting to position the symbol at the left or right side of the value. default 'left'
                                   ...
                                   <td>
                                      ...
                                      <option value="3">Transacción</option>
                                      <option value="4">Efecty/Baloto</option>
                                    </select>
   ...
   $('#TxtMedioPago').change(function(){
    ...
    if((valorCambiado == '2')||(valorCambiado == '3')||(valorCambiado == '4')){
     ...
     else if(valorCambiado == '1')
     {
          $('#Confirmacion').css('display','none');
                           ...
                           <table class="table product-overview">
                                  ...
                                  <?php 
                                  $sql ="SELECT Id_Ingreso_Tempora, Tipo_Pago_Temporal, Valor_Pago_Temporal, Voucher, Usuario_Ingreso FROM ingreso_temporal_usuario WHERE Usuario_Ingreso='".$IdUser."' and Pago_De='Traslado'";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
      ...
      if ($Tipo_Pago_Temporal==1) {
        $PagoRealizado="Efectivo";
      }
      elseif ($Tipo_Pago_Temporal==2) {
        $PagoRealizado="Tarjeta C.";
      }
      elseif ($Tipo_Pago_Temporal==3) {
        $PagoRealizado="Transacción";
      }
      elseif ($Tipo_Pago_Temporal==4) {
        $PagoRealizado="Efecty/Baloto";
      }

                                   ?>
                                 <tr >
                                    <td><a class="red" href="OrdenTrasladoCliente.php?PagoTemporal=<?php Echo($Id_Ingreso_Tempora) ?>">Eliminar</a></td>
                                    <td>
                                  ...
                                  <?php 
                                   ...
                                   ?>
                                    ...
                                    <?php 
                $sql ="SELECT SUM(Valor_Pago_Temporal) as SumaPagos FROM ingreso_temporal_usuario WHERE Usuario_Ingreso='".$IdUser."' and Pago_De='Traslado'";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
...
$PendientePago=$TotalPedido-$SumaPagos;
                 ?>
...
<form method="post" action="Traslado-Guardar.php" id="FormGuardarventa" autocomplete="off">
...
// Consecutivos Número Traslado
$sql ="SELECT Cons_Traslado FROM t_config WHERE Desarrollador='TEKSYSTEM S.A.S'";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
      $Cons_Traslado=$row['Cons_Traslado']+1;
 }
}
...
<input style="display: none;" type="text" name="TxtConsecutivoTraslado" value="<?php Echo($Cons_Traslado);?>">
...
$sql ="SELECT Id_Ingreso_Tempora,Valor_Pago_Temporal,Tipo_Pago_Temporal,Voucher FROM ingreso_temporal_usuario WHERE Usuario_Ingreso='".$IdUser."' and Pago_De='Traslado'";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
      $Id_Ingreso_Tempora=$row['Id_Ingreso_Tempora'];
...
<hr>
       ...
       <div class="col-xs-12 col-sm-7" style="margin-bottom: 10px;">
                                ...
                                <div class="input-group">
                                 ...
                                 <button type="submit" class="btn btn-danger btn-outline"><i class="fa fa-print"> </i> Finalizar e Imprimir Abono</button>
                                 </form>
