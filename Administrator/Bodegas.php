...
<html lang="en">
  ...
  $Valide=$_GET['Mensaje'];
    ...
    if ($Valide==1) {
        echo "<script>jQuery(function(){swal(\"¡ Bodega Creada!\", \"Correctamente \", \"success\");});</script>";
    };
     if ($Valide==2) {
        echo "<script>jQuery(function(){swal(\"¡ Bodega Actualizada!\", \"Correctamente \", \"success\");});</script>";
    };
    if ($Valide==22) {
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
        ...
        $Qr_Usuario_Encargado=$row['Usuario_Encargado'];
        $Qr_Usuario_Nombres=$row['Nombres'];
        $Qr_Usuario_Apellidos=$row['Apellidos'];
        ...
        $Qr_Nom_Ciudad=$row['Nom_Ciudad'];
					...
					<form action="Bodega-ActualizarBodega.php" method="post" id="FormUpdateBodega">
									<div class="modal-dialog">
										...
										<div class="col-xs-12 col-sm-6 ">
												...
												<input type="text" name="UpTxtNombre" class="input-large" placeholder="Nombre Bodega" style="text-transform:uppercase;" value="<?php Echo utf8_encode($Qr_Nom_Bodega) ?>">
							<input style="display: none;" type="text" name="TxtEditProv" value="<?php Echo utf8_encode($EditTask) ?>">
														...
														<div class="form-group">
															<label for="form-field-select-3">Cód Taller</label>

															<div class="input-group">
																<span class="input-group-addon">
																	<i class="ace-icon fa fa-bank"></i>
																</span>
												<input type="text" name="UpTxtCod" class="input-large" placeholder="Código Bodega" style="text-transform:uppercase;" value="<?php Echo utf8_encode($Qr_Cod_Bodega) ?>">
															</div>
														
														</div>
														
									<div class="form-group">
												<label for="form-field-select-3">Usuario Responsable</label>
												<div>
												<select class="chosen-select input-xxlarge" name="UpTxtUsuario" data-placeholder="Seleccionar...">
												<option selected="true" value="<?php Echo utf8_encode($Qr_Usuario_Encargado) ?>"><?php Echo utf8_encode($Qr_Usuario_Nombres." ".$Qr_Usuario_Apellidos) ?></option>
																<?php
$sql ="SELECT Id_Usuario, Nombres, Apellidos FROM t_usuarios WHERE Rol_Id_Rol=2 order by Nombres ASC";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
    	$SelectId_Usuario=$row['Id_Usuario'];
    	$SelectNom_Usuario=$row['Nombres']; 
    	$SelectApe_Usuario=$row['Apellidos'];             
    	echo ("<option value='".$SelectId_Usuario."'>".utf8_encode($SelectNom_Usuario." ".$SelectApe_Usuario)."</option>");
 }
}
...
?>
							...
							<input type="text" name="UpTxtDir" class="input-large" placeholder="Dirección de Taller" style="text-transform:uppercase;" value="<?php Echo utf8_encode($Qr_Dir_Bodega) ?>">
															</div>
								...
								<input name="UpTxtCelular" class="form-control input-mask-phone" type="text" id="form-field-mask-2" value="<?php Echo utf8_encode($Qr_Cel_Bodega) ?>" />
															</div>
								...
								<input name="UpTxtTel" class="form-control input-mask-phone" type="text" id="form-field-mask-2" value="<?php Echo utf8_encode($Qr_Tel_Bodega) ?>" />
															</div>
							...
							<input type="email" name="UpTxtCorreo" class="input-large" placeholder="E-mail Bodega" style="text-transform:uppercase;" value="<?php Echo utf8_encode($Qr_Correo_Bodega) ?>">
															</div>
												...
												<a href="Bodegas.php" class="btn btn-sm" data-dismiss="modal">
					...
					<?php 
					 ...
					 ?>
						...
						<div class="col-sm-12 col-xs-12">
							<a  data-toggle="modal" data-target="#modal-form" href="#"><span class="btn btn-danger pull-right"><i class="fa fa-plus-square"> </i> Nuevo Taller</span></a>
						<!-- Inicio Modal -->
								<div id="modal-form" class="modal" tabindex="-1">
							<form action="Bodega-CrearBodega.php" method="post" id="FormNuevaBodega">
									<div class="modal-dialog">
										<div class="modal-content">
											<div class="modal-header">
												<button type="button" class="close" data-dismiss="modal">&times;</button>
												<h4 class="blue bigger">Nueva Taller</h4>
											</div>
											...
											<div class="modal-body">
												...
												<input type="text" name="TxtNombre" class="input-large" placeholder="Nombre Taller" style="text-transform:uppercase;">
															</div>
														
														</div>
														
														<div class="form-group">
															<label for="form-field-select-3">Código Taller</label>

															<div class="input-group">
																<span class="input-group-addon">
																	<i class="ace-icon fa fa-bank"></i>
																</span>
												<input type="text" name="TxtCod" class="input-large" placeholder="Código Taller" style="text-transform:uppercase;">
															</div>
												...
												<label for="form-field-select-3">Usuario Responsable</label>
												<div>
												<select class="chosen-select input-xxlarge" name="TxtUsuario" data-placeholder="Seleccionar...">
...
?>
							...
							<input type="text" name="TxtDir" class="input-large" placeholder="Dirección de Taller" style="text-transform:uppercase;">
															</div>
							...
							<input type="email" name="TxtCorreo" class="input-large" placeholder="E-mail Bodega" style="text-transform:uppercase;">
															</div>
											...
											<table style="font-size: 12px;" id="dynamic-table"  class="table table-responsive table-striped table-bordered table-hover">
												<thead>
													...
													<tr class="warning">
														...
														<th class="tdcustom" style="width: 20%;">Taller</th>
														<th class="tdcustom" style="width: 20%;">Encargado</th>
														<th class="tdcustom" style="width: 20%;">Dir</th>
												...
												<tbody>
<?php 
$sql ="SELECT Nom_Ciudad,Id_Bodega, Cod_Bodega, Nom_Bodega, Usuario_Encargado, Dir_Bodega, Tel_Bodega,Cel_Bodega, Correo_Bodega, Ciudad_Id_Ciudad,Nombres,Apellidos FROM t_bodegas as A, t_ciudades as B, t_usuarios as C WHERE A.Ciudad_Id_Ciudad=B.Id_Ciudad and A.Usuario_Encargado=C.Id_Usuario "; 
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
        $Tb_Id_Bodega=$row['Id_Bodega'];
        $Tb_Cod_Bodega=$row['Cod_Bodega'];
        $Tb_Nom_Bodega=$row['Nom_Bodega'];
        $Tb_Usuario_Encargado=$row['Usuario_Encargado'];
        $Tb_Usuario_Nombres=$row['Nombres'];
        $Tb_Usuario_Apellidos=$row['Apellidos'];
        $Tb_Dir_Bodega=$row['Dir_Bodega'];
        $Tb_Tel_Bodega=$row['Tel_Bodega'];
        $Tb_Cel_Bodega=$row['Cel_Bodega'];
        $Tb_Correo_Bodega=$row['Correo_Bodega'];
        $Tb_Nom_Ciudad=$row['Nom_Ciudad'];
													 ?>
														...
														<a href="Bodegas.php?EditTask=<?php echo($Tb_Id_Bodega);?>" class="tooltip-primary blue" data-rel="tooltip" data-placement="top" title="Editar Taller">
																	<?php echo utf8_encode("TAL".$Tb_Cod_Bodega); ?>
														</a>
														</td>
														<td class="tdcustom" style="text-transform:uppercase;">
															<?php Echo utf8_decode($Tb_Nom_Bodega) ?>
																
															</td>
														<td class="tdcustom" style="text-transform:uppercase;"><?php echo utf8_encode($Tb_Usuario_Nombres." ".$Tb_Usuario_Apellidos); ?></td>
														<td class="tdcustom" style="text-transform:uppercase;"><?php echo utf8_encode($Tb_Dir_Bodega."- (".$Tb_Nom_Ciudad).")"; ?></td>
														<td class="tdcustom" style="text-transform:uppercase;"><a href="mailto:<?php echo utf8_encode($Tb_Correo_Bodega); ?>"><?php echo utf8_encode($Tb_Correo_Bodega); ?></a> </td>
														
														<td class="tdcustom" style="text-transform:uppercase;">
									
									<?php 
									$CadenaCel="<i class='fa fa-mobile-phone'> </i> ".$Tb_Cel_Bodega;
									$CadenaWhp="<i class='fa fa-phone'> </i> ".$Tb_Tel_Bodega;
														...
														<td class="center">
															<div class="hidden-sm hidden-xs action-buttons">
																...
																<a href="Proveedor-CrearOrden.php?ApeCliente=<?php echo($Tb_Nom_Prov);?>&DocSel=<?php echo($Tb_Id_Proveedor);?>&NomCliente=<?php echo($Tb_Contacto_Prov);?>" class="tooltip-danger red" data-rel="tooltip" data-placement="top" title="Crear Orden de Compra">
																	<i class="ace-icon fa fa-plus-square bigger-120"> </i> Ver Inventario
																</a>
