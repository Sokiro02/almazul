...
<html lang="en">
	<head>
		...
		<meta charset="utf-8" />
		<title>Informe Egresos Modasof</title>
					...
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li>
								...
								<a href="Informe-EgresosTienda.php">Egresos Tienda</a>
							</li>
...
<div class="widget-body">
		<h4 class="widget-title lighter">
			...
			<div class="row">
				<form action="Informe-EgresosTienda.php?QueryCon=<?php Echo($TxtConsultaVen) ?>" method="post" id="FormFechas" autocomplete="off">
...
if ($TxtConsultaVen!="") {
		$sql ="SELECT Id_Mov_Cuenta FROM t_mov_cuentas WHERE Fecha_Transf >='".$FechaStart."' and Fecha_Transf <='".$FechaEnd."' and  	Id_Cuenta_Sale='".$TxtConsultaVen."' order by Id_Mov_Cuenta  DESC";
		//Echo($sql);
		$result = $conexion->query($sql);
		if ($result->num_rows > 0) {
    	while ($row = $result->fetch_assoc()) { 
		$Lista=$Lista.$row['Id_Mov_Cuenta'].",";                  
 		}
	}
}
else
{
		$sql ="SELECT Id_Mov_Cuenta FROM t_mov_cuentas WHERE Fecha_Transf >='".$FechaStart."' and Fecha_Transf <='".$FechaEnd."' order by Id_Mov_Cuenta  DESC";
		//Echo($sql);
		$result = $conexion->query($sql);
		if ($result->num_rows > 0) {
    	while ($row = $result->fetch_assoc()) { 
...
if ($TxtConsultaVen!="") {
	...
	$sql ="SELECT * FROM t_mov_cuentas WHERE Id_Mov_Cuenta='".$Cadena[$i]."' and Id_Cuenta_Sale='".$TxtConsultaVen."'";
		//Echo($sql);
		$result = $conexion->query($sql);
}
else
{
	...
	$sql ="SELECT * FROM t_mov_cuentas WHERE Id_Mov_Cuenta='".$Cadena[$i]."'";
		//Echo($sql);
		$result = $conexion->query($sql);
}
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {  
        ...
        $Id_Cuenta_Sale=$row['Id_Cuenta_Sale'];
        $Id_Cuenta_Entra=$row['Id_Cuenta_Entra'];
        $Egreso_Id_Usuario=$row['Egreso_Id_Usuario'];
        $Por_Medio_de=$row['Por_Medio_de'];
        $dateSol = new DateTime($Fecha_Transf);
        $HoraSol=$dateSol->format('H:i:s a');
        $FechaSol=$dateSol->format('Y-m-d');
		}
}

// BENEFICIARIO DEL EGRESO
switch ($Id_Cuenta_Entra) {
    case 2015:
        $Beneficiario='Alex Baute';
        break;
    case 2016:
        $Beneficiario='Amalia Geles';
        break;
    case 2017:
        $Beneficiario='Inez Mazo';
        break;
    case 2018:
        $Beneficiario='Taller Valledupar';
        break;
     case 2019:
        $Beneficiario='Taller Barranquilla';
        break;
      case 2020:
        $Beneficiario='Rodrigo David Arregoces';
        break;
    default:
...
$sql ="SELECT * FROM t_usuarios WHERE Id_Usuario='".$Egreso_Id_Usuario."'";
		//Echo($sql);
		$result = $conexion->query($sql);
 if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {  
...
$sql ="SELECT * FROM t_tiendas WHERE Id_Tienda='".$Id_Cuenta_Sale."'";
		//Echo($sql);
		$result = $conexion->query($sql);
 if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {  
...
?>
			...
			<a data-rel="tooltip" data-placement="top" title="Detalle Compra" href="Pedido-Ver.php?PedidoCliente=<?php Echo($Cod_Pedido); ?>">
														...
														<td class="center">
															<?php Echo (fechasql($Fecha_Transf));?>		
														</td>
														...
														<td>
															<?php Echo utf8_encode($Detalle); ?>
														</td>
														
														
														<td class="center">
															<?php Echo utf8_encode("$ ".number_format($Valor_Transferido));  ?>
														</td>
