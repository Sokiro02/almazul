...
$sql ="SELECT * FROM t_facturas as A, t_clientes as B, t_usuarios as C WHERE num_consecutivo_sc='".$NumeroFactura."' and A.Cliente_Id_Cliente=B.Id_Cliente and A.Usuario_Vendedor=C.Id_Usuario";
	...
	$Num_Factura=$row['num_consecutivo_sc'];   
...
<html lang="en">
	<head>
		...
		<meta charset="utf-8" />
		<title>Crear Devolución Nota Entrega</title>
  ...
  $Valide=$_GET['Mensaje'];
    ...
    if ($Valide==1) {
     ...
     if ($Valide==333) {
      ...
      if ($Valide==332) {
        echo "<script>jQuery(function(){swal(\"¡ Nota Crédito Cargada!\", \"Correctamente \", \"success\");});</script>";
    };
     if ($Valide==222) {
        echo "<script>jQuery(function(){swal(\"¡ Nota Entrega Anulada Correctamente!\", \"Correctamente \", \"success\");});</script>";
    };
    if ($Valide==23) {
...
<!-- Fin de Primer Fila -->
						...
						<div class="clearfix">
										...
										<div>
<?php 
	$sql ="SELECT Id_Venta FROM t_ventas WHERE consecutivosc_id_consecutivosc='".$Num_Factura."' and Tienda_Id_Tienda='".$Tienda_Id_Tienda."'";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
							...
							<table style="font-size: 12px;" id="dynamic-table2"  class="table table-responsive table-striped table-bordered table-hover" width="100%">
												<thead>
													<tr class="warning">
														...
														<th class="tdcustom" style="width: 22%;">Cambio Talla</th>
														<?php 
...
<form action="Facturas-CambioTallaNE.php" method="post" Id="FormCambioTalla">
<input style="display: none;" type="text" name="TxtTallaSolicitada" value="<?php Echo($Talla_Solicitada)?>">
	...
	<input class="input-sm" type="text" name="TxtCantidad" value="<?php Echo($Cant_Solicitada)?>">
																<select name="TxtNuevaTalla">
																	<option value="">Seleccionar</option>
																	<?php 
$sql ="SELECT Id_Talla,Nom_Talla FROM t_tallas where talla_publicada='1'"; 
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
       ...
       Echo("<option value='".$Id_Talla."'>".$Nom_Talla."</option>");
	}
}
																	 ?>
																</select>
...
$sql ="SELECT Observa_Usuario FROM t_devoluciones Where Venta_Id_Venta='".$Cadena[$i]."'"; 
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
...
<form action="Facturas-NotaCr.php" method="post" Id="FormNotaCr">
	...
	<input class="input-sm" type="text" name="TxtCantidad" value="<?php Echo($Cant_Solicitada)?>">
	<input class="input-sm" type="txt" name="TxtObserva" placeholder="Indicar Causal">
