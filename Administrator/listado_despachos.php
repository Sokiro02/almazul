...
background: #8e24aa;
  ...
  $Valide=$_GET['Mensaje'];
    ...
    if ($Valide==333) {
        echo "<script>jQuery(function(){swal(\"¡ Orden Impresa!\", \"Correctamente \", \"success\");});</script>";
    };
    if ($Valide==2) {
		...
		<div class="page-content">
			...
			<!-- Inicio botones de acceso rápido -->
                ...
                <!-- Small boxes (Stat box) -->
                    ...
                    <h5 class="page-header">Seleccionar Despacho a consultar</h5>
                    <div class="row">
                        <table style="font-size: 12px;" id="dynamic-table"  class="table table-responsive table-striped table-bordered table-hover" width="100%">
                    	   <thead>
                    	       <tr class="warning">
                    					...
                    					<th class="tdcustom" style="width: 4%;">Fecha de Envio</th>
                    					<th class="tdcustom" style="width: 8%;">Tienda Recibe</th>
                    					<th class="tdcustom" style="width: 5%;">Status</th>
                    					<th class="tdcustom" style="width: 8%;">Usuario Envia</th>
                                        <th class="tdcustom" style="width: 8%;">Usuario Recibe</th>
...
$semanaantes=date("Y-m-d",strtotime($fecha_actual."- 2 week")); 

//echo($semanaantes);

                            $sql ="SELECT * FROM t_temporal_inventario_despachos WHERE fecha_despacho>='".$semanaantes."' order by fecha_despacho DESC;";  
                            ...
                            if ($result->num_rows > 0) {
                                while ($row = $result->fetch_assoc()) { 
                                    ...
                                    $status = $row['status_despacho'];
                                    $usuario_id_envia = $row['id_user'];
                                    $usuario_recibe = $row['id_user_recepcion'];
                                    $consulta = "SELECT CONCAT(Nombres,' ',Apellidos) as NomApe FROM t_usuarios WHERE Id_Usuario='".$usuario_id_envia."'";
                                    $resultado = $conexion->query($consulta) or die('Error:'.mysqli_error($conexion));;
                                    $fila=$resultado->fetch_assoc();
                                    $NombreUsuario = $fila['NomApe'];
                                    $consulta = "SELECT CONCAT(Nombres,' ',Apellidos) as NomApe FROM t_usuarios WHERE Id_Usuario='".$usuario_recibe."'";
                                    $resultado = $conexion->query($consulta) or die('Error:'.mysqli_error($conexion));;
                                    $fila=$resultado->fetch_assoc();
                                    $NombreUsuarioRec = $fila['NomApe'];
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <?php echo $NombreUsuarioRec;?>
                                    </td>
                                    <td>
                						<div class=" action-buttons">
                							<a target="_blank" href="Pdf_Despacho.php?DespachoId=<?php echo($id_despacho);?>" class="tooltip-error blue" data-rel="tooltip" data-placement="top" title="Imprimir Despacho">
                								<i class="ace-icon fa fa-print bigger-110"> </i> Imprimir
                							</a>
                                            <a href="despachos_detalle.php?Despacho=<?php echo($id_despacho);?>" class="tooltip-error green" data-rel="tooltip" data-placement="top" title="Recibir Despacho">
                                                <i class="ace-icon fa fa-edit bigger-110"> </i> Detalles
                                            </a>
                						</div>
                                    </td>
                                </tr>
