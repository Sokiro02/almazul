...
<html lang="en">
	<head>
		...
		<meta charset="utf-8" />
		<title>Abonos Separe Cliente</title>
...
?>
					...
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							<li>
								...
								<a href="Informe-PlanSepare.php">Regresar</a>
							</li>
						</ul><!-- /.breadcrumb -->
			...
			if ($AbonosFactura!="") {
			 ?>
				...
				<?php 
				$sql ="SELECT B.Id_Cliente, B.Nom_Cliente,B.Ape_Cliente FROM t_separados as A, t_clientes as B WHERE Num_Separe='".$AbonosFactura."' and A.Cliente_Id_Cliente=B.Id_Cliente";
	//Echo($sql);
...
$sql ="SELECT IFNULL(sum(Valor_Ingreso),0) as TotalAbonos FROM t_ingresos WHERE Separe_Cod_Separe='".$AbonosFactura."'";
	...
	$VrAbonos=$row['TotalAbonos'];                  
				 ...
				 ?>
								<h4>Abonos a Plan Separe <?php Echo($Nom_Cliente." ".$Ape_Cliente); ?> <br>
									Separado Nº: PLS <?php Echo($AbonosFactura); ?><br>
								</h4>
							 <table class="table product-overview">
                                  <thead>
                                    <th class="info" >Recibo de Caja Nº</th>
  ...
  $Cod_Recibo_Caja=$row['Cod_Recibo_Caja'];
                                ...
                                ?>
                                      ...
                                      <td><?php Echo($Cod_Recibo_Caja); ?></td>
                                  		...
                                  		<td><a  data-rel="tooltip" data-placement="top" title="Imprimir Recibo" href="Pdf_ReciboCajaSeparado.php?AbonoCliente=<?php Echo($Id_Ingreso) ?>&AbonosFactura=<?php Echo($AbonosFactura) ?>"><i class="fa fa-file-pdf-o bigger-110 red"></i></a></td>
                                  	</tr>
                                  	<?php 
                                  }
                              }
                              else
                              {
                              	?>
                              	<tr>
                              		<td colspan="6"><strong>No hay abonos en este Separado</strong></td>
                              	</tr>
                              	<?php
                              }
                                  	 ?>
                                  	 <tr>
                                  	 	<td class="success" colspan="3"><strong>Total Separado <?php Echo(formatomoneda($Total_Separe)); ?></strong></td>
                                  	 	...
                                  	 	<td class="success" colspan="2">
                                  	 		<strong>
                                  	 		Saldo: <?php Echo(formatomoneda($Total_Separe-$VrAbonos)) ;?></strong>
                                  	 	</td>
                                  	 </tr>
                                  	 <tr>
                                  	 	<td colspan="3">
                                  	<a  data-toggle="modal" data-target="#modal-form2" href="#"><span class="btn  btn-success btn-xs"><i class="fa fa-plus-square"> </i> Agregar Abono</span></a>
                                  	 	...
                                  	 	<td colspan="3">
                                  	 <a class="btn  btn-danger btn-xs" href="Informe-PlanSepare.php"><i class="fa fa-times bigger-110"> </i> Cerrar Vista</a>
                                  	 	</td>
                                  	 </tr>
                                  </tbody>
                         </table>
...
<!-- Inicio Modal -->
    ...
    <div>
      ...
      <form action="Clientes-GuardarAbonoSepare.php?AbonosFactura=<?php Echo($AbonosFactura); ?>" method="post" id="FormAnticipo" autocomplete="off">

<?php 

  $sql ="SELECT `Id_Config_Tienda`,`Consecutivo_ReciboCaja`, `Tienda_Id_Tienda` FROM `t_config_tienda` WHERE Tienda_Id_Tienda='".$MyIdTienda."'";
  //Echo($sql);
  $result = $conexion->query($sql);
  if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
                  ...
                  <div class="modal-dialog ">
                    <div class="modal-content">
                      <div class="modal-header">
                        ...
                        <h4 class="black bigger">Agregar Abono a Separado</h4>
                      </div>
    ...
    if((valorCambiado == '2')||(valorCambiado == '3')||(valorCambiado == '4')||(valorCambiado == '6')||(valorCambiado == '7')){
     ...
     else if(valorCambiado == '1')
     {
          $('#Confirmacion').css('display','none');
                                  ...
                                  <tr>
                                    <td>
                                      ¿ Cliente cancela el separado en totalidad?:
                                    </td>
                                    <td>
                      ...
                      <div class="modal-footer">
                        <a href="Clientes-AbonosSepare.php?AbonosFactura=<?php Echo($AbonosFactura) ?>" class="btn btn-sm btn-danger" data-dismiss="modal">
