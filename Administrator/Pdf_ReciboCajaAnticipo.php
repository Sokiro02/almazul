...
$ReciboCajaImp=$_GET['ReciboCajaImp'];
$PedidoImp=$_GET['PedidoImp'];

$sql ="SELECT  date_format(Fecha_Pedido,CONCAT(CONCAT(ELT(WEEKDAY(Fecha_Pedido) + 1, 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo')),', %d - ',CONCAT(ELT(MONTH(Fecha_Pedido), 'Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Agos', 'Sep', 'Oct', 'Nov', 'Dic')),'- %Y')) AS FechaPedido,B.Nombres,B.Apellidos, D.Nom_Tienda, D.Dir_Tienda, D.Cel_Tienda,C.Nom_Cliente, C.Ape_Cliente,C.Documento_Cliente, E.Nom_Ciudad, C.Cel1_Cliente FROM t_pedido as A, t_usuarios as B, t_clientes as C, t_tiendas as D, t_ciudades as E WHERE Cod_Pedido='".$PedidoImp."' and A.Pedido_Id_Usuario=B.Id_Usuario and A.Tienda_Id_Tienda=D.Id_Tienda and A.Cliente_Id_Cliente=C.Id_Cliente and C.Ciudad_Id_Ciudad=E.Id_Ciudad"; 
//Echo($sql);
...
$sql="SELECT date_format(Fecha_Ingreso,CONCAT(CONCAT(ELT(WEEKDAY(Fecha_Ingreso) + 1, 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo')),', %d - ',CONCAT(ELT(MONTH(Fecha_Ingreso), 'Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Agos', 'Sep', 'Oct', 'Nov', 'Dic')),'- %Y')) AS FechaIngreso From t_ingresos where Cod_Recibo_Caja='".$ReciboCajaImp."'";
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
...
/* seleccionamos el tipo, estilo y tamaño de la letra a utilizar */
        ...
        $pdf->Cell(96,8,utf8_decode('RECIBO DE CAJA N°'.$ReciboCajaImp),0,1,'C'); //Ancho, Alto, texto,borde,?,centrado
...
// Campo Número de Cliente //**************************************************************************************************************************************
        ...
        $pdf->SetFont('Arial','B', 8);
        $pdf->SetXY(23,33  );//Ubicación en PDF
        $pdf->Cell(20,3,'Beneficiario:',0,1,'L');
        ...
        $pdf->Cell(20,3,'Concepto:',0,1,'L');
                ...
                $pdf->Cell(20,3, strtoupper("ANTICIPO"),0,1,'L');
...
$sql ="SELECT Id_Ingreso, Concepto_Ingreso, Medio_Pago,Num_Transaccion,Valor_Ingreso,Nom_MedioPago FROM t_ingresos as A, t_medios_pago as B WHERE Cod_Recibo_Caja='".$ReciboCajaImp."' and A.Medio_Pago=B.Id_Medio_Pago";  
...
$pdf->AutoPrint();
                $pdf->Output("Facturas/ReciboCaja.pdf",'F');
        ...
        echo "<script language='javascript'>window.open('Clientes-AbonosOrdenes.php?NumPedido=".$PedidoImp."','_parent','');</script>";//para ver el archivo pdf generado
