					...
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li>
								...
								<a href="trasladostienda.php">Detalle Traslados Tienda</a>
							</li>
						</ul><!-- /.breadcrumb -->
...
<div class="widget-body">
		<h4 class="widget-title lighter">
			...
			<div class="row">
				<form action="trasladostienda.php?QueryCon=<?php Echo($TxtConsultaVen) ?>" method="post" id="FormFechas" autocomplete="off">
...
<div class="col-sm-12"><!-- Inicio Panel 12 -->
											...
											<ul class="nav nav-tabs" id="myTab">
												<li class="active">
													<a data-toggle="tab" href="#tabs-1">
														...
														Detalle Traslados Enviados <?php Echo($MiTienda); ?>
													</a>
												</li>
												...
												<i class="blue ace-icon fa fa-list bigger-120"></i>
														Detalle Traslados Recibidos <?php Echo($MiTienda); ?>
													</a>
												</li> 
											</ul>
										...
										<div class="table-header" style="background-color: #000;">
											Lista Traslados Tienda Modasof 
											<?php Echo($MiTienda); ?> 
											<?php 
												if ($FechaUno=="") {
												...
												{
													Echo("del ".fechaSql($FechaUno)." al ".fechaSql($FechaDos));
												}
											?> 
										</div>
...
if ($TxtConsultaVen=="") {
	$sql ="SELECT id_traslado FROM t_traslados WHERE fecha_solicitud >='".$FechaStart."' and fecha_solicitud <='".$FechaEnd."' order by id_traslado  DESC";
//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
	$Lista=$Lista.$row['id_traslado'].",";                  
 }
...
{
	$sql ="SELECT id_traslado FROM t_traslados WHERE id_tienda_desde='".$MyIdTienda."' and fecha_solicitud >='".$FechaStart."' and fecha_solicitud <='".$FechaEnd."' order by id_traslado  DESC";
//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
 ...
 ?>
                                    ...
                                    <th class="success"></th>
												...
												<thead>
													...
													<tr class="warning">
														<th class="tdcustom">Traslado Nº</th>
														<th class="tdcustom">Vendedor</th>
													
														<th class="tdcustom">Enviado A</th>
														<th class="tdcustom">Detalle</th>
														<th class="tdcustom">Solicitado</th>
														<th class="tdcustom">Despacho</th>
														<th class="tdcustom">Recibido</th>
...
if ($TxtConsultaVen=='') {
$sql ="SELECT * From  t_traslados_detalle  WHERE id_traslado='".$Cadena[$i]."'"; 
//Echo($sql);
$result = $conexion->query($sql);
}
else
{
	$sql ="SELECT * From  t_traslados_detalle  WHERE id_traslado='".$Cadena[$i]."' and id_tienda_desde='".$TxtConsultaVen."'"; 
//Echo($sql);
$result = $conexion->query($sql);
}
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
        ...
        $id        			=$row['id'];
        $Img_Referencia     =$row['imagen'];
        $cod_referencia     =$row['cod_referencia'];
     	$cod_ref_completa   =$row['cod_ref_completa'];
        $talla_id           =$row['talla_id'];
        $cantidad           =$row['cantidad'];
        $id_traslado        =$row['id_traslado'];
        $aprobado           =$row['aprobado'];
        $status             =$row['status'];
        $id_tienda_desde    =$row['id_tienda_desde'];
        $id_tienda_hasta    =$row['id_tienda_hasta'];
        $id_user            =$row['id_user'];
...
$sql ="SELECT * FROM t_traslados WHERE id_traslado='".$Cadena[$i]."'";
//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
...
$nombreusuario        	=ObtenernombreUsuario($id_user);
$nombretiendadesde      =ObtenernombreTienda($id_tienda_desde);
$nombretiendahasta      =ObtenernombreTienda($id_tienda_hasta);
							...
							<td class="center">
							<?php echo utf8_encode($id_traslado); ?>
							</td>
							<td class="tdcustom">
							<?php echo utf8_encode($nombreusuario); ?>
							</td>
						
							<td class="tdcustom">
							<?php echo utf8_encode($nombretiendahasta); ?>
							</td>
...
{
	$sql ="SELECT id_traslado FROM t_traslados WHERE id_tienda_hasta='".$MyIdTienda."' and fecha_solicitud >='".$FechaStart."' and fecha_solicitud <='".$FechaEnd."' order by id_traslado  DESC";
//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
                                    ...
                                    <th class="success"></th>
												...
												<thead>
													...
													<tr class="warning">
														<th class="tdcustom">Traslado Nº</th>
														<th class="tdcustom">Vendedor</th>
														<th class="tdcustom">Recibido de</th>
