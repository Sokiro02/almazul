...
<html lang="en">
	<head>
		...
		<meta charset="utf-8" />
		<title>Informe Plan Separe Tienda Modasof</title>
...
?>
					...
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li>
								...
								<a href="Informe-PlanSepare.php">Informe Planes Separe</a>
							</li>
			...
			if ($AbonosFactura!="") {
			 ?>
				...
				<?php 
				$sql ="SELECT B.Nom_Cliente,B.Ape_Cliente FROM t_separados as A, t_clientes as B WHERE Num_Separe='".$AbonosFactura."' and A.Cliente_Id_Cliente=B.Id_Cliente";
	//Echo($sql);
...
$sql ="SELECT IFNULL(sum(Valor_Ingreso),0) as TotalAbonos FROM t_ingresos WHERE Separe_Cod_Separe='".$AbonosFactura."'";
	Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
		...
		$PagoEn="Baloto/Efecty";
                              ...
                              else
                              {
                              	...
                              	<?php
                                  	 ...
                                  	 ?>
                                  	 <tr>
                                  	 	<td class="success" colspan="2"><strong>Total Factura <?php Echo(formatomoneda($Total_Separe)); ?></strong></td>
                                  	 ...
                                  	 <a class="btn btn-success" href=""><i class="fa fa-dollar bigger-110"> </i> Crear Abono</a>
                                  	 	...
                                  	 	<td colspan="3">
                                  	 <a class="btn btn-danger" href="Informe-PlanSepare.php"><i class="fa fa-times bigger-110"> </i> Cerrar Vista</a>
                                  	 	</td>
                                  	 </tr>
                                  </tbody>
                         </table>
...
<div class="widget-body">
		<h4 class="widget-title lighter">
			...
			<div class="row">
				<form action="Informe-PlanSepare.php?QueryCon=<?php Echo($TxtConsultaVen) ?>" method="post" id="FormFechas" autocomplete="off">
...
<div class="col-sm-12"><!-- Inicio Panel 12 -->
										...
										<div class="table-header" style="background-color: #000;">
											...
											<?php 
												...
												{
													Echo("Separados del ".fechaSql($FechaUno)." al ".fechaSql($FechaDos));
												}
											?> 
										</div>
...
if ($TxtConsultaVen=='') {
	$sql ="SELECT DISTINCT(Num_Separe) FROM t_separados WHERE Fecha_Separe >='".$FechaStart."' and Fecha_Separe <='".$FechaEnd."' order by Num_Separe  DESC";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
	$Lista=$Lista.$row['Num_Separe'].",";                  
 }
}
}
else
{
	$sql ="SELECT DISTINCT(Num_Separe) FROM t_separados WHERE Fecha_Separe >='".$FechaStart."' and Fecha_Separe <='".$FechaEnd."' and  Tienda_Id_Tienda='".$TxtConsultaVen."' order by Num_Separe  DESC";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
                                    ...
                                    <th class="success"></th>
												...
												<thead>
													...
													<tr class="warning">
														...
														<th class="tdcustom" >Estado del Pago</th>
														
														<th class="tdcustom" >Total Separado</th>
...
if ($TxtConsultaVen=='') {
	$sql ="SELECT date_format(Marca_Temporal,CONCAT(CONCAT(ELT(WEEKDAY(Marca_Temporal) + 1, 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo')),', %d - ',CONCAT(ELT(MONTH(Marca_Temporal), 'Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Agos', 'Sep', 'Oct', 'Nov', 'Dic')),'- %Y')) AS FechaTemporal,date_format(Fecha_Separe,CONCAT(CONCAT(ELT(WEEKDAY(Fecha_Separe) + 1, 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo')),', %d - ',CONCAT(ELT(MONTH(Fecha_Separe), 'Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Agos', 'Sep', 'Oct', 'Nov', 'Dic')),'- %Y')) AS FechaSolicitado, Total_Separe,Num_Separe, Separe_Paga, B.Nombres,B.Apellidos,C.Nom_Tienda, D.Nom_Cliente,D.Ape_Cliente,D.Documento_Cliente,Marca_Temporal,Observa_Separe FROM t_separados as A, t_usuarios as B, t_tiendas as C, t_clientes as D WHERE Num_Separe='".$Cadena[$i]."' and A.Usuario_Vendedor=B.Id_Usuario and A.Tienda_Id_Tienda=C.Id_Tienda and A.Cliente_Id_Cliente=D.Id_Cliente";
		//Echo($sql);
		$result = $conexion->query($sql);
}
	else
{
	$sql ="SELECT date_format(Marca_Temporal,CONCAT(CONCAT(ELT(WEEKDAY(Marca_Temporal) + 1, 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo')),', %d - ',CONCAT(ELT(MONTH(Marca_Temporal), 'Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Agos', 'Sep', 'Oct', 'Nov', 'Dic')),'- %Y')) AS FechaTemporal,date_format(Fecha_Separe,CONCAT(CONCAT(ELT(WEEKDAY(Fecha_Separe) + 1, 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo')),', %d - ',CONCAT(ELT(MONTH(Fecha_Separe), 'Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Agos', 'Sep', 'Oct', 'Nov', 'Dic')),'- %Y')) AS FechaSolicitado, Total_Separe,Num_Separe, Separe_Paga, B.Nombres,B.Apellidos,C.Nom_Tienda, D.Nom_Cliente,D.Ape_Cliente,D.Documento_Cliente,Marca_Temporal,Observa_Separe FROM t_separados as A, t_usuarios as B, t_tiendas as C, t_clientes as D WHERE Num_Separe='".$Cadena[$i]."' and Tienda_Id_Tienda='".$TxtConsultaVen."' and A.Usuario_Vendedor=B.Id_Usuario and A.Tienda_Id_Tienda=C.Id_Tienda and A.Cliente_Id_Cliente=D.Id_Cliente";
		//Echo($sql);
		$result = $conexion->query($sql);
}
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
        ...
        $Nom_Tienda=$row['Nom_Tienda'];
         $Observa_Separe=$row['Observa_Separe'];

        $Separe_Paga=$row['Separe_Paga'];
        if ($Separe_Paga==1) {
        	$Pagode="Pagado";
        }
        elseif ($Separe_Paga==2) {
        	$Pagode="Crédito";
        }
        elseif ($Separe_Paga==3) {
        	$Pagode="Facturada";
        }
...
$SubTotal=$Total_Separe/1.19;
$Iva=$Total_Separe-$SubTotal;
$TotalCompra=$Iva+$SubTotal;
$VrComision=($Total_Separe*1)/100;
//$VrAbonos=0;


$sql ="SELECT IFNULL(sum(Valor_Ingreso),0) as TotalAbonos FROM t_ingresos WHERE Separe_Cod_Separe='".$Cadena[$i]."'";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
...
?>
														...
														<td>
															...
															if ($Separe_Paga==1) {
																?>
																<a target="_blank"  data-rel="tooltip" data-placement="top" title="<?php Echo($Observa_Separe); ?>" href="#"><span class='badge bg-green'><?php Echo($Pagode); ?></span></a>
																<?php
															}
															elseif ($Separe_Paga==2)
															{
																?>
																<a target="_blank"  data-rel="tooltip" data-placement="top" title="<?php Echo($Observa_Separe); ?>" href="#"><span class='badge bg-red'><?php Echo($Pagode); ?></span></a>
																<?php
															}
															elseif ($Separe_Paga==3) {
																?>
																<a target="_blank"  data-rel="tooltip" data-placement="top" title="<?php Echo($Observa_Separe); ?>" href="#"><span class='badge bg-warning'><?php Echo($Pagode); ?></span></a>
																<?php
															}
												...
												<ul class="dropdown-menu dropdown-info dropdown-menu-right">
													...
													<li>
														<a target="_blank"  data-rel="tooltip" data-placement="top" title="Abonar-Cancelar" href="Clientes-AbonosSepare.php?AbonosFactura=<?php Echo($Num_Separe) ?>"><i class="fa fa-dollar bigger-100 blue"></i> Ver/Crear Abonos</a>
													</li>

													<?php 
													if ($Total_Separe==$VrAbonos) {
														?>

														<li>
														<a target="_blank"  data-rel="tooltip" data-placement="top" title="Detalle Factura" href="Clientes-DetallePlanSepare.php?NumeroFactura=<?php Echo($Num_Separe) ?>"><i class="fa fa-dollar bigger-100 blue"></i> Facturar</a>
													</li>
													...
													if ($IdRol==1) {
														...
														<li class="divider"></li>
													<li>
														<a style="position: relative;" data-rel="tooltip" data-placement="top" title="Anular Factura"  href="Clientes-AnularFactura.php?NumeroFactura=<?php Echo($Num_Separe) ?>"><i class="fa fa-close red "></i> Anular Factura</a>
													</li>
														<?php
													}
													 ?>
