...
$RefSel=$_GET['RefSel'];
$Datos="Ingreso a Cargar Inventario de la referencia ".$RefSel;
...
$RegistroDel=$_GET['RegistroDel'];
if ($RegistroDel!="") {
  $RefSel=$_GET['RefSel'];
  
  //RESTAMOS LA CANTIDAD AGREGADA
  $SqlBuscar = "SELECT Cantidad_Inv,Ref_Completa FROM t_inventario_ref WHERE Id_Registro_Inv='".$RegistroDel."'";
  $ResultadoBuscar = $conexion->query($SqlBuscar) or die(mysqli_error($conexion));
  $Fila = $ResultadoBuscar->fetch_assoc();

  $Cantidad = $Fila['Cantidad_Inv'];
  $Ref_Completa = $Fila['Ref_Completa'];



  $SqlActualizar ="UPDATE t_inventario SET Cantidad=Cantidad-'".$Cantidad ."' WHERE Referencia_Completa='".$Ref_Completa."'"; //ACTUALIZAMOS EL INVENTARIO
  $ResultActualizar = $conexion->query($SqlActualizar)or die (mysqli_error($conexion));
  
  
  //ELIMINAMOS EL REGISTRO DEL INVENTARIO INICIAL
 $sql="DELETE FROM t_inventario_ref WHERE Id_Registro_Inv='".$RegistroDel."'";
  $result=$conexion->query($sql);

  $Datos="Elimino referencia desde cargar inventario".$$RefSel;
  $Paginas= $_SERVER['PHP_SELF'];
  $seguridad = AgregarLog($IdUser,$Datos,$Paginas);
  
  // Retornar Página
  header("location:Cargar-Inventario.php?Mensaje=4&RefSel=".$RefSel."");
}
...
<html lang="en">
	<head>
		...
		<meta charset="utf-8" />
		<title>Ingreso Inventario Modasof</title>
...
color:#FFF;
  ...
  $Valide=$_GET['Mensaje'];
    ...
    if ($Valide==1) {
        echo "<script>jQuery(function(){swal(\"¡ Cantidad Agregada!\", \"Correctamente \", \"success\");});</script>";
    };
     if ($Valide==4) {
        echo "<script>jQuery(function(){swal(\"¡ Registro Eliminado!\", \"Correctamente \", \"success\");});</script>";
    };
     if ($Valide==2) {
                            ...
                            <div class="card-body">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-lg-9 col-md-9 col-sm-6">
                                      <h3 class="m-t-40">Ingreso Inventario Inicial</h3>
          
                                        <h4 class="box-title m-t-40">Diligenciar Campos <br> </h4>
                                        <p><?php //Echo($Detalle_Referencia); ?></p>
                                      
        <form action="Inventario-Guardar.php" method="post" id="FormSolicitud">

      <input style="display: none;" type="tex" name="TxtRef" value="<?php Echo($RefSel);?>">
                            <select class="col-lg-3 col-xs-12" name="TxtCantidad">
                                    <option value="">Seleccionar Cantidad</option>
                                    <?php 
                                    for ($i=1; $i <100 ; $i++) { 
                                      Echo("<option value='".$i."'>".$i." Und.</option>");
                                    }
                                     ?>
                            </select>
...
<?php 
    ...
    if ($IdUser==97) {
        ?>
          <select class="col-lg-3 col-xs-12" name="TxtAlmacen" required="true">
                                    <option value="">Seleccionar Tienda</option>
                                    <?php 
$sql ="SELECT Id_Tienda, Nom_Tienda FROM t_tiendas where Id_Tienda='10' order by Nom_Tienda ASC";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
      $SelectId_Tienda=$row['Id_Tienda'];
      $SelectNom_Tienda=$row['Nom_Tienda'];             
      echo ("<option value='".$SelectId_Tienda."' selected=''>".utf8_encode($SelectNom_Tienda)."</option>");
 }
}
                                     ?>
                                  </select>
...
if ($result->num_rows > 0) {
 ...
 ?>
                                  ...
                                  <!--  <select class="col-lg-3 col-xs-12" name="TxtUbica" required="true">
 ...
 -->
        <div>
   <button style="margin:15px;" class="btn btn-primary btn-rounded btn-sm"><i class="fa fa-plus-square"></i> Agregar Inventario </button>
        </div>
        </form>
                              ...
                              <div class="col-lg-9 col-md-9 col-sm-12">
                                 <hr>
     <h3>Inventario Ingresado al Sistema <i class="fa fa-caret-down"></i></h3>
       ...
       <div class="table-responsive">  
                     ...
                     <thead> 
                       ...
                       <tbody> 
                        <?php 
$sql ="SELECT date_format(Fecha_Ingreso,CONCAT(CONCAT(ELT(WEEKDAY(Fecha_Ingreso) + 1, 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo')),', %d - ',CONCAT(ELT(MONTH(Fecha_Ingreso), 'Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Agos', 'Sep', 'Oct', 'Nov', 'Dic')),'- %Y')) AS FechaCreada,Id_Registro_Inv,Ref_Completa, Cantidad_Inv, Fecha_Ingreso, Ubicacion, Tipo_Mov_Inv,Responsable_Id_Usuario,Fecha_Registro_Modasof, B.Nom_Tienda,C.Nombres, C.Apellidos FROM t_inventario_ref as A, t_tiendas as B, t_usuarios as C WHERE Tipo_Mov_Inv='Ingreso Inicial' and A.Tienda_Id_Tienda=B.Id_Tienda and A.Responsable_Id_Usuario=C.Id_Usuario and Inv_Ref='".$RefSel."' order by Nom_Tienda ASC";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
      ...
      $Fecha_Registro_Modasof=$row['Fecha_Registro_Modasof'];
      $Ubicacion=$row['Ubicacion'];
       $NomResponsable=$row['Nombres'];
        $ApeResponsable=$row['Apellidos'];
         $Nom_Tienda=$row['Nom_Tienda'];

 
                        ?>
                        <tr>
                          <td><?php Echo utf8_encode($Ref_Completa) ?></td>
                          <td><?php Echo utf8_encode($Cantidad_Inv) ?> Und.</td>
                           <td><?php Echo utf8_encode($Nom_Tienda) ?></td>
                            <td><?php Echo utf8_encode($FechaCreada) ?></td>
                             <td><?php Echo utf8_encode($NomResponsable." ".$ApeResponsable) ?></td>
                             <td>
                          <a style="display: none;" href="Cargar-Inventario.php?RegistroDel=<?php Echo($Id_Registro_Inv)?>&RefSel=<?php Echo($RefSel)?>"><i class="fa fa-trash red"></i></a>
                             </td>
                        </tr>
                         <?php 
                       }
