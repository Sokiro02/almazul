...
if ($Confirmation!="") {
	...
	if ($Id_PedidoDel!="") {
		...
		//Elimina todos los ingresos
		$sql ="DELETE FROM t_ingresos WHERE Pedido_Id_Pedido='".$Confirmation."'";  
		//echo($sql);
		$result = $conexion->query($sql);		
...
<html lang="en">
	<head>
		...
		<meta charset="utf-8" />
		<title>Mis Ventas Modasof</title>
...
<div class="col-sm-12"><!-- Inicio Panel 12 -->
											...
											<ul class="nav nav-tabs" id="myTab">
												<li class="active">
													<a data-toggle="tab" href="#tabs-1">
														...
														Ventas Tienda <?php Echo($MiTienda); ?>
													</a>
												</li>
										...
										<div class="table-header" style="background-color: #000;">
											Lista de Ventas <?php Echo($MiTienda); ?>
										</div>
...
if ($MyIdTienda=="") {
	$sql ="SELECT Id_Venta FROM t_ventas  order by Id_Venta  DESC";
//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
...
{
	$sql ="SELECT Id_Venta FROM t_ventas WHERE Tienda_Id_Tienda='".$MyIdTienda."' order by Id_Venta  DESC";
//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
                                    ...
                                    <th class="success"></th>
												...
												<thead>
													...
													<tr class="warning">
														...
														<th class="tdcustom" style="width: 10%;">Fecha Venta</th>
														<th class="tdcustom" style="width: 10%;">Cantidad</th>
														<th class="tdcustom" style="width: 10%;">Valor Prenda</th>
														<th class="tdcustom" style="width: 10%;">Valor Final</th>
														<th class="tdcustom" style="width: 10%;">Vr. Descuentos</th>
														<th class="tdcustom" style="width: 10%;">Vr. Adicionales</th>
														<th class="tdcustom" style="width: 10%;">Factura Nº</th>
														<th class="tdcustom" style="width: 10%;">O. Corte</th>
														<th class="tdcustom" style="width: 10%;">(-)</th>
														<th class="tdcustom" style="width: 10%;">(+)</th>
...
for($i=0; $i<$min; $i++)
{
$sql ="SELECT A.Vendedor_Id_Usuario,B.Nombres,B.Apellidos,B.Img_perfil,C.Nom_Cliente,C.Ape_Cliente,A.Valor_Prenda,A.Valor_Final,D.Img_Referencia,A.Ref_Vendida,A.Fecha_Solicitud,A.Factura_Id_Factura,A.Cant_Solicitada,E.Fecha_Factura From t_ventas as A, t_usuarios as B, t_clientes as C, t_referencias as D, t_facturas as E WHERE A.Id_Venta='".$Cadena[$i]."' and A.Vendedor_Id_Usuario=B.Id_Usuario and A.Cliente_Id_Cliente=C.Id_Cliente and A.Referencia_Id_Referencia=D.Cod_Referencia and A.Factura_Id_Factura=E.Num_Factura "; 
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
       
        $Fecha_Solicitud=$row['Fecha_Factura'];
        $Nombres=$row['Nombres'];
        ...
        if ($Valor_Final==$Valor_Prenda) {
        	$Valor_Descuento=0;
        	$ConDescuento="No";
        }
        elseif ($Valor_Final<$Valor_Prenda) {
        ...
        elseif ($Valor_Final>$Valor_Prenda) {
        	$Valor_Incremento=$Valor_Final-$Valor_Prenda;
        	$ConIncremento="Si";
        }
        ...
        $FechaSol=$dateSol->format('Y-m-d');
						...
						<img data-rel="tooltip" data-placement="top" title="<?php Echo($Nombres." ".$Apellidos."\r\n".$Fecha_Solicitud." ".$HoraSol) ?>" class="img-circle" src="<?php echo utf8_encode($AvatarVendedor); ?>" width="45px" height="45px">
														</td>
   ...
   <a style="position: relative;" data-rel="tooltip" data-placement="top" title="<?php Echo utf8_encode($Ref_Vendida);?>" class="image-link" href="<?php echo utf8_encode($Img_Referencia); ?>"><img src="<?php echo utf8_encode($Img_Referencia); ?>" width="45px" height="45px" >
   	<?php Echo($Ref_Vendida); ?>
    	</a>
    	
														</td>
														
														<td class="center">
															<?php Echo fechasql($Fecha_Solicitud);?>
															<br>
															<?php Echo($HoraSol); ?>		
														</td>
														<td class="center">
															...
															<td class="center">
															<?php Echo utf8_encode("$ ".number_format($Valor_Prenda));  ?>
														</td>
														<td class="center">
			
															<?php Echo utf8_encode("$ ".number_format($Valor_Final));  ?>
														</td>
													
														<td class="center">
															<?php Echo utf8_encode("$ ".number_format($Valor_Descuento));  ?>
														</td>
														<td class="center">
															<?php Echo utf8_encode("$ ".number_format($Valor_Incremento));  ?>
														</td>
														<td class="center">
															<?php Echo ($Factura_Id_Factura);?>		
														</td>

														<td class="center">
															<?php 

$sql ="SELECT Cod_Pedido FROM t_pedido WHERE Factura_Num_Factura='".$Factura_Id_Factura."'";
//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
...
else
{
	Echo("Si - #".$Cod_Pedido);
															...
															<td class="center">
															<?php Echo ($ConDescuento);?>		
														</td>
															<td class="center">
															<?php Echo ($ConIncremento);?>		
														</td>
    ...
    $date = new DateTime($Fecha_Solicitud);
						...
						<a data-rel="tooltip" data-placement="top" title="Detalles Solicitud" href="Panel-Produccion.php?Solicitud=<?php Echo($Cod_Solicitud_Prod); ?>">
							...
							<a class="image-link" href="<?php echo utf8_encode($Img_Referencia); ?>"><img src="<?php echo utf8_encode($Img_Referencia); ?>" width="40px" height="40px"></a>
								...
								<?php Echo utf8_encode($Cant_Solicitada) ?>Un.
														</td>
