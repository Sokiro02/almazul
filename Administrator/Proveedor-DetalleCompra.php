...
<html lang="en">
	<head>
		...
		<meta charset="utf-8" />
		<title>Detalle Compra Modasof</title>
...
<!-- INICIO TABLA -->
						<div class="clearfix">
										...
										<div>
						<table id="dynamic-table"  class="table table-responsive table-striped table-bordered table-hover" width="100%">
												<thead>
													<tr class="warning">
...
<?php 
	...
	$sql ="SELECT date_format(Fecha_Solicitud,CONCAT(CONCAT(ELT(WEEKDAY(Fecha_Solicitud) + 1, 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo')),', %d - ',CONCAT(ELT(MONTH(Fecha_Solicitud), 'Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Agos', 'Sep', 'Oct', 'Nov', 'Dic')),'- %Y')) AS FechaSolicitud,date_format(Fecha_Est_Llegada,CONCAT(CONCAT(ELT(WEEKDAY(Fecha_Est_Llegada) + 1, 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo')),', %d - ',CONCAT(ELT(MONTH(Fecha_Est_Llegada), 'Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Agos', 'Sep', 'Oct', 'Nov', 'Dic')),'- %Y')) AS FechaEstimada,date_format(Fecha_Recibido,CONCAT(CONCAT(ELT(WEEKDAY(Fecha_Recibido) + 1, 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo')),', %d - ',CONCAT(ELT(MONTH(Fecha_Recibido), 'Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Agos', 'Sep', 'Oct', 'Nov', 'Dic')),'- %Y')) AS FechaRecibido,Id_Orden_Compra, Cod_Orden_Prov, Cod_Orden_Modasof, Fecha_Solicitud, Marca_Temporal, Insumo_Cod_Insumo, Lote_Insumo, Cantidad_Solicitada, A.Costo_Insumo, Fecha_Est_Llegada,A.Bodega_Id_Bodega,D.Nom_Bodega, Estado_Insumo,B.Cod_Proveedor, B.Cod_Insumo,B.Url_Insumo, B.Nom_Insumo, B.Unidad_Insumo,C.Nombres,C.Apellidos,A.Fecha_Recibido, A.Observa_Compra, A.Cant_Recibida FROM t_orden_compra_insumos as A, t_insumos as B, t_usuarios as C,t_bodegas as D WHERE  A.Insumo_Cod_Insumo=B.Id_Insumo and A.Usuario_Responsable=C.Id_Usuario and Cod_Orden_Prov='".$NumeroOrden."' and A.Bodega_Id_Bodega=D.Id_Bodega"; 
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
        ...
        $Tb_Cant_Recibida=$row['Cant_Recibida'];
        ...
        $Tb_Observa_Compra=$row['Observa_Compra'];
          ...
          //$Tb_Cant_Recibida=CantidadRecibida($NumeroOrden,$Tb_Cod_Insumo,$Tb_Bodega_Id_Bodega);
													...
													<tr>
														...
														<td>
															<?php echo utf8_encode($Tb_Nom_Bodega); ?>
														</td>
														<td>
...
$CadenaRec="<i class='fa fa-check-circle-o green'> </i> <b class='green'>Recibidos: ".round($Tb_Cant_Recibida,1)." ".$Tb_Unidad_Insumo."</b>";
$CadenaPen="<i class='fa fa-clock-o red'> </i> <b class='red'>Pendientes: ".(round($Tb_Cantidad_Solicitada,1)-round($Tb_Cant_Recibida,1))." ".$Tb_Unidad_Insumo."</b>";
...
echo nl2br($cadena);
									 ?>
														...
														<td class="center">
															...
															$Resultado=$Tb_Cantidad_Solicitada-$Tb_Cant_Recibida;
															?>
														</td>
													<?php 
	 ...
	 <input type="number" step="any" name="Txt_Cant_Recibida" >
	...
	<input type="text" name="Txt_ObservaItem" class="input-sm" value="<?php echo ($Tb_Observa_Compra); ?>">		
		<button type="submit" class="btn btn-xs btn-danger tooltip-primary blue" data-rel="tooltip" data-placement="top" title="Verificar Pedido"><i class="fa fa-eye"></i></button>	
			...
			<input style="display: none;" type="text" name="Tb_Unidad_Insumo" value="<?php echo ($Tb_Unidad_Insumo); ?>">
			<input style="display: none;" type="text" name="Txt_IdOrdenCompra" value="<?php echo ($Tb_Id_Orden_Compra); ?>">
			<input style="display: none;" type="text" name="TxtIdInsumo" value="<?php echo ($Tb_Insumo_Cod_Insumo); ?>">
			...
			<input style="display: none;" type="text" name="Txt_Bodega_Id_Bodega" value="<?php echo ($Tb_Bodega_Id_Bodega); ?>">
													...
													elseif (round($Tb_Cant_Recibida,1)==round($Tb_Cantidad_Solicitada,1)) {
												Echo("<td colspan='2' class='center'><span class='label label-sm label-success'> RECIBIDO OK</span></td>");
													...
													elseif ($Tb_Cant_Recibida>$Tb_Cantidad_Solicitada) {
														...
														<td colspan="2">
										<?php 
										$Demas=$Tb_Cant_Recibida-$Tb_Cantidad_Solicitada;
										 ?>
										<span class="label label-sm label-warning"> + <?php Echo($Demas." ".$Tb_Unidad_Insumo) ?></span>
														</td>
