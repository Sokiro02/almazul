...
<html lang="en">
	<head>
		...
		<meta charset="utf-8" />
		<title>Informe Flujo de Caja Modasof</title>
...
<script src="https://modasof.com/espejo/assets/js/jquery.maskMoney.js" type="text/javascript"></script>
  ...
  $Valide=$_GET['Mensaje'];
    ...
    if ($Valide==27) {
        echo "<script>jQuery(function(){swal(\" Informe de Ingresos ".utf8_encode($ConSeleccionado)."\", \"Seleccione un rango de Fecha\", \"success\");});</script>";
    };
				...
				<div class="main-content-inner">
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li>
								<a href="Informe-flujocajatienda">Flujo de Caja Tienda <strong class="badge bg-black"><?php Echo($ConSeleccionado) ?> &nbsp;</strong> </a>
							</li>
...
<div class="widget-body">
		<h4 class="widget-title lighter">
			...
			<div class="row">
				<form action="Informe-flujocajatienda.php?QueryCon=<?php Echo($TxtConsultaVen) ?>" method="post" id="FormFechas" autocomplete="off">
...
<div class="col-sm-12"><!-- Inicio Panel 12 -->
										...
										<div class="table-header" style="background-color: #000;">
											...
											<?php 
												...
												{
													Echo("Ingresos del ".fechaSql($FechaUno)." al ".fechaSql($FechaDos));
												}
											?> 
										</div>
...
if ($TxtConsultaVen=="") {
	...
	$sql ="SELECT Id_Ingreso FROM t_ingresos WHERE Fecha_Ingreso >='".$FechaStart."' and Fecha_Ingreso <='".$FechaEnd."'  ORDER BY Id_Ingreso DESC";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
...
{
	$sql ="SELECT DISTINCT(Dia_Registro) FROM t_registro_caja WHERE Fecha_Registro >='".$FechaStart."' and Fecha_Registro <='".$FechaEnd."' and  Tienda_Id_Tienda='".$TxtConsultaVen."' order by Dia_Registro  DESC";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
	$Lista=$Lista.$row['Dia_Registro'].",";                 
 }
}
}
                                      ...
                                      <th class="success"></th>
												...
												<thead>
													...
													<tr class="warning">
														...
														<th class="tdcustom" >Apertura</th>
														<th class="tdcustom" >Efectivo</th>
														<th class="tdcustom" >Ing. Amalia</th>
														<th class="tdcustom" >Total Entradas</th>
														<th class="tdcustom" >Gastos</th>
														<th class="tdcustom" >Egresos</th>
														<th class="tdcustom" >Devoluciones</th>
														<th class="tdcustom" >Total Egresos</th>
														<th class="tdcustom" >R. Sistema</th>
														<th class="tdcustom" >R. Cierre</th>
														<th class="tdcustom" >Faltó $</th>
														<th class="tdcustom" >Sobró $</th>
...
{
	// Valor de Efectivo
$V1=($Cadena[$i]." 00:00:000");
$V2=($Cadena[$i]." 23:59:000");

	
// Valor de Apertura
$sql ="SELECT Valor_Confirmado FROM t_registro_caja  WHERE Tienda_Id_Tienda='".$TxtConsultaVen."' and  Tipo_Registro='1' and Dia_Registro='".$Cadena[$i]."' ";
		//Echo($sql);
		$result = $conexion->query($sql);
		if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {  
        $Valor_Apertura=$row['Valor_Confirmado'];
	}
}

// Valor de Cierre
$sql ="SELECT Valor_Confirmado FROM t_registro_caja  WHERE Tienda_Id_Tienda='".$TxtConsultaVen."' and  Tipo_Registro='2' and Dia_Registro='".$Cadena[$i]."' ";
		//Echo($sql);
		$result = $conexion->query($sql);
		if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {  
        $Valor_Cierre=$row['Valor_Confirmado'];
	}
}


// Valor Ingresos de Dinero Extras

$sql ="SELECT IFNULL(sum(Valor_ingresodinero),0) as Ingresodinero FROM t_ingreso_dinero as A WHERE Tienda_Id_Tienda='".$TxtConsultaVen."' and Fecha_Ingreso >='".$V1."' and Fecha_Ingreso <='".$V2."'";
		//Echo($sql);
		$result = $conexion->query($sql);
		if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {  
        $Ingresodinero=$row['Ingresodinero'];
	}
}


$sql ="SELECT IFNULL(sum(Valor_Ingreso),0) as IngresoEfectivo FROM t_ingresos as A WHERE Tienda_Id_Tienda='".$TxtConsultaVen."' and Fecha_Ingreso >='".$V1."' and Fecha_Ingreso <='".$V2."' and Medio_Pago='1'";
		//Echo($sql);
		$result = $conexion->query($sql);
		if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {  
...
$sql ="SELECT IFNULL(sum(Valor_Ingreso),0) as IngresoEfectivoKaren FROM t_ingresos as A WHERE Tienda_Id_Tienda='".$TxtConsultaVen."' and Fecha_Ingreso >='".$V1."' and Fecha_Ingreso <='".$V2."' and Medio_Pago='9'";
		//Echo($sql);
		$result = $conexion->query($sql);
		if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {  
        $IngresoEfectivoKaren=$row['IngresoEfectivoKaren'];
	}
}

// Total Gastos
$sql ="SELECT IFNULL(sum(Valor_gasto),0) as TotalGastos FROM t_gastos as A WHERE Tienda_Id_Tienda='".$TxtConsultaVen."' and Fecha_gasto >='".$V1."' and Fecha_gasto <='".$V2."'";
		//Echo($sql);
		$result = $conexion->query($sql);
		if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {  
...
// Total Egresos
$sql ="SELECT IFNULL(sum(Valor_Transferido),0) as EgresosTienda FROM t_mov_cuentas as A WHERE Id_Cuenta_Sale='".$TxtConsultaVen."' and Fecha_Transf >='".$V1."' and Fecha_Transf <='".$V2."'";
		//Echo($sql);
		$result = $conexion->query($sql);
		if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {  
        $EgresosTienda=$row['EgresosTienda'];
	}
}

// Total Devoluciones
$sql ="SELECT IFNULL(sum(valor_devolucion),0) as Devoluciondinero FROM t_devolucion_dinero  WHERE tienda_id_tienda='".$TxtConsultaVen."' and fecha_devolucion >='".$V1."' and fecha_devolucion <='".$V2."'";
		//Echo($sql);
		$result = $conexion->query($sql);
		if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {  
        $Devoluciondinero=$row['Devoluciondinero'];
	}
}

$FormatoDevolucion=$Devoluciondinero*-1;

$TotalEntradas=$Valor_Apertura+$IngresoEfectivo+$Ingresodinero+$IngresoEfectivoKaren;
$TotalSalidas=$TotalGastos+$EgresosTienda+$FormatoDevolucion;
$ReporteSistema=$TotalEntradas-$TotalSalidas-$Devoluciondinero;

$TotalEfectivo = $IngresoEfectivo+$IngresoEfectivoKaren;

if ($ReporteSistema>$Valor_Cierre) {
	$FaltaDinero=$ReporteSistema-$Valor_Cierre;
	$SobraDinero=0;
}
elseif ($ReporteSistema<$Valor_Cierre) {
	$SobraDinero=$Valor_Cierre-$ReporteSistema;
	$FaltaDinero=0;
}
else
{
...
?>
														...
														<td>
															<?php Echo utf8_encode("$ ".number_format($Valor_Apertura));  ?>
														</td>
														<td class="center">
															<?php Echo utf8_encode("$ ".number_format($TotalEfectivo));  ?>	
														</td>
														<td class="center">
															<?php Echo utf8_encode("$ ".number_format($Ingresodinero));  ?>	
														</td>
														<td class="center">
															<?php Echo utf8_encode("$ ".number_format($TotalEntradas));  ?>	
														</td>
														<td class="center">
															<?php Echo utf8_encode("$ ".number_format($TotalGastos));  ?>	
														</td>							
														<td>
															<?php Echo utf8_encode("$ ".number_format($EgresosTienda));  ?>
														</td>
														<td>
															<?php Echo utf8_encode("$ ".number_format($FormatoDevolucion));  ?>
														</td>
															<td>
															<?php Echo utf8_encode("$ ".number_format($TotalSalidas));  ?>
														</td>
														<td class="center">
															<?php Echo utf8_encode("$ ".number_format($ReporteSistema));  ?>
														</td>
														<td class="center">
															<?php Echo utf8_encode("$ ".number_format($Valor_Cierre));  ?>
														</td>
														<td class="center">
															<?php Echo utf8_encode("$ ".number_format($FaltaDinero));  ?>
														</td>
														<td class="center">
															<?php Echo utf8_encode("$ ".number_format($SobraDinero));  ?>
														</td>
													</tr>
													<?php 
