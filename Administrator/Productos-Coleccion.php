...
header("location:Productos-Coleccion.php?Mensaje=18");
}
...
<html lang="en">
	<head>
		...
		<meta charset="utf-8" />
		<title>Colecciones Modasof</title>
...
var dataString = 'Valcoleccion='+ texto;//se guarda en una variable nueva para posteriormente pasarla a busqueda.php
...
return false;    
  ...
  $Valide=$_GET['Mensaje'];
    ...
    if ($Valide==1) {
     ...
     if ($Valide==18) {
        echo "<script>jQuery(function(){swal(\"¡ Colección Eliminada!\", \"Correctamente \", \"success\");});</script>";
    };
     if ($Valide==2) {
        echo "<script>jQuery(function(){swal(\"¡ Colección Actualizada!\", \"Correctamente \", \"success\");});</script>";
    };
    if ($Valide==22) {
...
function(isConfirm){
  if (isConfirm) {
    window.location.href = 'Productos-Coleccion.php?Confirmation=<?php Echo($DeleteTask); ?>';
  } else {
    window.location.href = 'Productos-Coleccion.php';
  }
});
// swal({
					...
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li>
								...
								<a href="Productos-Coleccion.php">Colecciones</a>
							</li>
						</ul><!-- /.breadcrumb -->
...
if ($New==1) {
	...
	<!-- Inicio Modal -->
		<div >
							<form action="Coleccion-CrearColeccion.php" method="post" id="FormNuevoProveedor">
									<div class="modal-dialog ">
										<div class="modal-content">
											<div class="modal-header">
												<a href="Productos-Coleccion.php" type="button" class="close" data-dismiss="modal">&times;</a>
												<h4 class="black bigger">Nueva Colección</h4>
											</div>
											...
											<div class="modal-body">
												...
												<input type="text" autocomplete="off" name="TxtNombre" id="TxtNombre" class="input-large" placeholder="Nombre de la Colección" style="text-transform:uppercase;">
														...
														<div class="row">
															<div class="col-xs-12 col-sm-12">
																<label for="form-field-select-3">Fecha Publicación</label>
																<div class="input-group">
													<input class="form-control date-picker" name="TxtFechaInicio" id="id-date-picker-1" type="text" data-date-format="yyyy-mm-dd" />
																	<span class="input-group-addon">
											...
											<div class="modal-footer">
												<a href="Productos-Coleccion.php" class="btn btn-sm btn-danger" data-dismiss="modal">
...
$sql ="SELECT Id_Coleccion,Nom_Coleccion FROM t_colecciones WHERE Id_Coleccion='".$EditTask."'"; 
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
        $Qr_Nom_Coleccion=$row['Nom_Coleccion'];
...
<form action="Proveedor-ActualizarProveedor.php" method="post" id="FormUpdateProveedor">
									<div class="modal-dialog">
										<div class="modal-content">
											<div class="modal-header">
												...
												<h4 class="blue bigger">Editar Colección</h4>
											</div>
							...
							<input type="text" name="UpTxtNombre" class="input-large" placeholder="Nombre de colección" style="text-transform:uppercase;" value="<?php Echo utf8_encode($Qr_Nom_Coleccion) ?>">
							<input style="display: none;" type="text" name="TxtEditProv" value="<?php Echo utf8_encode($EditTask) ?>">
												...
												<div class="modal-footer">
												<a href="Productos-Coleccion.php" class="btn btn-sm" data-dismiss="modal">
													<i class="ace-icon fa fa-times"></i>
													Cancelar
												</a>
...
<?php 
 ...
 ?>
<!-- Link Ventana Modal -->
<a  href="Productos-Coleccion.php?New=1"><span class="btn btn-info pull-right"><i class="fa fa-plus-square"> </i> Crear Colección</span></a> 
<!-- Link Ventana Modal -->	
<!-- Link Ventana Modal -->
											...
											<table style="font-size: 12px;" id="dynamic-table"  class="table table-responsive table-striped table-bordered table-hover" width="100%">
												<thead>
													...
													<tr class="warning">
														...
														<th class="tdcustom" >Fecha Inicio</th>
												...
												<tbody>
	<?php 
$sql ="SELECT Id_Coleccion FROM t_colecciones order by Id_Coleccion ASC";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
$ListaColecciones=$ListaColecciones.$row['Id_Coleccion'].",";                  
 }
}
$CadenaColecciones=explode(",", $ListaColecciones);
//Split al Arreglo
$longitud = count($CadenaColecciones);
$min=$longitud-1;
...
for($i=0; $i<$min; $i++)
{
	$sql ="SELECT Id_Coleccion, Nom_Coleccion,Fecha_Inicio FROM t_colecciones  WHERE Id_Coleccion='".$CadenaColecciones[$i]."'"; 
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
        $Tb_Id_Coleccion=$row['Id_Coleccion'];
        $Tb_Nom_Coleccion=$row['Nom_Coleccion'];
        $Tb_Fecha_Inicio=$row['Fecha_Inicio'];
				 ...
				 ?>
														...
														<tr>
															<td class="tdcustom">
															<?php Echo utf8_encode($Tb_Nom_Coleccion) ?>
														...
														<td class="tdcustom"><?php echo utf8_encode($Tb_Fecha_Inicio); ?></td>
													...
													<a href="Productos-Coleccion.php?EditTask=<?php echo($Tb_Id_Coleccion);?>" class="tooltip-primary blue" data-rel="tooltip" data-placement="top" title="Editar Colección">
