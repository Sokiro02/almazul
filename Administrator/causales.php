...
if ($Confirmation!="") {
	$sql ="DELETE FROM t_causales WHERE id_causal='".$Confirmation."'";  
//echo($sql);
$result = $conexion->query($sql);
header("location:causales.php?Mensaje=18");
}
...
<html lang="en">
	<head>
		...
		<meta charset="utf-8" />
		<title>Causales Devolución Modasof</title>
...
return false;    
  ...
  $Valide=$_GET['Mensaje'];
    ...
    if ($Valide==1) {
        echo "<script>jQuery(function(){swal(\"¡ Causal Creado!\", \"Correctamente \", \"success\");});</script>";
    };
     if ($Valide==18) {
        echo "<script>jQuery(function(){swal(\"¡ Causal Eliminado!\", \"Correctamente \", \"success\");});</script>";
    };
     if ($Valide==2) {
        echo "<script>jQuery(function(){swal(\"¡ Causal Actualizado!\", \"Correctamente \", \"success\");});</script>";
    };
...
if (isset($_GET['DeleteTask'])) {
    ...
    if ($DeleteTask!="") {
    	...
    	swal({
  title: "¿Está seguro de eliminar este Causal?",
...
function(isConfirm){
  if (isConfirm) {
    window.location.href = 'causales.php?Confirmation=<?php Echo($DeleteTask); ?>';
  } else {
    window.location.href = 'causales.php';
  }
});
// swal({
					...
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li>
								...
								<a href="causales.php">Causales</a>
							</li>
						</ul><!-- /.breadcrumb -->
...
if ($New==1) {
	...
	<!-- Inicio Modal -->
		<div >
							<form action="causal-nuevo.php" method="post" id="FormNuevoProveedor">
									<div class="modal-dialog ">
										<div class="modal-content">
											<div class="modal-header">
												<a href="causales.php" type="button" class="close" data-dismiss="modal">&times;</a>
												<h4 class="black bigger">Nuevo Causal</h4>
											</div>
												...
												<div class="row">
													<div class="col-xs-12 col-sm-12 ">
														...
														<div class="form-group">
															<label for="form-field-select-3">Detalle</label>

															<div class="input-group">
																<span class="input-group-addon">
																	<i class="ace-icon fa fa-user"></i>
																</span>
												<input type="text" autocomplete="off" name="TxtNombre" id="TxtNombre" class="input-large" placeholder="Causal" style="text-transform:uppercase;">
															</div>
											...
											<div class="modal-footer">
												<a href="causales.php" class="btn btn-sm btn-danger" data-dismiss="modal">
...
$sql ="SELECT id_causal, nombre_causal FROM t_causales WHERE id_causal='".$EditTask."'"; 
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
        $Qr_id_causal=$row['id_causal'];
        $Qr_nombre_causal=$row['nombre_causal'];
 }
}
?>
<form action="causal-actualizar.php" method="post" id="FormUpdateProveedor">
									<div class="modal-dialog">
										<div class="modal-content">
											<div class="modal-header">
												
												<h4 class="blue bigger">Editar Causal</h4>
											</div>
										...
										<div class="col-xs-12 col-sm-12 ">
														...
														<div class="form-group">
															<label for="form-field-select-3">Nombre Causal</label>

															<div class="input-group">
																<span class="input-group-addon">
																	<i class="ace-icon fa fa-user"></i>
																</span>
							<input type="text" name="UpTxtNombre" class="input-large" placeholder="Nombre del Proveedor" style="text-transform:uppercase;" value="<?php Echo utf8_encode($Qr_nombre_causal) ?>">
							<input style="display: none;" type="text" name="TxtEditProv" value="<?php Echo utf8_encode($EditTask) ?>">
												...
												<div class="modal-footer">
												<a href="causales.php" class="btn btn-sm" data-dismiss="modal">
													<i class="ace-icon fa fa-times"></i>
													Cancelar
												</a>
...
<?php 
 ...
 ?>
<!-- Link Ventana Modal -->
<a  href="causales.php?New=1"><span class="btn btn-info pull-right"><i class="fa fa-plus-square"> </i> Nuevo Causal</span></a> 
<!-- Link Ventana Modal -->	
						...
						<div class="clearfix">
										...
										<div>
											<table style="font-size: 12px;" id="dynamic-table"  class="table table-responsive table-striped table-bordered table-hover" width="100%">
												...
												<tbody>
	<?php 
$sql ="SELECT id_causal FROM t_causales order by id_causal ASC";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
$ListaProveedores=$ListaProveedores.$row['id_causal'].",";                  
 }
}
...
for($i=0; $i<$min; $i++)
{
	$sql ="SELECT id_causal, nombre_causal FROM t_causales WHERE id_causal='".$CadenaProveedores[$i]."'";
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
        $Tb_id_causal=$row['id_causal'];
        $Tb_nombre_causal=$row['nombre_causal'];
}
}
				 ...
				 ?>
														...
														<tr>
															<td class="tdcustom">
															<?php Echo utf8_encode($Tb_id_causal) ?>
																
															</td>
															<td class="tdcustom">
															<?php Echo utf8_encode($Tb_nombre_causal) ?>
														...
														<td class="center">
															<div class=" action-buttons">
																<a href="causales.php?EditTask=<?php echo($Tb_id_causal);?>" class="tooltip-primary blue" data-rel="tooltip" data-placement="top" title="Editar Causal">
											...
											if ($SumaPedidos==0) {
											 ...
											 <a href="causales.php?DeleteTask=<?php echo($Tb_id_causal);?>" class="tooltip-danger red" data-rel="tooltip" data-placement="top" title="Eliminar Causal">
