...
header("location:Clientes-AnularFactura.php?Mensaje=111&NumeroFactura=".$Factura_Id_Factura."");
...
<!-- Fin de Primer Fila -->
				...
				<form action="AnularFactura.php" method="post" id="FormAnular">
							<!-- INICIO TABLA -->
						<div class="clearfix">
							<h3 class="center">1. Regrese todas las ventas relacionadas a esta factura al inventario </h3>
											<h3 class="center">2. Agregue el causal de la Anulación </h3>
											<h3 class="center">3. Guardar Movimiento en Factura </h3>
											<!-- <div class="pull-left tableTools-container"></div> -->
										</div>
										<div class="table-header">
											Detalle de Factura Nº: <?php Echo($Num_Factura); ?>
										</div>
...
<?php 
	...
	$sql ="SELECT * FROM t_ventas as A, t_referencias as B WHERE Factura_Id_Factura='".$Num_Factura."' and A.Referencia_Id_Referencia=B.Cod_Referencia and Tienda_Id_Tienda='".$TiendaSel."'"; 
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
		...
		$VrDescuento=$CalculoDescuento*100;
			?>
													...
													<tr>
														...
														if ($Estado_Anulado==0) {
															?>
															<a style="position: relative;" data-rel="tooltip" data-placement="top" title="Cargar a Inventario"  href="Clientes-AnularFactura.php?NumeroFactura=<?php Echo($NumeroFactura) ?>&MiVenta=<?php Echo($Id_Venta) ?>"><i class="fa fa-close red "></i> Regresar a Inventario</a>
															<?php
														}
														else
														{
        ...
        <textarea  class="autosize-transition form-control col-sm-12 col-xs-12" name="TxtDetalle" id="form-field-9" rows="5" maxlength="2000" placeholder="Indicar causal de anulación de la factura"><?php Echo($Observa_Factura."Factura anulada el día:".$Fecha_Anula) ?></textarea>
