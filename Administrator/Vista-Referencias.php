...
<html lang="en">
  ...
  $Valide=$_GET['Mensaje'];
    ...
    if ($Valide==1) {
        echo "<script>jQuery(function(){swal(\"¡ Colección Creada!\", \"Correctamente \", \"success\");});</script>";
    };
					...
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li>
								...
								<a href="Vista-Referencias.php">Referencias Modasof</a>
							</li>
						</ul><!-- /.breadcrumb -->
											...
											<table style="font-size: 12px;" id="dynamic-table"  class="table table-responsive table-striped table-bordered table-hover" width="100%">
												<thead>
													...
													<tr class="warning">
														...
														<th class="tdcustom" >Referencia Anterior</th>
														<th class="tdcustom" >Tipo Producto</th>
														<th class="tdcustom" >Colección</th>
												...
												<tbody>
	<?php 
$sql ="SELECT Cod_Referencia FROM t_referencias order by Id_Referencia ASC";  
...
for($i=0; $i<$min; $i++)
{
	$sql ="SELECT date_format(Fecha_Creacion,CONCAT(CONCAT(ELT(WEEKDAY(Fecha_Creacion) + 1, 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo')),', %d - ',CONCAT(ELT(MONTH(Fecha_Creacion), 'Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Agos', 'Sep', 'Oct', 'Nov', 'Dic')),'- %Y')) AS FechaCreacion,Id_Referencia, Cod_Referencia, Img_Referencia, Fecha_Creacion, Coleccion_Nom_Coleccion, Categoria_Id_Categoria_Prod, SubCategoria_Id_Subcategoria_Prod, Insumo_Ppal, Tipo_Tela, Estado_Ref, Costo_Proyectado_Pref, V_Mano_Obra_Ref, PVP_Ref, P_Mayor, Ref_Publicada, Tipo_Referencia,Nom_Cat_Producto,Ref_Antigua,Detalle_Antiguo,C.Nom_SubCat_Producto FROM t_referencias as A, t_categoria_producto as B, t_subcategoria_producto as C WHERE Cod_Referencia='".$Cadena[$i]."' and A.Categoria_Id_Categoria_Prod=B.Id_Cat_Producto and A.SubCategoria_Id_Subcategoria_Prod=C.Id_SubCat_Producto"; 
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
...
$UtilidadBruta=$Operacion3;
				 ...
				 ?>
														...
														<tr>
															<td class="tdcustom">
															<?php Echo utf8_encode($Id_Referencia) ?>	
															</td>
															<td class="tdcustom">
													...
													<img src="<?php echo $mostrar_img; ?>" width="50px" height="50px">
															...
															<td class="tdcustom">
															<?php Echo utf8_encode($Ref_Antigua); ?>	
															</td>
															<td class="tdcustom">
															<?php Echo utf8_encode($Nom_Cat_Producto." ".$Tipo_Tela) ?>	
															</td>
															<td class="tdcustom">
															<?php Echo utf8_encode($Coleccion_Nom_Coleccion) ?>		
															</td>
															<td class="tdcustom">
											...
											if ($IdRol==1) {
												?>
												<a target="_blank" href="Cargar-Inventario.php?RefSel=<?php Echo($Cod_Referencia); ?>&Referencia=<?php Echo($Coleccion_Nom_Coleccion); ?>" class="tooltip-success black" data-rel="tooltip" data-placement="top" title="Cargar a Inventario"><i class="fa fa-cloud-download bigger-170"></i></a>
													<a href="Ficha_Tecnical_Modasof.php?Ref=<?php Echo($Cod_Referencia); ?>" class="tooltip-primary blue" data-rel="tooltip" data-placement="top" title="Imprimir Ficha Técnica"><i class="fa fa-file-pdf-o bigger-150"></i>
													</a>
													<a target="_blank" href="Producto-modificar.php?RefSel=<?php Echo($Cod_Referencia); ?>" class="tooltip-success black" data-rel="tooltip" data-placement="top" title="Modificar Referencia"><i class="fa fa-edit bigger-170"></i></a>
													...
													<a target="_blank" href="Producto-Disponibilidad.php?RefSel=<?php Echo($Cod_Referencia); ?>&Referencia=<?php Echo($Coleccion_Nom_Coleccion); ?>" class="tooltip-success green" data-rel="tooltip" data-placement="top" title="Ver Disponibilidad"><i class="fa fa-eye bigger-170"></i></a>
												<?php
											}
											elseif ($IdRol==2)
											{
											 ...
											 <a target="_blank" href="Produccion-Solicitud.php?RefSel=<?php Echo($Cod_Referencia); ?>&Referencia=<?php Echo($Coleccion_Nom_Coleccion); ?>" class="tooltip-success black" data-rel="tooltip" data-placement="top" title="Solicitud de Producción"><i class="fa fa-cogs bigger-170"></i></a>
											...
											{
												?>
												<a target="_blank" href="Producto-DisponibilidadWeb.php?RefSel=<?php Echo($Cod_Referencia); ?>&Referencia=<?php Echo($Coleccion_Nom_Coleccion); ?>" class="tooltip-success green" data-rel="tooltip" data-placement="top" title="Ver Disponibilidad"><i class="fa fa-eye bigger-170"></i></a>
