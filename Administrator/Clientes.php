    ...
    while ($row = $result->fetch_assoc()) {                
        ...
        $Qr_Fechaano=$row['Fechaano'];
         ...
         $Largo_Pantalon=$row['Largo_Pantalon'];
					...
					<form action="Clientes-ActualizarCliente.php" method="post" id="FormUpdateCliente">
					...
					<?php 
					 ...
					 ?>
						...
						<div class="col-sm-12 col-xs-12">
							<a  data-toggle="modal" data-target="#modal-form" href="#"><span class="btn btn-danger pull-right"><i class="fa fa-plus-square"> </i> Nuevo Cliente</span></a>
						<!-- Inicio Modal -->
										...
										<div>
											<table style="font-size: 12px;" id="dynamic-table"  class="table table-responsive table-striped table-bordered table-hover">
												<thead>
													...
													<tr class="warning">
														<th class="tdcustom" >CLIENTE Nº</th>

														<th class="tdcustom" >Documento</th>
														<th class="tdcustom" >Nombres</th>
														<th class="tdcustom" >Teléfono</th>
...
$sql ="SELECT * FROM t_clientes as A, t_ciudades as B  WHERE A.Ciudad_Id_Ciudad=B.Id_Ciudad order by Id_Cliente Desc"; 
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
        ...
        $Tb_Correo_Cliente=$row['Correo_Cliente'];
         $Tb_Direccion=$row['Dir_Cliente'];
        $Tb_Nom_Ciudad=$row['Nom_Ciudad'];
													 ?>

														<tr>
														<td>
													<?php Echo utf8_decode($Tb_Id_Cliente) ?>
														
														</td>
														<td class="tdcustom" style="text-transform:uppercase;">
													<?php Echo utf8_decode($Tb_Documento_Cliente) ?>
															</td>
														
														<td class="tdcustom" style="text-transform:uppercase;"><?php echo utf8_encode($Tb_Nom_Cliente." ".$Tb_Ape_Cliente); ?></td>
														
														
														<td class="tdcustom" style="text-transform:uppercase;">
									
									<?php 
									$CadenaCel="<i class='fa fa-mobile-phone'> </i> ".$Tb_Cel1_Cliente;
									$CadenaWhp="<i class='fa fa-phone'> </i> ".$Tb_Cel2_Cliente;
...
echo nl2br($cadena);
									 ?>
															...
															<td>
																<?php Echo htmlentities($Tb_Direccion); ?>
															</td>
															<td>
																<?php Echo utf8_decode($Tb_Nom_Ciudad) ?>
															</td>
														...
														<td class="center">
															<a href="Clientes.php?EditTask=<?php echo($Tb_Id_Cliente);?>" class="tooltip-primary blue" data-rel="tooltip" data-placement="top" title="Editar Cliente">
																	<i class="fa fa-edit bigger-110"></i>
														</a>
														<a href="Vista-Cliente.php?Cartera=<?php echo($Tb_Id_Cliente);?>" class="tooltip-primary blue" data-rel="tooltip" data-placement="top" title="Editar Cliente">
