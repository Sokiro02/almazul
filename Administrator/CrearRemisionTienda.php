...
if ($Delete!="") {
	$sql="DELETE FROM t_salidas_remisiones  WHERE Id_Venta='".$Delete."'";
	$result=$conexion->query($sql);
	header("CrearRemisionTienda.php?Mensaje=1");
}
...
if ($PagoTemporal!="") {
  ...
  $result=$conexion->query($sql);
  header("CrearRemisionTienda.php");
}
...
if ($CancelVenta=="true") {
	$sql="DELETE FROM t_salidas_remisiones  WHERE Vendedor_Id_Usuario='".$IdUser."'";
	$result=$conexion->query($sql);
...
if ($result->num_rows > 0) {
 ...
 $sql ="SELECT Consecutivo_Salidas FROM t_config_tienda WHERE Tienda_Id_Tienda='".$MyIdTienda."'";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
      $Consecutivo_Salidas=$row['Consecutivo_Salidas']+1;
    }
  }
                                     ?>
                 
                                <h3 class="m-b-0 text-white">REMISIÓN Nº <?php Echo($Consecutivo_Salidas) ?> <br>
              ...
              <form action="PruebaEnterSalida.php" method="post" autocomplete="off">
...
$sql="SELECT C.Nom_Talla,Id_Venta, Cant_Solicitada, Talla_Solicitada, Tienda_Id_Tienda, A.Referencia_Id_Referencia, Fecha_Solicitud, Vendedor_Id_Usuario, Valor_Prenda,Valor_Final,B.Img_Referencia FROM t_salidas_remisiones as A, t_referencias as B, t_tallas as C  WHERE A.Referencia_Id_Referencia=B.Cod_Referencia and A.Talla_Solicitada=C.Id_Talla and Vendedor_Id_Usuario='".$IdUser."' and Estado_Venta='1' order by Referencia_Id_Referencia Desc";
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
...
//$VrDescuento=$CalculoDescuento*100;
                                     ?>
                                            ...
                                            <form action="Actualizar-Carrito3.php" method="post">
...
symbolPosition : 'left' // use this setting to position the symbol at the left or right side of the value. default 'left'
                                                ...
                                                <td style="padding-top: 20px;" align="center">
                                                	<a href="CrearRemisionTienda.php?Delete=<?php Echo($Id_Venta); ?>&Mensaje=1" data-rel="tooltip" data-placement="top" title="Eliminar Referencia"  class="text-inverse btn-xs" title="" data-toggle="tooltip" data-original-title="Delete"><i class="fa fa-trash bigger-150 red"></i></a>
...
$sql ="SELECT SUM(Valor_Final*Cant_Solicitada) as TotalPedido FROM t_salidas_remisiones WHERE Vendedor_Id_Usuario='".$IdUser."' and Estado_Venta='1'";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
...
$sql ="SELECT SUM(Cant_Solicitada) as SumaPrendas FROM t_salidas_remisiones WHERE Vendedor_Id_Usuario='".$IdUser."'and Estado_Venta='1'";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
      $SumaPrendas=$row['SumaPrendas'];
                                ...
                                <table class="table product-overview">
                                  <tr>
                                    <td>
                                     <!--  <a class="btn btn-danger btn-sm" href="CrearRemisionTienda.php?CancelVenta=true"><i class="fa fa-times"></i> Cancelar Venta</a> -->
                                    </td>
                                  </tr>
                                  <tr>
...
<form method="post" action="GuardarSalida.php" id="FormGuardarventa">
...
// Detalle venta Factura
    ...
    $sql ="SELECT Id_Venta,Referencia_Id_Referencia,Talla_Solicitada,Ref_Vendida, Cant_Solicitada FROM t_salidas_remisiones WHERE Vendedor_Id_Usuario='".$IdUser."' and Estado_Venta='1'";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
      $Id_Venta=$row['Id_Venta'];
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
      $Consecutivo_Salidas=$row['Consecutivo_Salidas']+1;
      ?>
      <input style="display: none;" type="text" name="FactNum" value="<?php Echo($Consecutivo_Salidas);?>">
      <?php
    }
  }
...
<hr>
 ...
 <textarea required name="Txtobservaciones" class="form-control" placeholder="Agregar Observaciones" ></textarea>
                                <table id="info" class="table product-overview">
                                 
                                </table>

                               <hr>
                                 <button type="submit" class="btn btn-danger btn-outline"><i class="fa fa-print"> </i> Finalizar e Imprimir Remisión</button>
                                 </form>
...
<!-- Inicio Modal -->
    ...
    <div>
              ...
              <form action="Cliente-Crear.php?ForVenta=4" method="post" id="FormNuevoCliente" autocomplete="off">
                  <div class="modal-dialog ">
                    <div class="modal-content">
                      <div class="modal-header">
                        <a href="CrearRemisionTienda.php" type="button" class="close" data-dismiss="modal">&times;</a>
                  ...
                  <div class="col-sm-12 col-xs-12 col-md-12">
                    ...
                    <div class="col-sm-8 col-xs-8 col-md-8">
                      ...
                      <div class="modal-footer">
                        <a href="CrearRemisionTienda.php" class="btn btn-sm btn-danger" data-dismiss="modal">
