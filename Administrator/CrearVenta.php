...
if ($Delete!="") {
	$sql="DELETE FROM t_ventas  WHERE Id_Venta='".$Delete."'";
	$result=$conexion->query($sql);
	header("CrearVenta.php?Mensaje=1");
}
...
// if ($CancelVenta=="true") {
// 	$sql="DELETE FROM t_ventas  WHERE Vendedor_Id_Usuario='".$IdUser."'";
// 	$result=$conexion->query($sql);
...
color:#FFF;
  ...
  $GetFactura=$_GET['Factura'];
    if ($Valide==21) {
       ...
       if ($Valide==222) {
        echo "<script>jQuery(function(){swal(\"¡Referencia sin inventario en tienda!\", \"Contactar al Administrador\", \"error\");});</script>";
    };


     if ($Valide==551) {
        echo "<script>jQuery(function(){swal(\"¡Error en el consecutivo !\", \" De la Salida Contable\", \"error\");});</script>";
    };
    ...
    if ($Valide==1) {
     ...
     if ($Valide==2) {
      ...
      if ($Valide==25) {
        echo "<script>jQuery(function(){swal(\"¡ Datos cliente Actualizado!\", \"Correctamente \", \"success\");});</script>";
    };
     if ($Valide==4) {
        echo "<script>jQuery(function(){swal(\"¡ Cliente Creado!\", \"Correctamente \", \"success\");});</script>";
    };
    if ($Valide==333) {
        echo "<script>jQuery(function(){swal(\"¡ Factura Creada Correctamente!\", \"FACT. Nº".$GetFactura." \", \"success\");});</script>";
    };
   ?>
...
// Agregamos un campo en la configuración de la tienda: 
  if ($MyIdTienda==2) { // Si la tienda es Barranquilla traer consecutivo de Facturas

    $LabelEtiqueta="Factura Nº";

     $sql ="SELECT Consecutivo_Factura FROM t_config_tienda WHERE Tienda_Id_Tienda='".$MyIdTienda."'";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
      $consecutivo_sc=$row['Consecutivo_Factura']+1;
    }
  }
}
else{ // Si es diferente traer consecutivo

   $LabelEtiqueta="Salida Contable Nº";
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
      $consecutivo_sc=$row['consecutivo_sc']+1;
                                ...
                                <h3 class="m-b-0 text-white"> <?php Echo($LabelEtiqueta."  ".$consecutivo_sc) ?> <a  data-toggle="modal" data-target="#modal-form3" href="#"><span class="btn  btn-success btn-xs"><i class="fa fa-search"> </i> Ver Tallas</span></a> <br><small><strong>Vendedor: <?php Echo utf8_encode($NomSesion) ?></strong></small> </h3>


              <form action="PruebaEnter_1.php" method="post" autocomplete="off">
...
$sql="SELECT C.Nom_Talla,Id_Venta, Cant_Solicitada, Talla_Solicitada, Tienda_Id_Tienda, A.Referencia_Id_Referencia, Fecha_Solicitud, Vendedor_Id_Usuario, Valor_Prenda,Valor_Final,B.Img_Referencia FROM t_ventas as A, t_referencias as B, t_tallas as C  WHERE A.Referencia_Id_Referencia=B.Cod_Referencia and A.Talla_Solicitada=C.Id_Talla and Vendedor_Id_Usuario='".$IdUser."' and Estado_Venta='1' order by Referencia_Id_Referencia Desc";
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
...
symbolPosition : 'left' // use this setting to position the symbol at the left or right side of the value. default 'left'
                                                ...
                                                <td style="padding-top: 20px;" align="center">
                                                	<a href="CrearVenta.php?Delete=<?php Echo($Id_Venta); ?>&Mensaje=1" data-rel="tooltip" data-placement="top" title="Eliminar Referencia"  class="text-inverse btn-xs" title="" data-toggle="tooltip" data-original-title="Delete"><i class="fa fa-trash bigger-150 red"></i></a>
...
$sql ="SELECT SUM(Valor_Final*Cant_Solicitada) as TotalPedido FROM t_ventas WHERE Vendedor_Id_Usuario='".$IdUser."' and Estado_Venta='1'";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
...
$sql ="SELECT SUM(Cant_Solicitada) as SumaPrendas FROM t_ventas WHERE Vendedor_Id_Usuario='".$IdUser."'and Estado_Venta='1'";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
      $SumaPrendas=$row['SumaPrendas'];
                                ...
                                <table class="table product-overview">
                                  ...
                                  <th class="pull-rigth warning" style="padding: 10px;" ><i class="fa fa-shopping-cart"> </i> <?php Echo($SumaPrendas); ?> Prendas
                                  </th>
                                  </tr>
                                  <tr>
                                    <td>Sub-Total:</td>
            ...
            <form action="GuardarIngresoVenta.php" method="post" id="FormPago" autocomplete="off" >
                                <table class="table product-overview">
                                  <tr>
...
$sql ="SELECT Distinct(notacredito_num) FROM t_devoluciones WHERE Fecha_Devolucion >='".$FechaInicioDia."' and Fecha_Devolucion <='".$FechaFinalDia."'  ORDER BY Id DESC";
    //Echo($sql);
    $result = $conexion->query($sql);
    if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
...
?>      
                        ...
                        <small style="color: blue;">Solo aplica para pagos con Nota Crédito</small>
                                ...
                                <table class="table product-overview">
                                  ...
                                  <?php 
                                  $sql ="SELECT Id_Ingreso_Tempora, Tipo_Pago_Temporal, Valor_Pago_Temporal, Voucher, Usuario_Ingreso,Nom_MedioPago FROM ingreso_temporal_usuario as A, t_medios_pago as B WHERE Usuario_Ingreso='".$IdUser."' and Pago_De='Factura' and A.Tipo_Pago_Temporal=B.Id_Medio_Pago";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
      ...
      $PagoRealizado=$row['Nom_MedioPago'];
                                   ...
                                   ?>
                                 <tr >
                                    <td><a class="red" href="CrearVenta.php?PagoTemporal=<?php Echo($Id_Ingreso_Tempora) ?>">Eliminar</a></td>
                                    <td>
...
$PendientePago=$ValorfinalFormulario-$SumaPagos;
                 ?>
...
<form method="post" action="GuardarVenta.php" id="FormGuardarventa">
...
// Detalle venta Factura
    ...
    $sql ="SELECT Id_Venta,Referencia_Id_Referencia,Talla_Solicitada,Ref_Vendida, Cant_Solicitada FROM t_ventas WHERE Vendedor_Id_Usuario='".$IdUser."' and Estado_Venta='1'";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
      $Id_Venta=$row['Id_Venta'];
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
      $Consecutivo_Factura=$row['Consecutivo_Factura']+1;
      ?>
      <input style="display: none;" type="text" name="FactNum" value="<?php Echo($Consecutivo_Factura);?>">
      <?php
    }
  }

// Número Salida Contable
 $sql ="SELECT consecutivo_sc FROM t_config_tienda WHERE Tienda_Id_Tienda='".$MyIdTienda."'";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
      $consecutivo_sc=$row['consecutivo_sc']+1;
      ?>
      <input style="display: none;" type="text" name="ScNum" value="<?php Echo($consecutivo_sc);?>">
      <?php
    }
  }

?>
    <input style="display: none;" type="text" name="ValorTotal" value="<?php Echo($ValorfinalFormulario);?>">
...
<?php 
    ...
    {
 ?>
     <h4 class="card-title">Seleccionar tipo de venta</h4>
...
<hr>
                              ...
                              <button type="submit" class="btn btn-danger btn-outline"><i class="fa fa-print"> </i> Imprimir Salida Contable</button>
