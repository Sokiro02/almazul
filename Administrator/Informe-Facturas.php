...
<html lang="en">
	<head>
		...
		<meta charset="utf-8" />
		<title>Informe Ventas Modasof</title>
					...
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li>
								...
								<a href="Informe-Facturas.php">Informe Facturación</a>
							</li>
							<li class="dropdown">
													...
													<ul class="dropdown-menu dropdown-info">
														<li>
				<a  href="Informe-Facturas.php">Ver todas las tiendas</a>
			</li>
														<?php
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
    	...
    	?>
    		<li>
				<a  href="Informe-Facturas.php?QueryCon=<?php Echo($SelectDown);?>&Mensaje=27"><?php Echo utf8_encode($NomDown); ?></a>
			</li>
    	<?php
 }
}
...
if ($result->num_rows > 0) {
									...
									<div class="modal-dialog">
										<div class="modal-content">
											<div class="modal-header">
												...
												<h4 class="blue bigger">Cambiar Fecha en Factura  Nº <?php Echo($NumeroFactura);?></h4>
											</div>
  ...
  <div class="input-group">
                                  ...
                                  <input required="true" class="form-control date-picker" id="id-date-picker-1" type="text" data-date-format="yyyy-mm-dd" name="TxtNuevaFecha" value="<?php Echo($Fecha_Factura)?>" />
  ...
  <div class="input-group">
                                ...
                                <select name="TxtTipoFactura" required="true" data-placeholder="Seleccionar...">
                                	...
                                	<option value="2">Crédito</option>
												...
												<a href="Informe-Facturas.php" class="btn btn-sm" data-dismiss="modal">
...
<div class="widget-body">
		<h4 class="widget-title lighter">
			...
			<div class="row">
				<form action="Informe-Facturas.php?QueryCon=<?php Echo($TxtConsultaVen) ?>" method="post" id="FormFechas" autocomplete="off">
					<div class="col-xs-12 col-sm-6">
...
//$FechaInicioDia=($anoactual."-".$mesactual."-01"." 00:00:000");
$FechaInicioDiacorta=($anoactual."-01-01");
$FechaFinalDia=($MarcaTemporal." 23:59:000");
$FechaFinalDiacorta=($MarcaTemporal);
...
<div class="col-sm-12"><!-- Inicio Panel 12 -->
										...
										<div class="table-header" style="background-color: #000;">
											...
											<?php 
													...
													Echo("Ventas del ".fechaSql($Desde)." al ".fechaSql($Hasta));
...
if ($TxtConsultaVen=="") {
	$sql ="SELECT DISTINCT(Num_Factura) FROM t_facturas WHERE Fecha_Factura >='".$FechaStart."' and Fecha_Factura <='".$FechaEnd."'  ORDER BY Num_Factura DESC";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
...
{
	$sql ="SELECT DISTINCT(Num_Factura) FROM t_facturas WHERE Fecha_Factura >='".$FechaStart."' and Fecha_Factura <='".$FechaEnd."' and  Tienda_Id_Tienda='".$TxtConsultaVen."' order by Num_Factura  DESC";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
                                    ...
                                    <th class="success"></th>
												...
												<thead>
													...
													<tr class="warning">
														...
														<th class="tdcustom" >Fecha<br><small>AAAA-MM-DD</small></th></th>
														...
														<th class="tdcustom" >Comisión</th>
														<th class="tdcustom" style="width: 10%;" >Ver Factura</th>
...
if ($TxtConsultaVen!="") {
	$sql ="SELECT date_format(Fecha_Factura,'%Y-%m-%d') AS FechaSolicitado,date_format(Marca_Temporal,CONCAT(CONCAT(ELT(WEEKDAY(Marca_Temporal) + 1, 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo')),', %d - ',CONCAT(ELT(MONTH(Marca_Temporal), 'Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Agos', 'Sep', 'Oct', 'Nov', 'Dic')),'- %Y')) AS FechaTemporal, Total_Factura,Num_Factura, Factura_Paga,Iva_Factura,Subtotal_Factura, B.Nombres,B.Apellidos,C.Nom_Tienda, D.Nom_Cliente,D.Ape_Cliente,Marca_Temporal,A.Tienda_Id_Tienda FROM t_facturas as A, t_usuarios as B, t_tiendas as C, t_clientes as D WHERE Num_Factura='".$Cadena[$i]."' and Tienda_Id_Tienda='".$TxtConsultaVen."' and A.Fecha_Factura >='".$FechaStart."' and A.Fecha_Factura <='".$FechaEnd."' and A.Usuario_Vendedor=B.Id_Usuario and A.Tienda_Id_Tienda=C.Id_Tienda and A.Cliente_Id_Cliente=D.Id_Cliente";
		//Echo($sql);
		$result = $conexion->query($sql);
}
else  
{
	$sql ="SELECT date_format(Marca_Temporal,CONCAT(CONCAT(ELT(WEEKDAY(Marca_Temporal) + 1, 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo')),', %d - ',CONCAT(ELT(MONTH(Marca_Temporal), 'Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Agos', 'Sep', 'Oct', 'Nov', 'Dic')),'- %Y')) AS FechaTemporal,date_format(Fecha_Factura,'%Y-%m-%d') AS FechaSolicitado,num_consecutivo_sc,Total_Factura, Num_Factura,Factura_Paga,Iva_Factura,Subtotal_Factura,B.Nombres,B.Apellidos,C.Nom_Tienda, D.Nom_Cliente,D.Ape_Cliente,Marca_Temporal,A.Tienda_Id_Tienda FROM t_facturas as A, t_usuarios as B, t_tiendas as C, t_clientes as D WHERE Num_Factura='".$Cadena[$i]."' and A.Fecha_Factura >='".$FechaStart."' and A.Fecha_Factura <='".$FechaEnd."' and A.Usuario_Vendedor=B.Id_Usuario and A.Tienda_Id_Tienda=C.Id_Tienda and A.Cliente_Id_Cliente=D.Id_Cliente";
		//Echo($sql);
		$result = $conexion->query($sql);
}
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
        ...
        $num_consecutivo_sc=$row['num_consecutivo_sc'];
...
?>
														...
														<td class="center">
															SC<?php Echo($num_consecutivo_sc) ?>
															<?php 
															$sql ="SELECT notacredito_num FROM t_devoluciones WHERE Num_Factura='".$num_consecutivo_sc."' and Tienda_Id_Tienda='".$Tienda_Id_Tienda."'";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
	...
	if ($notacredito_num!=0) {
	Echo("<span class='badge bg-red'>NC".$notacredito_num."</span>");
															}   
			...
			<a data-rel="tooltip" data-placement="top" title="Detalle Compra" href="Pedido-Ver.php?PedidoCliente=<?php Echo($Cod_Pedido); ?>">
														...
														<td>
															<?php 
															if ($Factura_Paga==1) {
																Echo("<span class='badge bg-green'>".$Pagode."</span>");
															}
															elseif ($Factura_Paga==2) 
															{
																Echo("<span class='badge bg-red'>".$Pagode."</span>");
															}
															elseif ($Factura_Paga==3) {
																Echo("<span class='badge bg-warning'>".$Pagode."</span>");
															}
														...
														<td class="center">
															...
															<?php Echo utf8_encode("$ ".number_format($VrAbonos));  ?>
														</td>
														<td class="center">
			
															<?php Echo utf8_encode("$ ".number_format($VrComision));  ?>
														</td>
			...
			<div class="btn-group">
												...
												<ul class="dropdown-menu dropdown-info dropdown-menu-right">
													...
													if ($IdRol==1) {
														...
														<li class="divider"></li>
													<?php 
													if ($num_consecutivo_sc!=0) {
														?>
														<li>
														<a target="_blank"  data-rel="tooltip" data-placement="top" title="Crear Devolución" href="Clientes-Devoluciones.php?NumeroFactura=<?php Echo($Num_Factura) ?>"><i class="fa fa-exchange bigger-100 blue"></i> Crear Devolución</a>
													</li>
														<?php
													}
													else
													{
														?>
														<li>
															<i class="fa fa-exchange bigger-100 blue"></i> Falta Salida Contable
														</li>
														<?php
													}
													...
													<li>
	<a style="position: relative;" data-rel="tooltip" data-placement="top" title="Anular Factura"  href="Clientes-AnularFactura.php?NumeroFactura=<?php Echo($Num_Factura) ?>&TiendaSel=<?php echo($Tienda_Id_Tienda);?>"><i class="fa fa-close red "></i> Anular Factura</a>
													</li>
													<li>
											<a style="position: relative;" data-rel="tooltip" data-placement="top" title="Editar Fecha"  href="Informe-Facturas.php?NumeroFactura=<?php Echo($Num_Factura) ?>&EditTask=1"><i class="fa fa-calendar red "></i> Editar Fecha</a>
													</li>
														<?php
													}
													 ?>
