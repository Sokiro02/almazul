...
<html lang="en">
	<head>
		...
		<meta charset="utf-8" />
		<title>Pedidos Entregados Modasof</title>
					...
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li>
								...
								<a href="PedidosEntregados.php">Pedidos Entregados</a>
							</li>
						</ul><!-- /.breadcrumb -->
...
<div class="widget-body">
		<h4 class="widget-title lighter">
			...
			<div class="row">
				<form action="PedidosEntregados.php?QueryCon=<?php Echo($MyIdTienda) ?>" method="post" id="FormFechas" autocomplete="off">
...
$treintadias=date("Y-m-d",strtotime($MarcaTemporal."- 4 week")); 
$FechaInicioDia=$treintadias." 00:00:0000";
$FechaInicioDiacorta=($treintadias);
...
?>
										...
										<div class="table-header" style="background-color: #000;">
											Pedidos Entregados Tienda <?php Echo($MiTienda); ?> del <?php echo fechasql($Desde); ?> al <?php echo fechasql($Hasta); ?>
										</div>
...
if ($MyIdTienda=="") {
	$sql ="SELECT Id_Pedido FROM t_pedido Where Estado_Pedido='10' and  Fecha_Pedido >='".$FechaStart."' and Fecha_Pedido <='".$FechaEnd."' order by Id_Pedido  DESC";
	//echo($sql);
	$result = $conexion->query($sql);
...
{
	$sql ="SELECT Id_Pedido FROM t_pedido WHERE Tienda_Id_Tienda='".$MyIdTienda."' and Estado_Pedido='10' and Fecha_Pedido >='".$FechaStart."' and Fecha_Pedido <='".$FechaEnd."' order by Id_Pedido  DESC";
	$result = $conexion->query($sql);
	//echo($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
                                    ...
                                    <th class="success"></th>
												...
												<thead>
													...
													<tr class="warning">
														...
														<th class="tdcustom" style="width: 10%;">Entregado el<br><small>AAAA-MM-DD</small></th>
														<th class="tdcustom" style="width: 10%;">Total Días</th>
...
for($i=0; $i<$min; $i++)
{
$sql ="SELECT date_format(Fecha_Pedido,'%Y-%m-%d') AS FechaPedido,date_format(Fecha_EntregaCliente,'%Y-%m-%d') AS FechaEntrega, Id_Pedido,Factura_Num_Factura, Cod_Pedido, Cliente_Id_Cliente, Fecha_Pedido, Total_Pedido, Descuento, Estado_Pedido,D.Nom_Estado_Pedido, Saldo_Abonado, Pedido_Id_Usuario, Fecha_EntregaCliente, Tienda_Id_Tienda, B.Nombres,B.Apellidos,B.Img_perfil,C.Nom_Cliente,C.Ape_Cliente,C.Avatar_Cliente FROM t_pedido as A, t_usuarios as B, t_clientes as C, t_estado_pedidos as D WHERE Id_Pedido='".$Cadena[$i]."' and A.Pedido_Id_Usuario=B.Id_Usuario and A.Cliente_Id_Cliente=C.Id_Cliente and A.Estado_Pedido=D.Id_Estado_Pedido"; 
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
		...
		$Factura_Num_Factura=$row['Factura_Num_Factura'];
        ...
        $Fecha_EntregaCliente=$row['Fecha_EntregaCliente'];
        ...
        $FechaSol=$dateSol->format('Y-m-d');
						...
						$DiasTotales=dias_transcurridos($Fecha_Pedido,$Fecha_EntregaCliente);
...
if ($result->num_rows > 0) {
    ...
    $TiendaDespacho=ObtenernombreTienda($Tienda_Id_Tienda);
    echo($TiendaDespacho);
            ?>
														...
														<td>
															<?php 
    $sql ="SELECT F.Img_Referencia, Estado_Solicitud_Cliente, G.Nom_Estado_Pedido FROM t_temporal_sol AS A, t_usuarios as B, t_bodegas as C, t_tallas as D, t_tiendas as E,t_referencias as F,t_estado_pedidos as G WHERE  A.Vendedor_Id_Usuario=B.Id_Usuario and A.Bodega_Id_Bodega=C.Id_Bodega and A.Talla_Solicitada=D.Id_Talla and A.Tienda_Id_Tienda=E.Id_Tienda and A.Referencia_Id_Referencia=F.Cod_Referencia  and Pedido_Id_Pedido='".$Cod_Pedido."' and A.Estado_Solicitud_Cliente=G.Id_Estado_Pedido  ORDER BY UNIX_TIMESTAMP(Fecha_Solicitud) DESC"; 
    //Echo($sql); 
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
   ...
   <a style="position: relative;" data-rel="tooltip" data-placement="top" title="<?php Echo utf8_encode($Nom_Estado_Pedido);?>" class="image-link" href="<?php echo utf8_encode($Img_Referencia); ?>"><img src="<?php echo utf8_encode($Img_Referencia); ?>" width="45px" height="45px" >
     ...
     <?php 
      ...
      ?>
														...
														<td class="center">
															FAC<?php Echo($Factura_Num_Factura) ?>
														</td>
