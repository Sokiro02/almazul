...
?>
      ...
      $Detienda=$_GET['Detienda'];
			if ($AbonosFactura!="") {
			 ?>
							<div class="col-sm-7 col-xs-7 center">
				<?php 
				$sql ="SELECT B.Id_Cliente, B.Nom_Cliente,B.Ape_Cliente FROM t_facturas as A, t_clientes as B WHERE num_consecutivo_sc='".$AbonosFactura."' and A.Cliente_Id_Cliente=B.Id_Cliente and A.Tienda_Id_Tienda='".$Detienda."'";
	//Echo($sql);
...
$sql ="SELECT Total_Factura FROM t_facturas WHERE num_consecutivo_sc='".$AbonosFactura."' and Tienda_Id_Tienda='".$Detienda."'";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
...
$sql ="SELECT IFNULL(sum(Valor_Ingreso),0) as TotalAbonos FROM t_ingresos WHERE consecutivosc_id_consecutivosc='".$AbonosFactura."' and Cliente_Id_Cliente='".$Id_Cliente."'";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
	$VrAbonos=$row['TotalAbonos'];                  
				 ...
				 ?>
								<h4>Anticipos de Cliente <?php Echo($Nom_Cliente." ".$Ape_Cliente); ?> <br>
									Nota Entrega Nº: NE <?php Echo($AbonosFactura); ?><br>
								</h4>
  ...
  $PagoEn=$row['Nom_MedioPago'];
                                ?>
                                      ...
                                      ?>
                                        ...
                                        if ($Cod_Recibo_Caja!=0) {
                                         ...
                                         ?>
                                        <a  data-rel="tooltip" data-placement="top" title="Imprimir Recibo" href="Pdf_ReciboCajaCopia.php?AbonoCliente=<?php Echo($Id_Ingreso) ?>&AbonosFactura=<?php Echo($AbonosFactura) ?>"><i class="fa fa-file-pdf-o bigger-110 red"></i></a>
                                        <?php 
                                      }
                                         ?>
                                      </td>
                              ...
                              else
                              {
                              	...
                              	<?php
                                  	 ...
                                  	 ?>
                                  	 <tr>
                                  	 	<td class="success" colspan="3"><strong>Total  <?php Echo(formatomoneda($Total_Factura)); ?></strong></td>
                                  	...
                                  	<a  data-toggle="modal" data-target="#modal-form2" href="#"><span class="btn  btn-success btn-xs"><i class="fa fa-plus-square"> </i> Agregar Abono</span></a>
                                  	 	...
                                  	 	<td colspan="3">
                                  	 <a class="btn  btn-danger btn-xs" href="Informe-VentasTienda.php"><i class="fa fa-times bigger-110"> </i> Cerrar Vista</a>
                                  	 	</td>
                                  	 </tr>
                                  </tbody>
                         </table>
...
<!-- Inicio Modal -->
    ...
    <div>
      ...
      <form action="Clientes-GuardarAbono.php?AbonosFactura=<?php Echo($AbonosFactura); ?>" method="post" id="FormAnticipo" autocomplete="off">
...
$sql ="SELECT Num_Factura FROM `t_facturas` WHERE num_consecutivo_sc='".$AbonosFactura."'";
  //Echo($sql);
  $result = $conexion->query($sql);
  if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
 ...
 ?>
                ...
                <input style="display: none;" type="text" value="<?php Echo($AbonosFactura) ?>" name="TxtNumFactura"> 
                 <input style="display: none;" type="text" value="<?php Echo($Num_Factura) ?>" name="Num_Factura"> 
