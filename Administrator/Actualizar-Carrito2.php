...
if ($ForCosto9>$PVP_Ref) {
	header("location:CrearPlanSepare.php?Mensaje=23");	
	//Echo($PVP_Ref);
}
else
{
	$sql="UPDATE t_plansepare SET Cant_Solicitada='".$TxtCantidad."', Valor_Final='".$ForCosto9."' WHERE Id_Venta='".$TxtIdVenta."'";
	$result = $conexion->query($sql);
	header("location:CrearPlanSepare.php?Mensaje=2");	
}
