...
<script>
$(document).ready(function(){
   $("#TxtClientes2").change(function () {
           $("#TxtClientes2 option:selected").each(function () {
            Select1 = $(this).val();
            //var Select2 = $("#TxtCantidad option:selected").val();
            //alert(Select1)
            $.post("SelectCliente.php", { Select1: Select1}, function(data){
...
background: -moz-linear-gradient(top, rgba(251,253,253,1) 0%, rgba(168,202,214,1) 100%);
background: -webkit-gradient(left top, left bottom, color-stop(0%, rgba(251,253,253,1)), color-stop(100%, rgba(168,202,214,1)));
background: -webkit-linear-gradient(top, rgba(251,253,253,1) 0%, rgba(168,202,214,1) 100%);
background: -o-linear-gradient(top, rgba(251,253,253,1) 0%, rgba(168,202,214,1) 100%);
background: -ms-linear-gradient(top, rgba(251,253,253,1) 0%, rgba(168,202,214,1) 100%);
background: linear-gradient(to bottom, rgba(251,253,253,1) 0%, rgba(168,202,214,1) 100%);
filter: progid:DXImageTransform.Microsoft.gradient( startColorstr='#fbfdfd', endColorstr='#a8cad6', GradientType=0 );
}
#panel-de{
	background: rgba(235,235,235,1);
background: -moz-linear-gradient(top, rgba(235,235,235,1) 0%, rgba(224,224,224,1) 47%, rgba(219,219,219,1) 100%);
background: -webkit-gradient(left top, left bottom, color-stop(0%, rgba(235,235,235,1)), color-stop(47%, rgba(224,224,224,1)), color-stop(100%, rgba(219,219,219,1)));
background: -webkit-linear-gradient(top, rgba(235,235,235,1) 0%, rgba(224,224,224,1) 47%, rgba(219,219,219,1) 100%);
background: -o-linear-gradient(top, rgba(235,235,235,1) 0%, rgba(224,224,224,1) 47%, rgba(219,219,219,1) 100%);
background: -ms-linear-gradient(top, rgba(235,235,235,1) 0%, rgba(224,224,224,1) 47%, rgba(219,219,219,1) 100%);
background: linear-gradient(to bottom, rgba(235,235,235,1) 0%, rgba(224,224,224,1) 47%, rgba(219,219,219,1) 100%);
filter: progid:DXImageTransform.Microsoft.gradient( startColorstr='#ebebeb', endColorstr='#dbdbdb', GradientType=0 );
  ...
  $Valide=$_GET['Mensaje'];
    ...
    if ($Valide==1) {
     ...
     if ($Valide==123) {
        echo "<script>jQuery(function(){swal(\"¡ Apertura Realizada!\", \"Correctamente \", \"success\");});</script>";
    };
     if ($Valide==124) {
        echo "<script>jQuery(function(){swal(\"¡ Cierre Realizado!\", \"Correctamente \", \"success\");});</script>";
    };
    if ($Valide==2) {
        echo "<script>jQuery(function(){swal(\"¡ Extra Cargada a Cliente!\", \"".$ClienteSel." \", \"success\");});</script>";
    };

    // Validación si tiene Pedidos pendientes por recibir

    $sql ="SELECT count(status_recibido) as PendienteRecibir FROM t_temporal_inventario WHERE id_tienda='".$MyIdTienda."' and status_recibido='ENVIADO'";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
      $PendienteRecibir=$row['PendienteRecibir'];
    }
  }


    if ($PendienteRecibir>0) {
    	echo "<script>jQuery(function(){swal(\"¡Tienes Despachos pendietes por Recibir!\", \"Verificar y recibir\", \"info\");});</script>";
    }
    ...
    while ($row = $result->fetch_assoc()) {                
        $AperturaCaja=$row['Valor_Caja'];
            ...
            if ($AperturaCaja=="") {
             ?>
							<li>
								<a  data-toggle="modal" data-target="#modal-form6" href="#">
									<h4 class="btn btn-success">Abrir Caja</h4>
								</a>
							</li>
							<?php 
						}
		...
		<?php 
            if ($AperturaCaja!="") {
             ?>
					<li>
						<a data-toggle="modal" data-target="#modal-form7" href="#">
							<h4 class="bg-red">Cerrar Caja</h4>
						</a>
					</li>
		<?php 
	}
		 ?>
						...
						<form action="ingresoclientes.php" method="post">	
						<div class="col-sm-1">
							<input type="hidden" name="tiendaregistra" value="<?php echo($MyIdTienda)?>">
							<input type="hidden" name="txtvalor" value="1">
						...
						<div class="col-sm-1">
							<input type="hidden" name="tiendaregistra" value="<?php echo($MyIdTienda)?>">
							<input type="hidden" name="txtvalor" value="-1">
							<button class="btn btn-primary "><i class="fa fa-minus-square"></i> </button>
						</div>
						</form>
						<div class="col-sm-7">
							<?php 
date_default_timezone_set("America/Bogota");
$diafrecuencia = date('Y-m-d');
$nuevafecha = strtotime ( '-365 day' , strtotime ( $diafrecuencia ) ) ;
$nuevafecha = date ( 'Y-m-d' , $nuevafecha );
$FechaInicioDia=($diafrecuencia." 00:00:000");
$FechaFinalDia=($diafrecuencia." 23:59:000");
							$frecuencia=totalfrecuencia($MyIdTienda,$FechaInicioDia,$FechaFinalDia)
							 ?>
								<h4 style="color: #C00000;"><strong> <?php echo($frecuencia); ?> </strong>Clientes han ingresado a la tienda hoy <?php echo (fechasql($FechaActual)) ?></h4>
							</div>
                      ...
                      <div class="modal-header">
                       ...
                       <table class="table product-overview center" style="width: 80%;">
                        <tr>
                          <td colspan="2">
                           Confirmar Valor
                                     <input class=" nav-search-input input-sm" type="text"  id="demo7" placeholder="$ 0" name="demo2"  />
...
<div id="panel-iz" class="col-sm-4 col-xs-12"><!-- Inicio Panel Derecho -->
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
      $Consecutivo_ReciboCaja=$row['Consecutivo_ReciboCaja']+1;
                                     ...
                                     ?>
        <h3 class="m-b-0 text-white">VENTA DE BONO Modasof<br><small><strong>Vendedor: <?php Echo utf8_encode($NomSesion) ?></strong></small> </h3>

                                                         

            <form action="GuardarIngresoBono.php" method="post" id="FormPago" autocomplete="off" >
     ...
     else if(valorCambiado == '1')
     {
          $('#Confirmacion').css('display','none');
                                 ...
                                 <hr>
                                  ...
                                  <?php 
                                  $sql ="SELECT Id_Ingreso_Tempora, Tipo_Pago_Temporal, Valor_Pago_Temporal, Voucher, Usuario_Ingreso,Nom_MedioPago FROM ingreso_temporal_usuario as A, t_medios_pago as B WHERE Usuario_Ingreso='".$IdUser."' and Pago_De='Anticipo Bono' and A.Tipo_Pago_Temporal=B.Id_Medio_Pago";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
                                 ...
                                 <tr >
                                  ...
                                  <?php 
                                   ...
                                   ?>
                                    ...
                                    <?php 
                $sql ="SELECT SUM(Valor_Pago_Temporal) as SumaPagos FROM ingreso_temporal_usuario WHERE Usuario_Ingreso='".$IdUser."' and Pago_De='Anticipo Bono'";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
      $SumaPagosAbono=$row['SumaPagos'];
 }
}
                 ?>
                                   ...
                                   <tr class="pull-rigth success">
                                    ...
                                    <td style="text-align: right;"><strong><?php Echo(formatomoneda($SumaPagosAbono)); ?></strong></td>
                                  </tr>
                                 
                                
                                </table>
<form method="post" action="GuardarReciboCajaBono.php" id="FormGuardarventa2">
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
      ...
      <input style="display: none;" type="text" name="TxtSumaPagos" value="<?php Echo($SumaPagosAbono);?>">
      <?php
    }
  }
  else
...
?>
<h4 class="card-title">Seleccionar Bono</h4> 
                                <select required="true" class="chosen-select input-xxlarge" name="TxtBono"  data-placeholder="Seleccionar...">
                            <option value="">Seleccionar Bono </option>
                  <?php
$sql ="SELECT Id_bono,Num_Bono FROM t_bonos WHERE Tienda_Id_Tienda='".$MyIdTienda."' and Estado_Bono='Sin Vender'  order by Num_Bono DESC";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
      $Id_bono=$row['Id_bono'];
      ...
      echo ("<option value='".$Id_bono."'>".utf8_encode($Num_Bono)."</option>");
 }
}
...
<div class="col-sm-4 col-xs-12"><!-- Inicio Botonería -->
	<a href="CrearVenta.php" style="color:white;">
          ...
          <a href="cc_crearsalida.php" style="color: white;">
        <div class="col-lg-12 col-xs-12">
          <!-- small box -->
       ...
       <a style="display: none;" href="CrearPlanSepare.php" style="color: white;">
        <div style="display: none;" class="col-lg-12 col-xs-12">
          <!-- small box -->
          <div  class="small-box bg-aqua">
            <div class="inner">
              <h3>PLAN SEPARE</h3>
        ...
        <a  href="CrearRemisionTienda.php" style="color: white">
        <div class="col-lg-12 col-xs-12">
          <!-- small box -->
        ...
        <a href="Egresos-tienda.php" style="color: white;">
         <div class="col-lg-12 col-xs-12">
          <!-- small box -->
          <div class="small-box bg-purple">
            <div class="inner">
              <h3>REGISTRAR EGRESO DE CAJA</h3>
        ...
        <a style="color: white;" href="gastos-tienda.php">
          <div class="col-lg-12 col-xs-12">
          <!-- small box -->
         ...
         <a style="color: white;" href="ingresodinero-tienda.php">
          <div class="col-lg-12 col-xs-12">
          <!-- small box -->
          <div class="small-box bg-black">
            <div class="inner">
              <h3>INGRESO DINERO </h3>
            </div>
            <div class="icon">
              <i class="fa fa-money"></i>
            </div>
...
<?php 	
 $sql ="SELECT anticipos_activo FROM t_config_tienda WHERE Tienda_Id_Tienda='".$MyIdTienda."'";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
...
if ($anticipos_activo=='Si') {

 ?>
<div id="panel-de" class="col-sm-4 col-xs-12"><!-- Inicio Panel Derecho -->
         <div class="col-md-12 col-xs-12">
        ...
        <h3 class="m-b-0 text-white">CREAR ANTICIPO  ORDEN CORTE<br><small><strong>Vendedor: <?php Echo utf8_encode($NomSesion) ?></strong></small> </h3>
            ...
            <form action="GuardarIngresoAnticipo.php" method="post" id="FormPago" autocomplete="off" >
                                <table class="table product-overview">
                                  ...
                                  <td>
                                     <input class=" nav-search-input input-sm" type="text"  id="demo5" placeholder="$ 0" name="demo1"  required="true" />
...
symbolPosition : 'left' // use this setting to position the symbol at the left or right side of the value. default 'left'
                                   ...
                                   <td>
                                     <select name="TxtMedioPago" id="TxtMedioPago2">
                                      <option value="">Seleccionar</option>
                                       <?php 
    $sql ="SELECT Id_Medio_Pago, Nom_MedioPago FROM t_medios_pago Where Estado_MedioPago='1' order by Id_Medio_Pago ASC";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
      ...
      Echo("<option value='".$Id_Medio_Pago."'>".$Nom_MedioPago."</option>");
                                     ...
                                     <script type="text/javascript">
   $('#TxtMedioPago2').change(function(){
    var valorCambiado =$(this).val();
    if((valorCambiado == '2')||(valorCambiado == '3')||(valorCambiado == '4')||(valorCambiado == '6')||(valorCambiado == '7')){
     
       $('#Confirmacion2').css('display','');
     }
     else if(valorCambiado == '1')
     {
          $('#Confirmacion2').css('display','none');
     }
});
    </script>
                                 ...
                                 <tr >
                                  ...
                                  <?php 
                                   ...
                                   ?>
                                    ...
                                    <?php 
                $sql ="SELECT SUM(Valor_Pago_Temporal) as SumaPagos FROM ingreso_temporal_usuario WHERE Usuario_Ingreso='".$IdUser."' and Pago_De='Anticipo'";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
...
?>
                          ...
                          <h4 class="card-title">Seleccionar Cliente</h4> 
                                <select required="true" class="chosen-select input-xxlarge" name="TxtClientes" id="TxtClientes2" data-placeholder="Seleccionar...">
                            <option value="">Empiece a digitar el documento </option>
                  <?php
$sql ="SELECT Id_Cliente,Documento_Cliente, Nom_Cliente,Ape_Cliente FROM t_clientes  order by Nom_Cliente ASC";  
$result = $conexion->query($sql);
...
?>      
                                ...
                                <br>
                                  <a  data-toggle="modal" data-target="#modal-form2" href="#"><span class="btn  btn-success btn-xs"><i class="fa fa-plus-square"> </i> Crear Nuevo Cliente</span></a>
                                <table id="info2" class="table product-overview">
...
{
	echo("<div id='panel-de' class='col-sm-4 col-xs-12'><p><h1>Solicitar al Admin Activación de Anticipos</h1></p></div>");
}
 ?>
                      ...
                      <div class="modal-header">
                       ...
                       <table class="table product-overview center" style="width: 80%;">
                        <tr>
                          <td colspan="2">
                           Confirmar Valor
                                     <input class=" nav-search-input input-sm" type="text"  id="demo6" placeholder="$ 0" name="demo1"  />
