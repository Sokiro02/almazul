...
$consulta_inventario ="SELECT  sum(valor_total) as suma,id_tienda,id_ref,img_ref,cod_ref,detalle_ref,nom_tienda,talla_id,cantidad,valor_unidad,valor_total,talla,id_user FROM t_temporal_inventario WHERE id_despacho='".$id_despachos."'";
...
$consul = "SELECT * FROM t_usuarios WHERE Id_Usuario='".$usuario_id."'";
$res = $conexion->query($consul) or die('Error:'.mysqli_error($conexion));
$enc = $res->fetch_assoc();
$Img_Perfil1 = $enc['Img_Perfil'];
$NomUser1 = $enc['Nombres'];
$ApeUser1 = $enc['Apellidos'];
		...
		<div class="breadcrumbs ace-save-state" id="breadcrumbs">
			<ul class="breadcrumb">
				<li>
					...
					<a href="listado_despachos.php">Atrás</a>
				</li>
			</ul><!-- /.breadcrumb -->						
		</div>
...
<h5>ENVIO REALIZADO POR:</h5>
<h5><img style="width: 15%;height: 15%;" class="nav-user-photo" src="../Administrator/<?php Echo utf8_encode($Img_Perfil1); ?>" alt="Jason's Photo" /> <?php Echo utf8_encode($NomUser1." ".$ApeUser1); ?></h5>
							...
							<table style="font-size: 12px;" id="dynamic-table"  class="table table-responsive table-striped table-bordered table-hover" width="100%">
												<thead>
													<tr class="warning">
                                                        ...
                                                        <th class="tdcustom" style="width: 5%;">Cliente?</th>
                                                        <th class="tdcustom" style="width: 10%;">Nombre Cliente</th>
													</tr>
												...
												<tbody>
<?php 
	$consulta_inventario ="SELECT  * FROM t_temporal_inventario WHERE id_despacho='".$id_despachos."'";
...
<a lass="btn btn-xs btn-success" target="_blank" href="codigos_barra2.php?ID=<?php Echo($id_despachos) ?>&ID_TIENDA=<?php Echo($id_tienda) ?>&TOTAL=<?php Echo($suma) ?>">
	<i class="fa fa-save"></i> Imprimir Codigos de Barra
</a>

<form style="display: none;" method="post" action="codigos_barra2.php" id="FormOrden">
