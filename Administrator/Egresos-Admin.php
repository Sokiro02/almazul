...
<input style="display: none;" type="text" name="ProyectoGasto" value="<?php Echo($ProyectoSel) ;?>">
				...
				<select class="chosen-select form-control"  name="CuentaRecibe" data-placeholder="Seleccionar Rubro...">
																<option value="">Seleccionar</option>
																	<?php
$sql ="SELECT Id_Usuario, Nombres, Apellidos FROM t_usuarios WHERE Estado_Id_Estado_Usuario='1' order by Nombres";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
    	$SelectId=$row['Id_Usuario'];
    	$SelectNombre=$row['Nombres'];    
    	$SelectApellido=$row['Apellidos'];             
    	echo ("<option value='".$SelectId."'>".utf8_encode($SelectNombre." ".$SelectApellido)."</option>");
 }
}
