...
if ($Confirmation!="") {
	$sql ="DELETE FROM t_gastos WHERE Id_Gasto='".$Confirmation."'";  
		echo($sql);
		$result = $conexion->query($sql);
		header("location:Informe-GastosTienda.php?Mensaje=18&start=".$FechaUno."&end=".$FechaDos."");
}
...
<html lang="en">
	<head>
		...
		<meta charset="utf-8" />
		<title>Informe Gastos Modasof</title>
...
$(document).ready(function(){
  ...
  $Valide=$_GET['Mensaje'];
    ...
    if ($Valide==28) {
        echo "<script>jQuery(function(){swal(\"¡ Gasto Editado!\", \"Correctamente \", \"success\");});</script>";
    };
     if ($Valide==18) {
        echo "<script>jQuery(function(){swal(\"¡ Gasto Eliminado!\", \"Correctamente \", \"success\");});</script>";
    };
    if ($Valide==27) {
    ...
    if (isset($_GET['NumGastoDel'])) {
	 $NumGastoDel=$_GET['NumGastoDel'];
	 $FechaUno=$_GET['start'];
	 $FechaDos=$_GET['end'];
    if ($NumGastoDel!="") {
    	...
    	swal({
  title: "¿Está seguro de eliminar el Gasto <?php echo($NumGastoDel); ?>?",
...
function(isConfirm){
  if (isConfirm) {
    window.location.href = 'Informe-GastosTienda.php?Confirmation=<?php Echo($NumGastoDel);?>&start=<?php echo($FechaUno) ?>&end=<?php Echo($FechaDos); ?>';
  } else {
    window.location.href = 'Informe-GastosTienda.php?start=<?php echo($FechaUno); ?>&end=<?php echo($FechaDos); ?>';
  }
});
// swal({
...
?>
			...
			<div class="main-content">
					...
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li>
								...
								<a href="index-ventas.php">Inicio</a>
							</li>

							<li>
								<i class="ace-icon fa fa-home"></i>
								<a href="Informe-GastosTienda.php">Gastos Tienda</a>
							</li>
				...
				?>
					...
					$MiGasto=$_GET['MiGasto'];
					$FechaUno=$_GET['start'];
					$FechaDos=$_GET['end'];
if ($MiGasto!="") {
	
$sql ="SELECT * FROM t_gastos WHERE Id_Gasto='".$MiGasto."'";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
      $Fecha_gasto=$row['Fecha_gasto'];        
       ...
       $Sel_Id_Rubro=$row['Rubro_id_rubro'];
       $Sel_Sub_rubro=$row['Sub_rubro'];
         
 }
}

$sql ="SELECT * FROM t_rubros WHERE Id_Rubro='".$Sel_Id_Rubro."' ";  
$result = $conexion->query($sql);
//echo($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
       $Rubro_Id_Rubro=$row['Id_Rubro'];
       $Nom_rubro=$row['Nom_rubro']; 
 }
}

$sql ="SELECT * FROM t_subrubros WHERE Id_subrubro='".$Sel_Sub_rubro."' ";  
$result = $conexion->query($sql);
//echo($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
       ...
       $Nom_Subrubro=$row['Nom_Subrubro']; 
					 ...
					 ?>
		<div id="DivEditar">
					<form action="Gastos-Editar.php" method="post" id="FormUpdateCliente">
						<input style="display: none;" type="text" name="TxtIdGasto" value="<?php Echo($MiGasto)?>">
						<input style="display: none;" type="text" name="start" value="<?php Echo($FechaUno)?>">
						<input style="display: none;" type="text" name="end" value="<?php Echo($FechaDos)?>">

									<div class="modal-dialog">
										<div class="modal-content">
											<div class="modal-header">
												
												<h4 class="blue bigger">Editar Gasto </h4>
											</div>
										...
										<div class="col-xs-12 col-sm-6 ">
														...
														<div class="form-group">
															<label for="form-field-select-3">Gasto Id Nº <?php Echo utf8_encode($MiGasto) ?></label>
											...
											<div>
			<input class="input-sm" type="text" id="demo1" placeholder="Valor Ingreso" name="demo1"  value="<?php Echo($Valor_gasto)?>" />
...
symbolPosition : 'left' // use this setting to position the symbol at the left or right side of the value. default 'left'
														...
														<div class="form-group">
							<label for="form-field-select-3">Editar Beneficiario :</label>
								<div class="input-group"> 
                                  <input required="true" class="form-control"  type="text" name="TxtBeneficiario" value="<?php Echo($Beneficiario)?>" />
                                  <span class="input-group-addon">
					...
					<br />
<select class="chosen-select form-control"  name="SelRubro" data-placeholder="Seleccionar Rubro...">

<option value="<?php echo($Rubro_Id_Rubro) ?>" selected><?php echo($Nom_rubro) ?></option>
																	<?php
...
<label for="form-field-select-3">Seleccionar Sub-Rubro</label>
					<br />
<select class="chosen-select form-control"  name="Sel_Sub_Rubro" data-placeholder="Seleccionar Rubro...">

<option value="<?php echo($Id_subrubro) ?>" selected><?php echo($Nom_Subrubro) ?></option>
																	<?php
$sql ="SELECT Id_subrubro, Nom_Subrubro FROM t_subrubros WHERE Subrubro_publicado=1";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
    	$Id_subrubro=$row['Id_subrubro'];
    	$Nom_Subrubro=$row['Nom_Subrubro'];             
    	echo ("<option value='".$Id_subrubro."'>".utf8_encode($Nom_Subrubro)."</option>");
 }
}
...
<div id="info">
					...
					<div class="form-group">
							<label for="form-field-select-3">Editar fecha del Gasto :</label>
								<div class="input-group"> 
                                  <input required="true" class="form-control date-picker" id="id-date-picker-1" type="text" data-date-format="yyyy-mm-dd" name="TxtNuevaFecha" value="<?php Echo($Fecha_gasto)?>" />
                                  <span class="input-group-addon">
					...
					<div class="form-group">
							<label for="form-field-select-3">Editar Descripción :</label>
								<div class="input-group"> 
                                  <input required="true" class="form-control"  type="text" name="TxtDescripcion" value="<?php Echo($Descripcion)?>" />
                                  <span class="input-group-addon">
                                    <i class="fa fa-user bigger-110"></i>
                                  </span>
                                </div>
												...
												<a href="Informe-GastosTienda.php" class="btn btn-sm" data-dismiss="modal">
...
<div class="widget-body">
		<h4 class="widget-title lighter">
			...
			<div class="row">
				<form action="Informe-GastosTienda.php?QueryCon=<?php Echo($TxtConsultaVen) ?>" method="post" id="FormFechas" autocomplete="off">
					<div class="col-xs-12 col-sm-6">
...
<?php 
if ($TxtConsultaVen!="") {
	$sql ="SELECT DISTINCT(Id_Gasto) FROM t_gastos WHERE Fecha_gasto >='".$FechaStart."' and Fecha_gasto <='".$FechaEnd."' and  Tienda_Id_Tienda='".$TxtConsultaVen."' and  Area_Id_Area<>'' order by Id_Gasto  DESC";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
	$Lista=$Lista.$row['Id_Gasto'].",";                  
 }
}
}
else
{
	$sql ="SELECT DISTINCT(Id_Gasto) FROM t_gastos WHERE Fecha_gasto >='".$FechaStart."' and Fecha_gasto <='".$FechaEnd."' and  Area_Id_Area<>'' order by Id_Gasto  DESC";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
                                    ...
                                    <th class="success"></th>
												...
												<thead>
													...
													<tr class="warning">
														...
														<th class="tdcustom" >Beneficiario</th>
														<th class="tdcustom" >Detalle</th>
														<th class="tdcustom" >Rubro</th>
														<th class="tdcustom" >Soporte</th>
														<th class="tdcustom" >Valor</th>
														<th class="tdcustom" >Acción</th>
...
if ($TxtConsultaVen!="") {
	$sql ="SELECT date_format(Fecha_gasto,CONCAT(CONCAT(ELT(WEEKDAY(Fecha_gasto) + 1, 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo')),', %d - ',CONCAT(ELT(MONTH(Fecha_gasto), 'Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Agos', 'Sep', 'Oct', 'Nov', 'Dic')),'- %Y')) AS FechaGasto,Fecha_gasto, Id_Gasto, Beneficiario,Descripcion,Url_soporte,Valor_gasto, Marca_temporal,Tienda_Id_Tienda,Sub_rubro,Usuario_id_usuario FROM t_gastos WHERE Id_Gasto='".$Cadena[$i]."' and Tienda_Id_Tienda='".$TxtConsultaVen."'";
		//Echo($sql);
		$result = $conexion->query($sql);
}
else
{
	$sql ="SELECT date_format(Fecha_gasto,CONCAT(CONCAT(ELT(WEEKDAY(Fecha_gasto) + 1, 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo')),', %d - ',CONCAT(ELT(MONTH(Fecha_gasto), 'Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Agos', 'Sep', 'Oct', 'Nov', 'Dic')),'- %Y')) AS FechaGasto,Fecha_gasto, Id_Gasto, Beneficiario,Descripcion,Url_soporte,Valor_gasto, Marca_temporal,Tienda_Id_Tienda,Sub_rubro,Usuario_id_usuario FROM t_gastos WHERE Id_Gasto='".$Cadena[$i]."'";
		//Echo($sql);
		$result = $conexion->query($sql);
}
...
?>
			...
			<a data-rel="tooltip" data-placement="top" title="Detalle Compra" href="Pedido-Ver.php?PedidoCliente=<?php Echo($Cod_Pedido); ?>">
														...
														<td>
															<?php Echo utf8_encode(ObtenernombreUsuario($Usuario_id_usuario)); ?>
														</td>
														<td class="center">
															<?php Echo(ObtenernombreTienda($Tienda_Id_Tienda));?>		
														</td>
														<td class="center">
															<?php Echo($FechaGasto);?>		
														</td>
														...
														<td class="tdcustom">
															<?php Echo utf8_encode($Beneficiario); ?>	
														</td>
														...
														<td>
															<?php Echo utf8_encode(Obtenersubrubro($Sub_rubro)); ?>
														</td>
														
														
															<td>
															<a target="_blank" href="<?php Echo ($Url_soporte); ?>">
																<i class="fa fa-eye"> </i> Ver Soporte
															</a>
															
														</td>
														<td class="center">
															<?php Echo utf8_encode("$ ".number_format($Valor_gasto));  ?>
														</td>
														 <td>
			<div class="btn-group">
												...
												<ul class="dropdown-menu dropdown-info dropdown-menu-right">
													...
													<li>
														<a data-rel="tooltip" data-placement="top" title="Editar Gasto" href="Informe-GastosTienda.php?MiGasto=<?php Echo($Cadena[$i]);?>&EditTask2=1&start=<?php echo($FechaUno); ?>&end=<?php echo($FechaDos) ?>"><i class="fa fa-edit bigger-100 blue"></i> Editar Gasto</a>
													</li>
													<li>
														<a style="position: relative;" data-rel="tooltip" data-placement="top" title="Eliminar Gasto"  href="Informe-GastosTienda.php?NumGastoDel=<?php Echo($Cadena[$i]) ?>&start=<?php echo($FechaUno); ?>&end=<?php echo($FechaDos) ?>"><i class="fa fa-trash red "></i> Eliminar Gasto</a>
													</li>
													...
													else
													{
														?>
														<li>
														<a style="position: relative;" data-rel="tooltip" data-placement="top" title="No tienes permisos"  href=""><i class="fa fa-close red "></i> Sin Acciones</a>
													</li>
