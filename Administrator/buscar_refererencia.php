...
$consulta="Select * From t_referencias WHERE 
    CONCAT_WS(' ',`Cod_Referencia`,`Detalle_Referencia`,`Ref_Antigua`,`Detalle_Antiguo`)  
    LIKE '%".$q."%'
    ";
...
if ($resconsulta->num_rows > 0){
    while ($fila=$resconsulta->fetch_assoc()){
        ...
        $Cod_Referencia = $fila['Cod_Referencia'];
        $Img_Referencia = $fila['Img_Referencia'];
        $Detalle_Referencia = $fila['Detalle_Referencia'];
        $PVP_Ref = $fila['PVP_Ref'];
        ?>
        <li class="<?php echo($Nom_CategoriaIns);?>">
            <?php
                $ruta_img = utf8_encode($Img_Referencia);
                $mostrar_img = "miniatura.php?x=80&y=80&file=".$ruta_img;
            ?>
            <img style="width: 150px;height: 150px;" width="300" height="300" alt="300x300" data-name="<?php echo $Cod_Referencia; ?>" data-price="<?php echo $PVP_Ref; ?>" data-foto="<?php echo ($mostrar_img); ?>" data-unidad="<?php echo "Unidad"; ?>" src="<?php echo $mostrar_img; ?>" class="img-responsive product_drag" />
            <div class="tags">
                <span class="label-holder">
                    <span id="InsumoSel<?php echo $Id_Referencia;?>" data-id="<?php echo $Cod_Referencia;?>"  class="label label-success arrowed"><i class="fa fa-plus-square"> </i> Agregar</span>
                    <input style="display: none;" type="text" name="ValorCod" id="ValorCod<?php echo $Id_Referencia;?>" value="<?php echo $Cod_Referencia; ?>">
                    <input style="display: none;" type="text" name="NombreInsumo" id="NombreInsumo<?php echo $Id_Referencia;?>" value="<?php echo $Detalle_Referencia; ?>">
                    <input style="display: none;" type="text" name="UrlInsumo" id="UrlInsumo<?php echo $Id_Referencia;?>" value="<?php echo $Img_Referencia; ?>">
                    <input style="display: none;" type="text" name="Promedio" id="Promedio<?php echo $Id_Referencia;?>" value="<?php echo $PVP_Ref; ?>">
                    <input style="display: none;" type="text" name="Unidad" id="Unidad<?php echo $Id_Referencia;?>" value="<?php echo "Unidad"; ?>">
                </span>
                 <span class="label-holder">
                    <span class="label label-info">Cod:<?php echo $Cod_Referencia; ?></span>
                </span>
                <span class="label-holder">
                    <span class="label label-info"><?php echo $Detalle_Referencia; ?></span>
                </span> 
            </div>
        </li>
        <script type="text/javascript">
          $("#InsumoSel<?php echo $Id_Referencia;?>").click(function(){
            var ValorCod = $("#ValorCod<?php echo $Id_Referencia;?>").val();
            var Nombre = $("#NombreInsumo<?php echo $Id_Referencia;?>").val();
            var ImgInsumo = $("#UrlInsumo<?php echo $Id_Referencia;?>").val();
            var PromInsumo = $("#Promedio<?php echo $Id_Referencia;?>").val();
            var UnidadInsumo = $("#Unidad<?php echo $Id_Referencia;?>").val();
        ...
        url: "AgregarReferenciaAjax.php",//la url adonde se va a mandar la cadena a buscar
        ...
        {
        Echo("<h5>Lo sentimos no se encontraron resultados en su busqueda</h5>");
    }
}
          ?>
									</ul>
...
/*
//CODIGO NO VALIDO
$sql ="SELECT Id_Insumo,Cod_Insumo, Nom_CategoriaIns,Categoria_Id_Categoria_Insumo, 
Nom_Insumo, Unidad_Insumo, Url_Insumo, Detalle_Insumo from t_insumos as A, 
t_categorias_insumos as B WHERE A.Categoria_Id_Categoria_Insumo=B.Id_Categoria_Insumo
 and Concatenar_Bus LIKE '%".$q."%' and Tipo_Insumo='1'"; 
//Echo($sql);
...
<input style="display: none;" type="text" name="Unidad" id="Unidad<?php echo $Id_Insumo;?>" value="<?php echo $Unidad_Insumo; ?>">
													...
													<span class="label-holder">
														<span class="label label-info">Cod:<?php echo $Cod_Insumo; ?></span>
													</span>
  ...
  if (inputValue === false) return false;
  if (inputValue === "") {
    swal.showInputError("Â¡Tienes que indicar la cantidad!");
    return false
  }
  swal("Correcto!", "A solicitado: " + inputValue, "success");
           $.ajax({//metodo ajax
type: "POST",//aqui puede  ser get o post
url: "AgregarInsumosAjax.php",//la url adonde se va a mandar la cadena a buscar
data:{ValorCod:ValorCod, Nombre:Nombre,inputValue:inputValue,PromInsumo:PromInsumo,ImgInsumo:ImgInsumo,UnidadInsumo:UnidadInsumo}, 
//cache: false,
success: function(html){//funcion que se activa al recibir un dato
$("#tablainsumos").html(html).show();// funcion jquery que muestra el div con identificador display, como formato html, tambien puede ser .text
...
{
	Echo("<h5>Lo sentimos no se encontraron resultados en su bÃºsqueda</h5>");
}
}
          ?>
									</ul>
