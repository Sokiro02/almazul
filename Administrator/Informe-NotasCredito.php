...
<html lang="en">
	<head>
		...
		<meta charset="utf-8" />
		<title>Informe Notas Crédito Modasof</title>
...
<script src="https://modasof.com/espejo/assets/js/jquery.maskMoney.js" type="text/javascript"></script>
  ...
  $Valide=$_GET['Mensaje'];
    ...
    if ($Valide==352) {
        echo "<script>jQuery(function(){swal(\"¡ Nota Crédito Creada!\", \"Correctamente \", \"success\");});</script>";
    };
    if ($Valide==27) {
					...
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li>
								...
								<a href="Informe-NotasCredito.php">Informe Notas Crédito</a>
							</li>
							<li class="dropdown">
													...
													<ul class="dropdown-menu dropdown-info">
														<li>
				<a  href="Informe-NotasCredito.php">Ver todas las tiendas</a>
			</li>
														<?php
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
    	...
    	?>
    		<li>
				<a  href="Informe-NotasCredito.php?QueryCon=<?php Echo($SelectDown);?>&Mensaje=27"><?php Echo utf8_encode($NomDown); ?></a>
			</li>
    	<?php
 }
}
...
<div class="widget-body">
		<h4 class="widget-title lighter">
			...
			<div class="row">
				<form action="Informe-NotasCredito.php?QueryCon=<?php Echo($TxtConsultaVen) ?>" method="post" id="FormFechas" autocomplete="off">
...
if ($TxtConsultaVen=="") {
	$sql ="SELECT Distinct(Cliente_Id_Cliente) FROM t_devoluciones WHERE Fecha_Devolucion >='".$FechaStart."' and Fecha_Devolucion <='".$FechaEnd."'  ORDER BY Id DESC";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
	$Lista=$Lista.$row['Cliente_Id_Cliente'].",";                  
 }
}
}
...
{
	$sql ="SELECT Distinct(Cliente_Id_Cliente) FROM t_devoluciones WHERE Fecha_Devolucion >='".$FechaStart."' and Fecha_Devolucion <='".$FechaEnd."' and  Tienda_Id_Tienda='".$TxtConsultaVen."' order by Id  DESC";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
...
if ($TxtConsultaVen!="") {
	$sql ="SELECT date_format(Fecha_Devolucion,CONCAT(CONCAT(ELT(WEEKDAY(Fecha_Devolucion) + 1, 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo')),', %d - ',CONCAT(ELT(MONTH(Fecha_Devolucion), 'Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Agos', 'Sep', 'Oct', 'Nov', 'Dic')),'- %Y')) AS FechaDev,Id,A.Fecha_Devolucion, Num_Factura,Total_Devolucion,Observa_Usuario,A.Ref_Completa, B.Nombres,B.Apellidos,C.Nom_Tienda, D.Nom_Cliente,D.Ape_Cliente,A.notacredito_num FROM t_devoluciones as A, t_usuarios as B, t_tiendas as C, t_clientes as D WHERE A.Cliente_Id_Cliente='".$Cadena[$i]."' and A.Tienda_Id_Tienda='".$TxtConsultaVen."' and A.Cambio_Id_Usuario=B.Id_Usuario and A.Tienda_Id_Tienda=C.Id_Tienda and A.Cliente_Id_Cliente=D.Id_Cliente";
		//Echo($sql);
		$result = $conexion->query($sql);
}
else  
{
	$sql ="SELECT date_format(Fecha_Devolucion,CONCAT(CONCAT(ELT(WEEKDAY(Fecha_Devolucion) + 1, 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo')),', %d - ',CONCAT(ELT(MONTH(Fecha_Devolucion), 'Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Agos', 'Sep', 'Oct', 'Nov', 'Dic')),'- %Y')) AS FechaDev,Id,A.Fecha_Devolucion, Num_Factura,Total_Devolucion,Observa_Usuario,A.Ref_Completa, B.Nombres,B.Apellidos,C.Nom_Tienda, D.Nom_Cliente,D.Ape_Cliente,A.notacredito_num FROM t_devoluciones as A, t_usuarios as B, t_tiendas as C, t_clientes as D WHERE A.Cliente_Id_Cliente='".$Cadena[$i]."' and A.Cambio_Id_Usuario=B.Id_Usuario and A.Tienda_Id_Tienda=C.Id_Tienda and A.Cliente_Id_Cliente=D.Id_Cliente";
		//Echo($sql);
		$result = $conexion->query($sql);
}
...
$sql ="SELECT SUM(Total_Devolucion) as TotalDevoluciones FROM t_devoluciones WHERE Cliente_Id_Cliente='".$Cadena[$i]."'";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
...
?>
	...
	Echo ($Ref_Completa."<br/>");               
														...
														<td>
															<?php 
															$sql ="SELECT SUM(Total_Devolucion) as TotalDevoluciones FROM t_devoluciones WHERE notacredito_num='".$notacredito_num."'";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
	$TotalDevoluciones=$row['TotalDevoluciones']; 
	Echo utf8_encode("$ ".number_format($TotalDevoluciones));               
 }
}
															...
															<td>
															<?php Echo utf8_encode("$ ".number_format($TotalRedimido));  ?>
														</td>
														<td class="center">
															<?php Echo utf8_encode("$ ".number_format($SaldoNotas));  ?>
														</td>
