...
?>
			...
			<div class="main-content">
				...
				<p>
			<a href="Lista-Referencias.php?categoriaSel=todos&nomcategoria=Todas" style='margin:1px;' type='button' class='btn btn-white btn-danger'><strong>VER TODAS</strong></a>
			<a href="Lista-Referencias.php?categoriaSel=ninguna&nomcategoria=" style='margin:1px;' type='button' class='btn btn-white btn-danger'><strong>LIMPIAR FILTRO</strong></a>
				<?php 
...
//Echo($sql);
	...
	echo("<a href='Lista-Referencias.php?categoriaSel=".$Id_Cat_Producto."&nomcategoria=".$Nom_Cat_Producto."' style='margin:1px;' type='button' class='btn btn-white btn-success'>".utf8_decode($Nom_Cat_Producto)."</a>");
 }
...
elseif ($categoriaSel=="") {
	$sql ="SELECT Cod_Referencia FROM t_referencias order by Id_Referencia DESC LIMIT 0 , 20";
}
else
{
	$sql ="SELECT Cod_Referencia FROM t_referencias WHERE Categoria_Id_Categoria_Prod='".$categoriaSel."' order by Id_Referencia ASC";
} 
$result = $conexion->query($sql);
//Echo($sql);
if ($result->num_rows > 0) {
...
for($i=0; $i<$min; $i++)
{
	$sql ="SELECT * FROM t_referencias  WHERE Cod_Referencia='".$Cadena[$i]."'"; 
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
...
//Echo($sql);
 ...
 mysqli_free_result($result);
				 ...
				 ?>
											...
											if ($IdRol==1) {
												?>
												<input class="icheckbox_flat-green" type='checkbox' name='IdDespublicar[]' value='<?php Echo($Id_Referencia); ?>' />
										
												<button class="btn btn-success btn-small"><i class="fa fa-check"></i> Despublicar</button>
