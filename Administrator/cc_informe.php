...
<html lang="en">
	<head>
		...
		<meta charset="utf-8" />
		<title>Informe Cuentas Cobro Modasof</title>
					...
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li>
								...
								<a href="cc_informe.php">Informe Cuentas Cobro</a>
							</li>
                              ...
                              else
                              {
                              	...
                              	<?php
                                  	 ...
                                  	 <a class="btn btn-success" href=""><i class="fa fa-dollar bigger-110"> </i> Crear Abono</a>
                                  	 	...
                                  	 	<td colspan="3">
                                  	 <a class="btn btn-danger" href="cc_informe.php"><i class="fa fa-times bigger-110"> </i> Cerrar Vista</a>
                                  	 	</td>
                                  	 </tr>
                                  </tbody>
                         </table>
...
<div class="widget-body">
		<h4 class="widget-title lighter">
			...
			<div class="row">
				<form action="cc_informe.php?QueryCon=<?php Echo($TxtConsultaVen) ?>" method="post" id="FormFechas" autocomplete="off">
...
if ($MyIdTienda=="") {
	$sql ="SELECT Id_Venta FROM t_salidas_cc WHERE Fecha_Solicitud >='".$FechaStart."' and Fecha_Solicitud <='".$FechaEnd."' order by Factura_Id_Factura  DESC";
}else{
	$sql ="SELECT Id_Venta FROM t_salidas_cc WHERE Fecha_Solicitud >='".$FechaStart."' and Fecha_Solicitud <='".$FechaEnd."' and Tienda_Id_Tienda='".$MyIdTienda."' order by Factura_Id_Factura  DESC";
}
 ...
 ?>
                                    ...
                                    <th class="success"></th>
												...
												<thead>
													...
													<tr class="warning">
														<th class="tdcustom" >Cuenta Nº</th>
														...
														<th class="tdcustom" >Valor Prenda</th>
														<th class="tdcustom" >Cantidad</th>
														<th class="tdcustom" >Total</th>
...
for($i=0; $i<$min; $i++)
{
	$sql ="SELECT date_format(Fecha_Solicitud,CONCAT(CONCAT(ELT(WEEKDAY(Fecha_Solicitud) + 1, 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo')),', %d - ',CONCAT(ELT(MONTH(Fecha_Solicitud), 'Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Agos', 'Sep', 'Oct', 'Nov', 'Dic')),'- %Y')) AS FechaSolicitud, Cant_Solicitada, Valor_Final,Factura_Id_Factura, B.Nombres,B.Apellidos,C.Nom_Tienda, D.Nom_Cliente,D.Ape_Cliente,D.Documento_Cliente,Fecha_Solicitud,Ref_Vendida,Tienda_Id_Tienda,A.Cliente_Id_Cliente,A.proveedor_id_proveedor FROM t_salidas_cc as A, t_usuarios as B, t_tiendas as C, t_clientes as D WHERE Id_Venta='".$Cadena[$i]."' and A.Vendedor_Id_Usuario=B.Id_Usuario and A.Tienda_Id_Tienda=C.Id_Tienda and A.proveedor_id_proveedor=D.Id_Cliente";
		//Echo($sql);
		$result = $conexion->query($sql);
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
        ...
        $Num_Remision=$row['Factura_Id_Factura'];
        ...
        $Marca_Temporal=$row['Fecha_Solicitud'];
        $nomtienda=ObtenernombreTienda($Tienda_Id_Tienda);
        $observacion=ObtenerComentarioRemision($Num_Remision);
        ...
        $totalprendas=$Cant_Solicitada*$Valor_Final;
...
?>
														...
														<td class="center">
															<?php Echo utf8_encode("$ ".number_format($totalprendas));  ?>
														</td>
														<td class="center">
															<?php Echo($nomtienda);?>		
														</td>
	...
	<td>
			...
			<a target="_blank" class=" btn btn-xs btn-danger" data-rel="tooltip" data-placement="top" title="Imprimir Remisión" href="cc_Pdf.php?FacturaImp=<?php Echo($Num_Remision); ?>&mitienda=<?php echo($Tienda_Id_Tienda) ?>&cliente=<?php echo($Cliente_Id_Cliente) ?>&proveedor=<?php echo($proveedor_id_proveedor) ?>"><i class="fa fa-file-pdf-o bigger-100 white"></i></a>	
	</td> 
