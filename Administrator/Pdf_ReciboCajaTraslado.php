...
// Campo Número de Cliente //**************************************************************************************************************************************
        ...
        $pdf->Cell(20,3,'Fecha documento:',0,1,'L');
                ...
                $pdf->SetFont('Arial','', 8);
                $pdf->SetXY(147,33  );//Ubicación en PDF
                $pdf->Cell(20,3,utf8_decode($FechaPedido),0,1,'L');
...
$sql ="SELECT Id_Ingreso, Concepto_Ingreso, Medio_Pago,Num_Transaccion,Valor_Ingreso FROM t_ingresos WHERE Cod_Recibo_Caja='".$ReciboCajaImp."'";  
...
$pdf->AutoPrint();
                $pdf->Output("Facturas/ReciboCajaT.pdf",'F');




        echo "<script language='javascript'>window.open('Facturas/ReciboCajaT.pdf','_blank','');</script>";

        echo "<script language='javascript'>window.open('OrdenTrasladoCliente.php?Mensaje=333','_parent','');</script>";//para ver el archivo pdf generado
