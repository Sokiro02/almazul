...
<div class="widget-body">
		<h4 class="widget-title lighter">
			...
			<div class="row">
				<form action="pedidosfecha.php?QueryCon=<?php Echo($MyIdTienda) ?>" method="post" id="FormFechas" autocomplete="off">
...
if ($MyIdTienda=="") {
	$sql ="SELECT Id_Pedido FROM t_pedido Where Tienda_Id_Tienda<>'2' AND Tienda_Id_Tienda<>'3'AND Tienda_Id_Tienda<>'4'AND Tienda_Id_Tienda<>'5'AND Tienda_Id_Tienda<>'6' AND Tienda_Id_Tienda<>'7' AND Tienda_Id_Tienda<>'8' AND Tienda_Id_Tienda<>'9' AND Tienda_Id_Tienda<>'10' and Fecha_Pedido >='".$FechaStart."' and Fecha_Pedido <='".$FechaEnd."' order by Id_Pedido  DESC";
	//echo($sql);
	$result = $conexion->query($sql);
                                    ...
                                    <th class="success"></th>
												...
												<thead>
													...
													<tr class="warning">
														...
														<th class="tdcustom" style="width: 5%;">Nº</th>
														<th class="tdcustom" style="width: 5%;">Nº Prendas</th>
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
        ...
        $numeroprendaspedido=Cantidadprendas($Cod_Pedido);
...
if ($result->num_rows > 0) {
    ...
    echo($TiendaDespacho);
            ?>
														...
														<td>
															<?php 
    $sql ="SELECT F.Img_Referencia, Estado_Solicitud_Cliente, G.Nom_Estado_Pedido, A.Cant_Solicitada,A.Referencia_Id_Referencia FROM t_temporal_sol AS A, t_usuarios as B, t_bodegas as C, t_tallas as D, t_tiendas as E,t_referencias as F,t_estado_pedidos as G WHERE  A.Vendedor_Id_Usuario=B.Id_Usuario and A.Bodega_Id_Bodega=C.Id_Bodega and A.Talla_Solicitada=D.Id_Talla and A.Tienda_Id_Tienda=E.Id_Tienda and A.Referencia_Id_Referencia=F.Cod_Referencia  and Pedido_Id_Pedido='".$Cod_Pedido."' and A.Estado_Solicitud_Cliente=G.Id_Estado_Pedido  ORDER BY UNIX_TIMESTAMP(Fecha_Solicitud) DESC"; 
    //Echo($sql); 
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
...
<li>
   <a style="position: relative;" data-rel="tooltip" data-placement="top" title="<?php Echo utf8_encode($Nom_Estado_Pedido);?>" class="image-link" href="<?php echo utf8_encode($Img_Referencia); ?>"><img src="<?php echo utf8_encode($Img_Referencia); ?>" width="45px" height="45px" >
   		...
   		<?php echo ("Estado:".$Nom_Estado_Pedido) ;?>
   		<?php echo ("Ref:".$Referencia_Id_Referencia) ;?>
