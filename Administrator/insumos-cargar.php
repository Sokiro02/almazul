...
header("location:insumos-cargar.php?Mensaje=18");
}
...
$result = $conexion->query($sql);
header("location:insumos-cargar.php?Mensaje=15&TAB=tabs-3");
}
...
<html lang="en">
	<head>
		...
		<meta charset="utf-8" />
		<title>Cargar Insumos Modasof</title>
...
$(document).ready(function(){
   $("#TxtCategoria").change(function () {
	...
	<?php 
  $Valide=$_GET['Mensaje'];
  $CodInsumo=$_GET['CodInsumo'];

    if ($Valide==2) {
        echo "<script>jQuery(function(){swal(\"¡Cantidad en Cero!\", \"No es admitida\", \"error\");});</script>";
    };
    if ($Valide==1) {
        echo "<script>jQuery(function(){swal(\"¡ Insumo Creado!\", \"Correctamente \", \"success\");});</script>";
    };
     if ($Valide==111) {
        echo "<script>jQuery(function(){swal(\"¡ Insumo Cargado al Inventario!\", \"Correctamente \", \"success\");});</script>";
    };
    ...
    if ($Valide==112) {
        echo "<script>jQuery(function(){swal(\"¡ Se ajusto el inventario del Cód.!\", \"".$CodInsumo." \", \"success\");});</script>";
    };
     if ($Valide==11) {
...
function(isConfirm){
  if (isConfirm) {
    window.location.href = 'insumos-cargar.php?Confirmation=<?php Echo($DeleteInsumo); ?>';
  } else {
    window.location.href = 'insumos-cargar.php';
  }
});
// swal({
					...
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li>
								...
								<a href="insumos-cargar.php">Insumos</a>
							</li>
						</ul><!-- /.breadcrumb -->
				...
				?>
						<div class="row">
							...
							<div class="col-sm-6 col-xs-12">
								<form action="insumos-cargar.php" method="post">
									<div class="form-group">
															<label for="form-field-select-3">Seleccionar</label>
															<div>
												<select class="chosen-select input-xxlarge" name="EditTask" data-placeholder="Seleccionar...">
													<option value="">Seleccionar</option>
														<?php
$sql ="SELECT Id_Insumo, Cod_Insumo FROM t_insumos WHERE Tipo_Insumo='1' ORDER BY Cod_Insumo ASC";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
    	$SelectId_Insumo=$row['Id_Insumo'];
    	$SelectCod_Insumo=$row['Cod_Insumo'];             
    	echo ("<option value='".$SelectId_Insumo."'>".utf8_encode($SelectCod_Insumo)."</option>");
 }
}
...
<hr>
	...
	<?php 
$EditTask = $_POST['EditTask']; // Variable tomada de Tabla parte inferior
...
?>
		...
		<form action="insumo-guardarinventario.php" method="post" id="FormActualizarInsumo" enctype="multipart/form-data">
				...
				<input type="text" style="display: none;" name="TxtIdInsumo" value="<?php Echo($EditTask); ?>">
									...
									<div class="modal-dialog">
										<div class="modal-content ">
											<div class="modal-header">
												<a href="insumos-cargar.php" type="button" class="close" data-dismiss="modal">&times;</a>
												<h4 class="black bigger">Agregar Inventario a Código: <?php Echo($Qr_Cod_Insumo) ?></h4>
											</div>
									...
									<div class="form-group">
															<label for="form-field-select-3">Ingresar Inventario en unidad de medida <strong><?php Echo utf8_encode($Qr_Unidad_Insumo) ?></strong></label>

															<div>
	<input style="display: none;" type="text" name="TxtIdInsumo" name="Qr_Cod_Insumo" value="<?php Echo($EditTask)?>">
	<input style="display: none;" type="text" name="Qr_Cod_Insumo" name="Qr_Cod_Insumo" value="<?php Echo($Qr_Cod_Insumo)?>">
	<input style="display: none;" type="text" name="Qr_Unidad_Insumo" name="Qr_Cod_Insumo" value="<?php Echo($Qr_Unidad_Insumo)?>">
	<input type="number" step="any" name="CantidadInventario" class="input-lg" value="0">
															</div>

														
											</div>
									
									<?php Echo utf8_encode($Qr_Nom_CategoriaIns); ?>
									<br>
									<?php Echo utf8_encode($Qr_Nom_SubCategoriaIns); ?>
											<hr>							
											...
											<div class="modal-footer">
												<a href="Insumos-cargar.php" class="btn btn-sm" data-dismiss="modal">
...
$sql ="SELECT Distinct(Cod_Insumo_Cod) FROM t_inventario_telas WHERE Cantidad_Inv<>'0' order by Id_inventario_tela Asc";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
$ListaInsumos=$ListaInsumos.$row['Cod_Insumo_Cod'].",";                  
 }
}
                                   ...
                                   <th class="success"></th>
												...
												<thead>
													...
													<tr class="warning">
														...
														<th class="tdcustom" >Categoria</th>
														<th class="tdcustom" >Cod Insumo</th>
														<th class="tdcustom" >Cantidad</th>
														<th class="tdcustom" >Unidad</th>
														<th class="tdcustom" >Valor Promedio</th>
														<th class="tdcustom" >Total Inventario</th>
														<th class="tdcustom" style="width: 15%">Ajustar</th>
...
for($i=0; $i<$min; $i++)
{
$sql ="SELECT Id_inventario_tela,Cod_Insumo_Cod,Cantidad_Inv,Medida_Inv FROM t_inventario_telas WHERE Cod_Insumo_Cod='".$CadenaInsumos[$i]."'"; 
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
    	$Id_inventario_tela=$row['Id_inventario_tela'];               
        $Cod_Insumo_Cod=$row['Cod_Insumo_Cod'];
        $Cantidad_Inv=$row['Cantidad_Inv'];
        $Medida_Inv=$row['Medida_Inv'];
													 ...
													 ?>
														...
														<tr>
														<td>
															<?php echo utf8_encode($Id_inventario_tela); ?>	
														</td>
														<td>
<?php 
	$sql ="SELECT Nom_CategoriaIns FROM t_categorias_insumos as A, t_insumos as B WHERE  B.Cod_Insumo='".$CadenaInsumos[$i]."' and B.Categoria_Id_Categoria_Insumo=A.Id_Categoria_Insumo"; 
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
...
$sql ="SELECT Nom_SubCategoriaIns FROM t_subcategorias_insumos as A, t_insumos as B WHERE  B.Cod_Insumo='".$CadenaInsumos[$i]."' and B.SubCategoria_Id_SubCategoria_Insumo=A.Id_SubCategoria_Insumo"; 
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
    	$Nom_SubCategoriaIns=$row['Nom_SubCategoriaIns'];                
	}
}


 ?>
 <?php echo strtoupper(utf8_encode($Nom_CategoriaIns." ".$Nom_SubCategoriaIns)); ?>	
														</td>
														<td class="center">
															<?php echo strtoupper(utf8_encode($Cod_Insumo_Cod)); ?>	
														</a>
														</td>
														
														<td class="center">
															<?php Echo strtoupper(round($Cantidad_Inv,2)) ?>		
															</td>
														<td>
															<?php Echo strtoupper(utf8_encode($Medida_Inv)) ?>
														</td>
														<td>
															<?php 
	$sql ="SELECT AVG(Costo_Insumo) as CostoPromedio FROM t_insumos as A, t_inventario_telas as B WHERE A.Cod_Insumo='".$CadenaInsumos[$i]."' and A.Cod_Insumo=B.Cod_Insumo_Cod"; 
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
        ...
        $CostoPromedio=$row['CostoPromedio'];
								 ...
								 ?>	
												...
												<?php echo (formatomoneda($CostoPromedio)); ?>
														
														</td>

													
															<?php 
															$TotalInventario=$CostoPromedio*$Cantidad_Inv;
															 ?>
															
														
														<td class="center">
															<?php Echo utf8_encode("$ ".number_format($TotalInventario));  ?>
														</td>
														<td>
									<form action="insumo-ajustarinventario.php" method="post">
			<input style="display: none;" type="text" name="IdInventario" value="<?php Echo($Id_inventario_tela) ?>">
			<input type="number" step="any" name="ActualizarInventario" class="input-small" value="<?php Echo($Cantidad_Inv)?>">
			<input style="display: none;" type="text" name="CodInsumo" class="input-small" value="<?php Echo($Cod_Insumo_Cod)?>">
