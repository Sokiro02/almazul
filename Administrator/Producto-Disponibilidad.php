...
color:#FFF;
  ...
  $Valide=$_GET['Mensaje'];
    ...
    if ($Valide==21) {
     ...
     if ($Valide==22) {
        echo "<script>jQuery(function(){swal(\"Lo sentimos, nuestro sistema ha Bloqueado Módulo Pedidos!\", \"Por favor contactarse con Gerencia Modasof\", \"error\");});</script>";
    };
    ...
    while ($row = $result->fetch_assoc()) {   
      ...
      echo ("<option value='".$SelectId_Talla."'>".utf8_encode($SelectNom_Talla)."</option>");
                    ...
                    <select class="chosen-select input-xxlarge" id="TxtBodega" name="TxtBodega" data-placeholder="Seleccionar...">
                      <option value="">Seleccionar ...</option>
                                    <?php 
$sql ="SELECT Id_Tienda, Nom_Tienda FROM t_tiendas order by Cod_tienda ASC";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
...
if ($result->num_rows > 0) {
 ...
 -->
                            ...
                            $ruta_img = utf8_encode($Img_Referencia);
                                                        $mostrar_img = "miniatura.php?x=100&y=100&file=".$ruta_img;
                            ?>
                          <img src="<?php echo $mostrar_img; ?>" width="150px" height="150px">
                                        </div>
                                        <h4>P.V.P <?php Echo(Formatomoneda($PVP_Ref)); ?></h4>
                                      
                                        <div>
                                          <a  data-toggle="modal" data-target="#modal-form2" href="#"><span class="btn  btn-warning "><i class="fa fa-bell"> </i> Notificar a Tienda</span></a>
                                          <hr>
                                          <a href="Produccion-SolicitudCliente.php?RefSel=<?php Echo($RefSel); ?>&OrdenCorte=900" class="btn btn-danger"><i class="fa fa-cut bigger-170"></i> Solicitar a Producción</a>
                                          <hr>
                                           <a  href="traslados.php"><span class="btn  btn-success "><i class="fa fa-exchange"> </i> Solicitar Traslado</span></a>
                                          <hr>
  ...
  $sql="SELECT Id_Tienda FROM t_tiendas Where Estado_Tienda='1' order by Cod_tienda ASC";
  $result=$conexion->query($sql);
  if ($result->num_rows >0) {
    while ($row = $result->fetch_assoc ()) {
      ...
      else
      {
        ...
        <?php
           ...
           ?>   
              ...
              <?php 
              $sql="SELECT  IFNULL(sum(Cant_Solicitada),0) as TotalProduccion FROM t_temporal_sol as A WHERE Referencia_Id_Referencia='".$RefSel."' and Estado_Solicitud_Cliente='9' And Tienda_Id_Tienda='".$CadenaTiendas[$i]."'";
  $result=$conexion->query($sql);
      ...
      $TotalProduccion=$row['TotalProduccion'];  
        Echo($TotalProduccion." Und.");                
               ...
               ?>
                ...
                <tr>
        <?php 
            $sql="SELECT  IFNULL(sum(Cant_Solicitada),0) as TotalporTalla,B.Nom_Talla FROM t_temporal_sol as A, t_tallas as B WHERE Referencia_Id_Referencia='".$RefSel."'  and A.Talla_Solicitada=B.Id_Talla  And Tienda_Id_Tienda='".$CadenaTiendas[$i]."' and Estado_Solicitud_Cliente='9' GROUP BY Talla_Solicitada";
          // Echo($sql);
  $result=$conexion->query($sql);
  if ($result->num_rows >0) {
    while ($row = $result->fetch_assoc ()) {
      ...
      else
      {
        ...
        <?php
           ...
           ?>
               ...
               <tr>
       <?php 
            $sql="SELECT  SUM(Cant_Solicitada) as TotalporTalla,B.Nom_Talla FROM t_temporal_sol as A, t_tallas as B WHERE Referencia_Id_Referencia='".$RefSel."'  and A.Talla_Solicitada=B.Id_Talla  And Tienda_Id_Tienda='".$CadenaTiendas[$i]."' and Estado_Solicitud_Cliente='9' GROUP BY Talla_Solicitada";
            //Echo($sql);
  $result=$conexion->query($sql);
  if ($result->num_rows >0) {
    while ($row = $result->fetch_assoc ()) {
