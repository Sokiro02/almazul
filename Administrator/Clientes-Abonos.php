      ...
      $Detienda=$_GET['Detienda'];
			if ($AbonosFactura!="") {
			 ?>
				...
				<?php 
				$sql ="SELECT B.Id_Cliente, B.Nom_Cliente,B.Ape_Cliente FROM t_facturas as A, t_clientes as B WHERE Num_Factura='".$AbonosFactura."' and A.Cliente_Id_Cliente=B.Id_Cliente and A.Tienda_Id_Tienda='".$Detienda."'";
	//Echo($sql);
...
$sql ="SELECT Total_Factura FROM t_facturas WHERE Num_Factura='".$AbonosFactura."' and Tienda_Id_Tienda='".$Detienda."'";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
...
$sql ="SELECT IFNULL(sum(Valor_Ingreso),0) as TotalAbonos FROM t_ingresos WHERE Factura_Cod_Factura='".$AbonosFactura."' and Tienda_Id_Tienda='".$Detienda."'";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
