...
$sql ="SELECT * FROM t_separados as A, t_clientes as B, t_usuarios as C WHERE Num_Separe='".$NumeroFactura."' and A.Cliente_Id_Cliente=B.Id_Cliente and A.Usuario_Vendedor=C.Id_Usuario";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
	$Fecha_Factura=$row['Fecha_Separe'];                  
	$Num_Factura=$row['Num_Separe'];   
	...
	$Total_Factura=$row['Total_Separe']; 
	$Factura_Paga=$row['Separe_Paga']; 
	if ($Factura_Paga==3) {
	  	$Pagode="Facturado";
	  } 
 }
...
<html lang="en">
	<head>
		...
		<meta charset="utf-8" />
		<title>Detalle Separado Modasof</title>
...
<h5><i class="fa fa-dollar"></i> Separado Número: PLS<?php Echo utf8_encode($Num_Factura) ?></h5>
<!-- <h5><i class="fa fa-phone"></i> <?php Echo utf8_encode($Qr_Cel1_Cliente."-".$Qr_Cel2_Cliente) ?></h5> -->
<h5><i class="fa fa-calendar"></i> Fecha Separe: <?php Echo (FechaSql($Fecha_Factura)) ?> </h5>
...
<div class="col-sm-4 center">
	<h5>SEPARADO NÚMERO: PLS<?php Echo utf8_encode($Num_Factura) ?></h5>
...
<div class="infobox infobox-green">
	...
	<div class="infobox-data">
		...
		<div class="infobox-content">Total Separado</div>
	</div>
	<?php 
	if ($Pagode=='Facturado') {
	?>
	<h4>Plan Separe Facturado</h4>
	<?php
	}
	else
	{
		?>
		 <a href="FacturarPlanSepare.php?NumeroFactura=<?php Echo($NumeroFactura); ?>" type="submit" class="btn btn-danger btn-outline"><i class="fa fa-print"> </i> Facturar e Imprimir</a>
		<?php
	}
...
<!-- Fin de Primer Fila -->
							...
							<form action="testrr.php" method="post">
							<!-- INICIO TABLA -->
						<div class="clearfix">
											<!-- <div class="pull-left tableTools-container"></div> -->
										</div>
										<div class="table-header">
											Detalle de Separado Nº: PLS<?php Echo($Num_Factura); ?>
										</div>
...
<?php 
	...
	$sql ="SELECT * FROM t_plansepare as A, t_referencias as B WHERE Factura_Id_Factura='".$Num_Factura."' and A.Referencia_Id_Referencia=B.Cod_Referencia"; 
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
