...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
        ...
        $Tienda_Id_Tienda1=$row['Tienda_Id_Tienda'];
...
$SqlActualizar ="UPDATE t_ventas SET Estado_Devolucion='1' WHERE Id_Venta='".$MiVenta."'"; //ACTUALIZAMOS EL INVENTARIO
...
header("location:Clientes-Devoluciones.php?Mensaje=111&NumeroFactura=".$Factura_Id_Factura."");


}

$sql ="SELECT * FROM t_facturas as A, t_clientes as B, t_usuarios as C WHERE Num_Factura='".$NumeroFactura."' and A.Cliente_Id_Cliente=B.Id_Cliente and A.Usuario_Vendedor=C.Id_Usuario";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
...
<html lang="en">
	<head>
		...
		<meta charset="utf-8" />
		<title>Crear Devolución Factura</title>
...
<script src="https://modasof.com/espejo/assets/js/ImageSelect.jquery.js"></script>
  ...
  $Valide=$_GET['Mensaje'];
    ...
    if ($Valide==1) {
     ...
     if ($Valide==333) {
        echo "<script>jQuery(function(){swal(\"¡ Cambio por talla realizado!\", \"Correctamente \", \"success\");});</script>";
    };
      if ($Valide==332) {
        echo "<script>jQuery(function(){swal(\"¡ Prenda Cargada a nota Crédito!\", \"No olvide guardar la Nota Crédito \", \"success\");});</script>";
    };
     if ($Valide==222) {
        echo "<script>jQuery(function(){swal(\"¡ Factura Anulada Correctamente!\", \"Correctamente \", \"success\");});</script>";
    };
    if ($Valide==23) {
					...
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li>
								...
								<!-- <a href="Informe-Facturas.php">Regresar</a> -->
							</li>
						</ul><!-- /.breadcrumb -->
...
<h5><i class="fa fa-dollar"></i> Salida Contable Nº: <?php Echo utf8_encode($num_consecutivo_sc) ?></h5>
<!-- <h5><i class="fa fa-phone"></i> <?php Echo utf8_encode($Qr_Cel1_Cliente."-".$Qr_Cel2_Cliente) ?></h5> -->
<h5><i class="fa fa-calendar"></i> Fecha Factura: <?php Echo (FechaSql($Fecha_Factura)) ?> </h5>
...
<div class="col-sm-4 center">
	...
	<h5>SALIDA CONTABLE Nº: <?php Echo utf8_encode($num_consecutivo_sc) ?></h5>
...
<div class="infobox infobox-green">
 ...
 <form action="Facturas-guardarNotaCr.php" method="post" Id="FormCambioTalla">

<div class="col-sm-4 center">
	<?php 

	// Número Factura
 $sql ="SELECT Consecutivo_Notas FROM t_config_tienda WHERE Tienda_Id_Tienda='1'";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
      $Consecutivo_Notas=$row['Consecutivo_Notas']+1;
      ?>
      <input style="display: none;" type="text" name="TxtConsecutivonotas" value="<?php Echo($Consecutivo_Notas);?>">
      <?php
    }
  }
	 ?>

	 <input style="display: none;" type="text" name="TxtNumFactura" value="<?php Echo($num_consecutivo_sc);?>">
	<input style="display: none;" type="text" name="TxtTienda" value="<?php Echo($Tienda_Id_Tienda);?>">

	<h5 style="display: none;">NOTA CRÉDITO: <?php Echo utf8_encode($Consecutivo_Notas) ?></h5>
	<button type="submit" class="btn btn-danger btn-xs"><i class="fa fa-save"></i> Guardar</button>	

<?php 

 $sql ="SELECT SUM(Total_Devolucion) as TotalDevoluciones FROM t_devoluciones WHERE Cliente_Id_Cliente='".$Cliente_Id_Cliente."' and Num_Factura='".$num_consecutivo_sc."'";
    //Echo($sql);
    $result = $conexion->query($sql);
    if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
...
<div class="infobox infobox-red">
	...
	<div class="infobox-data">
		<span class="infobox-data-number"><?php Formatomoneda($TotalDevoluciones); ?></span>
		<div class="infobox-content">Total Nota Crédito</div>
	</div>
...
<!-- Fin de Primer Fila -->
						...
						<div class="clearfix">
										...
										<div class="table-header">
											Detalle de Salida Nº: <?php Echo($num_consecutivo_sc); ?>
										</div>
										...
										<div>
<?php 
	$sql ="SELECT Id_Venta FROM t_ventas WHERE Factura_Id_Factura='".$Num_Factura."' and Tienda_Id_Tienda='".$Tienda_Id_Tienda."'";
	Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
							...
							<table style="font-size: 12px;" id="dynamic-table2"  class="table table-responsive table-striped table-bordered table-hover" width="100%">
												<thead>
													<tr class="warning">
														...
														if ($IdRol==1) {
														 ?>
														<th class="tdcustom" style="width: 30%;">Nota Crédito</th>
														<?php 
													}
														 ?>
													</tr>
...
for($i=0; $i<$min; $i++)
{
	$sql ="SELECT * FROM t_ventas as A, t_referencias as B WHERE Id_Venta='".$Cadena[$i]."' and A.Referencia_Id_Referencia=B.Cod_Referencia"; 
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
...
Echo("<span class='green'>Devolución Creada</span>");
$sql ="SELECT id,Observa_Usuario,notacredito_num,Cambio_Id_Usuario,fecha_guardada FROM t_devoluciones Where Venta_Id_Venta='".$Cadena[$i]."'"; 
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
        $Observa_Usuario=$row['Observa_Usuario'];
         $idnotacredito=$row['id'];
         $notacredito_num=$row['notacredito_num'];
         $Cambio_Id_Usuario=$row['Cambio_Id_Usuario'];
         $fecha_guardada=$row['fecha_guardada'];

         $nombreusuario=ObtenernombreUsuario($Cambio_Id_Usuario);

       Echo ("<br><strong>Causal:</strong>".utf8_encode($Observa_Usuario));
	}
}
// Validamos si ya le dieron guardar o no a la nota crédito 
	if ($notacredito_num==0) {
		Echo("<br><strong><span class='red'><i class='fa fa-warning'> </i> NO HA GUARDADO LA NOTA CRÉDITO</span></strong>");
	}
	else
	{
		Echo("<br><strong><span class='green'><i class='fa fa-check'> </i>NOTA CRÉDITO NÚMERO :".$notacredito_num."</span><br>Guardada por : ".$nombreusuario."<br>Fecha: ".$fecha_guardada."</strong>");
	}

?>
<form action="Facturas-cancelarNotaCr.php" method="post" Id="FormNotaCr">
	<input style="display: none;" type="text" name="TxtTallaSolicitada" value="<?php Echo($Talla_Solicitada)?>">
	<input style="display: none;" type="text" name="TxtIdVenta" value="<?php Echo($Id_Venta)?>">
	<input style="display: none;" type="text" name="TxtFactura" value="<?php Echo($num_consecutivo_sc)?>">
	<?php // Cuidado envío Salida contable con dato de Factura ?>
	<input style="display: none;" type="text" name="TxtSalidacontable" value="<?php Echo($NumeroFactura)?>">
	<input style="display: none;" type="text" name="TxtReferencia" value="<?php Echo($Referencia_Id_Referencia)?>">
	<input style="display: none;" type="text" name="TxtRefVendida" value="<?php Echo($Ref_Vendida)?>">
	<input style="display: none;" type="text" name="TxtTienda" value="<?php Echo($Tienda_Id_Tienda)?>">
	<input style="display: none;" type="text" name="TxtValor" value="<?php Echo($Valor_Final)?>">
	<input style="display: none;" type="text" name="TxtCliente" value="<?php Echo($Cliente_Id_Cliente)?>">
	<input style="display: none;" type="text" name="TxtIdNotacredito" value="<?php Echo($idnotacredito)?>">
	
	<input style="display: none;" class="input-sm" type="text" name="TxtCantidad" value="<?php Echo($Cant_Solicitada)?>">


		<button type="submit" class="btn btn-danger btn-xs"><i class="fa fa-close"></i> Eliminar Prenda en Nota Crédito</button>
	</form>
<?php
												}
												else{
												 ...
												 ?>
<form action="Facturas-NotaCr.php" method="post" Id="FormNotaCr">
	<input style="display: none;" type="text" name="TxtTallaSolicitada" value="<?php Echo($Talla_Solicitada)?>">
	<input style="display: none;" type="text" name="TxtIdVenta" value="<?php Echo($Id_Venta)?>">
	<input style="display: none;" type="text" name="TxtFactura" value="<?php Echo($num_consecutivo_sc)?>">
	<?php // Cuidado envío Salida contable con dato de Factura ?>
	...
	<input class="input-sm" type="text" name="TxtCantidad" value="<?php Echo($Cant_Solicitada)?>">

	<select required="true" name="TxtObserva">
			<option value="">Seleccionar Causal</option>
																	<?php 
$sql ="SELECT id_causal,nombre_causal FROM t_causales ORDER BY nombre_causal ASC"; 
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
        $id_causal=$row['id_causal'];
        $nombre_causal=$row['nombre_causal'];
       Echo("<option value='".$nombre_causal."'>".$nombre_causal."</option>");
	}
}
																	 ?>
																</select>
	<button type="submit" class="btn btn-success btn-xs"><i class="fa fa-refresh"></i> Cargar Nota Crédito</button>
															</form>
															<?php } ?>
														</td>
														<?php 
