...
if ($Delete!="") {
	$sql="DELETE FROM t_plansepare  WHERE Id_Venta='".$Delete."'";
	$result=$conexion->query($sql);
	header("CrearPlanSepare.php?Mensaje=1");
}
...
// if ($CancelVenta=="true") {
// 	$sql="DELETE FROM t_plansepare  WHERE Vendedor_Id_Usuario='".$IdUser."'";
// 	$result=$conexion->query($sql);
...
color:#FFF;
  ...
  $GetFactura=$_GET['Factura'];
    ...
    if ($Valide==333) {
        echo "<script>jQuery(function(){swal(\"¡ Plan Separe Creado Correctamente!\", \"SEPARE. Nº".$GetFactura." \", \"success\");});</script>";
    };
   ?>
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
      $Consecutivo_PlanSepare=$row['Consecutivo_PlanSepare']+1;
                                ...
                                <h3 class="m-b-0 text-white">PLAN SEPARE Nº <?php Echo($Consecutivo_PlanSepare) ?> <a  data-toggle="modal" data-target="#modal-form3" href="#"><span class="btn  btn-success btn-xs"><i class="fa fa-search"> </i> Ver Tallas</span></a> <br><small><strong>Vendedor: <?php Echo utf8_encode($NomSesion) ?></strong></small> </h3>


              <form action="PruebaEnterSepare.php" method="post" autocomplete="off">
...
$sql="SELECT C.Nom_Talla,Id_Venta, Cant_Solicitada, Talla_Solicitada, Tienda_Id_Tienda, A.Referencia_Id_Referencia, Fecha_Solicitud, Vendedor_Id_Usuario, Valor_Prenda,Valor_Final,B.Img_Referencia FROM t_plansepare as A, t_referencias as B, t_tallas as C  WHERE A.Referencia_Id_Referencia=B.Cod_Referencia and A.Talla_Solicitada=C.Id_Talla and Vendedor_Id_Usuario='".$IdUser."' and Estado_Venta='1' order by Referencia_Id_Referencia Desc";
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
...
//$VrDescuento=$CalculoDescuento*100;
                                     ?>
                                            ...
                                            <form action="Actualizar-Carrito2.php" method="post">
...
symbolPosition : 'left' // use this setting to position the symbol at the left or right side of the value. default 'left'
                                                ...
                                                <td style="padding-top: 20px;" align="center">
                                                	<a href="CrearPlanSepare.php?Delete=<?php Echo($Id_Venta); ?>&Mensaje=1" data-rel="tooltip" data-placement="top" title="Eliminar Referencia"  class="text-inverse btn-xs" title="" data-toggle="tooltip" data-original-title="Delete"><i class="fa fa-trash bigger-150 red"></i></a>
...
$sql ="SELECT SUM(Valor_Final*Cant_Solicitada) as TotalPedido FROM t_plansepare WHERE Vendedor_Id_Usuario='".$IdUser."' and Estado_Venta='1'";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
...
$sql ="SELECT SUM(Cant_Solicitada) as SumaPrendas FROM t_plansepare WHERE Vendedor_Id_Usuario='".$IdUser."'and Estado_Venta='1'";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
...
<form method="post" action="GuardarPlanSepare.php" id="FormGuardarventa">
...
// Detalle Plan Separe
    ...
    $sql ="SELECT Id_Venta,Referencia_Id_Referencia,Talla_Solicitada,Ref_Vendida, Cant_Solicitada FROM t_plansepare WHERE Vendedor_Id_Usuario='".$IdUser."' and Estado_Venta='1'";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
      $Id_Venta=$row['Id_Venta'];
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
      $Consecutivo_PlanSepare=$row['Consecutivo_PlanSepare']+1;
      ?>
      <input style="display: none;" type="text" name="FactNum" value="<?php Echo($Consecutivo_PlanSepare);?>">
      <?php
    }
  }
...
<hr>
                               ...
                               <hr>
                                 <button type="submit" class="btn btn-danger btn-outline"><i class="fa fa-print"> </i> Guardar Plan Separe</button>
                                 </form>
...
<!-- Inicio Modal -->
    ...
    <div>
              ...
              <form action="Cliente-Crear.php?ForVenta=6" method="post" id="FormNuevoCliente" autocomplete="off">
                  <div class="modal-dialog ">
                    <div class="modal-content">
                      <div class="modal-header">
                        <a href="CrearPlanSepare.php" type="button" class="close" data-dismiss="modal">&times;</a>
                      ...
                      <div class="modal-footer">
                        <a href="CrearPlanSepare.php" class="btn btn-sm btn-danger" data-dismiss="modal">
                      ...
                      <div class="modal-body">
                        <div class="row">
                          <div class="col-xs-12 col-sm-6 ">
                            ...
                            <?php
$sql ="SELECT Id_Talla, Nom_Talla FROM t_tallas  order by Nom_Talla DESC LIMIT 9";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
