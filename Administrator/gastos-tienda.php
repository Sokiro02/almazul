				...
				<div class="main-content-inner">
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li class="active"><a href="Gastos-Tienda.php"><i class="fa fa-dollar"> </i> Gastos</a></li>
						</ul><!-- /.breadcrumb -->
					...
					<!-- 	<div class="page-header">
						...
						<div class="row">
							<div class="col-xs-12">
								<!-- PAGE CONTENT BEGINS -->
								<form class="form-horizontal" role="form" id="FormGastos" method="post" action="Guardargasto.php" enctype="multipart/form-data">
								...
								<div>
									<label for="form-field-select-3">Seleccione de donde sale el gasto: </label>

															<br />
															<select class="chosen-select form-control"  name="TipoSalida" data-placeholder="Seleccionar Rubro...">
																<option value="">Seleccionar</option>
																<option value="1">Caja de la Tienda</option>
																<option value="2">Cuenta de Modasof</option>
															</select>
														</div>
...
if ($FechaDos=="") {
	...
	$sql ="SELECT Valor_Gasto FROM t_gastos WHERE Tienda_Id_Tienda='".$MyIdTienda."' and Fecha_Gasto >='".$FechaStart."' and Fecha_Gasto <='".$FechaEnd."' order by Id_Gasto  DESC";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
	$Lista=$row['Valor_Gasto']+$Lista;                  
 }
}
?>
					...
					<div class="col-xs-12 col-sm-6">
											<div class="widget-box">
												<div class="widget-header">
													<h4 class="widget-title">Total Gastos:<?php Echo(formatomoneda($Lista)); ?> // <?php echo ($MarcaTemporal);  ?></h4>
...
$sql="SELECT Valor_Gasto,Marca_Temporal,Id_Gasto,Descripcion,Fecha_Gasto FROM t_gastos WHERE Tienda_Id_Tienda='".$MyIdTienda."' and Fecha_Gasto >='".$FechaStart."' and Fecha_Gasto <='".$FechaEnd."' order by Id_Gasto  DESC";
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
        ...
        $Descripcion=$row['Descripcion'];
							 ...
							 ?>	
      <li><a href="#">
      	<strong><?php Echo utf8_encode($Id_Gasto); ?>-Reportado el: <?php echo($Reportado); ?></strong><br>
      	<?php echo($Descripcion) ?>
      	<br>
      	Fecha gasto:<?php echo($Fecha_Gasto);?>
      	<span class="pull-right badge bg-blue"><?php Echo formatomoneda($Valor_Gasto); ?></span></a></li>
              ...
              <a class="btn btn-success" href="Informe-GastosTienda.php">Ver todos los gastos</a>
            </div>
          </div>
          <!-- /.widget-user -->
        </div>
