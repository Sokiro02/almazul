...
if (isset($_POST['enviar'])) {
	if (is_array($_POST['TxtCod'])) {
		$selected = '';
		$cantidad_codigos = count($_POST['TxtCod']);
        $codigos = $_POST['TxtCod'];
		$current = 0;
		foreach ($_POST['cant_dev'] as $key => $value) {
		  $cant_devolver[] = $value; //GUARDA LAS CANTIDADES A DEVOLVER
		}
    }
}
?>
...
<html lang="en">
	<head>
		...
		<meta charset="utf-8" />
		<title>Crear Devolucion Modasof</title>
...
function Traslados(id,accion) {
    if(accion=='GUARDAR'){
        if ( confirm("Estas seguro de realizar la Devolución?") ) {
                $.post("Clientes-CrearDevolucionGuardar.php", { Codigo: id, Accion: accion}, function(data){
                    if(data.trim() == "true"){
                        alert("Devolución realizada con exito"+data);
                        location.href = "www.google.com"; 
                        //location.reload(true);
                    } else {
                        alert("Error al devolver la referencia"+data);
                        location.href = "www.google.com"; 
                    }
                });          
            return true;
        ...
        if ( confirm("Estas seguro de CANCELAR la Devolución?") ) {
                $.post("Clientes-CrearDevolucionGuardar.php", { Codigo: id, Accion: accion}, function(data){
                    if(data.trim() == "true"){
                        alert("Devolución CANCELADA con exito");
                        location.href = "Informe-VentasTienda.php"; 
                        //location.reload(true);
                    } else {
                        alert("Error al devolver la referencia"+data);
                    }
					...
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li>
								...
								<a href="Informe-VentasTienda.php">Regresar</a>
							</li>
						</ul><!-- /.breadcrumb -->
...
<h5><img style="width: 15%;height: 15%;" class="nav-user-photo" src="../Administrator/<?php Echo utf8_encode($Img_Perfil); ?>" alt="Jason's Photo" /> <?php Echo utf8_encode($Nom_Vendedor." ".$Ape_Vendedor); ?></h5>
...
<h5><i class="fa fa-dollar"></i> Factura Número: <?php Echo utf8_encode($Num_Factura) ?></h5>
...
<h5><i class="fa fa-money"></i> Pago de: <?php Echo utf8_encode($Pagode) ?> </h5>
...
<div class="col-sm-4 center">
	<h5>FACTURA NÚMERO: <?php Echo utf8_encode($Num_Factura) ?></h5>
...
<div class="infobox infobox-green">
	...
	<div class="infobox-data">
		...
		<div class="infobox-content">Total Factura</div>
	</div>
...
<!-- Fin de Primer Fila -->
						...
						<div class="clearfix">
										...
										<div class="table-header">
											Seleccione las prendas que requieren cambio de la Factura Nº: <?php Echo($Num_Factura); ?>
										</div>
							...
							<table style="font-size: 12px;" id="dynamic-table2"  class="table table-responsive table-striped table-bordered table-hover" width="100%">
												<thead>
													<tr class="warning">
                                                        ...
                                                        <th class="tdcustom" style="width: 5%;">Cant. Devolver</th>
                                                        <th class="tdcustom" style="width: 5%;">Valor. Devolver</th>
													</tr>
...
<?php
$i=0;
foreach ($_POST['TxtCod'] as $key => $value) { //Busqueda de Referencias Seleccionadas (Id_Venta))
	$sql ="SELECT * FROM t_ventas as A, t_referencias as B WHERE Factura_Id_Factura='".$Num_Factura."' and Id_Venta = '".$value."' and A.Referencia_Id_Referencia=B.Cod_Referencia"; 
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
			...
			?>
													...
													<tr>
														<td>
                                                            ...
                                                            <input style="display: none;" type="text" name="idventa[]" value="<?php echo utf8_encode($Id_Venta); ?>">
														</td>
														<td>
                                                        ...
                                                        <td>
                                                        <?php
                                                            echo $cant_devolver[$i];
                                                            ?>
                                                            <input style="display: none;" type="text" name="cant_devolver[]" value="<?php echo utf8_encode($cant_devolver[$i]); ?>">
                                                            
                                                        </td>
                                                        <td>
                                                        <?php 
                                                            $total = $cant_devolver[$i]*$Valor_Final;
                                                            echo utf8_encode(Formatomoneda($total));
                                                            $i=$i+1; //VARIABLE PARA USAR EN EL ARREGLO DE CANT_DEVOLVER
                                                            $cant_din_dev = $cant_din_dev + $total;//SUMA LA CNTIDAD DE DINERO A DEVOLVER
                                                        ?>
                                                        </td>
													</tr>
                                                ...
                                                <tr>
                                                    ...
                                                    <td></td>
                                                	<td></td>
                                                	<td><strong>Total Devolver:</strong></td>
                                                    <td><?php 
                                                        echo utf8_encode(Formatomoneda($cant_din_dev));
                                                    ?></td>
                                                </tr>
                                                </tfoot>                                                
											</table>
                                            <input style="display: none;" type="text" name="NumeroFactura" value="<?php Echo($NumeroFactura) ?>">
											<div class="modal-footer center">
												<a  onclick="Traslados(<?php echo $Id_Venta;?>,'CANCELAR')" class="btn btn-sm" data-dismiss="modal">
													<i class="ace-icon fa fa-times"></i>
													Cancelar
												</a>

												<button onclick="Traslados(<?php echo $Id_Venta;?>,'GUARDAR')" class="btn btn-sm btn-primary" name="enviar">
