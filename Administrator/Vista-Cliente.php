...
$ClienteCartera=$_GET['Cartera'];

$sql ="SELECT * FROM t_clientes as A, t_ciudades as B  WHERE A.Ciudad_Id_Ciudad=B.Id_Ciudad and Id_Cliente='".$ClienteCartera."'"; 
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
        $Tb_Id_Cliente=$row['Id_Cliente'];
        $Tb_Documento_Cliente=$row['Documento_Cliente'];
        $Tb_Ape_Cliente=$row['Ape_Cliente'];
        $Tb_Nom_Cliente=$row['Nom_Cliente'];
        $Tb_Cel1_Cliente=$row['Cel1_Cliente'];
        $Tb_Tel_Cliente=$row['Tel_Cliente'];
        $Tb_Cel2_Cliente=$row['Cel2_Cliente'];
        $Tb_Correo_Cliente=$row['Correo_Cliente'];
...
$sql="SELECT IFNULL(sum(Total_Factura),0) as TotalFacturado From t_facturas where  Cliente_Id_Cliente='".$ClienteCartera."' and Factura_Paga <>'3'";
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
...
$sql="SELECT IFNULL(sum(Total_Pedido),0) as TotalOrdenes From t_pedido where  Cliente_Id_Cliente='".$ClienteCartera."' and Estado_Pedido<>'10'";
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
...
$sql="SELECT IFNULL(sum(Valor_Ingreso),0) as TotalAbonos From t_ingresos where  Cliente_Id_Cliente='".$ClienteCartera."'";
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
...
$SumaCompras=$TotalOrdenes+$TotalFacturado;
$SaldoDinero=$SumaCompras-$TotalAbonos;
					...
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li>
								...
								<a href="Clientes2.php">Clientes</a>
							</li>
						</ul><!-- /.breadcrumb -->
...
<div class="col-sm-12 col-xs-12"><!-- Inicio Panel Inferior-->
                    ...
                    <div class="col-lg-3 col-xlg-2 col-md-4">
                        <div class="card">
                            <div class="card-body">
                                ...
                                <center class="m-t-30"> <img src="<?php Echo($AvatarCliente) ?>" class="img-circle" width="150" />
                                    <h4 class="card-title m-t-10">Cliente</h4>
                                    <h6 style="text-transform:uppercase;" class="card-subtitle"><?php Echo utf8_encode($Tb_Nom_Cliente."".$Tb_Ape_Cliente) ?></h6>
                            ...
                            <div class="card-body"> <small class="text-muted">Email address </small>
                                <h6><?php Echo($Tb_Correo_Cliente) ?></h6> <small class="text-muted p-t-30 db">Celular</small>
                                <h6><?php Echo($Tb_Cel1_Cliente." // ".$Tb_Cel2_Cliente) ?></h6> <small class="text-muted p-t-30 db">Address</small>
                                 <small class="text-muted p-t-30 db">Perfil Modasof</small>
                                <br/>
                                <a href="mailto=<?php Echo($Tb_Correo_Cliente) ?>" class="btn btn-circle btn-secondary"><i class="fa fa-envelope"></i></a>
                                <button class="btn btn-circle btn-secondary"><i class="fa fa-phone"> <?php Echo($Tb_Cel1_Cliente); ?></i></button>
                    ...
                    <div class="col-lg-9 col-xlg-10 col-md-8">
                        <div class="card">
                            ...
                            <ul class="nav nav-tabs profile-tab" role="tablist">
                                <li class="nav-item"> <a class="nav-link active" data-toggle="tab" href="#home" role="tab">Cartera</a> </li>
                                <li class="nav-item"> <a class="nav-link" data-toggle="tab" href="#profile" role="tab">Profile</a> </li>
                                <li class="nav-item"> <a class="nav-link" data-toggle="tab" href="#settings" role="tab">Settings</a> </li>
                            </ul>
                            <!-- Tab panes -->
                            <div class="tab-content">
                                <div class="tab-pane active" id="home" role="tabpanel"> <!-- Inicio Panel Cartera -->
                                    <div class="card-body">
          ...
          <div class="small-box bg-red">
            <div class="inner">
              <h3>
              	<?php 
              	Echo(Formatomoneda($SumaCompras));
              	?>	
              </h3>
        ...
        <div class="col-lg-4 col-xs-12">
          ...
          <div class="small-box bg-green">
            <div class="inner">
              <h3>
              	<?php 
              	Echo(Formatomoneda($TotalAbonos));
              	?>	
              </h3>
        ...
        <div class="col-lg-4 col-xs-12">
          ...
          <div class="small-box bg-blue">
            <div class="inner">
              <h3>
              	<?php 
              	Echo(Formatomoneda($SaldoDinero));
              	?>	
              </h3>
        ...
        <!-- ./col -->
        <h3>Total Facturas <?php Echo(Formatomoneda($TotalFacturado)); ?></h3>
        <div>

        <?php 

// ************** Arreglo Total Insumos ***************//
$sql ="SELECT Id_Factura FROM t_facturas WHERE Cliente_Id_Cliente='".$ClienteCartera."' and Factura_Paga <>'3' order by Id_Factura DESC";  
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
$Lista=$Lista.$row['Id_Factura'].",";                  
 }
}
...
// ************** Arreglo Total Insumos ***************//
 ?>
        	 <table class="table product-overview">
                                  ...
                                  <th class="pull-rigth info"  >Factura Nº</th>
                                   <th class="pull-rigth info"  >Salida Contable</th>
                                  <th class="pull-rigth info" >Factura a</th>
                                   <th class="pull-rigth info" > Tienda</th>
                                  <th class="pull-rigth info" > Fecha</th>
                                 
                                  <th class="pull-rigth info" >Abonos</th>
                                  <th class="pull-rigth info" >Total Factura</th>
                                  <th class="pull-rigth info" >Devoluciones</th>
                                   <th class="pull-rigth danger" >Debe</th>
                                   <th class="pull-rigth success" >A favor</th>
                                  </thead>
                                  <tbody>
                <?php 
...
$sql="SELECT Num_Factura, Total_Factura, Factura_Paga,Fecha_Factura,B.Id_Tienda, Nom_Tienda,A.num_consecutivo_sc From t_facturas as A, t_tiendas as B where  A.Id_Factura='".$Cadena[$i]."' and A.Tienda_Id_Tienda=B.Id_Tienda ";
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
        ...
        $Id_Tiendafactura=$row['Id_Tienda'];


        if ($num_consecutivo_sc==0) {
            $numerofinalfactura= $Num_Factura;
        }else{
             $numerofinalfactura=$num_consecutivo_sc;
        }
...
if ($num_consecutivo_sc==0) {
    ...
    $sql="SELECT IFNULL(sum(Valor_Ingreso),0) as AbonosFactura From t_ingresos where  Factura_Cod_Factura='".$Num_Factura."' and Cliente_Id_Cliente='".$ClienteCartera."' and Tienda_Id_Tienda='".$Id_Tiendafactura."'";
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
        $AbonosFactura=$row['AbonosFactura'];
    }
}
     ...
     $sql="SELECT IFNULL(sum(Valor_Ingreso),0) as AbonosFactura From t_ingresos where  consecutivosc_id_consecutivosc='".$num_consecutivo_sc."' and Cliente_Id_Cliente='".$ClienteCartera."' and Tienda_Id_Tienda='".$Id_Tiendafactura."'";
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
...
$sql="SELECT IFNULL(sum(Total_Devolucion),0) as TotalDevol From t_devoluciones where  Num_Factura='".$numerofinalfactura."' and Cliente_Id_Cliente='".$ClienteCartera."'";
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
        $TotalDevol=$row['TotalDevol'];
    }
}

$SaldoFactura=$Total_Factura-$AbonosFactura;
                ...
                ?>
                                    ...
                                    if ($num_consecutivo_sc==0) {
                                        echo("<td>".$Num_Factura."</td>");
                                        echo("<td>0</td>");
                                    }else{
                                        echo("<td>0</td>");
                                         echo("<td>".$num_consecutivo_sc."</td>");
                                    }
                                     ...
                                     ?>
                                      ...
                                      <td>
                                  		...
                                  		<td>
                                  				<?php Echo(Formatomoneda($AbonosFactura)); ?>
                                  		</td>
                                  		<td>
                                  				<?php Echo(Formatomoneda($Total_Factura)); ?>
                                  		</td>
                                      <td>
                                          <?php Echo(Formatomoneda($TotalDevol)); ?>
                                      </td>
                                      <td class="danger">
                                          <?php Echo(Formatomoneda($SaldoFactura)); ?>
                                      </td>
                                      <td class="success">
                                          <?php Echo(Formatomoneda($TotalDevol)); ?>
                                      </td>
        ...
        <!--  Tabla de Pedidos -->
         ...
         <!-- ./col -->
        <h3>Total Pedidos <?php Echo(Formatomoneda($TotalOrdenes)); ?></h3>
        <div>

        <?php 

// ************** Arreglo Total Insumos ***************//
$sql ="SELECT Cod_Pedido FROM t_pedido WHERE Cliente_Id_Cliente='".$ClienteCartera."' and Estado_Pedido<>'10' order by Cod_Pedido DESC";  
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
$Lista2=$Lista2.$row['Cod_Pedido'].",";                  
 }
}
$Cadena2=explode(",", $Lista2);
//Split al Arreglo
$longitud2 = count($Cadena2);
$min2=$longitud2-1;
// ************** Arreglo Total Insumos ***************//
 ?>
           <table class="table product-overview">
                                  ...
                                  <th class="pull-rigth info"  >Pedido Nº</th>
                                  <th class="pull-rigth info"  >Estado</th>
                                   <th class="pull-rigth info" > Tienda</th>
                                  <th class="pull-rigth info" > Fecha</th>
                                 
                                  <th class="pull-rigth info" >Abonos</th>
                                  <th class="pull-rigth info" >Total Pedido</th>
                                   <th class="pull-rigth danger" >Debe</th>
                                  </thead>
                                  <tbody>
                <?php 
  ...
  for($x=0; $x<$min2; $x++)
{
$sql="SELECT Cod_Pedido, Fecha_Pedido, Total_Pedido,Nom_Tienda,Estado_Pedido From t_pedido as A, t_tiendas as B where  Cod_Pedido='".$Cadena2[$x]."' and A.Tienda_Id_Tienda=B.Id_Tienda";
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
...
$sql="SELECT IFNULL(sum(Valor_Ingreso),0) as AbonosPedidos From t_ingresos where  Pedido_Id_Pedido='".$Cadena2[$x]."'";
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
        $AbonosPedidos=$row['AbonosPedidos'];
    }
}

$SaldoPedido=$Total_Pedido-$AbonosPedidos;



                ?>
                <?php  if ($Estado_Pedido!=14) 
                {
    ...
    $Color_Estado=$row['Color_Estado'];
      Echo("<span class='label' style='background-color:".$Color_Estado."'>".utf8_encode($Nom_Estado_Pedido)."</span> <span class='action-icons'></span>");
            ...
            ?>
                                      ...
                                      <td>
                                          <?php Echo(fechasql($Fecha_Pedido)); ?>
                                      </td>
                                      
                                      <td>
                                          <?php Echo(Formatomoneda($AbonosPedidos)); ?>
                                      </td>
                                      <td>
                                          <?php Echo(Formatomoneda($Total_Pedido)); ?>
                                      </td>
                                     
                                      <td class="danger">
                                          <?php Echo(Formatomoneda($SaldoPedido)); ?>
                                      </td>
