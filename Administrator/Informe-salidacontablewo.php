...
<div class="widget-body">
		<h4 class="widget-title lighter">
			...
			<div class="row">
				<form action="Informe-salidacontablewo.php?QueryCon=<?php Echo($TxtConsultaVen) ?>" method="post" id="FormFechas" autocomplete="off">
					<div class="col-xs-12 col-sm-6">
...
$sql ="SELECT IFNULL(sum(Valor_Ingreso),0) as TotalAbonos FROM t_ingresos WHERE consecutivosc_id_consecutivosc='".$Cadena[$i]."' and Tienda_Id_Tienda='".$TiendaOk."'";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
...
$sql ="SELECT cliente_id_cliente FROM t_facturas WHERE num_consecutivo_sc='".$Cadena[$i]."'";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
...
?>
												...
												<ul class="dropdown-menu dropdown-info dropdown-menu-right">
													<li>
		<a target="_blank" data-rel="tooltip" data-placement="top" title="Imprimir Nota Entrega" href="Pdf_FacturaNEwo.php?FacturaImp=<?php Echo($Cadena[$i]); ?>&DeTienda=<?php echo($TiendaOk) ?>"><i class="fa fa-file-pdf-o bigger-100 blue"></i> Imprimir Nota Entrega</a>
													</li>
													<li>
