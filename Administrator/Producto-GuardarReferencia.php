...
if ($TxtInsumo1=="" ){
    echo "Error, sin seleccionar Insumo Principal";
    exit();
}
if ($TxtTotalCosto==""){
    echo "Error";
...
if($filesize > $max){
    echo "ERROR AL RECIBIR LOS DATOS DE TAMAÑO DE ARCHIVO";
    echo "Error";
    exit();
...
if ($Ref1=="") { 
    echo "ERROR, NO SE INGRESO LA CATEGORIA DEL PRODUCTO REF1";
    echo "Error";
    header("location:Lista-Referencias.php?Mensaje=551"); // Categoria de Producto
    exit();
}
elseif ($Ref2=="") {
    echo "ERROR, NO SE SELECCIONO LA SUB CATEGORIA DEL PRODUCTO, REF2";
    echo "Error";
    header("location:Lista-Referencias.php?Mensaje=552"); // SubCategoria de Producto
    exit();
}
elseif ($Ref3=="") {
    echo "ERROR, NO SE SELECCIONO EL INSUMO PRINCIPAL. AL RECIBIR LOS DATOS DE REF3";
    echo "Error";
...
elseif ($Ref4=="") {
    echo "ERROR, NO SE SELECCIONO EL INSUMO SECUNDARIO. AL RECIBIR LOS DATOS DE REF4";
    echo "Error";
...
$sql ="SELECT Id_Temporal FROM t_temporal_ref WHERE Orden_Temporal='".$IdUser."'"; 
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
...
// Eliminar Diseño en Tabla Temporal 
  ...
  $result = $conexion->query($sql);
    ...
    //echo "DEBIO GUARDAR CORRECTAMENTE";
    //exit();
