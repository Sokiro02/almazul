...
//ApeCliente=VALLEDUPAR 82 S.A.S&DocSel=29&NomCliente=SAD
...
$Datos = "Se ingreso a Crear Orden de Proveedor: ".$Qr_Id_Proveedor." ".$Qr_Nom_Prov;
$seguridad = AgregarLog($IdUser,$Datos,"Proveedor-CrearOrden.php");
...
$result = $conexion->query($sql);
if($result){
    $Datos = "Se elimino la siguiente orden de compra: ".$DeleteItem;
    $seguridad = AgregarLog($IdUser,$Datos,"Proveedor-CrearOrden.php");
}
...
<h5><i class="fa fa-user"></i> ORDEN NÚMERO: OCP<?php Echo utf8_encode($OCI) ?></h5>
...
<div class="col-sm-4 center">
	<h5>ORDEN NÚMERO: OCP<?php Echo utf8_encode($OCI) ?></h5>
<?php 
...
<?php
$sql ="SELECT Id_Insumo, Nom_Insumo,Cod_Insumo,Url_Insumo FROM t_insumos WHERE Proveedor_Id_Proveedor='".$DocSel."'";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
    	$SelectIdInsumo=$row['Id_Insumo'];
    	$SelectNombreInsumo=$row['Nom_Insumo'];   
    	$SelectCodInsumo=$row['Cod_Insumo'];  
    	$SelectUrlInsumo=$row['Url_Insumo'];             
    	
    	Echo("<option value='".$SelectIdInsumo."' data-img-src='".$SelectUrlInsumo."'>".$SelectNombreInsumo."-".$SelectCodInsumo."</option>");
 }
}
									...
									<input type="number" id="TxtCantidad" step="any" class="input-xxlarge" name="TxtCantidad" required="true">
														...
														<div class="form-group col-xs-12 col-sm-2">
																...
																<div class="input-group">
																	...
																	<select required="true" class="chosen-select input-medium" name="TxtLote">
		...
		<div id="info2">
						...
						<div class="clearfix">
										...
										<div class="table-header">
											Orden de Compra Nº: OCP<?php Echo($OCI); ?>
										</div>
