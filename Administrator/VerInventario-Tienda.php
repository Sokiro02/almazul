...
<html lang="en">
	<head>
		...
		<meta charset="utf-8" />
		<title>Inventario Tiendas Modasof</title>
...
<script src="https://modasof.com/espejo/assets/js/jquery.maskMoney.js" type="text/javascript"></script>
  ...
  $Valide=$_GET['Mensaje'];
    ...
    if ($Valide==1) {
     ...
     if ($Valide==1) {
        echo "<script>jQuery(function(){swal(\"¡ Cantidad Ajustada!\", \"Correctamente \", \"success\");});</script>";
    };
    if ($Valide==27) {
						...
						<ul class="breadcrumb">
							...
							<li class="dropdown">
													...
													<ul class="dropdown-menu dropdown-info">
														<li>
				<a  href="VerInventario-Tienda.php">Ver todas las tiendas</a>
			</li>
														<?php
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
    	...
    	?>
    		<li>
				<a  href="VerInventario-Tienda.php?QueryCon=<?php Echo($SelectDown);?>&Mensaje=27"><?php Echo utf8_encode($NomDown); ?></a>
			</li>
    	<?php
 }
}
    	...
    	$Costo_Proyectado_Pref=$row['Costo_Proyectado_Pref'];
					 ...
					 ?>
						...
						<div class="modal-footer">
												<a href="VerInventario-Tienda.php" class="btn btn-sm" data-dismiss="modal">
...
if ($TxtConsultaVen=="") {
	$sql ="SELECT DISTINCT(Referencia_Completa) FROM t_inventario WHERE Cantidad<>'0' ORDER BY Referencia_Completa DESC";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
	$Lista=$Lista.$row['Referencia_Completa'].",";                  
 }
}
}
...
{
	$sql ="SELECT DISTINCT(Referencia_Completa) FROM t_inventario  WHERE  Id_Tienda='".$TxtConsultaVen."' and Cantidad<>'0'  order by Referencia_Completa  DESC";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
                                    ...
                                    <th class="success"></th>
												...
												<thead>
													...
													<tr class="warning">
														...
														<th class="tdcustom" >Costo Prod.</th>	
														<th class="tdcustom" >Costo Comercial</th>	
														<th class="tdcustom" >Total Activo</th>
														<th class="tdcustom" >Ajustar</th>
														<th class="tdcustom" >Editar</th>
													</tr>
													<tr>
														<th class="tdcustom" >Registro</th>
														<th class="tdcustom" >Tienda</th>
...
$sql ="SELECT Id,Cantidad,Referencia_Completa,Referencia  FROM t_inventario WHERE Referencia_Completa='".$Cadena[$i]."' and Id_Tienda='".$TxtConsultaVen."' ";
		//Echo($sql);
		$result = $conexion->query($sql);
		if ($result->num_rows > 0) {
    		while ($row = $result->fetch_assoc()) {                
        	$Cantidad_Inv=$row['Cantidad'];
        	$Id_Registro_Inv=$row['Id'];
        	$Ref_Completa=$row['Referencia_Completa'];
        	$Referencia=$row['Referencia'];
				}
			}

$sql ="SELECT B.Nom_Cat_Producto,A.Detalle_Referencia,A.Ref_Antigua,A.PVP_Ref,A.Cod_Referencia  FROM t_referencias as A, t_categoria_producto as B WHERE A.Categoria_Id_Categoria_Prod=B.Id_Cat_Producto and Cod_Referencia='".$Referencia."'";
		//Echo($sql);
		$result = $conexion->query($sql);
		if ($result->num_rows > 0) {
    		while ($row = $result->fetch_assoc()) {                
        	$Nom_Cat_Producto=$row['Nom_Cat_Producto'];
        	$Inv_Ref=$row['Cod_Referencia'];
        	$Detalle_Referencia=$row['Detalle_Referencia'];
        	$Ref_Antigua=$row['Ref_Antigua'];
        	$PVP_Ref=$row['PVP_Ref'];
				}
			}


// VERIFICAR COSTO DE PRODUCCIÓN DE LA PRENDA

//$sql ="SELECT SUM(Prom_Temporal*Cant_Temporal) AS CostoProduccion FROM t_temporal_ref2 WHERE Referencia_Temporal='".$Referencia_Id_Referencia."' ";
$sql ="SELECT SUM(Costo_Insumo_Ref*Cant_Solicitada) AS CostoProduccion FROM t_insumos_ref WHERE Referencia_Cod_Referencia='".$Referencia."' ";
//$sql ="SELECT SUM(Prom_Temporal*Cant_Temporal) AS CostoProduccion FROM t_temporal_ref2 WHERE Referencia_Temporal='".$Referencia_Id_Referencia."' ";
		//Echo($sql);
		$result = $conexion->query($sql);
		if ($result->num_rows > 0) {
...
$sql ="SELECT V_Mano_Obra_Ref  FROM t_referencias WHERE Cod_Referencia='".$Referencia."' ";
		//Echo($sql);
		$result = $conexion->query($sql);
		if ($result->num_rows > 0) {
    		while ($row = $result->fetch_assoc()) {                
			...
			$CostoPrenda=$CostoProduccion+$ManoObra;
			$TotalCosto=$CostoPrenda*$Cantidad_Inv;
			$TotalComercial=$PVP_Ref*$Cantidad_Inv;
...
?>
													...
													<?php Echo($Id_Registro_Inv) ?>
														...
														<td class="center">
															<?php Echo($Cantidad_Inv);?>	
														</td>
														<td>
															<?php Echo utf8_encode("$ ".number_format($TotalCosto));  ?>
														</td>
														<td>
															<?php Echo utf8_encode("$ ".number_format($PVP_Ref));  ?>
														</td>
															<td>
															<?php Echo utf8_encode("$ ".number_format($TotalComercial));  ?>
														</td>
														<?php 
														if ($IdRol==1) {
														 ?>
														<td>
		<form action="inventario-ajustarinventario.php" method="post">
			<input style="display: none;" type="text" name="IdInventario" value="<?php Echo($Id_Registro_Inv) ?>">
			<input  type="number"  name="ActualizarInventario" class="input-small" value="<?php Echo($Cantidad_Inv)?>">
			<input style="display: none;" type="text"  name="RefAjustada" class="input-small" value="<?php Echo($Ref_Completa)?>">
			<input style="display: none;" type="text"  name="TiendaSel" class="input-small" value="<?php Echo($TxtConsultaVen)?>">
								...
								<td class="center">
													...
													<a href="Ficha_Tecnica2_Modasof.php?Ref=<?php Echo($Referencia); ?>&&get_tienda=<?php echo($TxtConsultaVen);?>" class="tooltip-primary blue" data-rel="tooltip" data-placement="top" title="Imprimir Ficha Técnica">
																	 <i class="fa fa-file-pdf-o bigger-150"></i>
														</a>
													<a target="_blank" href="Producto-modificar.php?RefSel=<?php echo($Referencia); ?>">Modificar</a>
															</div>
...
$('#dynamic-table thead tr:eq(1) th').each( function () {
    ...
    "lengthMenu": [[7000, 8000, 9000, -1], [7000, 8000, 9000, "All"]],
