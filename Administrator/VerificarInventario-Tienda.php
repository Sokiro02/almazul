					...
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li>
								...
								<a href="VerificarInventario-Tienda.php">Informe Inventarios</a>
							</li>
							<li class="dropdown">
													...
													<ul class="dropdown-menu dropdown-info">
														<li>
				<a  href="VerificarInventario-Tienda.php">Ver todas las tiendas</a>
			</li>
														<?php
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
    	...
    	?>
    		<li>
				<a  href="VerificarInventario-Tienda.php?QueryCon=<?php Echo($SelectDown);?>&Mensaje=27"><?php Echo utf8_encode($NomDown); ?></a>
			</li>
    	<?php
 }
}
    	...
    	$Costo_Proyectado_Pref=$row['Costo_Proyectado_Pref'];
					 ...
					 ?>
						...
						<div class="modal-footer">
												<a href="VerificarInventario-Tienda.php" class="btn btn-sm" data-dismiss="modal">
...
if ($TxtConsultaVen=="") {
	$sql ="SELECT DISTINCT(Referencia_Completa) FROM t_inventario ORDER BY Referencia_Completa DESC";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
...
{
	$sql ="SELECT DISTINCT(Referencia_Completa) FROM t_inventario  WHERE  Id_Tienda='".$TxtConsultaVen."'  order by Referencia_Completa  DESC";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
...
else
{
		...
		if ($result->num_rows > 0) {
			...
			$InventarioActivo=InventarioReal($Ref_Completa,$Id_Tienda);
			$TotalCosto=$CostoProduccion*$InventarioActivo;
			$TotalComercial=$PVP_Ref*$InventarioActivo;
...
?>
													...
													<?php Echo($Id_Registro_Inv) ?>
														...
														<td class="center">
															<?php Echo(InventarioReal($Ref_Completa,$Id_Tienda));?>	
														</td>
														<td>
