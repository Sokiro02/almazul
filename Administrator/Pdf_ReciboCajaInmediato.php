...
$latienda=$_GET['latienda'];
//$AbonosFactura=$_GET['AbonosFactura'];

$sql ="SELECT  Cod_Recibo_Caja,A.Fecha_Ingreso,Nombres,Apellidos,Nom_Cliente,Ape_Cliente,Documento_Cliente,Nom_Ciudad,Cel1_Cliente FROM t_ingresos as A, t_clientes as C, t_ciudades as E, t_usuarios as F WHERE Cod_Recibo_Caja='".$AbonoCliente."' and A.Tienda_Id_Tienda='".$latienda."' and A.Cliente_Id_Cliente=C.Id_Cliente and A.Ingreso_Id_Usuario=F.Id_Usuario and C.Ciudad_Id_Ciudad=E.Id_Ciudad"; 
//Echo($sql);
...
$sql ="SELECT * FROM t_tiendas WHERE Id_Tienda='".$latienda."'"; 
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
...
$sql ="SELECT SUM(Valor_Ingreso) as TotalAnticipo FROM t_ingresos WHERE Cod_Recibo_Caja='".$AbonoCliente."' and Tienda_Id_Tienda='".$latienda."'"; 
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
...
/* seleccionamos el tipo, estilo y tamaño de la letra a utilizar */
        ...
        $pdf->Cell(96,8,utf8_decode('RECIBO DE CAJA N°'.$Cod_Recibo_Caja),0,1,'C'); //Ancho, Alto, texto,borde,?,centrado
...
// Campo Número de Cliente //**************************************************************************************************************************************
        ...
        $pdf->Cell(20,3,'Fecha documento:',0,1,'L');
                ...
                $pdf->SetFont('Arial','', 8);
                $pdf->SetXY(147,33  );//Ubicación en PDF
                $pdf->Cell(20,3,utf8_decode($FechaIngreso),0,1,'L');
        ...
        $pdf->Cell(20,3,'Concepto:',0,1,'L');
                ...
                $pdf->Cell(20,3, strtoupper($Concepto_Ingreso),0,1,'L');
...
$sql ="SELECT Id_Ingreso, Concepto_Ingreso, Medio_Pago,Num_Transaccion,Valor_Ingreso,Nom_MedioPago FROM t_ingresos as A, t_medios_pago as B WHERE Cod_Recibo_Caja='".$Cod_Recibo_Caja."' and A.Medio_Pago=B.Id_Medio_Pago and Tienda_Id_Tienda='".$latienda."'";  
...
$pdf->Cell(20,5,utf8_encode("$ ".number_format($TotalAnticipo)),1,1,'R',1);
//$pdf->Line(178,87,178,91);  //linea que divide la caja
        ...
        $pdf->Cell(20,3 ,utf8_decode('FIRMA, C.C Y SELLO DEL BENEFICIARIO '),0,1,'L');
        $pdf->SetXY(136,93  );//Ubicación en PDF
        $pdf->Cell(62,30,'',1,1,'C'); //Caja para datos de factura totales //Ancho, Alto, texto,borde,?,centrado

        $pdf->SetFont('Arial','B', 8);
        $pdf->SetXY(22,119  );//Ubicación en PDF
        $pdf->Cell(20,3 ,utf8_decode('Este recibo se emitió desde Modasof el día: '.$TiempoActual),0,1,'L');
                // $pdf->SetFont('Arial','B', 8);
 ...
 //******************************************************************************************************************************
        ...
        $ValorLetras=convertir($TotalAnticipo);
...
$pdf->AutoPrint();
                $pdf->Output("Facturas/ReciboCajaInmediato.pdf",'I');




        echo "<script language='javascript'>window.open('Facturas/ReciboCajaInmediato.pdf','_blank','');</script>";

        echo "<script language='javascript'>window.open('index-ventas.php','_parent','');</script>";//para ver el archivo pdf generado
