...
if ($Confirmation!="") {
	$sql ="DELETE FROM t_gastos WHERE Id_Gasto='".$Confirmation."'";  
		echo($sql);
		$result = $conexion->query($sql);
		header("location:Informe-Cuadrecaja.php?Mensaje=18&start=".$FechaUno."&end=".$FechaDos."");
}
...
<html lang="en">
	<head>
		...
		<meta charset="utf-8" />
		<title>Informe Arqueo Caja Modasof</title>
...
$(document).ready(function(){
  ...
  $Valide=$_GET['Mensaje'];
    ...
    if ($Valide==28) {
        echo "<script>jQuery(function(){swal(\"¡ Registro Editado!\", \"Correctamente \", \"success\");});</script>";
    };
     if ($Valide==18) {
...
function(isConfirm){
  if (isConfirm) {
    window.location.href = 'Informe-Cuadrecaja.php?Confirmation=<?php Echo($NumGastoDel);?>&start=<?php echo($FechaUno) ?>&end=<?php Echo($FechaDos); ?>';
  } else {
    window.location.href = 'Informe-Cuadrecaja.php?start=<?php echo($FechaUno); ?>&end=<?php echo($FechaDos); ?>';
  }
});
// swal({
					...
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li>
								...
								<a href="Informe-Cuadrecaja.php">Arqueo Caja Diario</a>
							</li>
...
$sql ="SELECT * FROM t_registro_caja WHERE Id_Registro_Caja='".$MiGasto."'";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
       ...
       $Valor_Confirmado=$row['Valor_Confirmado'];        
					...
					<form action="Registrocaja-Editar.php" method="post" id="FormUpdateCliente">
						<input style="display: none;" type="text" name="TxtIdRegistro" value="<?php Echo($MiGasto)?>">
									...
									<div class="modal-dialog">
										<div class="modal-content">
											<div class="modal-header">
												...
												<h4 class="blue bigger">Editar Cuadre</h4>
											</div>
										...
										<div class="col-xs-12 col-sm-6 ">
														...
														<div class="form-group">
															<label for="form-field-select-3">Registro Id Nº <?php Echo utf8_encode($MiGasto) ?></label>
											...
											<div>
			<input class="input-sm" type="text" id="demo1" placeholder="Valor Ingreso" name="demo1"  value="<?php Echo($Valor_Confirmado)?>" />
...
<div class="widget-body">
		<h4 class="widget-title lighter">
			...
			<div class="row">
				<form action="Informe-Cuadrecaja.php?QueryCon=<?php Echo($TxtConsultaVen) ?>" method="post" id="FormFechas" autocomplete="off">
					<div class="col-xs-12 col-sm-6">
...
<?php 
if ($TxtConsultaVen!="") {
	$sql ="SELECT Id_Registro_Caja FROM t_registro_caja WHERE Fecha_Registro >='".$FechaStart."' and Fecha_Registro <='".$FechaEnd."' and  Tienda_Id_Tienda='".$TxtConsultaVen."' order by Id_Registro_Caja  DESC";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
	$Lista=$Lista.$row['Id_Registro_Caja'].",";                  
 }
}
}
else
{
	$sql ="SELECT Id_Registro_Caja FROM t_registro_caja WHERE Fecha_Registro >='".$FechaStart."' and Fecha_Registro <='".$FechaEnd."' order by Id_Registro_Caja  DESC";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
                                    ...
                                    <th class="success"></th>
												...
												<thead>
													...
													<tr class="warning">
														...
														<th class="tdcustom" >Registrado el</th>
														<th class="tdcustom" >Dia Reporte</th>
														<th class="tdcustom" >Tipo de Reporte</th>
...
if ($TxtConsultaVen!="") {
	$sql ="SELECT * FROM t_registro_caja WHERE Id_Registro_Caja='".$Cadena[$i]."' and Tienda_Id_Tienda='".$TxtConsultaVen."'";
		//Echo($sql);
		$result = $conexion->query($sql);
}
else
{
	$sql ="SELECT * FROM t_registro_caja WHERE Id_Registro_Caja='".$Cadena[$i]."'";
		//Echo($sql);
		$result = $conexion->query($sql);
}
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {  
        ...
        $Id_Registro_Caja=$row['Id_Registro_Caja'];
        ...
        $Reporte_Usuario=$row['Reporte_Usuario'];
        $dateSol = new DateTime($Fecha_Registro);
        $HoraSol=$dateSol->format('H:i:s a');
        $FechaSol=$dateSol->format('Y-m-d');
	}
}

// VALIDACIÓN DE CIERRE O APERTURA

$sql ="SELECT * FROM t_tiendas WHERE Id_Tienda='".$Tienda_Id_Tienda."'";
		//Echo($sql);
		$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {  
...
$sql ="SELECT * FROM t_usuarios WHERE Id_Usuario='".$Usuario_Id_Usuario."'";
		//Echo($sql);
		$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {  
...
?>
			...
			<a data-rel="tooltip" data-placement="top" title="Detalle Compra" href="Pedido-Ver.php?PedidoCliente=<?php Echo($Cod_Pedido); ?>">
														...
														<td class="center">
															<?php Echo(fechasql($Dia_Registro));?>		
														</td>
														<td class="tdcustom">
															<?php 
															if ($Tipo_Registro==1) {
																Echo utf8_encode("Apertura");
															}
															else
															{
																Echo utf8_encode("Cierre");
															}
														...
														<td class="center">
															<?php Echo utf8_encode("$ ".number_format($Valor_Confirmado));  ?>
														</td>
														 <td>
			<div class="btn-group">
												...
												<ul class="dropdown-menu dropdown-info dropdown-menu-right">
													...
													<li>
														<a data-rel="tooltip" data-placement="top" title="Editar Registro" href="Informe-Cuadrecaja.php?MiGasto=<?php Echo($Cadena[$i]);?>&EditTask2=1&start=<?php echo($FechaUno); ?>&end=<?php echo($FechaDos) ?>"><i class="fa fa-edit bigger-100 blue"></i> Editar Registro</a>
													</li>
