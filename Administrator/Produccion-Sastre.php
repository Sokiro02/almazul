...
$FechaInicioDiacorta=($MarcaTemporal);
...
<html lang="en">
	<head>
		...
		<meta charset="utf-8" />
		<title>Producción Modasof desde <?php echo fechasql($FechaUno); ?> al <?php echo fechasql($FechaDos); ?></title>
...
document.cookie = 'cross-site-cookie=bar; SameSite=None; Secure';
  ...
  $Valide=$_GET['Mensaje'];
    ...
    if ($Valide==1) {
     ...
     if ($Valide==333) {
        echo "<script>jQuery(function(){swal(\"¡ Pedido Entregado !\", \"Correctamente \", \"success\");});</script>";
    };
    if ($Valide==18) {
					...
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li>
								...
								<a href="Produccion-Sastre.php">Producción Día</a>
							</li>
						</ul><!-- /.breadcrumb -->
...
<div class="widget-body">
		<h4 class="widget-title lighter">
			...
			<div class="row">
				<form action="Produccion-Sastre.php?QueryCon=<?php Echo($MyIdTienda) ?>" method="post" id="FormFechas" autocomplete="off">
...
?>
										...
										<div class="table-header" style="background-color: #000;">
											Producción  desde <?php echo fechasql($Desde); ?> al <?php echo fechasql($Hasta); ?>
										</div>
...
// ************** Arreglo Total Referencias ***************//
if ($MyIdTienda=="") {
	$sql ="SELECT Distinct(Referencia_Id_Referencia) FROM t_temporal_sol WHERE Estado_Solicitud_Cliente='1' and  Fecha_Solicitud >='".$FechaStart."' and Fecha_Solicitud <='".$FechaEnd."' GROUP BY Talla_Solicitada, Referencia_Id_Referencia";
	//echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
	$Lista=$Lista.$row['Referencia_Id_Referencia'].",";                  
 }
}
}
else
...
// ************** Arreglo Total Insumos ***************//
 ?>
									<table style="font-size: 12px;" id="dynamic-table"  class="table table-responsive table-striped table-bordered table-hover cell-border"  width="100%">
	...
	$sql ="SELECT DISTINCT(Talla_Solicitada) FROM `t_temporal_sol` WHERE `Estado_Solicitud_Cliente`='1' and Fecha_Solicitud >='".$FechaStart."' and Fecha_Solicitud <='".$FechaEnd."'";
	$result = $conexion->query($sql);
	//echo($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
						 ...
						 <th class="success"></th>
												...
												<thead>
													...
													<tr class="warning">
														<th>Img</th>
														<th>Detalle</th>
														<th class="tdcustom" style="width: 10%;">Nº Pedidos</th>
														<th class="tdcustom" style="width: 10%;">Referencia</th>
														<th class="tdcustom" style="width: 10%;">Color</th>
<?php 

// Verifica las tallas solicitadas
    ...
    while ($row = $result->fetch_assoc()) { 
	$Talla_Solicitada=$row['Talla_Solicitada']; 
	$NombreTalla=ObtenernombreTalla($Talla_Solicitada);

	?>
			<th class="tdcustom"><?php echo($NombreTalla); ?></th>
	<?php 
...
?>
												<th>
													<?php 
// Verifica las cantidades por las tallas solicitadas

		$sql ="SELECT IFNULL(sum(Cant_Solicitada),0) as total from t_temporal_sol where Estado_Solicitud_Cliente='1'  and Fecha_Solicitud >='".$FechaStart."' and Fecha_Solicitud <='".$FechaEnd."'";
	//echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
	$TotalConsulta=$row['total'];                  
 }
}

$sql ="SELECT COUNT(DISTINCT(Talla_Solicitada)) as totaltallas from t_temporal_sol where Estado_Solicitud_Cliente='1'and Fecha_Solicitud >='".$FechaStart."' and Fecha_Solicitud <='".$FechaEnd."'";
	//echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
	$totaltallas=$row['totaltallas'];                  
 }
}
...
for($i=0; $i<$min; $i++)
{
$sql ="SELECT * FROM t_temporal_sol WHERE Referencia_Id_Referencia='".$Cadena[$i]."' "; 
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
        ...
        $Fecha_Solicitud=$row['Fecha_Solicitud'];
													 ...
													 ?>
														...
														<tr>
						<td>
							<?php $ImagenRef=ObtenerimagenRef($Referencia_Id_Referencia); ?>
							 <a style="position: relative;" data-rel="tooltip" data-placement="top" title="<?php Echo utf8_encode($Referencia_Id_Referencia);?>" class="image-link" href="<?php echo utf8_encode($ImagenRef); ?>"><img src="<?php echo utf8_encode($ImagenRef); ?>" width="45px" height="45px" >
             </a>
						</td>	
						<td>
							<?php 
							$DetalleRef=ObtenerDetalleRef($Referencia_Id_Referencia);  
							echo ($DetalleRef);
							?>
						</td>
						<td>
							<?php 
							// Verifica los pedidos de esa Referencia

							$sql ="SELECT * FROM t_temporal_sol WHERE Referencia_Id_Referencia='".$Cadena[$i]."' and Estado_Solicitud_Cliente='1'  and Fecha_Solicitud >='".$FechaStart."' and Fecha_Solicitud <='".$FechaEnd."'"; 
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
        ...
        <a target="_blank" href="Pedido-Ver.php?PedidoCliente=<?php Echo($Pedido_Id_Pedido);?>">
        	<span class='label' style='background-color:#D15B47'><?php echo($Pedido_Id_Pedido) ?></span>
        </a>
<br>
<br>	
        <?php
					...
					<td class="center">
						<?php echo($Referencia_Id_Referencia); ?>
					</td>
					<td class="center">

						<?php 
						$InsumoPpalRef=InsumoPpalRef($Referencia_Id_Referencia);
						$ColorPpalRef=ColorPpalRef($InsumoPpalRef);
						echo strtoupper($ColorPpalRef); ?>
					</td>
				<?php 
    ...
    while ($row = $result->fetch_assoc()) { 
	$Talla_Solicitada=$row['Talla_Solicitada']; 
	$ContadorPrendas=ObtenercantidadProduccion($Referencia_Id_Referencia,$Talla_Solicitada,$FechaStart,$FechaEnd);
				?>

				<td class="center"><?php 
				if ($ContadorPrendas!=0) {
					echo("<strong>".$ContadorPrendas."</strong>"); 
				}
				else
				{
				...
				<?php
				 ...
				 <td>
				 	...
				 	<?php 
				 	 $SumatotalPrendas=SumatotalPrendas($Referencia_Id_Referencia,$FechaStart,$FechaEnd);
				 	 Echo $SumatotalPrendas;
				 	 ?>
...
$('#dynamic-table thead tr:eq(1) th').each( function () {
    ...
    var table = $('#dynamic-table').DataTable({
    	responsive:true,
    	"order": [[ <?php echo($totaltallas+5); ?>, "Desc" ]],
        orderCellsTop: true,
        "language": {
    ...
    "lengthMenu": [[500, 700, 1000, -1], [500, 700, 1000, "All"]],
            ...
            pageTotal8 = api
                .column( <?php echo($totaltallas+5); ?>, { page: 'current'} )
                .data()
                .reduce( function (a, b) {
             ...
             // Update footer
            $( api.column( <?php echo($totaltallas+5); ?> ).footer() ).html(
                'Total'+format2(pageTotal8,'' )
            );
