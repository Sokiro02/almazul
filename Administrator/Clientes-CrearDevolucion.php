...
$Datos="Ingreso a Devolucion de referencias de la factura: ".$NumeroFactura;
...
<!-- Fin de Primer Fila -->
							...
							<form action="testdev.php" method="post">
							<!-- INICIO TABLA -->
						<div class="clearfix">
							...
							<table style="font-size: 12px;" id="dynamic-table2"  class="table table-responsive table-striped table-bordered table-hover" width="100%">
												<thead>
													<tr class="warning">
                                                        ...
                                                        <th class="tdcustom" style="width: 5%;">Devolver</th>
...
<?php 
	...
	$sql ="SELECT * FROM t_ventas as A, t_referencias as B WHERE Factura_Id_Factura='".$Num_Factura."' and A.Referencia_Id_Referencia=B.Cod_Referencia"; 
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
			...
			?>
													...
													<tr>
                                                        ...
                                                        <td>
                                                            	<script>
                                                            		...
                                                            		function habilitar(value,id)
                                                            		{
                                                            			if(value==true)
                                                            			{
                                                                            ...
                                                                            //$("#"+id+"option[value='1']").attr("selected",true);
                                                            			}else if(value==false){
                                                            				// deshabilitamos
                                                                            combo = document.getElementById(id);
                                                                            combo.disabled=true;
                                                                ...
                                                                <select name="cant_dev[]" id="<?php echo $Id_Venta;?>" disabled="">                                                    
                                                            <?php 
                                                                $cnt = $Cant_Solicitada;
                                                                for ($i=1;$i<=$cnt;$i++){
                                                                    ?>
					   ...
					   <input style="display: none;" type="text" name="NumeroFactura" value="<?php Echo($NumeroFactura) ?>">
						<label class="pos-rel">
						<input type="checkbox"  onchange="habilitar(this.checked,'<?php echo $Id_Venta;?>');" class="ace" name="TxtCod[]" value="<?php Echo($Id_Venta)?>" />
															<span class="lbl"></span>
														</label>
											...
											<div class="modal-footer">
												...
												<button type="submit" class="btn btn-sm btn-primary" name="enviar">
