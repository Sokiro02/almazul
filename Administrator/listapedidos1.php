		...
		<?php 
$get_estado=$_GET['get_estado'];


	date_default_timezone_set("America/Bogota");
							$MarcaTemporal = date('Y-m-d');
				$quincedias=date("Y-m-d",strtotime($MarcaTemporal."- 2 week")); 	
							$FechaInicioDia = ($quincedias . " 00:00:000");
...
if ($get_estado==9) {
	?>
	<a href='Pedidos.php' style='margin:1px;' type='button' class='btn btn-white btn-danger'>Pedidos Recibidos</a>

		<a href='Pedidos.php?get_estado=9' style='margin:1px;' type='button' class='btn  btn-danger'>Pendientes Taller</a>

		<a href='Pedidos.php?get_estado=8' style='margin:1px;' type='button' class='btn btn-white btn-danger'>En Tránsito</a>
		<hr>
<hr>
...
<a href='Pedidos.php?get_estado=9' style='margin:1px;' type='button' class='btn btn-white btn-danger'>Pendientes Taller</a>

<a href='Pedidos.php?get_estado=8' style='margin:1px;' type='button' class='btn  btn-danger'>En Tránsito</a>
		<hr>
...
<div class="col-md-12">
	<span><strong>
		Pedidos 
					<?php echo ($MiTienda); ?>
								<?php
								if ($FechaUno == "") {
				echo (" del " . fechaSql($quincedias) . " al " . fechaSql($FechaUno));
								} else {
	...
	<form action="Pedidos.php?get_estado=8" method="post" id="FormFechas" autocomplete="off">
	...
	?>
	<a href='Pedidos.php' style='margin:1px;' type='button' class='btn  btn-danger'>Pedidos Recibidos</a>
...
$get_anoactual=Date('Y');
$get_ano_anterior=$get_anoactual-1;

// ************** Arreglo Total Insumos ***************//
if ($MyIdTienda=="") {
	$sql ="SELECT distinct(Id_Pedido) FROM t_pedido WHERE Estado_Pedido<>'10' and Estado_Pedido<>'17' and Estado_Pedido<>'14' and YEAR(Fecha_Pedido)>='".$get_ano_anterior."'  order by Fecha_Pedido  DESC";
...
}elseif($MyIdTienda!="" && $get_estado==9)
{
	$sql ="SELECT distinct(Id_Pedido) FROM t_pedido WHERE Tienda_Id_Tienda='".$MyIdTienda."' and Estado_Pedido<>'10' and Estado_Pedido<>'17' and Estado_Pedido<>'9'  and Estado_Pedido<>'14' and Estado_Pedido<>'8'  order by Fecha_Pedido  DESC";
	//echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
...
}elseif ($MyIdTienda!="" && $get_estado==8) {
	$sql ="SELECT distinct(Id_Pedido) FROM t_pedido WHERE Tienda_Id_Tienda='".$MyIdTienda."' and Estado_Pedido='8' and Fecha_Pedido >='" . $FechaStart . "' and Fecha_Pedido <='" . $FechaEnd . "'  order by Fecha_Pedido  DESC";
	//echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
	$Lista=$Lista.$row['Id_Pedido'].",";                  
		...
		$sql ="SELECT distinct(Id_Pedido) FROM t_pedido WHERE Tienda_Id_Tienda='".$MyIdTienda."' and Estado_Pedido='9' and YEAR(Fecha_Pedido)>='".$get_ano_anterior."' order by Fecha_Pedido  DESC";
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
	$Lista=$Lista.$row['Id_Pedido'].",";                  
