...
<html lang="en">
	<head>
		...
		<meta charset="utf-8" />
		<title>Informe Recibos de Caja Modasof</title>
...
<script src="https://modasof.com/espejo/assets/js/jquery.maskMoney.js" type="text/javascript"></script>
	...
	<?php 
  $Valide=$_GET['Mensaje'];
  $OrdenSeleccionado=$_GET['OrdenSeleccionado'];
    ...
    if ($Valide==1) {
     ...
     if ($Valide==3) {
        echo "<script>jQuery(function(){swal(\"¡ hola!\", \"Correctamente \", \"success\");});</script>";
    };
    if ($Valide==28) {
        echo "<script>jQuery(function(){swal(\"¡ Recibo Editado !\", \"Correctamente \", \"success\");});</script>";
    };
    if ($Valide==27) {
        echo "<script>jQuery(function(){swal(\" Recibo Transferido a Orden ".utf8_encode($OrdenSeleccionado)."\", \"Taller Seleccionado\", \"success\");});</script>";
    };
...
?>
			...
			<div class="main-content">
					...
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li>
								...
								<a href="Informe-RecibosdeCaja.php">Informe Recibos de Caja</a>
							</li>
				...
				$UpReciboCaja=$_GET['UpReciboCaja'];
				$UpIngreso=$_GET['UpIngreso'];
				$UpValIn=$_GET['UpValIn'];
					...
					<form action="Clientes-transferirrecibo.php" method="post" id="FormUpdateCliente">
									<div class="modal-dialog">
										<div class="modal-content">
											<div class="modal-header">
												
												<h4 class="blue bigger">Asociar Recibo de Caja</h4>
											</div>
										...
										<div class="col-xs-12 col-sm-6 ">
														...
														<div class="form-group">
															<label for="form-field-select-3">Recibo de Caja Nº <?php Echo utf8_encode($UpReciboCaja) ?></label>

															<div class="input-group">
																<span class="input-group-addon">
																	<i class="ace-icon fa fa-user"></i>
																</span>
												<input disabled="true" type="text" name="UpTxtNombres" class="input-large" placeholder="Indicar " style="text-transform:uppercase;" value="<?php Echo utf8_encode(formatomoneda($UpValIn)) ?>">
															</div>
											...
											<div class="form-group">
							<label for="form-field-select-3">Asociar a Nueva Orden</label>

<div class="input-group">
	<input style="display: none;" type="text" name="TxtIdIngreso" value="<?php Echo($UpIngreso) ?>">
	<select class="" name="TxtNuevoReciboCaja" data-placeholder="Seleccionar...">
                                <option selected="true" value="<?php Echo utf8_encode($Qr_Id_Ciudad) ?>"><?php Echo utf8_encode($Qr_Nom_Ciudad) ?></option>
                                <?php
$sql ="SELECT Cod_Pedido,Nom_Cliente, Ape_Cliente FROM t_pedido as A, t_clientes as B  WHERE Estado_Pedido<>'10' and A.Cliente_Id_Cliente=B.Id_Cliente order by Cod_Pedido DESC";  
    ...
    while ($row = $result->fetch_assoc()) {   
      $Cod_Pedido=$row['Cod_Pedido'];        
       $Nom_Cliente=$row['Nom_Cliente'];        
        ...
        $PedidoCliente=$Nom_Cliente." ".$Ape_Cliente;        
      echo ("<option value='".$Cod_Pedido."'>PDC".utf8_encode($Cod_Pedido." - ".$PedidoCliente)."</option>");
 }
}
...
?>
												...
												<a href="Informe-RecibosdeCaja.php" class="btn btn-sm" data-dismiss="modal">
					...
					<?php 
					 ...
					 <?php 
					$EditTask2 = $_GET['EditTask2'];
						if ($EditTask2!="") {

				$UpReciboCaja=$_GET['UpReciboCaja'];
				$UpIngreso=$_GET['UpIngreso'];
				$UpValIn=$_GET['UpValIn'];
$sql ="SELECT Fecha_Ingreso,Valor_Ingreso,Medio_Pago,Num_Transaccion,Nom_MedioPago FROM t_ingresos as A, t_medios_pago as B WHERE Id_Ingreso='".$UpIngreso."' and A.Medio_Pago=B.Id_Medio_Pago";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
      $Fecha_Ingreso=$row['Fecha_Ingreso'];        
       ...
       $Nom_MedioPago=$row['Nom_MedioPago'];        
					...
					<form action="Clientes-EditarRecibo.php" method="post" id="FormUpdateCliente">
						<input style="display: none;" type="text" name="TxtIdIngreso" value="<?php Echo($UpIngreso)?>">
									...
									<div class="modal-dialog">
										<div class="modal-content">
											<div class="modal-header">
												...
												<h4 class="blue bigger">Editar Recibo de Caja</h4>
											</div>
										...
										<div class="col-xs-12 col-sm-6 ">
											...
											<div>
			<input class="input-sm" type="text" id="demo1" placeholder="Valor Ingreso" name="demo1"  value="<?php Echo($Valor_Ingreso)?>" />
...
symbolPosition : 'left' // use this setting to position the symbol at the left or right side of the value. default 'left'
											...
											<div class="form-group">
							<label for="form-field-select-3">Seleccione la nueva fecha del Recibo :</label>

  <div class="input-group">
                                  
                                  <input required="true" class="form-control date-picker" id="id-date-picker-1" type="text" data-date-format="yyyy-mm-dd" name="TxtNuevaFecha" value="<?php Echo($Fecha_Ingreso)?>" />
                                  <span class="input-group-addon">
                                    <i class="fa fa-calendar bigger-110"></i>
                                        ...
                                        <option value="<?php Echo($Medio_Pago) ?>"><?php Echo($Nom_MedioPago) ?></option>
                                       <?php 
...
if ($result->num_rows > 0) {
   ...
   $('#TxtMedioPago').change(function(){
    ...
    if((valorCambiado == '2')||(valorCambiado == '3')||(valorCambiado == '4')||(valorCambiado == '6')){
     ...
     <input type="text"  name="TxtVoucher" value="<?php Echo($Num_Transaccion)?>" placeholder="Nº Confirmación">
												</div>
			...
			if ($AbonosFactura!="") {
			 ?>
				...
				<?php 
				$sql ="SELECT B.Nom_Cliente,B.Ape_Cliente FROM t_facturas as A, t_clientes as B WHERE Num_Factura='".$AbonosFactura."' and A.Cliente_Id_Cliente=B.Id_Cliente";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
...
$sql ="SELECT Total_Factura FROM t_facturas WHERE Num_Factura='".$AbonosFactura."'";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
...
$sql ="SELECT IFNULL(sum(Valor_Ingreso),0) as TotalAbonos FROM t_ingresos WHERE Factura_Cod_Factura='".$AbonosFactura."'";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
	$VrAbonos=$row['TotalAbonos'];                  
				 ...
				 ?>
								<h4>Abonos de Cliente <?php Echo($Nom_Cliente." ".$Ape_Cliente); ?> <br>
									Factura Nº: FAC <?php Echo($AbonosFactura); ?><br>
								</h4>
...
$sql ="SELECT date_format(Fecha_Ingreso,CONCAT(CONCAT(ELT(WEEKDAY(Fecha_Ingreso) + 1, 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo')),', %d - ',CONCAT(ELT(MONTH(Fecha_Ingreso), 'Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Agos', 'Sep', 'Oct', 'Nov', 'Dic')),'- %Y')) AS IngresoAbono,Valor_Ingreso, Medio_Pago,Num_Transaccion,Fecha_Ingreso, B.Nombres,B.Apellidos,Nom_MedioPago FROM t_ingresos as A, t_usuarios as B,t_medios_pago as C WHERE Factura_Cod_Factura='".$AbonosFactura."' and A.Ingreso_Id_Usuario=B.Id_Usuario and A.Medio_Pago=B.Id_Medio_Pago order by Id_Ingreso ASC";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
	...
	$PagoEn=$row['Nom_MedioPago'];
                                ...
                                ?>
                                  	<tr>
                                  		...
                                  		<td><a  data-rel="tooltip" data-placement="top" title="Imprimir Recibo" href=""><i class="fa fa-file-pdf-o bigger-110 red"></i></a></td>
                                  	</tr>
                                  	<?php 
                                  }
                              }
                              else
                              {
                              	?>
                              	<tr>
                              		<td colspan="6"><strong>No hay abonos en esta Factura</strong></td>
                              	</tr>
                              	<?php
                              }
                                  	 ?>
                                  	 <tr>
                                  	 	<td class="success" colspan="2"><strong>Total Factura <?php Echo(formatomoneda($Total_Factura)); ?></strong></td>
                                  	 ...
                                  	 <tr>
                                  	 	<td colspan="3">
                                  	 <a class="btn btn-success" href=""><i class="fa fa-dollar bigger-110"> </i> Crear Abono</a>
                                  	 	</td>
                                  	 	<td colspan="3">
                                  	 <a class="btn btn-danger" href="Informe-RecibosdeCaja.php"><i class="fa fa-times bigger-110"> </i> Cerrar Vista</a>
                                  	 	</td>
                                  	 </tr>
                                  </tbody>
                         </table>
...
<div class="widget-body">
		<h4 class="widget-title lighter">
			...
			<div class="row">
				<form action="Informe-RecibosdeCaja.php?QueryCon=<?php Echo($TxtConsultaVen) ?>" method="post" id="FormFechas" autocomplete="off">
					<div class="col-xs-12 col-sm-6">
...
<div class="col-sm-12"><!-- Inicio Panel 12 -->
										...
										<div class="table-header" style="background-color: #000;">
											...
											<?php 
												...
												{
													Echo("Recibos de Caja del ".fechaSql($FechaUno)." al ".fechaSql($FechaDos));
												}
											?> 
										</div>
...
if ($TxtConsultaVen=='') {
	$sql ="SELECT DISTINCT(Cod_Recibo_Caja) FROM t_ingresos WHERE Fecha_Ingreso >='".$FechaStart."' and Fecha_Ingreso <='".$FechaEnd."' and Cod_Recibo_Caja<>'0' order by Cod_Recibo_Caja  ASC";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
	$Lista=$Lista.$row['Cod_Recibo_Caja'].",";                  
 }
}
}
else
{
	$sql ="SELECT DISTINCT(Cod_Recibo_Caja) FROM t_ingresos WHERE Fecha_Ingreso >='".$FechaStart."' and Fecha_Ingreso <='".$FechaEnd."' and  Tienda_Id_Tienda='".$TxtConsultaVen."' and Cod_Recibo_Caja<>'0' order by Cod_Recibo_Caja  ASC";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
 ...
 ?>
                                    ...
                                    <th class="success"></th>
												...
												<thead>
													...
													<tr class="warning">
														<th class="tdcustom" >Recibo de Caja</th>
														<th class="tdcustom" >Vendedor</th>
														...
														<th class="tdcustom" >Confirmación</th>
														<th class="tdcustom" >Concepto</th>
														<th class="tdcustom" >Valor Ingreso</th>
														
														<th class="tdcustom" style="width: 15%;" >Acciones</th>
...
if ($TxtConsultaVen=='') {
	$sql ="SELECT date_format(Fecha_Ingreso,CONCAT(CONCAT(ELT(WEEKDAY(Fecha_Ingreso) + 1, 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo')),', %d - ',CONCAT(ELT(MONTH(Fecha_Ingreso), 'Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Agos', 'Sep', 'Oct', 'Nov', 'Dic')),'- %Y')) AS FechaIngreso,Fecha_Ingreso,Concepto_Ingreso,Medio_Pago,Valor_Ingreso,Num_Transaccion,Cod_Recibo_Caja,Id_Ingreso,Ingreso_Id_Usuario,Tienda_Id_Tienda,Cliente_Id_Cliente,Medio_Pago FROM t_ingresos WHERE Cod_Recibo_Caja='".$Cadena[$i]."'";
		//Echo($sql);
		$result = $conexion->query($sql);
}
	else
{
	$sql ="SELECT date_format(Fecha_Ingreso,CONCAT(CONCAT(ELT(WEEKDAY(Fecha_Ingreso) + 1, 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo')),', %d - ',CONCAT(ELT(MONTH(Fecha_Ingreso), 'Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Agos', 'Sep', 'Oct', 'Nov', 'Dic')),'- %Y')) AS FechaIngreso,Fecha_Ingreso,Concepto_Ingreso,Medio_Pago,Valor_Ingreso,Num_Transaccion,Cod_Recibo_Caja,Id_Ingreso,Ingreso_Id_Usuario,Tienda_Id_Tienda,Cliente_Id_Cliente,Medio_Pago FROM t_ingresos  WHERE Cod_Recibo_Caja='".$Cadena[$i]."' and Tienda_Id_Tienda='".$TxtConsultaVen."'";
		//Echo($sql);
		$result = $conexion->query($sql);
}
...
// CONFIRMACIÓN DE QUE  CLIENTE
$sql ="SELECT * FROM t_medios_pago WHERE Id_Medio_Pago='".$Medio_Pago."'";
		//Echo($sql);
		$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {  
...
?>
														...
														<td class="center">
															RC<?php Echo($Cod_Recibo_Caja) ?>
														</td>
														
														<td>
															<?php Echo utf8_encode(obtenernombreUsuario($Ingreso_Id_Usuario)); ?>
														</td>
														<td class="center">
															<?php Echo utf8_encode(obtenernombreTienda($Tienda_Id_Tienda)); ?>
														</td>
														...
														<td class="center">
												<?php Echo utf8_encode(obtenerdocumentoCliente($Cliente_Id_Cliente)); ?>	
														</td>
														
														
														<td class="tdcustom">
															<?php Echo utf8_encode(obtenernombreCliente($Cliente_Id_Cliente)); ?>		
														</td>
														<td>
														<?php Echo($PagoEn); ?>
														</td>
														<td>
														<?php Echo($Num_Transaccion); ?>
														</td>
														<td>
														<?php Echo($Concepto_Ingreso); ?>
														</td>
														<td class="center">
															<?php Echo utf8_encode("$ ".number_format($Valor_Ingreso));  ?>
														</td>
												...
												<ul class="dropdown-menu dropdown-info dropdown-menu-right">
													<li>
						<a target="blank"  data-rel="tooltip" data-placement="top" title="Imprimir Recibo" href="Pdf_ReciboCajaInmediatoCopia.php?AbonoCliente=<?php Echo($Cod_Recibo_Caja); ?>&latienda=<?php echo($Tienda_Id_Tienda); ?>"><i class="fa fa-file-pdf-o bigger-100 blue"></i> Imprimir Recibo</a>
													</li>
													...
													if ($IdRol==1) {
														...
														<li class="divider"></li>
													<li>
														<a style="position: relative;" data-rel="tooltip" data-placement="top" title="Transferir Recibo"  href="Informe-RecibosdeCaja.php?UpReciboCaja=<?php Echo($Cod_Recibo_Caja) ?>&UpIngreso=<?php Echo($Id_Ingreso); ?>&EditTask=1&UpValIn=<?php Echo($Valor_Ingreso) ?>"><i class="fa fa-exchange green "></i> Transferir a otra Orden</a>
													</li>
													<li>
														<a style="position: relative;" data-rel="tooltip" data-placement="top" title="Anular Factura"  href="Informe-RecibosdeCaja.php?UpReciboCaja=<?php Echo($Cod_Recibo_Caja) ?>&UpIngreso=<?php Echo($Id_Ingreso); ?>&EditTask2=1&UpValIn=<?php Echo($Valor_Ingreso) ?>"><i class="fa fa-edit red "></i> Editar Recibo</a>
													</li>
														<?php
													}
													 ?>
...
activaTab('<?php Echo($TabActive); ?>');
   ...
   function format2(n, currency) {
    ...
    $('#example').DataTable( {
        ...
        "paging":   true,
        "info":     true,
        "aLengthMenu": [[5000, 7000, 10000, -1], [5000, 7000, 10000, "Todas"]],
    "pageLength": 100,
