...
if ($PVP_Ref=="") { // Si no hay valor = Referencia solicittada no exise
  header("location:CrearPlanSepare.php?Mensaje=21"); 
}
else{

$sql ="SELECT Referencia_Id_Referencia,Nom_Talla FROM t_plansepare as A, t_tallas as B WHERE Referencia_Id_Referencia='".$Ref."'and A.Talla_Solicitada=B.Id_Talla and Factura_Id_Factura='0'";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
...
//if ($RefCompleta==$RefLector ) {
  //header("location:CrearPlanSepare.php?Mensaje=22");
//}
if ($Id_Talla=="") {
 header("location:CrearPlanSepare.php?Mensaje=21");
}
...
if ($result->num_rows > 0) {
  ...
  if ($CantidadInventario>0) {
    ...
    $sql="INSERT INTO t_plansepare (Cant_Solicitada, Talla_Solicitada, Tienda_Id_Tienda, Referencia_Id_Referencia,Ref_Vendida, Fecha_Solicitud, Vendedor_Id_Usuario, Valor_Prenda, Valor_Final,Estado_Venta) VALUES ('".$Minimo."','".$Id_Talla."','".$Tienda."','".$Ref."','".$RefLector."','".$TiempoActual."','".$IdUser."','".$PVP_Ref."','".$PVP_Ref."','".$EstadoVenta."')";
$result = $conexion->query($sql);

header("location:CrearPlanSepare.php");
    
  }
  else
  {
    header("location:CrearPlanSepare.php?Mensaje=222");
  }
