...
$IdDespachoRecibido=$_GET['IdDespachoRecibido'];
$SolCliente=$_GET['SolCliente'];
$SolProd=$_GET['SolProd'];


include("Lib/seguridad.php");
$Datos="Ingreso a Despachos";
$Paginas= $_SERVER['PHP_SELF'];
$seguridad = AgregarLog($IdUser,$Datos,$Paginas);

if ($Recibido==1) {
$MarcarDespachoRecibido=2;

 

if ($SolCliente!="") {

	$sql="UPDATE t_detalle_despachos SET Fecha_Recibe_Despacho='".$TiempoActual."', Recibe_Id_Usuario='".$IdUser."', Estado_Detalle_Despacho='".$MarcarDespachoRecibido."' WHERE Id_Detalle_Despacho='".$IdDespachoRecibido."'";
      $result=$conexion->query($sql);
 Echo($sql);

//Ingresamos al inventario la solicitud de nuestro cliente

	$sql="SELECT  Cant_Solicitada, Talla_Solicitada, Tienda_Id_Tienda, Referencia_Id_Referencia,B.Nom_Talla,Pedido_Id_Pedido FROM t_temporal_sol as A, t_tallas as B  WHERE Id_Temporal_Sol='".$SolCliente."' and A.Talla_Solicitada=B.Id_Talla";
		$result = $conexion->query($sql);
		if ($result->num_rows > 0) {
    		while ($row = $result->fetch_assoc()) {                
        	$Cant_Solicitada=$row['Cant_Solicitada'];
        	$Talla_Solicitada=$row['Talla_Solicitada'];
        	$NomTallaSol=$row['Nom_Talla'];
        	...
        	$RefCompleta=$Referencia_Id_Referencia."-".$NomTallaSol;
        	}
    	}

 ////////// Inicio Actualizar Pedido ///////////

// Actualización de despacho recibido
 $Estado=1;

$sql="UPDATE t_temporal_sol SET Recibido_Despacho='".$Estado."' WHERE Id_Temporal_Sol='".$SolCliente."'";
$result=$conexion->query($sql);

 // Suma de solicitudes en 1 
 $sql="SELECT IFNULL(sum(Recibido_Despacho),0) as SumaDespacho FROM t_temporal_sol Where Pedido_Id_Pedido='".$Pedido_Id_Pedido."'";
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
...
// Suma Total de Solicitudes
 $sql="SELECT COUNT(Recibido_Despacho) as TotalSolicitues FROM t_temporal_sol Where Pedido_Id_Pedido='".$Pedido_Id_Pedido."'";
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
...
$PedidoRecibido=9;
  ...
  $sql="UPDATE t_pedido SET Estado_Pedido='".$PedidoRecibido."' WHERE Cod_Pedido='".$Pedido_Id_Pedido."'";
$result = $conexion->query($sql);
}
 /////////// Final Actualizar Pedido ///////////

// Ingreso a Inventario 

   $Ubicacion="Bodega";
   $TipoMov="Ingreso Prod";

	$sql="INSERT INTO t_inventario_ref(Tienda_Id_Tienda, Solicitud_Cl, Inv_Ref, Talla_Id_Talla, Ref_Completa, Cantidad_Inv, Fecha_Ingreso, Ubicacion, Tipo_Mov_Inv,Responsable_Id_Usuario,Fecha_Registro_Modasof) VALUES ('".$Tienda_Id_Tienda."','".$SolCliente."','".$Referencia_Id_Referencia."','".$Talla_Solicitada."','".$RefCompleta."','".$Cant_Solicitada."','".$TiempoActual."','".$Ubicacion."','".$TipoMov."','".$IdUser."','".$TiempoActual."');";
	$result=$conexion->query($sql);
 //Echo($sql);

 // Comentario de Envio
$ComentarioAutomatico=utf8_decode("Solicitud Recibida e ingresada a inventario");

$sql=("INSERT INTO t_comentarios_produccion_cliente (Usuario_id_usuario, Fecha_Comentario, Comentario_Prod, Solicitud_Cod_Orden) VALUES ('".$IdUser."','".$TiempoActual."','".$ComentarioAutomatico."','".$SolCliente."')");
$result = $conexion->query($sql);
//Echo($sql);
header("location:Despachos.php?Mensaje=13"); 
...
$MarcarDespachoRecibido=2;
 ...
 //Echo($sql);
	...
	//Ingresamos al inventario la solicitud de Producción

	$sql="SELECT  Cant_Solicitada, Talla_Solicitada, Tienda_Id_Tienda, Referencia_Id_Referencia,B.Nom_Talla FROM t_solicitudes_prod as A, t_tallas as B  WHERE Cod_Solicitud_Prod='".$SolProd."' and A.Talla_Solicitada=B.Id_Talla";
	//Echo($sql);
		$result = $conexion->query($sql);
		if ($result->num_rows > 0) {
    		while ($row = $result->fetch_assoc()) {                
   ...
   $Ubicacion="Bodega";
   $TipoMov="Ingreso de Prod";

	$sql="INSERT INTO t_inventario_ref(Tienda_Id_Tienda, Solicitud_Prod, Inv_Ref, Talla_Id_Talla, Ref_Completa, Cantidad_Inv, Fecha_Ingreso, Ubicacion, Tipo_Mov_Inv,Responsable_Id_Usuario,Fecha_Registro_Modasof) VALUES ('".$Tienda_Id_Tienda."','".$SolProd."','".$Referencia_Id_Referencia."','".$Talla_Solicitada."','".$RefCompleta."','".$Cant_Solicitada."','".$TiempoActual."','".$Ubicacion."','".$TipoMov."','".$IdUser."','".$TiempoActual."');";
	$result=$conexion->query($sql);
 //Echo($sql);
...
$sql=("INSERT INTO t_comentarios_produccion (Usuario_id_usuario, Fecha_Comentario, Comentario_Prod, Solicitud_Cod_Orden) VALUES ('".$IdUser."','".$TiempoActual."','".$ComentarioAutomatico."','".$SolProd."')");
$result = $conexion->query($sql);

Echo($sql);
  header("location:Despachos.php?Mensaje=13&Stock=1"); 
...
return false;    
  ...
  $OrdenDespacho=$_GET['OrdenDespacho'];
    ...
    if ($Valide==2) {
        echo "<script>jQuery(function(){swal(\"¡ Orden de Des!\", \"Correctamente \", \"success\");});</script>";
    };

     if ($Valide==13) {
        echo "<script>jQuery(function(){swal(\"¡ Despacho Recibido y Agregado al Inventario!\", \"Correctamente \", \"success\");});</script>";
    };
...
?>
					...
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li>
								...
								<a href="Despachos.php">Despachos PDC</a>
							</li>

							<li>
								<i class="ace-icon fa fa-list"></i>
								<a href="Despachos.php?Stock=1">Despachos SLP</a>
							</li>
	...
	if ($New==1) {
		...
		$Desapacho=$_GET['Despacho']

		?>
		<!-- Inicio Modal -->
								<div >

							<form action="Despacho-SubirGuia.php" method="post" id="FormNuevoInsumo" enctype="multipart/form-data">
									<div class="modal-dialog">
										<div class="modal-content ">
											<div class="modal-header">
												<a href="Despachos.php" type="button" class="close" data-dismiss="modal">&times;</a>
												<h4 class="black bigger">Subir Soporte </h4>
											</div>
									...
									<div class="form-group">
										<label for="form-field-select-3">Guía</label>
										<div >
									<input name="fotoportada"  type="file" id="id-input-file-1" class="col-xs-10 col-sm-5"" />
									<input name="TxtDespacho" value="<?php Echo($Desapacho);?>" style="display: none;"/>
										</div>
									</div>
									...
									<hr>
											...
											<div class="col-xs-12 col-sm-6 ">
									<div class="form-group">
										<label for="form-field-select-3">Número Guía</label>
										<div >
											<input name="TxtGuia"  type="text"  class="input-xxlarge" />
										</div>
									</div>
										<hr>
											...
											<div class="modal-footer">
												<a href="Despachos.php" class="btn btn-sm" data-dismiss="modal">
										...
										<div>
											<table style="font-size: 12px;" id="dynamic-table"  class="table table-responsive table-striped table-bordered table-hover" width="100%">
												<thead>
													...
													<tr class="warning">
														<th class="tdcustom">Remisión</th>		
														<th class="tdcustom">PDC</th>
														<th>Estado</th>
														<th class="tdcustom" style="width: 15%">Enviado a</th>
                            							<th class="tdcustom" style="width: 15%">Fecha. Envío</th>
														<th class="tdcustom" style="width: 15%">Fecha. Recibe</th>
                            							<th class="tdcustom" style="width: 15%">Referencia</th>
														<th class="tdcustom" >Cantidad</th>
														<th class="tdcustom" >Transportadora</th>
														<th  class="tdcustom" >Guia</th>
														
													</tr>
													<tr>
														<th class="tdcustom">Remisión</th>		
														<th class="tdcustom">SLC</th>
														<th>Estado</th>
														<th class="tdcustom" style="width: 15%">Enviado a</th>
                            							<th class="tdcustom" style="width: 15%">Fecha. Envío</th>
														<th class="tdcustom" style="width: 15%">Fecha. Recibe</th>
...
if ($Stock==1) {
	$sql ="SELECT date_format(Fecha_Envia_Despacho,CONCAT(CONCAT(ELT(WEEKDAY(Fecha_Envia_Despacho) + 1, 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo')),', %d - ',CONCAT(ELT(MONTH(Fecha_Envia_Despacho), 'Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Agos', 'Sep', 'Oct', 'Nov', 'Dic')),'- %Y')) AS DespachoEnviado, date_format(Fecha_Recibe_Despacho,CONCAT(CONCAT(ELT(WEEKDAY(Fecha_Recibe_Despacho) + 1, 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo')),', %d - ',CONCAT(ELT(MONTH(Fecha_Recibe_Despacho), 'Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Agos', 'Sep', 'Oct', 'Nov', 'Dic')),'- %Y')) AS DespachoRecibido, Id_Detalle_Despacho, Despacho_Cod_Despacho, Fecha_Envia_Despacho, Fecha_Recibe_Despacho, Nombres,Apellidos, Url_Despacho, Cant_Enviada, Solicitud_Produccion, Solicitud_Cliente, Estado_Detalle_Despacho, Transportadora, Num_Guia, Observa_Despacho, Publica_Despacho,B.Referencia_Id_Referencia,B.Pedido_Id_Pedido,D.Nom_Talla FROM t_detalle_despachos AS A, t_solicitudes_prod as B, t_usuarios as C, t_tallas as D WHERE  A.Solicitud_Produccion=B.Cod_Solicitud_Prod and A.Envia_Id_Usuario=C.Id_Usuario and B.Talla_Solicitada=D.Id_Talla  ORDER BY UNIX_TIMESTAMP(Fecha_Envia_Despacho) DESC"; 
}
else{
$sql ="SELECT date_format(Fecha_Envia_Despacho,CONCAT(CONCAT(ELT(WEEKDAY(Fecha_Envia_Despacho) + 1, 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo')),', %d - ',CONCAT(ELT(MONTH(Fecha_Envia_Despacho), 'Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Agos', 'Sep', 'Oct', 'Nov', 'Dic')),'- %Y')) AS DespachoEnviado, date_format(Fecha_Recibe_Despacho,CONCAT(CONCAT(ELT(WEEKDAY(Fecha_Recibe_Despacho) + 1, 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo')),', %d - ',CONCAT(ELT(MONTH(Fecha_Recibe_Despacho), 'Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Agos', 'Sep', 'Oct', 'Nov', 'Dic')),'- %Y')) AS DespachoRecibido, Id_Detalle_Despacho, Despacho_Cod_Despacho, Fecha_Envia_Despacho, Fecha_Recibe_Despacho, Nombres,Apellidos, Url_Despacho, Cant_Enviada,  Solicitud_Cliente, Estado_Detalle_Despacho, Transportadora, Num_Guia, Observa_Despacho, Publica_Despacho,B.Referencia_Id_Referencia,B.Pedido_Id_Pedido,D.Nom_Talla,C.Nombres,C.Apellidos,E.Nom_Tienda FROM t_detalle_despachos AS A, t_temporal_sol as B, t_usuarios as C, t_tallas as D, t_tiendas as E WHERE  A.Solicitud_Cliente=B.Id_Temporal_Sol and A.Envia_Id_Usuario=C.Id_Usuario and B.Talla_Solicitada=D.Id_Talla and B.Tienda_Id_Tienda=E.Id_Tienda  ORDER BY UNIX_TIMESTAMP(Fecha_Envia_Despacho) DESC"; 
}
    //Echo($sql); 
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
    $DespachoEnviado=$row['DespachoEnviado'];
    $DespachoRecibido=$row['DespachoRecibido']; 
    $Id_Detalle_Despacho=$row['Id_Detalle_Despacho']; 
    $Despacho_Cod_Despacho=$row['Despacho_Cod_Despacho'];
    $Fecha_Envia_Despacho=$row['Fecha_Envia_Despacho'];
    $Fecha_Recibe_Despacho=$row['Fecha_Recibe_Despacho']; 
    $Nombres=$row['Nombres'];  
    $Apellidos=$row['Apellidos']; 
    $Url_Despacho=$row['Url_Despacho']; 
    $Referencia_Id_Referencia=$row['Referencia_Id_Referencia'];   
    $Cant_Enviada=$row['Cant_Enviada'];
    $Solicitud_Produccion=$row['Solicitud_Produccion'];
    $Solicitud_Cliente=$row['Solicitud_Cliente'];
    $Transportadora=$row['Transportadora']; 
    $Num_Guia=$row['Num_Guia'];
    $Estado_Detalle_Despacho=$row['Estado_Detalle_Despacho'];
     $Observa_Despacho=$row['Observa_Despacho'];

     $Pedido_Id_Pedido=$row['Pedido_Id_Pedido'];
     $Nom_Talla=$row['Nom_Talla'];  
     $Nom_Tienda=$row['Nom_Tienda'];   

    if ($Estado_Detalle_Despacho==1) {
      $NomEstado="Enviado";
    }
    elseif ($Estado_Detalle_Despacho==2) {
      $NomEstado="Recibido";
    }
    elseif ($Estado_Detalle_Despacho==3) {
      $NomEstado="Acabados";
    }
     elseif ($Estado_Detalle_Despacho==10) {
      $NomEstado="Enviado";
    }
    $date = new DateTime($Fecha_Solicitud);
				 ...
				 ?>
														...
														<tr>
															<td class="center">
														REM<?php Echo($Despacho_Cod_Despacho); ?>
													...
													<td>
														...
														if ($Stock==1) {
															?>
															SLP<?php Echo utf8_encode($Solicitud_Produccion); ?>
															<?php
														}
														else
														{
															?>
															PDC<?php Echo utf8_encode($Pedido_Id_Pedido); ?>
															<?php
														}
													...
													<td>
															<?php 
															if ($Estado_Detalle_Despacho==1) {
																 Echo("<span class='badge bg-yellow'>".$NomEstado."</span>");
															}
															if ($Estado_Detalle_Despacho==2) {
																 Echo("<span class='badge bg-green'>".$NomEstado."</span>");
															}
															 ?>
														</td>
														<td>
                          ...
                          <td>
                           <span class="tooltip-primary blue" data-rel="tooltip" data-placement="top" title="<?php Echo utf8_encode($Nombres." ".$Apellidos) ?>"><?php Echo($DespachoEnviado); ?></span>
                          </td>
							<td class="tdcustom">
								<?php
								if ($DespachoRecibido=="") {
									...
									if ($Stock==1) {
										?>
										<a class="badge bg-red" href="Despachos.php?Recibido=1&IdDespachoRecibido=<?php Echo($Id_Detalle_Despacho); ?>&SolProd=<?php Echo($Solicitud_Produccion) ?>">Marcar como Recibido</a>
										<?php
									}
									else
									{
										?>
										<a class="badge bg-red" href="Despachos.php?Recibido=1&IdDespachoRecibido=<?php Echo($Id_Detalle_Despacho); ?>&SolCliente=<?php Echo($Solicitud_Cliente) ?>">Marcar como Recibido</a>
										<?php
									}
								 ...
								 echo ($DespachoRecibido); 
								 }
								 ?>
							</td>
							<td class="tdcustom">
								<?php echo utf8_encode($Referencia_Id_Referencia."-".$Nom_Talla); ?>
							</td>
                            <td class="tdcustom">
                              <?php echo utf8_encode($Cant_Enviada);?> Und.
                            </td>
								 <td class="tdcustom">
                              <?php echo utf8_encode($Transportadora);?> 
                            </td>						
									...
									if ($Url_Despacho!="") {
										?>
				<a class="image-link" href="<?php echo utf8_encode($Url_Despacho); ?>"><?php Echo($Num_Guia) ;?> <i class="fa fa-eye"></i></a>
										<?php
									}
									else 
									{
				...
				<a href="Despachos.php?New=1&Despacho=<?php Echo($Despacho_Cod_Despacho); ?>" class="red" >Subir Guía <i class="fa fa-cloud-upload"> </i></a>
				<?php 
			}
					 ?>
