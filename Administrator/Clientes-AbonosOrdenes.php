...
$DeleteRecibo=$_GET['DeleteRecibo'];
$AbonosPedido=$_GET['NumPedido'];

if ($DeleteRecibo!="") {
  $sql="DELETE FROM t_ingresos WHERE Id_Ingreso='".$DeleteRecibo."'";
  $result = $conexion->query($sql);
  header("location:Clientes-AbonosOrdenes.php?NumPedido=".$AbonosPedido."&Mensaje=111");
}
...
<script src="https://modasof.com/espejo/assets/js/jquery.maskMoney.js" type="text/javascript"></script>
  ...
  $Valide=$_GET['Mensaje'];
    ...
    if ($Valide==1) {
     ...
     if ($Valide==111) {
        echo "<script>jQuery(function(){swal(\"¡ Abono Eliminado!\", \"Correctamente \", \"success\");});</script>";
    };
    if ($Valide==27) {
...
?>
			...
			if ($AbonosPedido!="") {
			 ?>
				...
				<?php 
	$sql ="SELECT B.Id_Cliente, B.Nom_Cliente,B.Ape_Cliente FROM t_pedido as A, t_clientes as B WHERE Cod_Pedido='".$AbonosPedido."' and A.Cliente_Id_Cliente=B.Id_Cliente";
	//Echo($sql);
	$result = $conexion->query($sql);
...
$sql ="SELECT Total_Pedido FROM t_pedido WHERE Cod_Pedido='".$AbonosPedido."'";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
	$Total_Factura=$row['Total_Pedido'];
}
}

$sql ="SELECT IFNULL(sum(Valor_Ingreso),0) as TotalAbonos FROM t_ingresos WHERE Pedido_Id_Pedido='".$AbonosPedido."'";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
	$VrAbonos=$row['TotalAbonos'];                  
				 ...
				 ?>
								<h4>Anticipos de Cliente <?php Echo($Nom_Cliente." ".$Ape_Cliente); ?> <br>
									Pedido Nº: ODC <?php Echo($AbonosPedido); ?><br>
								</h4>
...
$sql ="SELECT Id_Ingreso,date_format(Fecha_Ingreso,CONCAT(CONCAT(ELT(WEEKDAY(Fecha_Ingreso) + 1, 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo')),', %d - ',CONCAT(ELT(MONTH(Fecha_Ingreso), 'Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Agos', 'Sep', 'Oct', 'Nov', 'Dic')),'- %Y')) AS IngresoAbono,Valor_Ingreso, Medio_Pago,Num_Transaccion,Fecha_Ingreso, B.Nombres,B.Apellidos, Cod_Recibo_Caja,Nom_MedioPago FROM t_ingresos as A, t_usuarios as B, t_medios_pago AS C WHERE Pedido_Id_Pedido='".$AbonosPedido."' and A.Ingreso_Id_Usuario=B.Id_Usuario and A.Medio_Pago=C.Id_Medio_Pago order by Id_Ingreso ASC";
  ...
  $Cod_Recibo_Caja=$row['Cod_Recibo_Caja'];
	...
	$PagoEn=$row['Nom_MedioPago'];
            ...
            <td>
              <a  data-rel="tooltip" data-placement="top" title="Imprimir Recibo" href="Pdf_ReciboCajaAnticipo.php?PedidoImp=<?php Echo($AbonosPedido) ?>&ReciboCajaImp=<?php Echo($Cod_Recibo_Caja) ?>"><i class="fa fa-file-pdf-o bigger-110 red"></i></a>
              <!--  <a  data-rel="tooltip" data-placement="top" title="Eliminar Abono" href="Clientes-AbonosOrdenes.php?NumPedido=<?php Echo($AbonosPedido) ?>&DeleteRecibo=<?php Echo($Id_Ingreso) ?>"><i class="fa fa-trash bigger-110 red"></i></a> -->
            </td>
                                  	</tr>
                                  	<?php 
                                  }
                              ...
                              else
                              {
                              	...
                              	<?php
                                  	 ...
                                  	 ?>
                                  	 <tr>
                                  	 	<td class="success" colspan="3"><strong>Total Factura <?php Echo(formatomoneda($Total_Factura)); ?></strong></td>
                                  	...
                                  	<a  data-toggle="modal" data-target="#modal-form2" href="#"><span class="btn  btn-success btn-xs"><i class="fa fa-plus-square"> </i> Agregar Abono</span></a>
                                  	 	...
                                  	 	<td colspan="3">
                                  	 <a class="btn  btn-danger btn-xs" href="Pedidos.php"><i class="fa fa-times bigger-110"> </i> Cerrar Vista</a>
                                  	 	</td>
                                  	 </tr>
                                  </tbody>
                         </table>
...
<!-- Inicio Modal -->
    ...
    <div>
      ...
      <form action="Clientes-GuardarAbonoPedido.php?AbonosPedido=<?php Echo($AbonosPedido); ?>" method="post" id="FormAnticipo" autocomplete="off">
...
<?php 
 ...
 ?>
                ...
                <input style="display: none;" type="text" value="<?php Echo($AbonosPedido) ?>" name="TxtNumPedido"> 
                  ...
                  <div class="modal-dialog ">
                    <div class="modal-content">
                      <div class="modal-header">
                        ...
                        <h4 class="black bigger">Agregar Anticipo</h4>
                      </div>
                       ...
                       <table class="table product-overview center" style="width: 80%;">
                       	<tr>
                       		...
                       		<td colspan="2">
                                  ...
                                  <input required="true" class="form-control" id="id-date-picker-1" type="text" data-date-format="yyyy-mm-dd" name="TxtFechaAnticipo" value="" />
...
$FechaFinalDia=($MarcaTemporal." 23:59:000");
	...
	$sql ="SELECT Distinct(notacredito_num) FROM t_devoluciones WHERE Fecha_Devolucion >='".$FechaInicioDia."' and Fecha_Devolucion <='".$FechaFinalDia."' and Cliente_Id_Cliente='".$Id_Cliente."'  ORDER BY Id DESC";
    //Echo($sql);
    $result = $conexion->query($sql);
    if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
    $Lista2=$Lista2.$row['notacredito_num'].",";                  
 }
}
if ($Lista2!=0) {
 ...
 ?>
                                   ...
                                   <tr> 
                                   <td colspan="2">
                                      <select name="valornotasel"  data-placeholder="Seleccionar...">
...
<br>
                        <small style="color: green;">Cliente tiene Notas Crédito</small>
                                   </td>
                                   
                                  </tr>
                                  <?php 
} // Final de validación si tiene o no notas crédito

else
{
	echo("<tr><td class='red'>Cliente no tiene Notas Crédito </td></tr>");
}
                                   ?>
                                 ...
                                 <!--  <tr>
                                    <td>
                                      ¿ Cliente cancela la factura en totalidad?:
                                    </td>
                                    <td>
                      ...
                      <div class="modal-footer">
                        <a href="Clientes-Abonos.php?AbonosPedido=<?php Echo($AbonosPedido) ?>" class="btn btn-sm btn-danger" data-dismiss="modal">
