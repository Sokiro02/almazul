...
$CostoAdicional=FormatoMascara($demo9);

$ValorActualizado=$TxtValorPrenda+$CostoAdicional;


if ($Update!="") {
	$sql="UPDATE t_temporal_sol SET Observa_Cliente='".$TxtDetalle."', Valor_Adicional='".$CostoAdicional."', Valor_Final='".$ValorActualizado."' WHERE Id_Temporal_Sol='".$Update."'";
...
$(document).ready(function(){
   $("#TxtClientes").change(function () {
           $("#TxtClientes option:selected").each(function () {
            ...
            //alert(Select1)
            $.post("SelectClienteOrden.php", { Select1: Select1}, function(data){
                $("#info").html(data); 
            });            
        });
   })
...
contador++;
alert('!Pedido Creado!, verificar datos en tabla de pedidos');
return true;
}
</script>
...
color:#FFF;
  ...
  $Valide=$_GET['Mensaje'];
    ...
    if ($Valide==21) {
     ...
     if ($Valide==555) {
        echo "<script>jQuery(function(){swal(\"¡Verificar Lista de Clientes!\", \"Cliente ya creado\", \"error\");});</script>";
    };
    if ($Valide==22) {
        echo "<script>jQuery(function(){swal(\"¡Lo Sentimos!\", \"No ha incluido ningún valor\", \"error\");});</script>";
    };
    if ($Valide==1) {
        echo "<script>jQuery(function(){swal(\"¡ Orden  Actualizada!\", \"Correctamente \", \"success\");});</script>";
    };
     if ($Valide==333) {
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
      $Cons_PedidosCl=$row['Cons_PedidosCl']+1;
                                ...
                                <h3 class="m-b-0 text-white">Orden de Corte Nº PDC<?php Echo($Cons_PedidosCl) ?></h3>
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
      $SumaPedido=$row['SumaPedido'];
                                    ...
                                    <table class="table product-overview">
                                        <thead>
                                            <tr>
                                                ...
                                                <th style="width: 5%;">Vr. Adicional</th>
                                    ...
                                    <?php 
$sql="SELECT C.Nom_Talla,B.Img_Referencia,Id_Temporal_Sol, Bodega_Id_Bodega, Cant_Solicitada, Talla_Solicitada, Tienda_Id_Tienda, Referencia_Id_Referencia, Fecha_Solicitud, Solicitud_Id_Usuari, Valor_Prenda,Valor_Final, Observa_Cliente, Valor_Adicional FROM t_temporal_sol as A, t_referencias as B, t_tallas as C  WHERE A.Referencia_Id_Referencia=B.Cod_Referencia and A.Talla_Solicitada=C.Id_Talla and Solicitud_Id_Usuari='".$IdUser."' order by Id_Temporal_Sol Desc";
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
...
$VrDescuento=$CalculoDescuento*100;
                                     ?>
                    <form autocomplete="off" action="OrdenCliente.php?Update=<?php Echo($Id_Temporal_Sol); ?>&Mensaje=1" method="post" >
                      <input style="display: none;" type="text" name="TxtValorPrenda" value="<?php Echo( $Valor_Prenda );?>">
                                            <tr>
                                                <td>
                                                ...
                                                <td>
                                                    ...
                                                    <h5><?php Echo($Referencia_Id_Referencia."-".$Nom_Talla."   ")  ?> </h5> 
                                                ...
                                                <!-- <td class="center" style="padding-top: 70px;">
                                                 ...
                                                 <td class="center" style="padding-top: 60px;">
                                                  <input class=" nav-search-input input-sm" type="text" value="<?php Echo(formatomoneda($Valor_Adicional)) ?>" id="demo9" placeholder="$ 0" name="demo9"  required="true" />
                                                  <small>Valores adicionales</small>
...
symbolPosition : 'left' // use this setting to position the symbol at the left or right side of the value. default 'left'
                                                ...
                                                <td style="padding-top: 70px;" align="center">
                                                	<a href="OrdenCliente.php?Delete=<?php Echo($Id_Temporal_Sol); ?>" data-rel="tooltip" data-placement="top" title="Eliminar Referencia" href="javascript:void(0)" class="text-inverse" title="" data-toggle="tooltip" data-original-title="Delete"><i class="fa fa-trash bigger-150 red"></i></a>
                                                  <button type="submit" class="btn btn-success btn-xs" data-rel="tooltip" data-placement="top" title="Actualizar Solicitud"><i class="fa fa-refresh"></i></button>
                                                </td>
                                            </tr>
                                       </form>
                                       <?php 
...
$sql ="SELECT SUM(Valor_Final*Cant_Solicitada) as TotalPedido FROM t_temporal_sol WHERE  Solicitud_Id_Usuari='".$IdUser."'"; 
//Echo($sql); 
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
...
$sql ="SELECT SUM(Cant_Solicitada) as SumaPrendas FROM t_temporal_sol WHERE Solicitud_Id_Usuari='".$IdUser."'";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
...
<form method="post" action="Pedido-Guardar.php" id="FormGuardarventa" autocomplete="off" onsubmit="return mifuncion();">
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
      $Cons_RecibosCaja=$row['Consecutivo_ReciboCaja']+1;
 }
}
...
?>
 ...
 <select required="true" class="chosen-select input-xxlarge" name="TxtTaller"  data-placeholder="Seleccionar...">
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
      ...
      echo ("<option value='".$SelectId_Cliente."'>".htmlentities($SelectDocumento_Cliente." - ".$Nom_Cliente." ".$Ape_Cliente)."</option>");
 }
}
...
<hr>
       ...
       <div class="col-xs-12 col-sm-7" style="margin-bottom: 10px;">
                                ...
                                <div class="input-group">
                                 ...
                                 <button type="submit" class="btn btn-danger btn-outline"><i class="fa fa-print"> </i> Finalizar Orden de Corte</button>
                                 </form>
...
<!-- Inicio Modal -->
    <div id="modal-form2" class="modal" tabindex="-1">
               <!-- Inicio Modal -->
    <div>
              <form autocomplete="off" action="Cliente-Crear.php" method="post" id="FormNuevoCliente">
