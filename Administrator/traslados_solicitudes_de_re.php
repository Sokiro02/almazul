...
$Datos="Ingreso (ACEPTAR TRASLADO) a Detalle de traslado de referencias nro de traslado: ".$codigo;
...
function Traslados(id,accion) {
    if(accion=='GUARDAR'){
        if ( confirm("Estas seguro realizar el traslado?") ) {
                $.post("GuardarTraslados2.php", { Codigo: id, Accion: accion}, function(data){
                    if(data.trim() == "true"){
                        alert("Traslado recibido con exito, prendas ingresadas a su inventario y descontadas a la tienda que envió.");
                        location.href = "traslados_recibir.php?TxtTienda=<?php echo $id_tienda_hasta; ?>"; 
                        //location.reload(true);
                    } else {
                        alert("Error al guardar el traslado"+data);
                    }
                });          
            return true;
        } else {
...
<a href="traslados_recibir.php?TxtTienda=<?php echo $id_tienda_hasta; ?>"><button type="reset" id="cancelar" class="btn btn-xs btn-warning"><i class="fa fa-plus-square"></i> CANCELAR</button></a>
</div>
