...
<html lang="en">
	<head>
		...
		<meta charset="utf-8" />
		<title>Notas Entrega Modasof</title>
					...
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li>
								...
								<a href="Misventasne.php">Detalle Salidas Contables</a>
							</li>
						</ul><!-- /.breadcrumb -->
...
<div class="widget-body">
		<h4 class="widget-title lighter">
			...
			<div class="row">
				<form action="Misventasne.php?QueryCon=<?php Echo($TxtConsultaVen) ?>" method="post" id="FormFechas" autocomplete="off">
...
<div class="col-sm-12"><!-- Inicio Panel 12 -->
											...
											<ul class="nav nav-tabs" id="myTab">
												<li class="active">
													<a data-toggle="tab" href="#tabs-1">
														...
														Detalle Salidas Contables <?php Echo($MiTienda); ?>
													</a>
												</li>
										...
										<div class="table-header" style="background-color: #000;">
											Lista Salidas Contables Modasof 
											<?php Echo($MiTienda); ?> 
											<?php 
												if ($FechaUno=="") {
...
if ($TxtConsultaVen=="") {
	$sql ="SELECT DISTINCT(Id_Venta) FROM t_ventas WHERE Fecha_Solicitud >='".$FechaStart."' and Fecha_Solicitud <='".$FechaEnd."' and consecutivosc_id_consecutivosc<>'0' order by Id_Venta  DESC";
//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
...
{
	$sql ="SELECT DISTINCT(Id_Venta) FROM t_ventas WHERE Tienda_Id_Tienda='".$MyIdTienda."' and Fecha_Solicitud >='".$FechaStart."' and Fecha_Solicitud <='".$FechaEnd."' and consecutivosc_id_consecutivosc<>'0' order by Id_Venta  DESC";
//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
...
if ($TxtConsultaVen=='') {
$sql ="SELECT Vendedor_Id_Usuario,Valor_Prenda,Valor_Final,Ref_Vendida,Factura_Id_Factura,Cant_Solicitada,Cliente_Id_Cliente,Referencia_Id_Referencia,Tienda_Id_Tienda,consecutivosc_id_consecutivosc From t_ventas  WHERE Id_Venta='".$Cadena[$i]."' and consecutivosc_id_consecutivosc<>0"; 
//Echo($sql);
$result = $conexion->query($sql);
}
else
{
	$sql ="SELECT Vendedor_Id_Usuario,Valor_Prenda,Valor_Final,Ref_Vendida,Factura_Id_Factura,Cant_Solicitada,Cliente_Id_Cliente,Referencia_Id_Referencia,Tienda_Id_Tienda,consecutivosc_id_consecutivosc From t_ventas  WHERE Id_Venta='".$Cadena[$i]."' and Tienda_Id_Tienda='".$TxtConsultaVen."' and consecutivosc_id_consecutivosc<>0"; 
//Echo($sql);
$result = $conexion->query($sql);
}
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
        ...
        $Factura_Id_Factura=$row['consecutivosc_id_consecutivosc'];
...
$Fecha_Solicitud=ObtenerfechaSalidaContable($Factura_Id_Factura,$Tienda_Id_Tienda);
$Img_Referencia=ObtenerimagenRef($Referencia_Id_Referencia);
$TelaReferencia=ObtenertipotelaRef($Referencia_Id_Referencia);
							...
							<td class="center">
							<?php echo utf8_encode(ObtenernombreUsuario($Vendedor_Id_Usuario)); ?>
							</td>
							<td class="tdcustom">
   ...
   <a target="_blank" href="<?php echo utf8_encode($Img_Referencia); ?>">
   	<img src="<?php echo utf8_encode($Img_Referencia); ?>" width="45px" height="45px" ><br>
   	<?php Echo($Ref_Vendida); ?>
    	</a>
    	
														</td>
														<td>
															<?php Echo utf8_encode($TelaReferencia);?>
														</td>
...
$sql ="SELECT Cod_Pedido FROM t_pedido WHERE consecutivosc_id_consecutivosc='".$Factura_Id_Factura."' and Tienda_Id_Tienda='".$Tienda_Id_Tienda."'";
//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
...
{
	Echo("Si - #<a target='_blank' href='Pedido-Ver.php?PedidoCliente=".$Cod_Pedido."'>".$Cod_Pedido."</a>");
}                
 }
 mysqli_free_result($result);
}
