...
if ($PVP_Ref=="") { // Si no hay valor = Referencia solicittada no exise
  header("location:cc_crearsalida.php?Mensaje=21"); 
}
else{
...
if ($CantidadInventario>0) {
    $sql="INSERT INTO t_salidas_cc (Cant_Solicitada, Talla_Solicitada, Tienda_Id_Tienda, Referencia_Id_Referencia,Ref_Vendida, Fecha_Solicitud, Vendedor_Id_Usuario, Valor_Prenda, Valor_Final,Estado_Venta) VALUES ('".$Minimo."','".$Id_Talla."','".$Tienda."','".$Ref."','".$RefLector."','".$TiempoActual."','".$IdUser."','".$PVP_Ref."','".$PVP_Ref."','".$EstadoVenta."')";
//Echo($sql)
$result = $conexion->query($sql);

header("location:cc_crearsalida.php");
}

  else
  {
      header("location:cc_crearsalida.php?Mensaje=222");
  }
