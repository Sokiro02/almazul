...
function(isConfirm){
  if (isConfirm) {
    window.location.href = 'Pedidos.php?Confirmation=<?php Echo($NumPedidoDel); ?>';
  } else {
					...
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li>
								...
								<a href="despachos-costo.php">Detalle Despachos</a>
							</li>
							<li class="dropdown">
													...
													<ul class="dropdown-menu dropdown-info">
														<li>
				<a  href="despachos-costo.php">Ver todas las tiendas</a>
			</li>
														<?php
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
    	...
    	?>
    		<li>
				<a  href="despachos-costo.php?QueryCon=<?php Echo($SelectDown);?>&Mensaje=27"><?php Echo utf8_encode($NomDown); ?></a>
			</li>
    	<?php
 }
}
...
<div class="widget-body">
		<h4 class="widget-title lighter">
			...
			<div class="row">
				<form action="despachos-costo.php?QueryCon=<?php Echo($TxtConsultaVen) ?>" method="post" id="FormFechas" autocomplete="off">
...
<div class="col-sm-12"><!-- Inicio Panel 12 -->
											...
											<ul class="nav nav-tabs" id="myTab">
												<li class="active">
													<a data-toggle="tab" href="#tabs-1">
														...
														Despachos Tienda <?php Echo($MiTienda); ?>
													</a>
												</li>
										...
										<div class="table-header" style="background-color: #000;">
											...
											<?php 
												...
												{
													Echo("Despachos del ".fechaSql($FechaUno)." al ".fechaSql($FechaDos));
												}
											?> 
										</div>
...
if ($TxtConsultaVen=="") {
$sql ="SELECT id FROM t_temporal_inventario WHERE fecha_ingreso >='".$FechaStart."' and fecha_ingreso <='".$FechaEnd."'  ORDER BY id  DESC";
//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
...
{
	$sql ="SELECT id FROM t_temporal_inventario WHERE id_tienda='".$TxtConsultaVen."' and fecha_ingreso >='".$FechaStart."' and fecha_ingreso <='".$FechaEnd."' order by id  DESC";
//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
							...
							<table style="font-size: 12px;" id="dynamic-table"  class="table table-responsive table-striped table-bordered table-hover" width="100%">
                                    ...
                                    <th class="success"></th>
												...
												<thead>
													...
													<tr class="warning">
														<th class="tdcustom" style="width: 5%;">Id_Despacho</th>
														<th class="tdcustom" style="width: 10%;">Enviado por</th>
														<th class="tdcustom" style="width: 10%;">Fecha</th>
														<th class="tdcustom" style="width: 10%;">Enviado a</th>
														<th class="tdcustom" style="width: 10%;">Despacho Nº</th>
														
														<th class="tdcustom" style="width: 10%;">Referencia</th>
														<th class="tdcustom" style="width: 10%;">Cantidad</th>
														<th class="tdcustom" style="width: 10%;">¿Cliente?</th>
														<th class="tdcustom" style="width: 10%;">Estado</th>
														<th class="tdcustom" style="width: 10%;">Costo</th>
														<th class="tdcustom" style="width: 10%;">MDO</th>
														<th class="tdcustom" style="width: 10%;">Ad</th>
														<th class="tdcustom" style="width: 10%;">Valor</th>
...
if ($TxtConsultaVen=='') {
$sql ="SELECT valor_total,id, id_tienda, nom_tienda, id_ref, img_ref, cod_ref, detalle_ref, talla_id, cantidad, valor_unidad, valor_total, fecha_ingreso, id_user, talla, status_recibido, codigo_generado, id_despacho, cliente, id_solicitud_prod, id_cliente_sol,B.Nombres,B.Apellidos,B.Img_perfil FROM t_temporal_inventario as A, t_usuarios as B WHERE id='".$Cadena[$i]."' and A.id_user=B.Id_Usuario"; 
//Echo($sql);
$result = $conexion->query($sql);
}
else
{
	$sql ="SELECT valor_total,id, id_tienda, nom_tienda, id_ref, img_ref, cod_ref, detalle_ref, talla_id, cantidad, valor_unidad, valor_total, fecha_ingreso, id_user, talla, status_recibido, codigo_generado, id_despacho, cliente, id_solicitud_prod, id_cliente_sol,B.Nombres,B.Apellidos,B.Img_perfil FROM t_temporal_inventario as A, t_usuarios as B WHERE id='".$Cadena[$i]."' and A.id_user=B.Id_Usuario and id_tienda='".$TxtConsultaVen."'"; 
		//Echo($sql);
		$result = $conexion->query($sql);
}
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
		...
		$status_recibido=$row['status_recibido'];
...
$sql ="SELECT Cod_Referencia FROM t_referencias WHERE Id_Referencia='".$id_ref."' ";
		//Echo($sql);
		$result = $conexion->query($sql);
		if ($result->num_rows > 0) {
    		while ($row = $result->fetch_assoc()) {                
        	$Codigo_ReferenciaInv=$row['Cod_Referencia'];
				}
			}



$sql ="SELECT IFNULL(SUM(Prom_Temporal*Cant_Temporal),0) AS CostoProduccion FROM t_temporal_ref2 WHERE 	Referencia_Temporal='".$Codigo_ReferenciaInv."' ";
		//Echo($sql);
		$result = $conexion->query($sql);
		if ($result->num_rows > 0) {
    		while ($row = $result->fetch_assoc()) {                
...
if ($CostoProduccion==0) {
	$sql ="SELECT SUM(Costo_Insumo_Ref*Cant_Solicitada) AS CostoProduccion FROM t_insumos_ref WHERE Referencia_Cod_Referencia='".$Codigo_ReferenciaInv."' ";
		//Echo($sql);
		$result = $conexion->query($sql);
		if ($result->num_rows > 0) {
    		while ($row = $result->fetch_assoc()) {                
        	$CostoProducciontabla1=$row['CostoProduccion'];
				}
			}
			$valorcosto=$CostoProducciontabla1;
}
else
{
	$valorcosto=$CostoProduccion;
}
			
$sql ="SELECT V_Mano_Obra_Ref  FROM t_referencias WHERE Cod_Referencia='".$Codigo_ReferenciaInv."' ";
		//Echo($sql);
		$result = $conexion->query($sql);
		if ($result->num_rows > 0) {
    		while ($row = $result->fetch_assoc()) {                
        	$ManoObra=$row['V_Mano_Obra_Ref'];
				}
			}
			...
			else
			{
				$ManoObra=0;
						...
						<?php Echo($Nombres." ".$Apellidos." ".$fecha_ingreso." ".$HoraSol); ?>
														</td>
													

														<td class="tdcustom">
															<?php Echo (fechasql($fecha_ingreso)) ?>	
														</td>
															<td class="tdcustom">
															<?php Echo strtoupper(utf8_encode($nom_tienda)) ?>	
														</td>
														</td>
															<td class="tdcustom">
						<a data-rel="tooltip" data-placement="top" title="Ver Despacho"  href="despachos_detalle.php?Despacho=<?php Echo (utf8_encode($id_despacho)) ?>"> <?php Echo (utf8_encode($id_despacho)) ?></a>
														...
														<td class="center">
															...
															<td class="tdcustom">
															<?php Echo strtoupper(utf8_encode($cliente)) ?>	
														</td>

															<td class="center">
															<?php Echo ($status_recibido);?>		
														</td>
														<td>				
   									<?php Echo utf8_encode("$ ".number_format($valorcosto));  ?>
														</td>
														<td>				
   									<?php Echo utf8_encode("$ ".number_format($ManoObra));  ?>
														</td>
														<td>				
   									<?php Echo utf8_encode("$ ".number_format(10000));  ?>
														</td>
														<td class="center">
															<?php Echo utf8_encode("$ ".number_format($valor_total));  ?>
														</td>
										...
										<div>
											<table style="font-size: 12px;" id="dynamic-table2"  class="table table-responsive table-striped table-bordered table-hover" width="100%">
												<thead>
													...
													<tr class="warning">
														<th class="tdcustom" style="width: 8%;">Solicitado por</th>
														<th class="tdcustom" style="width: 8%;">Solicitud Nº</th>
 														<th class="tdcustom" style="width: 20%;">Fecha Solicitud</th>
 														<th class="tdcustom" style="width: 8%;">Img</th>
														...
														<th class="tdcustom" style="width: 10%;">Taller</th>
														<th class="tdcustom" style="width: 10%;">Enviar a</th>
...
<?php 
 ...
 $sql ="SELECT date_format(Fecha_Solicitud,CONCAT(CONCAT(ELT(WEEKDAY(Fecha_Solicitud) + 1, 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo')),', %d - ',CONCAT(ELT(MONTH(Fecha_Solicitud), 'Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Agos', 'Sep', 'Oct', 'Nov', 'Dic')),'- %Y')) AS CreadaFecha,B.Nombres,B.Apellidos,B.Img_Perfil,Id_Solicitud_Prod,Cod_Solicitud_Prod, C.Nom_Bodega, Cant_Solicitada, D.Nom_Talla, E.Nom_Tienda, Referencia_Id_Referencia, Estado_Solicitud_Prod, Fecha_Solicitud, F.Img_Referencia FROM t_solicitudes_prod AS A, t_usuarios as B, t_bodegas as C, t_tallas as D, t_tiendas as E,t_referencias as F WHERE  A.Solicitud_Id_Usuario=B.Id_Usuario and A.Bodega_Id_Bodega=C.Id_Bodega and A.Talla_Solicitada=D.Id_Talla and A.Tienda_Id_Tienda=E.Id_Tienda and A.Referencia_Id_Referencia=F.Cod_Referencia and Estado_Solicitud_Prod<>'4' ORDER BY UNIX_TIMESTAMP(Fecha_Solicitud) desc"; 
    //Echo($sql); 
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
    ...
    $date = new DateTime($Fecha_Solicitud);
						...
						<img data-rel="tooltip" data-placement="top" title="<?php Echo($Nombres." ".$Apellidos."\r\n".$Fecha_Solicitud) ?>" class="img-circle" src="<?php echo utf8_encode($Img_Perfil); ?>" width="45px" height="45px">
														</td>
														<td class="">
													
						<a data-rel="tooltip" data-placement="top" title="Detalles Solicitud" href="Panel-Produccion.php?Solicitud=<?php Echo($Cod_Solicitud_Prod); ?>">
															SLP<?php Echo utf8_encode($Cod_Solicitud_Prod) ?>
								 ...
								 <span class="date"><?php Echo($CreadaFecha); ?>  <?php echo $date->format('H:i:s a');  ?></span>
														</td>
														<td class="center">
      ...
      $NomEstado="Pendiente";
      Echo("<span class='label label-danger'> ".$NomEstado."</span> <span class='action-icons'></span>");
    }
    elseif ($Estado_Solicitud_Prod==2) {
      $NomEstado="Producción";
       Echo("<span class='label label-warning'> ".$NomEstado."</span> <span class='action-icons'></span>");
    }
    ...
    elseif ($Estado_Solicitud_Prod==3) {
      $NomEstado="Acabados";
       Echo("<span class='label label-success'> ".$NomEstado."</span> <span class='action-icons'></span>");
    }
