...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
        ...
        $Proveedor=$row['proveedor_id_proveedor'];
        
    }
}

$sql="SELECT IFNULL(sum(Valor_Final),0) as SumaRecibo From t_salidas_cc where Factura_Id_Factura='".$FacturaImp."' and Tienda_Id_Tienda='".$mitienda."' and Cliente_Id_Cliente='".$getcliente."'";
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
...
$sql="SELECT Observa_Remision From t_cc where Num_Remision='".$FacturaImp."'";
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
...
$sql="SELECT * From t_clientes as A, t_ciudades as B where Id_Cliente='".$getproveedor."' and A.Ciudad_Id_Ciudad=B.Id_Ciudad";
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
        $P_Nom_Cliente=$row['Nom_Cliente'];
        $P_Ape_Cliente=$row['Ape_Cliente'];
        $P_Documento_Cliente=$row['Documento_Cliente'];
        $P_Dir_Cliente=$row['Dir_Cliente'];
        $P_Correo_Cliente=$row['Correo_Cliente'];
        $P_Nom_Ciudad_Cli=$row['Nom_Ciudad'];
    }
}
...
/* seleccionamos el tipo, estilo y tamaño de la letra a utilizar */
        ...
        $pdf->Cell(96,8,utf8_decode('CUENTA DE COBRO Nº '.$FacturaImp),0,1,'C'); //Ancho, Alto, texto,borde,?,centrado
        ...
        $pdf->SetXY(10,8  );//Ubicación en PDF
        $pdf->Cell(30,3,utf8_decode($P_Nom_Cliente." ".$Ape_Cliente),0,1,'L');


        $pdf->SetFont('Arial','B', 8);
        $pdf->SetXY(10,11  );//Ubicación en PDF
        $pdf->Cell(30,3,utf8_decode($P_Documento_Cliente),0,1,'L');


        $pdf->SetFont('Arial','B', 8);
        $pdf->SetXY(10,14  );//Ubicación en PDF
        $pdf->Cell(30,3,utf8_decode($P_Dir_Cliente),0,1,'L'); 

        $pdf->SetFont('Arial','B', 8);
        $pdf->SetXY(10,17  );//Ubicación en PDF
        $pdf->Cell(30,3,utf8_decode($P_Nom_Ciudad_Cli),0,1,'L'); 

        $pdf->SetFont('Arial','B', 8);
        $pdf->SetXY(10,20  );//Ubicación en PDF
        $pdf->Cell(30,3,utf8_decode($P_Correo_Cliente),0,1,'L'); 

        $pdf->SetFont('Arial','B', 8);
        $pdf->SetXY(10,23  );//Ubicación en PDF
        $pdf->Cell(30,3,'No responsable de IVA',0,1,'L');      

       
        $pdf->SetXY(10,32  );//Ubicación en PDF
        
        $pdf->Cell(196,20,'',1,1,'C'); //Caja para datos de Cliente y Factura //Ancho, Alto, texto,borde,?,centrado
...
// Campo Número de Cliente //**************************************************************************************************************************************
        ...
        $pdf->Cell(10,3,utf8_decode('Teléfono:'),0,1,'L');
                ...
                $pdf->Cell(10,3,'(605) 582-0046',0,1,'L');
        ...
        $pdf->Cell(20,3,utf8_decode('Fecha:'),0,1,'L');
        ...
        $pdf->Cell(20,3,'Elaborado por:',0,1,'L');
        ...
        $pdf->Cell(20,3,utf8_decode('Salida de Tienda:'),0,1,'L');
...
$sql ="SELECT Ref_Vendida,Referencia_Id_Referencia,Cant_Solicitada,Detalle_Antiguo, Valor_Final FROM t_salidas_cc as A, t_referencias as B WHERE Factura_Id_Factura='".$FacturaImp."' and Tienda_Id_Tienda='".$mitienda."' and Cliente_Id_Cliente='".$getcliente."'  and A.Referencia_Id_Referencia=B.Cod_Referencia";  
...
if ($result->num_rows > 0) {
  ...
  //******************************************************************************************************************************
        ...
        $pdf->Cell(20,3 ,utf8_decode('Entregado por - C.C: '),0,1,'L');
...
//******************************************************************************************************************************
        ...
        $pdf->SetFont('Arial','B', 8);
        $pdf->SetXY(10,117  );//Ubicación en PDF
        $pdf->Cell(20,3 ,utf8_decode('ELABORADO: '),0,1,'L');
        $pdf->Line(32,119,90,119);  //linea para firma

        $pdf->SetFont('Arial','B', 8);
        $pdf->SetXY(93,117  );//Ubicación en PDF
        $pdf->Cell(20,3 ,utf8_decode('ACEPTADA FIRMADO Y/O SELLO Y FECHA: '),0,1,'L');
        $pdf->Line(160,119,203,119);  //linea para firma
 ...
 //******************************************************************************************************************************
        ...
        //$pdf->Line(57,119,105,119);  //linea para firma
        $ValorLetras=convertir($PrendaSubtotal);
        $pdf->SetFont('Arial','B', 6);
        $pdf->SetXY(10,106  );//Ubicación en PDF
        $pdf->Cell(20,6 ,utf8_decode('Valor en letras:'.$ValorLetras),0,1,'L');
        $pdf->SetFont('Arial','B', 6);
        $pdf->SetXY(10,101  );//Ubicación en PDF
        $pdf->MultiCell(134,2 ,utf8_decode(''),0,'L','J');

      


        $pdf->SetFont('Arial','B', 9);
        $pdf->SetXY(10,88  );//Ubicación en PDF
        $pdf->MultiCell(180,3 ,(utf8_decode("CONCEPTO:".$Observa_Remision)),0,'L','J');


         // FINAL CUADRO DE VALORES
        $pdf->SetFont('Arial','B', 8);
        $pdf->SetXY(3,123  );//Ubicación en PDF
        $pdf->MultiCell(204,3 ,('Nota: En virtud de lo contemplado en el art. 4 de la ley 1943 de 2018; adicionando el paragrafo tercero al art. 437 del ET, quienes ostenten en calidad de no Responsable de IVA no estan obligados a expedir factura de venta por las ventas de bienes o prestacion de servicios que realicen.'),0,'C','J');
...
//$pdf->Close();
$pdf->Output('Facturas/Cuentacobro-Modasof.pdf','I');

        echo "<script language='javascript'>window.open('Facturas/Cuentacobro-Modasof.pdf','_blank','');</script>";


        echo "<script language='javascript'>window.open('cc_informe.php?Mensaje=333','_parent','');</script>";//para ver el archivo pdf generado
