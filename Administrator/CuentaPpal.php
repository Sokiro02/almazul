					...
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li>
								...
								<a href="CuentaPpal.php.php">Cuentas Modasof</a>
							</li>
...
<div class="col-sm-12 col-xs-12"><!-- Inicio Panel 12-->
<div class="clearfix">
...
<table style="font-size: 12px;" id="dynamic-table"  class="table table-responsive table-striped table-bordered table-hover" width="100%">
	<thead>
		...
		<tr class="warning">
			<th class="tdcustom">Nombre Cuenta</th>
			<th class="tdcustom" >Ingresos</th>
...
$sql ="SELECT Id_Usuario FROM t_usuarios WHERE Estado_Id_Estado_Usuario='1' order by Nombres asc";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
$Lista=$Lista.$row['Id_Usuario'].",";                  
 }
}
...
$sql="SELECT IFNULL(sum(Valor_Transferido),0) as IngresosCuenta FROM t_mov_cuentas Where Id_Cuenta_Entra='".$Cadena[$i]."'";
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
        $IngresosCuenta=$row['IngresosCuenta'];
        }
    }

$sql="SELECT IFNULL(sum(Valor_Transferido),0) as SalidasCuenta FROM t_mov_cuentas Where Id_Cuenta_Sale='".$Cadena[$i]."'";
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
        $SalidasCuenta=$row['SalidasCuenta'];
        }
    }


$sql ="SELECT * FROM t_usuarios WHERE Id_Usuario='".$Cadena[$i]."'";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
    	$Nombres=$row['Nombres'];
...
<tr>
	...
	<td>
		<?php Echo(formatomoneda($IngresosCuenta)); ?>
	</td>
	<td>
		<?php Echo(formatomoneda($SalidasCuenta)); ?>
	</td>
	<td>
		<?php Echo(formatomoneda(0)); ?>
	</td>
	<td>
		<?php Echo(formatomoneda(0));?>
	</td>
