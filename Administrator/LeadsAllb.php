...
// Eliminar Insumo de Pedido 

$DeleteItem=$_GET['DeleteItem'];
$NotificaId=$_GET['Notifica'];

if ($DeleteItem=="Si") {
	$sql ="UPDATE t_llamadas SET Llamada_Pendiente='No'  WHERE Id_Llamada='".$NotificaId."'";  
$result = $conexion->query($sql);
header("location:LeadsModasof.php");
}
// 
?>
<!DOCTYPE html>
					...
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li>
								...
								<a href="LeadsModasof.php">Llamadas</a>
							</li>
						</ul><!-- /.breadcrumb -->
...
<div class="col-sm-12 col-xs-12"><!-- Inicio Panel Inferior-->
 ...
 <?php 
$LlamadaSel=$_GET['LlamadaSel'];
if ($LlamadaSel=="") {
	$sql ="SELECT Id_Llamada FROM t_llamadas WHERE Llamada_Pendiente='Si'";  
}
else
{
	$sql ="SELECT Id_Llamada FROM t_llamadas WHERE Id_Llamada='".$LlamadaSel."'";
}

$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
$Lista=$Lista.$row['Id_Llamada'].",";                  
 }
}
...
$sql ="SELECT date_format(Fecha_Notificacion,CONCAT(CONCAT(ELT(WEEKDAY(Fecha_Notificacion) + 1, 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo')),', %d - ',CONCAT(ELT(MONTH(Fecha_Notificacion), 'Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Agos', 'Sep', 'Oct', 'Nov', 'Dic')),'- %Y')) AS CreadaFecha,B.Nom_Tienda, Canal_Lead,Nombre_Lead, Celular_Lead,Fecha_Notificacion, Observacion_Admin, Estado_Llamada,Nom_Estado_Llamada FROM t_llamadas as A, t_tiendas as B,t_estado_llamadas as C  WHERE A.Tienda_Id_Tienda=B.Id_Tienda and A.Estado_Llamada=C.Id_Estado_LLamada  and Id_Llamada='".$Cadena[$i]."' order by Id_Llamada ASC"; 
//Echo($sql); 
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
      ...
      $Nom_Estado_Llamada=$row['Nom_Estado_Llamada'];
 ...
 <div class="col-md-3">
          ...
          <div class="box box-primary direct-chat direct-chat-primary">
            <div class="box-header with-border">
              <h3 class="box-title"><?php Echo($Nombre_Lead); ?>: <small class="blue"><i class="fa fa-phone"></i> <?php Echo($Celular_Lead); ?></small>
              	<a href="LeadsModasof.php?DeleteItem=Si&Notifica=<?php Echo($Cadena[$i]); ?>"><span class="badge badge-danger"><i class="fa fa-close "></i> Quitar Notificación</span></a>
              </h3>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
              ...
              <div class="direct-chat-messages">
                ...
                <div class="direct-chat-msg">
                  <div class="direct-chat-info clearfix">
                    <span class="direct-chat-name pull-left">Alex Baute</span>
                    <span class="direct-chat-timestamp pull-right"><?php Echo($CreadaFecha); ?>  <?php echo $date->format('H:i a');  ?></span>
                  </div>
                  <!-- /.direct-chat-info -->
                 <!--  <img class="direct-chat-img" src="../dist/img/user1-128x128.jpg" alt="Message User Image"> --><!-- /.direct-chat-img -->
                  <div class="direct-chat-text">
                    <strong><?php Echo($Nom_Tienda);?>:</strong> <?php Echo($Observacion_Admin); ?>
                    <span class="badge badge-success"><?php Echo($Nom_Estado_Llamada); ?></span>
                  </div>
                  <!-- /.direct-chat-text -->
                </div>
                <!-- /.direct-chat-msg -->
            ...
            <?php
// Contador de LLamadas 
$sql ="SELECT date_format(Fecha_Comentario,CONCAT(CONCAT(ELT(WEEKDAY(Fecha_Comentario) + 1, 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo')),', %d - ',CONCAT(ELT(MONTH(Fecha_Comentario), 'Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Agos', 'Sep', 'Oct', 'Nov', 'Dic')),'- %Y')) AS FechaC, Id_Seg_Llamada,Llamada_Id_llamada, B.Nombres,B.Apellidos,B.Img_Perfil,Comentario_Llamada, Fecha_Comentario FROM t_seguimiento_llamadas as A, t_usuarios as B  WHERE A.Vendedor_Id_Usuario=B.Id_Usuario and Llamada_Id_llamada='".$Cadena[$i]."' order by Id_Seg_Llamada DESC";  
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
      $FechaC=$row['FechaC'];
      $Id_Seg_Llamada=$row['Id_Seg_Llamada'];
      $Llamada_Id_llamada=$row['Llamada_Id_llamada'];
	  $Nombres=$row['Nombres'];
      $Apellidos=$row['Apellidos'];
      $Comentario_Llamada=$row['Comentario_Llamada'];
      $Img_Perfil=$row['Img_Perfil'];
      $Fecha_Comentario=$row['Fecha_Comentario'];
      $date2= new DateTime($Fecha_Comentario);
                                     ?>
                <div class="direct-chat-msg right">
                  <div class="direct-chat-info clearfix">
                    <span class="direct-chat-name pull-right"><?php Echo utf8_encode($Nombres." ".$Apellidos); ?></span>
                    <span class="direct-chat-timestamp pull-left"><?php Echo($FechaC); ?>  <?php echo $date2->format('H:i a');  ?></span>
                  </div>
                  <!-- /.direct-chat-info -->
                  <img class="direct-chat-img" src="<?php Echo($Img_Perfil); ?>" alt="Message User Image"><!-- /.direct-chat-img -->
                  <div class="direct-chat-text">
                    <?php Echo utf8_encode($Comentario_Llamada); ?>
                  </div>
                  <!-- /.direct-chat-text -->
                </div>
<?php 
		...
		else
		{
			...
			<div class="direct-chat-text red">
                    <?php Echo ("Sin Atención"); ?>
                  </div>
 ...
 ?>
            ...
            <div class="box-footer">
              <form action="Llamada-GuardarComentario.php?LlamadaSel=<?php Echo($LlamadaSel); ?>" method="post">
                <div class="input-group">
                	 <select class="chosen-select"  name="TxtEstado" data-placeholder="Seleccionar...">
                      <option value="">Resultado de la Llamada</option>
                           <?php 
$sql ="SELECT Id_Estado_LLamada, Nom_Estado_Llamada FROM t_estado_llamadas order by Nom_Estado_Llamada ASC";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
      $SelectId_Estado_LLamada=$row['Id_Estado_LLamada'];
      $SelectNom_Estado_Llamada=$row['Nom_Estado_Llamada'];             
      echo ("<option value='".$SelectId_Estado_LLamada."'>".utf8_encode($SelectNom_Estado_Llamada)."</option>");
 }
}
                                     ?>
                                  
                        </select>
                  <input type="text" name="TxtComentario" placeholder="Mensaje" class="form-control">
                  <input type="text" style="display: none;" name="TxtIdLlamada" value="<?php Echo($Cadena[$i]);?>" class="form-control">
