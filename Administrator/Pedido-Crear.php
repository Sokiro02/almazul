...
// Consecutivos Recibos de Caja
$sql ="SELECT Cons_RecibosCaja FROM t_config WHERE Desarrollador='TEKSYSTEM S.A.S'";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
      $Cons_RecibosCaja=$row['Cons_RecibosCaja']+1;
 }
}
...
$("#btnbuscar").click(function(){ //se crea la funcioin keyup
var texto = $('.busca').val();//se recupera el valor de la caja de texto y se guarda en la variable texto

var dataString = 'palabra='+ texto;//se guarda en una variable nueva para posteriormente pasarla a busqueda.php
if(texto==''){
alert("Introduzca en el campo su búsqueda");
//si no tiene ningun valor la caja de texto no realiza ninguna accion
...
url: "busquedaClientes.php",//la url adonde se va a mandar la cadena a buscar
data: dataString,
//cache: false,
success: function(html){//funcion que se activa al recibir un dato
$("#VistaCliente").html(html).show();// funcion jquery que muestra el div con identificador display, como formato html, tambien puede ser .text
}
});
...
<script type="text/javascript">
$(document).ready(function(){
$("#btnbuscarphone").click(function(){ //se crea la funcioin keyup
var texto = $('.buscaphone').val();//se recupera el valor de la caja de texto y se guarda en la variable texto

var dataString = 'palabraphone='+ texto;//se guarda en una variable nueva para posteriormente pasarla a busqueda.php
if(texto==''){
alert("Introduzca en el campo su búsqueda");
//si no tiene ningun valor la caja de texto no realiza ninguna accion
...
$.ajax({//metodo ajax
type: "POST",//aqui puede  ser get o post
url: "busquedaClientes.php?Phone=1",//la url adonde se va a mandar la cadena a buscar
data: dataString,
//cache: false,
success: function(html){//funcion que se activa al recibir un dato
$("#VistaCliente").html(html).show();// funcion jquery que muestra el div con identificador display, como formato html, tambien puede ser .text
...
color:#FFF;
  ...
  $Valide=$_GET['Mensaje'];
    ...
    if ($Valide==23) {
        echo "<script>jQuery(function(){swal(\"¡ Pedido Creado Correctamente!\", \"".$ClienteSel." \", \"success\");});</script>";
    };
   ?>
					...
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li>
								...
								<a href="Lista-Referencias.php">Regresar a la Galeria</a>
							</li>
							<li>
								<i class="ace-icon fa fa-users"></i>
								<a href="OrdenCliente.php">Orden Cliente</a>
							</li>
						</ul><!-- /.breadcrumb -->
...
<!-- Small boxes (Stat box) -->
      <div class="row">
          <!-- ./col -->
    <?php 
    $ClientePedido=$_GET['ClientePedido'];
     $sql ="SELECT B.Nom_Ciudad,Id_Cliente, Documento_Cliente,Avatar_Cliente, Nom_Cliente, Ape_Cliente, Ciudad_Id_Ciudad, Correo_Cliente, Tel_Cliente, Cel1_Cliente, Cel2_Cliente, Tipo_Cliente from t_clientes as A, t_ciudades as B  WHERE  Id_Cliente='".$ClientePedido."' and A.Ciudad_Id_Ciudad=B.Id_Ciudad";     
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
      $SumaPedido=$row['SumaPedido'];
                                     ...
                                     ?>
              <h4>Total Pedido Nº PDC<?php Echo($Cons_PedidosCl); ?></h4>
              <h3><?php Echo(formatomoneda($SumaPedido)); ?></h3>
              <a data-rel="tooltip" data-placement="top" title="Agregar Pago" data-toggle="modal" data-target="#modal-form2" href="#" class="btn btn-success btn-xs">Realizar Pago</a>
              <a class="btn btn-danger btn-xs">Cancelar Pedido</a>
 ...
 <!-- ============================================================== -->
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="card">
                            <div class="card-body">
                                <h5 class="card-title"><strong>RESUMEN PEDIDO Nº PDC<?php Echo($Cons_PedidosCl);  ?></strong></h5>
                                <div class="table-responsive">
                                    <table class="table table-bordered">
                                        ...
                                        <tbody>
                                          <?php 
$sql="SELECT D.Nom_Cat_Producto,C.Nom_Talla,B.Img_Referencia,Id_Temporal_Sol, Bodega_Id_Bodega, Cant_Solicitada, Talla_Solicitada, Tienda_Id_Tienda, Referencia_Id_Referencia, Fecha_Solicitud, Solicitud_Id_Usuari, Valor_Prenda,Valor_Final, Observa_Cliente FROM t_temporal_sol as A, t_referencias as B, t_tallas as C, t_categoria_producto as D WHERE A.Referencia_Id_Referencia=B.Cod_Referencia and A.Talla_Solicitada=C.Id_Talla and B.Categoria_Id_Categoria_Prod=D.Id_Cat_Producto and Solicitud_Id_Usuari='".$IdUser."' order by Id_Temporal_Sol Desc";
//Echo($sql);
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
...
$VrDescuento=$CalculoDescuento*100;
                                     ...
                                     ?>
                                            <tr>
                                                <td class="center"><img src="<?php Echo($Img_Referencia); ?>" alt="iMac" width="40"></td>
                                                <td><?php Echo($Nom_Cat_Producto); ?> REF.<?php Echo($Referencia_Id_Referencia."-".$Nom_Talla); ?></td>
                                                <td class="center"><?php Echo($Cant_Solicitada); ?> Unidad</td>
                                                <td class="font-200 center"><?php Echo(formatomoneda($Valor_Prenda)); ?></td>
                                                <td class="font-200 center"><strong><?php Echo(round($VrDescuento,0)); ?> %</strong></td>
                                                
                                                 <td class="font-200 center"><?php Echo(formatomoneda($TotalPedido)); ?></td>
                                            </tr>
                                            <?php 
                                          }
                                        }
                                             ...
                                             <tr>
                                                <td colspan="5" class="font-200" align="right"><strong>TOTAL PEDIDO</strong></td>
                                                <td class="font-200 center"><strong><?php Echo(formatomoneda($SumaPedido)); ?></strong></td>
                                    ...
                                    <div role="tabpanel" class="tab-pane active" id="iprofile">
                                        <div class="row">
                                            ...
                                            <!-- <div class="col-md-12 ml-auto">
                                                <h4 class="card-title m-t-30">Información General Cliente</h4>
         ...
         <div class="col-lg-4 col-xs-6">
          ...
          <div class="small-box bg-yellow">
            ...
            <a href="#" class="small-box-footer">
              Ver Devoluciones <i class="fa fa-arrow-circle-right"></i>
            </a>
          </div>
        </div>
        ...
        <div class="col-lg-4 col-xs-6">
          ...
          <div class="small-box bg-red">
            ...
            <a href="#" class="small-box-footer">
              Ver Cartera <i class="fa fa-arrow-circle-right"></i>
            </a>
          </div>
        </div>
...
<!-- Inicio Modal -->
		<div id="modal-form2" class="modal" tabindex="-1">
							<div class="modal-dialog ">
										<div class="modal-content">
											<div class="modal-header">
												...
												<h4 class="black bigger"><i class="fa fa-circle-info"> El abono mínimo solicitado es del 50% (<?php Echo(formatomoneda($SumaPedido/2)); ?>)</i>
				...
				<form action="Pedido-guardar.php" method="post">		

        <input style="display: none;" type="text" name="TxtConsecutivo" value="<?php Echo($Cons_PedidosCl);?>">	
        <input style="display: none;" type="text" name="TxtCliente" value="<?php Echo($ClientePedido);?>">  
        <input style="display: none;" type="text" name="TxtSumaPedido" value="<?php Echo($SumaPedido);?>">
        <input style="display: none;" type="text" name="TxtConsecutivoReciboCaja" value="<?php Echo($Cons_RecibosCaja);?>">
        <?php 
        if ($MyIdTienda!="") {
          ?>
        ...
        {
          ?>
          <input style="display: none;" type="text" name="TxtTienda" value="<?php Echo($Tienda_Id_Tienda);?>">
          <?php
        }
      ...
      <div id="PagoEfectivo" class="col-xs-12 col-sm-12" style="margin-bottom: 10px;">

        <div class="col-sm-3 col-xs-12 center">
          <strong>Pago en Efectivo:</strong>
        </div>
       <div class="col-sm-6 col-xs-12">
        <input class="nav-search-input input-xxlarge" type="text" value="$0" id="demo1" placeholder="$ 0" name="demo1"  required="true" />
...
symbolPosition : 'left' // use this setting to position the symbol at the left or right side of the value. default 'left'
      ...
      <div id="PagoTarjeta" class="col-xs-12 col-sm-12" style="margin-bottom: 10px;">
        <div class="col-sm-3 col-xs-12 center">
          <strong>Pago en Tarjeta:</strong>
        </div>
       <div class="col-sm-3 col-xs-12">
        <input class=" nav-search-input input-xxlarge" type="text" value="$0" id="demo2" placeholder="$ 0" name="demo2"  required="true" />
...
symbolPosition : 'left' // use this setting to position the symbol at the left or right side of the value. default 'left'
     ...
     <div class="col-sm-3 col-xs-12">
        <input type="text" name="TxtNumVoucher" class="input-xlarge" placeholder="Nº Voucher">
      </div>
    ...
    <div id="PagoTransferencia" class="col-xs-12 col-sm-12" style="margin-bottom: 10px;">
        <div class="col-sm-3 col-xs-12 center">
          <strong>Transferencia:</strong>
        </div>
       <div class="col-sm-3 col-xs-12">
        <input class=" nav-search-input input-xxlarge" type="text" value="$0" id="demo3" placeholder="$ 0" name="demo3"  required="true" />
...
symbolPosition : 'left' // use this setting to position the symbol at the left or right side of the value. default 'left'
     ...
     <div class="col-sm-3 col-xs-12">
        <input type="text" name="TxtTransaccion" class="input-xlarge" placeholder="Nº Transacción">
      </div>

     
    </div>
		    
     <div id="PagoEfecty" class="col-xs-12 col-sm-12" style="margin-bottom: 10px;">
        <div class="col-sm-3 col-xs-12 center">
          <strong>Efecty/Baloto:</strong>
        </div>
       <div class="col-sm-3 col-xs-12">
        <input class=" nav-search-input input-xxlarge" type="text" value="$0" id="demo4" placeholder="$ 0" name="demo4"  required="true" />
...
symbolPosition : 'left' // use this setting to position the symbol at the left or right side of the value. default 'left'
     ...
     <div class="col-sm-3 col-xs-12">
        <input type="text" name="TxtNumEfecty" class="input-xlarge" placeholder="Nº Transacción">
      </div>

     
    </div>
        
     <div id="TotalPago" class="col-xs-12 col-sm-12" style="margin-bottom: 10px;">
       ...
       <div class="col-xs-12 col-sm-6" style="margin-bottom: 10px;">
                                ...
                                <div class="input-group">
                                  ...
                                  <input required="true" class="form-control date-picker" id="id-date-picker-1" type="text" data-date-format="yyyy-mm-dd" name="TxtFechaEntrega" value="" />
      ...
      <button class="btn  btn-danger" type="submit"><i class="fa fa-check"> </i> Finalizar Pedido Nº PDC<?php Echo($Cons_PedidosCl); ?></button>
      </div>
