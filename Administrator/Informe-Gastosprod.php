					...
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li>
								...
								<a href="index-produccion.php">Inicio</a>
							</li>

							<li>
								<i class="ace-icon fa fa-home"></i>
								<a href="Informe-Gastosprod.php">Gastos Tienda</a>
							</li>
...
<div class="widget-body">
		<h4 class="widget-title lighter">
			...
			<div class="row">
				<form action="Informe-Gastosprod.php?QueryCon=<?php Echo($TxtConsultaVen) ?>" method="post" id="FormFechas" autocomplete="off">
...
<?php 
	...
	$sql ="SELECT Id_Gasto FROM t_gastos WHERE Fecha_gasto >='".$FechaStart."' and Fecha_gasto <='".$FechaEnd."' and  Area_Id_Area='".$MyIdTaller."' order by Id_Gasto  DESC";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
                                    ...
                                    <th class="success"></th>
												...
												<thead>
													...
													<tr class="warning">
														...
														<th class="tdcustom" >Subido por</th>
														<th class="tdcustom" >Taller</th>
...
for($i=0; $i<$min; $i++)
{
	...
	$sql ="SELECT date_format(A.Fecha_gasto,CONCAT(CONCAT(ELT(WEEKDAY(A.Fecha_gasto) + 1, 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo')),', %d - ',CONCAT(ELT(MONTH(A.Fecha_gasto), 'Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Agos', 'Sep', 'Oct', 'Nov', 'Dic')),'- %Y')) AS FechaGasto,Fecha_gasto, Id_Gasto, Beneficiario,Descripcion,Url_soporte,Valor_gasto, Marca_temporal,B.Nombres, B.Apellidos,C.Nom_Subrubro FROM t_gastos as A, t_usuarios as B, t_subrubros as C WHERE Id_Gasto='".$Cadena[$i]."' and A.Usuario_id_usuario=B.Id_Usuario and A.Sub_rubro=C.Id_subrubro";
		//Echo($sql);
		$result = $conexion->query($sql);
...
?>
			...
			<a data-rel="tooltip" data-placement="top" title="Detalle Compra" href="Pedido-Ver.php?PedidoCliente=<?php Echo($Cod_Pedido); ?>">
														...
														<td>
															<?php Echo utf8_encode($Nom_Subrubro); ?>
														</td>
