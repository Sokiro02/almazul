...
if ($DeleteItem != "") {
    ...
    $ssql2     = "UPDATE t_solicitudes_prod SET Existencias_Ref=Existencias_Ref+'" . $NCantidad . "' WHERE Id_Solicitud_Prod='" . $Id_Solicitud_Prod . "'";
    ...
    if ($result) {
        header("location:DespachosM.php?TxtTienda=" . $Cod_Tienda . "&Mensaje=63&TxtNomTienda=" . $Nom_Tienda2);
        ...
        header("location:DespachosM.php?TxtTienda=" . $id_tienda . "&Mensaje=64&TxtNomTienda=" . $Nom_Tienda2);
    }
}
...
<!-- Fin de Primer Fila -->
										...
										<div class="col-sm-12">
											<div class="widget-box">
												<div class="widget-header">
													<h4 class="widget-title">Cargar INVENTARIO DE REFERENCIAS</h4>
		...
		<form method="post" action="DespachosM-guardar.php" id="FormOrden" onsubmit="return mifuncion();">
...
$consultas                 = "SELECT * FROM t_solicitudes_prod WHERE Estado_Solicitud_Prod ='" . $valor_centro_distribucion . "'and Existencias_Ref>0 and Bodega_Id_Bodega='".$MyIdTaller."'";
$resultado_consulta        = $conexion->query($consultas) or die('Error:' . mysqli_error($conexion));
$cantidad                  = $resultado_consulta->num_rows;
$i                         = 0;
if ($resultado_consulta->num_rows > 0) {
    while ($fila_consulta = $resultado_consulta->fetch_assoc()) {
        $codigo_de_referencia = $fila_consulta['Referencia_Id_Referencia'];
        $Existencia           = $fila_consulta['Existencias_Ref'];
        $Talla_Id             = $fila_consulta['Talla_Solicitada'];
        $Id_Solicitud         = $fila_consulta['Id_Solicitud_Prod'];
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                echo ("<option value='" . $Id_Solicitud . "-" . $SelectIdInsumo . "-" . $Nombre_Talla . "-" . $Talla_Id . "-" . $Existencia . "' data-img-src='" . $mostrar_img . "'>" . $SelectCodInsumo . " " . $SelectNombreInsumo . " Talla: " . $Nombre_Talla . " Existencia: " . $Existencia . "</option>");
            }
        }
    }
}
...
<div class="form-group  col-sm-1">
    <label style="display: none;" for="form-field-select-3">Cliente?</label>
    <div>
        <input style="display: none;" type="checkbox" name="cliente" value="si">
    </div>
</div>
				...
				<tr>
					...
					<td>
						<div class=" action-buttons">
							<a href="DespachosM.php?Delete=<?php echo ($id); ?>&TxtTienda=<?php echo ($id_tienda); ?>&TxtNomTienda=<?php echo ($Nom_Tienda2); ?>" class="tooltip-error red" data-rel="tooltip" data-placement="top" title="Eliminar Item">
...
<br />
<div class="center">
    <a onclick="return mifuncion2();" href="DespachosM-Guardar2.php?IDTIENDA=<?php echo $Cod_Tienda; ?>&NomTienda=<?php echo $Nom_Tienda2; ?>">
        <div class="infobox degrade-verde" id="">
            <div class="infobox-data">
    		  <span class="infobox-data-number"><i class="fa fa-save"></i> FINALIZAR ORDEN</span>
            </div>
        </div>
    </a>
</div>
