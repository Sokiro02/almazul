...
$Datos="Ingreso a Generar Codigos de Barra";
...
if ($DeleteItem!="") {
	$sql ="DELETE FROM t_temporal_codigos WHERE id='".$DeleteItem."'";  
    $result = $conexion->query($sql) or die('Error:'.mysqli_error($conexion));
    if ($result){
        header("location:Generar-CodigosBarra.php?Mensaje=63");
        ...
        header("location:Generar-CodigosBarra.php?Mensaje=64");
    }
}
...
<html lang="en">
	<head>
		...
		<meta charset="utf-8" />
		<title>GENERAR CODIGOS DE BARRA Modasof</title>
...
<!-- Fin de Primer Fila -->
										...
										<div class="col-sm-12">
											<div class="widget-box">
												<div class="widget-header">
													<h4 class="widget-title">GENERAR CODIGOS DE BARRA PARA REFERENCIAS</h4>
				...
				<form method="post" action="CodigosGuardar.php" id="FormOrden">
	...
	<div>
<select class="my-select" id="TxtInsumoSel" name="TxtInsumoSel">
	<option value="">Seleccionar Referencias</option>

<?php
$sql ="SELECT * FROM t_referencias";  
$result = $conexion->query($sql) or die('Error:'.mysqli_error($conexion));;
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
    	...
    	Echo("<option value='".$SelectIdInsumo."' data-img-src='".$mostrar_img."'>".$SelectCodInsumo."</option>");
 }
}
?>
</select>
...
<input style="display: none;" type="text" name="TxtStatus" value="PROCESANDO">
 <div class="form-group  col-sm-1">
	...
	<option style="text-transform:uppercase;" value="NA">NA</option>
																	<?php
$sql ="SELECT Id_Talla, Nom_Talla FROM t_tallas order by Nom_Talla ASC";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
    	$SelectIdTalla=$row['Id_Talla'];
    	$SelectNombreTalla=$row['Nom_Talla'];             
    	echo ("<option style='text-transform:uppercase;' value='".$SelectIdTalla."'>".utf8_decode($SelectNombreTalla)."</option>");
 }
}
...
?>
					...
					<!-- <select required="true" class="chosen-select input-xxlarge" name="TxtCantidad" id="TxtCantidad" data-placeholder="Seleccionar...">
									...
									<input type="number" id="TxtCantidad" step="any" class="input-xxlarge" name="TxtCantidad" required="true">
						...
						<div class="clearfix">
							...
							<table style="font-size: 12px;" id="dynamic-table"  class="table table-responsive table-striped table-bordered table-hover" width="100%">
												...
												<tbody>
<?php 
	$consulta_inventario ="SELECT * FROM t_temporal_codigos WHERE id_user='".$IdUser."' order by id desc";
				...
				<tr>
					...
					<td>
						<div class=" action-buttons">
							<a href="Generar-CodigosBarra.php?Delete=<?php echo($id);?>" class="tooltip-error red" data-rel="tooltip" data-placement="top" title="Eliminar Item">
...
<div class="center">
<form method="post" action="codigos_barra3.php" id="FormOrden">
    <input style="display: none;" type="text" name="ID" value="<?php Echo($IdUser) ?>">
