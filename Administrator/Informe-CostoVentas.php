...
<html lang="en">
	<head>
		...
		<meta charset="utf-8" />
		<title>Informe Costos-Ventas Modasof</title>
					...
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li>
								...
								<a href="Informe-CostoVentas.php">Costos vs Ventas Tienda</a>
							</li>
...
<div class="widget-body">
		<h4 class="widget-title lighter">
			...
			<div class="row">
				<form action="Informe-CostoVentas.php?QueryCon=<?php Echo($TxtConsultaVen) ?>" method="post" id="FormFechas" autocomplete="off">
...
<div class="col-sm-12"><!-- Inicio Panel 12 -->
										...
										<div class="table-header" style="background-color: #000;">
											...
											<?php 
												...
												{
													Echo("Egresos del ".fechaSql($FechaUno)." al ".fechaSql($FechaDos));
												}
											?> 
										</div>
...
if ($TxtConsultaVen!="") {
		$sql ="SELECT Id_venta FROM t_ventas WHERE Fecha_Solicitud >='".$FechaStart."' and Fecha_Solicitud <='".$FechaEnd."' and  	Tienda_id_Tienda='".$TxtConsultaVen."' order by Id_venta  DESC";
		//Echo($sql);
		$result = $conexion->query($sql);
		if ($result->num_rows > 0) {
    	while ($row = $result->fetch_assoc()) { 
...
{
		$sql ="SELECT Id_venta FROM t_ventas WHERE Fecha_Solicitud >='".$FechaStart."' and Fecha_Solicitud <='".$FechaEnd."' order by Id_venta  DESC";
		//Echo($sql);
		$result = $conexion->query($sql);
		if ($result->num_rows > 0) {
    	while ($row = $result->fetch_assoc()) { 
                                        ...
                                        <th class="success"></th>
												...
												<thead>
													...
													<tr class="warning">
														...
														<th class="tdcustom" >Cant. Vendida</th>
														<th class="tdcustom" >Insumos</th>
														<th class="tdcustom" >M.Obra</th>
														<th class="tdcustom" >Total Costo</th>
														<th class="tdcustom" >Vr. Prenda</th>
														<th class="tdcustom" >Vr. Vendida</th>
														<th class="tdcustom" >Descuento</th>
														<th class="tdcustom" >Utilidad Bruta</th>
...
if ($TxtConsultaVen!="") {
	...
	$sql ="SELECT * FROM t_ventas WHERE Id_venta='".$Cadena[$i]."' and Tienda_id_Tienda='".$TxtConsultaVen."'";
		//Echo($sql);
		$result = $conexion->query($sql);
...
{
	// Todos los movimientos sin filtro de tienda
	$sql ="SELECT Referencia_Id_Referencia, Ref_Vendida,Valor_Final,Valor_Prenda,Vendedor_id_usuario,Tienda_id_Tienda,Cant_Solicitada,Factura_Id_Factura,Fecha_Solicitud,consecutivosc_id_consecutivosc FROM t_ventas WHERE Id_venta='".$Cadena[$i]."'";
		//Echo($sql);
		$result = $conexion->query($sql);
...
// VERIFICAR EL VENDEDOR QUE REALIZÓ EL EGRESO

$sql ="SELECT * FROM t_usuarios WHERE Id_Usuario='".$Vendedor_id_usuario."'";
		//Echo($sql);
		$result = $conexion->query($sql);
 if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {  
...
$sql ="SELECT * FROM t_facturas WHERE Num_Factura='".$Factura_Id_Factura."'";
		//Echo($sql);
		$result = $conexion->query($sql);
 if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {  
...
// VERIFICAR LA TIENDA QUE REALIZÓ EL EGRESO
...
$sql ="SELECT SUM(Costo_Insumo_Ref*Cant_Solicitada) AS CostoProduccion FROM t_insumos_ref WHERE Referencia_Cod_Referencia='".$Referencia_Id_Referencia."' ";
//$sql ="SELECT SUM(Prom_Temporal*Cant_Temporal) AS CostoProduccion FROM t_temporal_ref2 WHERE Referencia_Temporal='".$Referencia_Id_Referencia."' ";
		//Echo($sql);
		$result = $conexion->query($sql);
		if ($result->num_rows > 0) {
...
$sql ="SELECT V_Mano_Obra_Ref  FROM t_referencias WHERE Cod_Referencia='".$Referencia_Id_Referencia."' ";
		//Echo($sql);
		$result = $conexion->query($sql);
		if ($result->num_rows > 0) {
    		while ($row = $result->fetch_assoc()) {                
			...
			$TotalCosto=$CostoPrenda*$Cant_Solicitada;
			$TotalComercial=$Valor_Final*$Cant_Solicitada;
...
if ($Valor_Final==0) {
	...
	$Operacion1=((int)$TotalCosto/(int) $Valor_Final);
...
?>
														...
														<td class="center">
															SC<?php Echo($consecutivosc_id_consecutivosc) ?><br>
														...
														<td class="center">
															<?php Echo utf8_encode("$ ".number_format($CostoProduccion));  ?>
														</td>
														<td class="center">
														...
														<td>
															<?php Echo(round($VrDescuento,0)); ?> %
														</td>
														<td class="tdcustom">
															<?php Echo(round($UtilidadBruta,1)); ?>%	
															</td>
														 <td>
			<div class="btn-group">
												...
												<ul class="dropdown-menu dropdown-info dropdown-menu-right">
													...
													if ($IdRol==1) {
														?>
													<li>
														<a target="_blank"  data-rel="tooltip" data-placement="top" title="Editar Costos de la Prenda" href="Producto-modificar.php?RefSel=<?php Echo($Referencia_Id_Referencia) ?>"><i class="fa fa-edit bigger-100 blue"></i> Editar Costos</a>
													</li>
