...
include("Lib/seguridad.php");
    $Datos="Ingreso al Informe de la Tienda: ".$TxtConsultaVen;
					...
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li>
								...
								<a href="Informe-IngresosTienda.php">Informe Ingresos</a>
							</li>

							<li>
								<i class="ace-icon fa fa-home"></i>
								<a href="CajaTienda.php">Mi Caja</a>
							</li>
...
<div class="widget-body">
		<h4 class="widget-title lighter">
			...
			<div class="row">
				<form action="Informe-IngresosTienda.php?QueryCon=<?php Echo($TxtConsultaVen) ?>" method="post" id="FormFechas" autocomplete="off">
...
<?php 
	...
	$sql ="SELECT Id_Ingreso FROM t_ingresos WHERE Fecha_Ingreso >='".$FechaStart."' and Fecha_Ingreso <='".$FechaEnd."' and  Tienda_Id_Tienda='".$TxtConsultaVen."' order by Id_Ingreso  DESC";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
...
if ($TxtConsultaVen!="") {
	$sql ="SELECT date_format(A.Fecha_Ingreso,CONCAT(CONCAT(ELT(WEEKDAY(A.Fecha_Ingreso) + 1, 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo')),', %d - ',CONCAT(ELT(MONTH(A.Fecha_Ingreso), 'Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Agos', 'Sep', 'Oct', 'Nov', 'Dic')),'- %Y')) AS FechaIngreso,Id_Ingreso, Medio_Pago,Num_Transaccion,Valor_Ingreso,Concepto_Ingreso, B.Nombres,B.Apellidos,C.Nom_Tienda, D.Nom_Cliente,D.Ape_Cliente FROM t_ingresos as A, t_usuarios as B, t_tiendas as C, t_clientes as D WHERE Id_Ingreso='".$Cadena[$i]."' and Tienda_Id_Tienda='".$TxtConsultaVen."' and A.Ingreso_Id_Usuario=B.Id_Usuario and A.Tienda_Id_Tienda=C.Id_Tienda and A.Cliente_Id_Cliente=D.Id_Cliente";
		//Echo($sql);
		$result = $conexion->query($sql);
}
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {  
        ...
        //$dateSol = new DateTime($FechaIngreso);
