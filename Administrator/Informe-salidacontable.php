...
<html lang="en">
	<head>
		...
		<meta charset="utf-8" />
		<title>Informe Salidas Contables Tienda Modasof</title>
...
<script src="https://modasof.com/espejo/assets/js/jquery.maskMoney.js" type="text/javascript"></script>
  ...
  $Fact=$_GET['Fact'];
    ...
    if ($Valide==1) {
     ...
     if ($Valide==3) {
        echo "<script>jQuery(function(){swal(\"¡ Plan Separe Facturado!\", \"Correctamente \", \"success\");});</script>";
    };
    if ($Valide==2) {
					...
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li>
								...
								<a href="Informe-salidacontable.php">Informe Salidas contables</a>
							</li>
...
$NumeroFactura=$_GET['NumeroFactura'];
				...
				$sql ="SELECT Fecha_Factura,Factura_Paga FROM t_facturas Where num_consecutivo_sc='".$NumeroFactura."'";  
    ...
    while ($row = $result->fetch_assoc()) {   
      ...
      elseif ($Factura_Paga==2) {
      	$Pagode="Crédito";
					...
					<form action="Clientes-EditarFactura.php" method="post" id="FormUpdateFactura">
						<input style="display: none;" type="text" name="TxtFactura" value="<?php Echo($NumeroFactura)?>">
									<div class="modal-dialog">
										<div class="modal-content">
											<div class="modal-header">
												
												<h4 class="blue bigger">Cambiar Fecha en Salida Contable  Nº <?php Echo($NumeroFactura);?></h4>
											</div>
							...
							<label for="form-field-select-3">Seleccione la nueva fecha de la Salida :</label>

  <div class="input-group">
                                  
                                  <input required="true" class="form-control date-picker" id="id-date-picker-1" type="text" data-date-format="yyyy-mm-dd" name="TxtNuevaFecha" value="<?php Echo(fechasql($Fecha_Factura))?>" />
                                  <span class="input-group-addon">
                                    <i class="fa fa-calendar bigger-110"></i>
												...
												<div class="form-group">
							<label for="form-field-select-3">Seleccionar Tipo de Pago:</label>

  <div class="input-group">
                                  
                                <select name="TxtTipoFactura" required="true" data-placeholder="Seleccionar...">
                                	<option selected="true" value="<?php Echo($Factura_Paga)?>"><?php Echo($Pagode); ?></option>
                                	...
                                	<option value="2">Crédito</option>
												...
												<a href="Informe-salidacontable.php" class="btn btn-sm" data-dismiss="modal">
...
<div class="widget-body">
		<h4 class="widget-title lighter">
			...
			<div class="row">
				<form action="Informe-salidacontable.php?QueryCon=<?php Echo($TxtConsultaVen) ?>" method="post" id="FormFechas" autocomplete="off">
					<div class="col-xs-12 col-sm-6">
...
if ($TxtConsultaVen=='') {
	$sql ="SELECT DISTINCT(num_consecutivo_sc) FROM t_facturas WHERE Fecha_Factura >='".$FechaStart."' and Fecha_Factura <='".$FechaEnd."' and estado_sc='1' order by num_consecutivo_sc  DESC";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
	$Lista=$Lista.$row['num_consecutivo_sc'].",";                 
 }
}
}
else
{
	$sql ="SELECT DISTINCT(num_consecutivo_sc) FROM t_facturas WHERE Fecha_Factura >='".$FechaStart."' and Fecha_Factura <='".$FechaEnd."' and  Tienda_Id_Tienda='".$TxtConsultaVen."' and estado_sc='1' order by num_consecutivo_sc  DESC";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
 ...
 ?>
                                    ...
                                    <th class="success"></th>
												...
												<thead>
													...
													<tr class="warning">
														<th class="tdcustom" >Nota Entrega</th>
														...
														<th class="tdcustom" >Sub-Total</th>
														<th class="tdcustom" >I.V.A</th>
														<th class="tdcustom" >Total NE</th>
														<th class="tdcustom" >Abonos</th>
														<th class="tdcustom" >Pendiente</th>
...
if ($TxtConsultaVen=='') {
	$sql ="SELECT date_format(Marca_Temporal,CONCAT(CONCAT(ELT(WEEKDAY(Marca_Temporal) + 1, 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo')),', %d - ',CONCAT(ELT(MONTH(Marca_Temporal), 'Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Agos', 'Sep', 'Oct', 'Nov', 'Dic')),'- %Y')) AS FechaTemporal,date_format(Fecha_Factura,CONCAT(CONCAT(ELT(WEEKDAY(Fecha_Factura) + 1, 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo')),', %d - ',CONCAT(ELT(MONTH(Fecha_Factura), 'Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Agos', 'Sep', 'Oct', 'Nov', 'Dic')),'- %Y')) AS FechaSolicitado, Total_Factura,num_consecutivo_sc, Factura_Paga,Marca_Temporal,Observa_Factura,Tienda_Id_Tienda,Usuario_Vendedor,Iva_Factura,Subtotal_Factura FROM t_facturas WHERE num_consecutivo_sc='".$Cadena[$i]."' and Fecha_Factura >='".$FechaStart."' and Fecha_Factura <='".$FechaEnd."'";
		//Echo($sql);
		$result = $conexion->query($sql);
}
	else
{
	$sql ="SELECT date_format(Marca_Temporal,CONCAT(CONCAT(ELT(WEEKDAY(Marca_Temporal) + 1, 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo')),', %d - ',CONCAT(ELT(MONTH(Marca_Temporal), 'Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Agos', 'Sep', 'Oct', 'Nov', 'Dic')),'- %Y')) AS FechaTemporal,date_format(Fecha_Factura,CONCAT(CONCAT(ELT(WEEKDAY(Fecha_Factura) + 1, 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo')),', %d - ',CONCAT(ELT(MONTH(Fecha_Factura), 'Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Agos', 'Sep', 'Oct', 'Nov', 'Dic')),'- %Y')) AS FechaSolicitado, Total_Factura,num_consecutivo_sc, Factura_Paga,Marca_Temporal,Observa_Factura,Tienda_Id_Tienda,Usuario_Vendedor,Iva_Factura,Subtotal_Factura FROM t_facturas WHERE num_consecutivo_sc='".$Cadena[$i]."' and Tienda_Id_Tienda='".$TxtConsultaVen."' and Fecha_Factura >='".$FechaStart."' and Fecha_Factura <='".$FechaEnd."'";
		//Echo($sql);
		$result = $conexion->query($sql);
}
...
$SubTotal=$Total_Factura/1.19;
$Iva=$Total_Factura-$SubTotal;
$TotalCompra=$Iva+$SubTotal;
$VrComision=($Total_Factura*1)/100;
//$VrAbonos=0;

$sql ="SELECT cliente_id_cliente FROM t_facturas WHERE num_consecutivo_sc='".$Cadena[$i]."' and Fecha_Factura >='".$FechaStart."' and Fecha_Factura <='".$FechaEnd."'";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
        $Elcliente=$row['cliente_id_cliente'];                
 }
}



$sql ="SELECT IFNULL(sum(Valor_Ingreso),0) as TotalAbonos FROM t_ingresos WHERE consecutivosc_id_consecutivosc='".$Cadena[$i]."' and Cliente_Id_Cliente='".$Elcliente."'";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
...
?>
														...
														<td class="center">
															NE<?php Echo($Cadena[$i]) ?>
														</td>
													
													
														<td>
															<?php Echo utf8_encode(ObtenernombreUsuario($Usuario_Vendedor)); ?>
														</td>
														...
														<td class="center">
															<?php Echo utf8_encode(ObtenerdocumentoCliente($Elcliente)); ?>		
														</td>
														
														
														<td class="tdcustom">
															<?php Echo utf8_encode(ObtenernombreCliente($Elcliente)); ?>	
														</td>
														...
														<td>
															...
															if ($Factura_Paga==1) {
																?>
																<a target="_blank"  data-rel="tooltip" data-placement="top" title="<?php Echo($Observa_Factura); ?>" href="#"><span class='badge bg-green'><?php Echo($Pagode); ?></span></a>
																<?php
															}
															elseif ($Factura_Paga==2)
															{
																?>
																<a target="_blank"  data-rel="tooltip" data-placement="top" title="<?php Echo($Observa_Factura); ?>" href="#"><span class='badge bg-red'><?php Echo($Pagode); ?></span></a>
																<?php
															}
															elseif ($Factura_Paga==3) {
																?>
																<a target="_blank"  data-rel="tooltip" data-placement="top" title="<?php Echo($Observa_Factura); ?>" href="#"><span class='badge bg-warning'><?php Echo($Pagode); ?></span></a>
																<?php
															}
														...
														<td>
															<?php Echo utf8_encode("$ ".number_format($Subtotal_Factura));  ?>
														</td>
														
														
															<td class="center">
															<?php Echo utf8_encode("$ ".number_format($Iva_Factura));  ?>
														</td>
														<td class="center">
															<?php 
															if ($TotalCompra<0) {
																$TotalCompra=0;
																Echo utf8_encode("$ ".number_format($TotalCompra)); 
															}
															else
															{
																Echo utf8_encode("$ ".number_format($TotalCompra)); 
														...
														<td class="center">
															...
															<?php 
																...
																Echo utf8_encode("$ ".number_format($VrAbonos));	
														...
														<td class="center">
															...
															<?php Echo utf8_encode("$ ".number_format($VrSaldo));  ?>
														</td>
												...
												<ul class="dropdown-menu dropdown-info dropdown-menu-right">
													<li>
		<a target="_blank" data-rel="tooltip" data-placement="top" title="Imprimir Nota Entrega" href="Pdf_FacturaNE.php?FacturaImp=<?php Echo($Cadena[$i]); ?>&DeTienda=<?php echo($TiendaOk) ?>"><i class="fa fa-file-pdf-o bigger-100 blue"></i> Imprimir Nota Entrega</a>
													</li>
													<li>
														<a target="_blank"  data-rel="tooltip" data-placement="top" title="Abonar-Cancelar" href="Clientes-AbonosNE.php?AbonosFactura=<?php Echo($Cadena[$i]) ?>&&Detienda=<?php echo($TiendaOk) ?>"><i class="fa fa-dollar bigger-100 blue"></i> Ver/Crear Abonos</a>
													</li>

													<li>
														<a target="_blank"  data-rel="tooltip" data-placement="top" title="Crear Devolución" href="Clientes-DevolucionesNE.php?NumeroFactura=<?php Echo($Cadena[$i]) ?>"><i class="fa fa-exchange bigger-100 blue"></i> Cambio por Talla</a>
													</li>
													<li>
														<a target="_blank"  data-rel="tooltip" data-placement="top" title="Detalle Factura" href="Clientes-DetalleNotaEntrega.php?NumeroFactura=<?php Echo($Cadena[$i]) ?>&DeTienda=<?php echo($TiendaOk) ?>"><i class="fa fa-eye bigger-100 blue"></i> Detalle Nota Entrega</a>
													</li>
													...
													if ($IdRol==1) {
														...
														<li class="divider"></li>
													<li>
														<a style="position: relative;" data-rel="tooltip" data-placement="top" title="Anular Salida"  href="clientes-anularsalidacontable.php?NumeroFactura=<?php Echo($Cadena[$i]) ?>"><i class="fa fa-close red "></i> Anular Salida</a>
													</li>
													<li>
											<a style="position: relative;" data-rel="tooltip" data-placement="top" title="Editar Fecha"  href="Informe-salidacontable.php?NumeroFactura=<?php Echo($Cadena[$i]) ?>&EditTask=1"><i class="fa fa-calendar red "></i> Editar Fecha</a>
													</li>
														<?php
													}
													 ?>
