...
if ($ConfirmationBaja!="") {
	...
	$sql=("UPDATE t_bonos SET Estado_Bono='".$EstadoBono."' WHERE Id_bono='".$ConfirmationBaja."'");
   //echo($sql);
   $result = $conexion->query($sql);
   
	
   header("location:Informe-Bonos.php?Mensaje=335");
...
<html lang="en">
	<head>
		...
		<meta charset="utf-8" />
		<title>Informe Bonos Modasof</title>
...
<script src="https://modasof.com/espejo/assets/js/jquery.maskMoney.js" type="text/javascript"></script>
  ...
  $OrdenSeleccionado=$_GET['OrdenSeleccionado'];
    ...
    if ($Valide==332) {
        echo "<script>jQuery(function(){swal(\"¡Lo Sentimos!\", \"El número de bono ya Existe\", \"error\");});</script>";
    };
    if ($Valide==1) {
        echo "<script>jQuery(function(){swal(\"¡ Insumo Creado!\", \"Correctamente \", \"success\");});</script>";
    };
     if ($Valide==333) {
        echo "<script>jQuery(function(){swal(\"¡ Bono Creado!\", \"Correctamente \", \"success\");});</script>";
    };
     if ($Valide==334) {
        echo "<script>jQuery(function(){swal(\"¡ Bono Actualizado!\", \"Correctamente \", \"success\");});</script>";
    };
     if ($Valide==334) {
        echo "<script>jQuery(function(){swal(\"¡ Bono Redimido!\", \"Correctamente \", \"success\");});</script>";
    };
    if ($Valide==28) {
    ...
    if (isset($_GET['NumBonoBaja'])) {
	 $NumBonoBaja=$_GET['NumBonoBaja'];
...
function(isConfirm){
  if (isConfirm) {
    window.location.href = 'Informe-Bonos.php?ConfirmationBaja=<?php Echo($NumBonoBaja); ?>';
  } else {
    window.location.href = 'Informe-Bonos.php';
  }
});
// swal({
					...
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li>
								...
								<a href="Informe-Bonos.php">Informe Bonos</a>
							</li>
							<li>
								<i class="ace-icon fa fa-plus-square"></i>
								<?php 
	if ($IdRol==1) {
		echo("<a href='Informe-Bonos.php?EditTask=1' class='red'>CREAR BONO</a>");
	}
 ?>
							</li>
...
<?php 
					$EditTask2 = $_GET['EditTask2'];
						if ($EditTask2!="") {
	$UpBono=$_GET['UpBono'];
	$sql ="SELECT Num_Bono,Fecha_Bono,Tienda_Id_Tienda,B.Nom_Tienda,Valor_Comercial FROM t_bonos as A, t_tiendas as B WHERE Id_bono='".$UpBono."' and A.Tienda_Id_Tienda=B.Id_Tienda";
	//Echo($sql);
	$result = $conexion->query($sql);
	...
	$Nom_Tienda=$row['Nom_Tienda'];
					 ...
					 ?>
								<div id="DivEditar">
						<form action="Bonos-Actualizar.php" method="post" id="FormUpdateCliente">

						<input style="display: none;" type="text" name="TxtIdBono" value="<?php echo($UpBono); ?>">

									<div class="modal-dialog">
										<div class="modal-content">
											<div class="modal-header">
												
												<h4 class="blue bigger">Actualizar Bono Modasof</h4>
											</div>
										...
										<div class="col-xs-12 col-sm-6 ">
														...
														<div class="form-group">
															<label for="form-field-select-3">Ingrese el número del Bono </label>
				...
				<input type="text" name="TxtNumBono" class="input-sm" value="<?php Echo($Num_Bono); ?>">								
			<!-- <input class="input-sm" type="text" id="demo1" placeholder="Valor Ingreso" name="demo1"  required="true" value="" /> -->
...
symbolPosition : 'left' // use this setting to position the symbol at the left or right side of the value. default 'left'
							...
							<div class="form-group">
								<label for="form-field-select-3">Seleccione la tienda a la que entrega el Bono </label>
													 <select name="TxtTienda" id="TxtTienda">
                                        <option selected="true" value="<?php Echo($Tienda_Id_Tienda); ?>"><?php Echo($Nom_Tienda); ?></option>
                                       <?php 
    $sql ="SELECT Id_Tienda, Nom_Tienda FROM t_tiendas Where Estado_Tienda='1' order by Id_Tienda ASC";  
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
      ...
      Echo("<option value='".$Id_Tienda."'>".$Nom_Tienda."</option>");
                                       ...
                                       ?>
											...
											<div class="form-group">
							<label for="form-field-select-3">Fecha de entrega del Bono :</label>

  <div class="input-group">
                                  
                                  <input required="true" class="form-control date-picker" id="id-date-picker-1" type="text" data-date-format="yyyy-mm-dd" name="TxtNuevaFecha" value="" />
                                  <span class="input-group-addon">
                                    <i class="fa fa-calendar bigger-110"></i>
												...
												<div class="form-group">
															<label for="form-field-select-3">Editar el Valor </label>
															<div class="input-group">
																<span class="input-group-addon">
																	<i class="ace-icon fa fa-user"></i>
																</span>
			...
			<input class="input-sm" type="text" id="demo1" placeholder="Valor Ingreso" name="demo1"  required="true" value="<?php Echo($Valor_Comercial); ?>" />
...
symbolPosition : 'left' // use this setting to position the symbol at the left or right side of the value. default 'left'
												...
												<a href="Informe-Bonos.php" class="btn btn-sm" data-dismiss="modal">
		 ...
		 ?>
					...
					<form action="Bonos-Crear.php" method="post" id="FormUpdateCliente">
									...
									<div class="modal-dialog">
										<div class="modal-content">
											<div class="modal-header">
												...
												<h4 class="blue bigger">Crear Bono Modasof</h4>
											</div>
										...
										<div class="col-xs-12 col-sm-6 ">
											...
											<div>
				<input type="text" name="TxtNumBono" class="input-sm">								
...
<div class="widget-body">
		<h4 class="widget-title lighter">
			...
			<div class="row">
				<form action="Informe-Bonos.php?QueryCon=<?php Echo($TxtConsultaVen) ?>" method="post" id="FormFechas" autocomplete="off">
					<div class="col-xs-12 col-sm-6">
...
$FechaInicioDia=($Anoactual."-01-01 00:00:000");
...
<div class="col-sm-12"><!-- Inicio Panel 12 -->
										...
										<div class="table-header" style="background-color: #000;">
											...
											<?php 
												...
												{
													Echo("Bonos del ".fechaSql($FechaUno)." al ".fechaSql($FechaDos));
												}
											?> 
										</div>
...
if ($TxtConsultaVen=='') {
	$sql ="SELECT DISTINCT(Id_bono) FROM t_bonos WHERE Fecha_Bono >='".$FechaStart."' and Fecha_Bono <='".$FechaEnd."' order by Id_bono  ASC";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
	$Lista=$Lista.$row['Id_bono'].",";                  
 }
}
}
else
{
	$sql ="SELECT DISTINCT(Id_bono) FROM t_bonos WHERE Fecha_Bono >='".$FechaStart."' and Fecha_Bono <='".$FechaEnd."' and  Tienda_Id_Tienda='".$TxtConsultaVen."' order by Id_bono  ASC";
	//Echo($sql);
	$result = $conexion->query($sql);
	if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
                                    ...
                                    <th class="success"></th>
                                     <th class="success"></th>
												...
												<thead>
													...
													<tr class="warning">
														<th class="tdcustom" >Número de Bono</th>
														<th class="tdcustom" >Estado</th>
														<th class="tdcustom" >Tienda</th>
														<th class="tdcustom" >Fecha</th>
														<th class="tdcustom" >Creado Por</th>
														<th class="tdcustom" >Vendido Por</th>
														<th class="tdcustom" >Valor Comercial</th>
														<th class="tdcustom" >Vendido A</th>
														<th class="tdcustom" >Documento Cl</th>
...
if ($TxtConsultaVen=='') {
$sql ="SELECT date_format(Fecha_Bono,CONCAT(CONCAT(ELT(WEEKDAY(Fecha_Bono) + 1, 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo')),', %d - ',CONCAT(ELT(MONTH(Fecha_Bono), 'Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Agos', 'Sep', 'Oct', 'Nov', 'Dic')),'- %Y')) AS FechaBono,Id_bono, Num_Bono,Estado_Bono, Valor_Comercial,Usuario_Vende,Cliente_Id_Cliente,Usuario_Crea,Tienda_Id_Tienda FROM t_bonos WHERE Id_bono='".$Cadena[$i]."'";
		//Echo($sql);
		$result = $conexion->query($sql);
}
	else
{
$sql ="SELECT date_format(Fecha_Bono,CONCAT(CONCAT(ELT(WEEKDAY(Fecha_Bono) + 1, 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo')),', %d - ',CONCAT(ELT(MONTH(Fecha_Bono), 'Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Agos', 'Sep', 'Oct', 'Nov', 'Dic')),'- %Y')) AS FechaBono,Id_bono, Num_Bono,Estado_Bono, Valor_Comercial,Usuario_Vende,Cliente_Id_Cliente,Usuario_Crea,Tienda_Id_Tienda FROM t_bonos WHERE Id_bono='".$Cadena[$i]."' and Tienda_Id_Tienda='".$TxtConsultaVen."'";
		//Echo($sql);
		$result = $conexion->query($sql);
}
...
?>
														...
														<td class="center">
															BONO-<strong><?php Echo($Num_Bono) ?></strong> 
														</td>
														<td class="center">
															<?php 
															if ($Estado_Bono=="Sin Vender") {
																echo("<span class='badge bg-red'>".$Estado_Bono."</span>");
															}
															elseif ($Estado_Bono=="Redimido") {
																echo("<span class='badge bg-green'>".$Estado_Bono."</span>");
															}
															elseif ($Estado_Bono=="Vendido") {
																echo("<span class='badge bg-orange'>".$Estado_Bono."</span>");
															}
															 ?>
														...
														<td>
															<?php Echo utf8_encode(ObtenernombreUsuario($Usuario_Crea)); ?>
														</td>
														<td>
															<?php Echo utf8_encode(ObtenernombreUsuario($Usuario_Vende)); ?>
														</td>
													
														<td class="center">
															<?php Echo utf8_encode("$ ".number_format($Valor_Comercial));  ?>
														</td>
														<td>
												...
												<ul class="dropdown-menu dropdown-info dropdown-menu-right">
													<?php 
												$Document=(ObtenerdocumentoCliente($Cliente_Id_Cliente));
													if ($Document!="") {
														?>
														<li>
														<a  data-rel="tooltip" data-placement="top" title="Bono Redimido" href="Informe-Bonos.php?NumBonoBaja=<?php echo utf8_encode($Id_bono); ?>"><i class="fa fa-refresh bigger-100 green"></i> Redimir Bono</a>
													</li>
													...
													<li>
														<a style="position: relative;" data-rel="tooltip" data-placement="top" title="Anular Factura"  href="Informe-Bonos.php?UpBono=<?php Echo($Id_bono) ?>&EditTask2=1"><i class="fa fa-edit red "></i> Editar Bono</a>
													</li>
														<?php
													}
													 ?>
