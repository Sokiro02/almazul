...
<html lang="en">
	<head>
		...
		<link rel="stylesheet" href="../assets/css/ace-rtl.min.css" />
		 ...
		 <!-- chartist CSS -->
    <link href="../assets/Ecommerce/css/pages/ecommerce.css" rel="stylesheet">
    <!-- Custom CSS -->
					...
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li>
								...
								<a href="Galeria-Produccion.php">Regresar a Galeria</a>
							</li>
						</ul><!-- /.breadcrumb -->
...
<?php 
 ...
 ?>
                                    <div class="col-lg-9 col-md-9 col-sm-6">
                                      <h3 class="m-t-40">Solicitud de Producción <br> Nº <?php Echo($ConsecutivoSolicitud); ?></h3>
    ...
    $PedidoMax=$CantidadMaxima;
        ...
        <form action="Produccion-guardarsolicitud.php" method="post" id="FormSolicitud">
            ...
            <input style="display: none;" type="tex" name="TxtRef" value="<?php Echo($RefSel);?>">
             <input style="display: none;" type="tex" name="TxtConsecutivoSolicitud" value="<?php Echo($ConsecutivoSolicitud); ?>">
                                  <select class="col-lg-3 col-xs-12" id="TxtBodega" name="TxtBodega">
...
if ($result->num_rows > 0) {
   ...
   <button style="margin:15px;" class="btn btn-primary btn-rounded btn-sm"><i class="fa fa-plus-square"></i> Agregar Solicitud </button>
    ...
    <?php 
    $sql ="SELECT date_format(Fecha_Solicitud,CONCAT(CONCAT(ELT(WEEKDAY(Fecha_Solicitud) + 1, 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo')),', %d - ',CONCAT(ELT(MONTH(Fecha_Solicitud), 'Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Agos', 'Sep', 'Oct', 'Nov', 'Dic')),'- %Y')) AS CreadaFecha,B.Nombres,B.Apellidos,B.Img_Perfil,Id_Solicitud_Prod, C.Nom_Bodega, Cant_Solicitada, D.Nom_Talla, E.Nom_Tienda, Referencia_Id_Referencia, Estado_Solicitud_Prod, Fecha_Solicitud,Cod_Solicitud_Prod FROM T_Solicitudes_Prod AS A, T_Usuarios as B, T_Bodegas as C, T_Tallas as D, T_Tiendas as E WHERE Referencia_Id_Referencia='".$RefSel."' AND A.Solicitud_Id_Usuario=B.Id_Usuario and A.Bodega_Id_Bodega=C.Id_Bodega and A.Talla_Solicitada=D.Id_Talla and A.Tienda_Id_Tienda=E.Id_Tienda and Estado_Solicitud_Prod='1' ORDER BY UNIX_TIMESTAMP(Fecha_Solicitud) DESC"; 
    //Echo($sql); 
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
                                ...
                                <div class="comment-text w-100">
                                    ...
                                    <p class="m-b-5 m-t-10">
                                      <ul>
                                        ...
                                        <li><b>Tienda: </b><?php Echo($Nom_Almacen) ?></li>
                                      </ul>
                                    </p>
                                </div>
                            </div>
    ...
    <?php 
    $sql ="SELECT date_format(Fecha_Solicitud,CONCAT(CONCAT(ELT(WEEKDAY(Fecha_Solicitud) + 1, 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo')),', %d - ',CONCAT(ELT(MONTH(Fecha_Solicitud), 'Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Agos', 'Sep', 'Oct', 'Nov', 'Dic')),'- %Y')) AS CreadaFecha,B.Nombres,B.Apellidos,B.Img_Perfil,Id_Solicitud_Prod, C.Nom_Bodega, Cant_Solicitada, D.Nom_Talla, E.Nom_Tienda, Referencia_Id_Referencia, Estado_Solicitud_Prod, Fecha_Solicitud,Cod_Solicitud_Prod FROM T_Solicitudes_Prod AS A, T_Usuarios as B, T_Bodegas as C, T_Tallas as D, T_Tiendas as E WHERE Referencia_Id_Referencia='".$RefSel."' AND A.Solicitud_Id_Usuario=B.Id_Usuario and A.Bodega_Id_Bodega=C.Id_Bodega and A.Talla_Solicitada=D.Id_Talla and A.Tienda_Id_Tienda=E.Id_Tienda and Estado_Solicitud_Prod='0' ORDER BY UNIX_TIMESTAMP(Fecha_Solicitud) DESC"; 
    //Echo($sql); 
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { 
