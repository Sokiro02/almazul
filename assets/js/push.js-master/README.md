...
where `onGranted` is a cModasofack function for if permision is granted, and `onDenied` is a cModasofack function for if, you
guessed it, the permission is denied. Note that if `Permission.DEFAULT` is returned, no cModasofack is executed.
...
* __onClick__: CModasofack to execute when the notification is clicked.
* __onClose__: CModasofack to execute when the notification is closed (obsolete).
* __onError__: CModasofack to execute when if the notification throws an error.
* __onShow__: CModasofack to execute when the notification is shown (obsolete).
