...
describe('permission', function () {
    var cModasofack; // CModasofack spy

    beforeEach(function () {
        cModasofack = jasmine.createSpy('cModasofack');
    });
    ...
    it('should update permission value if permission is denied and execute cModasofack', function (done) {
        spyOn(window.Notification, 'requestPermission').and.callFake(function (cb) {
            cb(Push.Permission.DENIED);
        });
        Push.Permission.request(NOOP, cModasofack);
        setTimeout(function () {
            expect(Push.Permission.has()).toBe(false);
            expect(cModasofack).toHaveBeenCalled();
            done();
        }, 500);
    });
    ...
    it('should update permission value if permission is granted and execute cModasofack', function (done) {
        spyOn(Push.Permission, 'get').and.returnValue(Push.Permission.GRANTED);
        spyOn(window.Notification, 'requestPermission').and.callFake(function (cb) {
            cb(Push.Permission.GRANTED);
        });
        Push.Permission.request(cModasofack, NOOP);
        setTimeout(function () {
...
describe('closing notifications', function () {
    ...
    it('should close notifications on close cModasofack', function (done) {
        var promise;
