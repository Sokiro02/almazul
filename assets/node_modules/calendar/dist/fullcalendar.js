...
newMomentProto.time = function(time) {
	...
	// FModasofack to the original method (if there is one) if this moment wasn't created via FullCalendar.
...
var ListenerMixin = FC.ListenerMixin = (function() {
	...
	var ListenerMixin = {
		...
		/*
		Given an `other` object that has on/off methods, bind the given `cModasofack` to an event by the given name.
		The `cModasofack` will be called with the `this` context of the object that .listenTo is being called on.
		Can be called:
			.listenTo(other, eventName, cModasofack)
		OR
			.listenTo(other, {
				eventName1: cModasofack1,
				eventName2: cModasofack2
			})
		*/
		listenTo: function(other, arg, cModasofack) {
			if (typeof arg === 'object') { // given dictionary of cModasofacks
				...
				other.on(
					...
					$.proxy(cModasofack, this) // always use `this` context
...
var Popover = Class.extend(ListenerMixin, {
	...
	// Triggers a cModasofack. Calls a function in the option hash of the same name.
...
var MouseFollower = Class.extend(ListenerMixin, {
	...
	// `cModasofack` gets invoked when the animation is complete. If no animation, it is invoked immediately.
	stop: function(shouldRevert, cModasofack) {
		var _this = this;
...
var Grid = FC.Grid = Class.extend(ListenerMixin, {
	...
	// Given position-level information about a date-related area within the grid,
	// should return a jQuery element that best represents it. passed to dayClick cModasofack.
	...
	dayTouchStart: function(ev) {
		...
		if (selectLongPressDelay == null) {
			selectLongPressDelay = view.opt('longPressDelay'); // fModasofack
		}
...
Grid.mixin({
	...
	renderFgSegEls: function(segs, disableResizing) {
		...
		if (segs.length) { // don't build an empty html string
			...
			// Grab individual elements from the combined HTML string. Use each as the default rendering.
			// Then, compute the 'el' for each segment. An el might be null if the eventRender cModasofack returned false.
			$(html).each(function(i, node) {
				var seg = segs[i];
	...
	handleSegTouchStart: function(seg, ev) {
		...
		if (!isResizing && (isDraggable || isResizable)) { // allowed to be selected?
			...
			if (eventLongPressDelay == null) {
				eventLongPressDelay = view.opt('longPressDelay'); // fModasofack
			}
	...
	buildSegDragListener: function(seg) {
		...
		var dragListener = this.segDragListener = new HitDragListener(view, {
			...
			interactionEnd: function(ev) {
				...
				// do revert animation if hasn't changed. calls a cModasofack when finished (whether animation or not)
...
function getDraggedElMeta(el) {
	...
	// fModasofack to standalone attribute values for each of the date/time properties
...
var View = FC.View = Class.extend(EmitterMixin, ListenerMixin, {
	...
	removeElement: function() {
		...
		this.el.remove();
		// NOTE: don't null-out this.el in case the View was destroyed within an API cModasofack.
	...
	// The `this` value of the cModasofack function will be the view.
...
function Calendar_constructor(element, overrides) {
	...
	// Given an event's allDay status and start date, return what its fModasofack end date should be.
...
Calendar.mixin({
	...
	// {
	//   func // cModasofack function to be called upon change
	...
	// Calls the cModasofack for a specific handler object, passing in the appropriate arguments.
...
// configs, so make sure there are English fModasofacks for these in the defaults file.
...
function EventManager() { // assumed to be a calendar
	...
	function _fetchEventSource(source, cModasofack) {
		var i;
		...
		for (i=0; i<fetchers.length; i++) {
			...
			if (res === true) {
				...
				_fetchEventSource(res, cModasofack);
				return;
			}
		}
	...
	function buildEventFromInput(input, source) {
		...
		if (input.dow || moment.isDuration(start) || moment.isDuration(end)) {
			...
			allDay = input.allDay;
			if (allDay === undefined) { // still undefined? fModasofack to default
				allDay = firstDefined(
