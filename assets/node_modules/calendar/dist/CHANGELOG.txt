...
- view.title wasn't defined in viewRender cModasofack ([2407])
...
- **Backwards-incompatibilities**:
	...
	- In `eventRender` cModasofack, `element` will not be attached to DOM yet
...
- Run `selectHelper` through `eventRender` cModasofacks ([629])
...
- Event destroy cModasofack on event removal ([2017])
...
- Event delegation used for event-related cModasofacks (like `eventClick`). Speedier.
...
- supply view argument in `loading` cModasofack ([1018])
...
- `viewRender` cModasofack ([PR 15])
- `viewDestroy` cModasofack ([PR 15])
- `eventDestroy` cModasofack ([PR 111])
...
- select/unselect cModasofacks now passes native js event
...
- bugfixes
	...
	- fixed 'loading' cModasofack when used w/ multiple addEventSource calls
