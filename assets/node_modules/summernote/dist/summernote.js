...
(function (factory) {
  ...
  if (!Array.prototype.reduce) {
     ...
     *
     * @param {Function} cModasofack
     * @param {Value} [initialValue]
     * @return {Value}
     *
     * @see http://goo.gl/WNriQD
     */
    Array.prototype.reduce = function (cModasofack) {
      ...
      for (; k < len; k++) {
        if (k in t) {
          value = cModasofack(value, t[k], k, t);
        }
      }
      return value;
    };
  ...
  if (!Array.prototype.map) {
     ...
     */
    Array.prototype.map = function (cModasofack, thisArg) {
      var T, A, k;
      if (this === null) {
      ...
      if (typeof cModasofack !== 'function') {
        throw new TypeError(cModasofack + ' is not a function');
      }
      ...
      while (k < len) {
        ...
        if (k in O) {
          kValue = O[k];
          mappedValue = cModasofack.call(T, kValue, k, O);
          A[k] = mappedValue;
        }
        k++;
      }
  ...
  var Editor = function (handler) {
    ...
    var triggerOnBeforeChange = function ($editable) {
      ...
      handler.bindCustomEvent(
        $holder, $editable.data('cModasofacks'), 'before.command'
    ...
    var triggerOnChange = function ($editable) {
      var $holder = dom.makeLayoutInfo($editable).holder();
      handler.bindCustomEvent(
        $holder, $editable.data('cModasofacks'), 'change'
      )($editable.html(), $editable);
    };
    ...
    this.insertImage = function ($editable, sUrl, filename) {
      async.createImage(sUrl, filename).then(function ($image) {
        ...
        var $holder = dom.makeLayoutInfo($editable).holder();
        handler.bindCustomEvent(
          $holder, $editable.data('cModasofacks'), 'image.upload.error'
        )();
      });
    };
    ...
    this.removeMedia = function ($editable, value, $target) {
      ...
      handler.bindCustomEvent(
        $(), $editable.data('cModasofacks'), 'media.delete'
      )($target, $editable);
  ...
  var EventHandler = function () {
    ...
    /**
     * @param {jQuery} $holder
     * @param {Object} cModasofacks
     * @param {String} eventNamespace
     * @returns {Function}
     */
    var bindCustomEvent = this.bindCustomEvent = function ($holder, cModasofacks, eventNamespace) {
      return function () {
        var cModasofack = cModasofacks[func.namespaceToCamel(eventNamespace, 'on')];
        if (cModasofack) {
          cModasofack.apply($holder[0], arguments);
        }
    ...
    this.insertImages = function (layoutInfo, files) {
      ...
      var cModasofacks = $editable.data('cModasofacks');
      var options = $editor.data('options');

      // If onImageUpload options setted
      if (cModasofacks.onImageUpload) {
        bindCustomEvent($holder, cModasofacks, 'image.upload')(files);
      // else insert Image as dataURL
      } else {
        $.each(files, function (idx, file) {
          var filename = file.name;
          if (options.maximumImageFileSize && options.maximumImageFileSize < file.size) {
            bindCustomEvent($holder, cModasofacks, 'image.upload.error')(options.langInfo.image.maximumFileSizeError);
          } else {
    ...
    this.attach = function (layoutInfo, options) {
      ...
      layoutInfo.editable().data('cModasofacks', {
    ...
    this.attachCustomEvent = function (layoutInfo, options) {
      ...
      $editable.focus(bindCustomEvent($holder, cModasofacks, 'focus'));
      $editable.blur(bindCustomEvent($holder, cModasofacks, 'blur'));

      $editable.keydown(function (event) {
        if (event.keyCode === key.code.ENTER) {
          bindCustomEvent($holder, cModasofacks, 'enter').call(this, event);
        }
        bindCustomEvent($holder, cModasofacks, 'keydown').call(this, event);
      });
      $editable.keyup(bindCustomEvent($holder, cModasofacks, 'keyup'));

      $editable.on('mousedown', bindCustomEvent($holder, cModasofacks, 'mousedown'));
      $editable.on('mouseup', bindCustomEvent($holder, cModasofacks, 'mouseup'));
      $editable.on('scroll', bindCustomEvent($holder, cModasofacks, 'scroll'));

      $editable.on('paste', bindCustomEvent($holder, cModasofacks, 'paste'));
      
      // [workaround] IE doesn't have input events for contentEditable
      //  - see: https://goo.gl/4bfIvA
      var changeEventName = agent.isMSIE ? 'DOMCharacterDataModified DOMSubtreeModified DOMNodeInserted' : 'input';
      $editable.on(changeEventName, function () {
        bindCustomEvent($holder, cModasofacks, 'change')($editable.html(), $editable);
      });

      if (!options.airMode) {
        layoutInfo.toolbar().click(bindCustomEvent($holder, cModasofacks, 'toolbar.click'));
        layoutInfo.popover().click(bindCustomEvent($holder, cModasofacks, 'popover.click'));
      }
      ...
      if (dom.isTextarea(list.head($holder))) {
        $holder.closest('form').submit(function (e) {
          ...
          bindCustomEvent($holder, cModasofacks, 'submit').call(this, e, $holder.code());
        });
      }
      ...
      // fire init event
      bindCustomEvent($holder, cModasofacks, 'init')(layoutInfo);
  ...
  $.extend($.summernote, {
     ...
     * event has name and cModasofack function.
     * 
     * ``` 
   ...
   *                // cModasofack function name when button clicked 
   *                event : 'hello',
