...
You can bind to any event defined below by either using an options cModasofack handler, or the standard jQuery .on method.

Example using options cModasofack handler:

```javascript
$('#tree').treeview({
  // The naming convention for cModasofack's is to prepend with `on`
