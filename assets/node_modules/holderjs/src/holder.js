...
(function(register, global, undefined) {
	...
	var Holder = {
		...
		run: function(userOptions) {
			...
			renderSettings.noFontFModasofack = options.noFontFModasofack ? options.noFontFModasofack : false;
				...
				if (imageHasSrc) {
					if (imageAttr.src.indexOf(options.domain) === 0) {
						...
						if (imageRendered) {
							...
							//If the placeholder has not been rendered, check if the image exists and render a fModasofack if it doesn't
	...
	/**
	 * Modifies the DOM to fit placeholders and sets up resizable image cModasofacks (for fluid and automatically sized placeholders)
	 *
	 * @private
	 * @param el Image DOM element
	...
	function prepareDOMElement(mode, el, flags, _renderSettings) {
		...
		if (flags.font) {
			...
			//Only run the <canvas> webfont fModasofack if noFontFModasofack is false, if the node is not an image, and if canvas is supported
			if (!renderSettings.noFontFModasofack && el.nodeName.toLowerCase() === 'img' && App.setup.supportsCanvas && renderSettings.renderer === 'svg') {
	...
	function buildSceneGraph(scene) {
      ...
      units: scene.theme.units ? scene.theme.units : App.defaults.units,
		...
		if (!tpdata) {
			throw 'Holder: staging fModasofack not supported yet.';
		}
	 ...
	 * @param cModasofack CModasofack function executed if the element is invisible
	 */
	function dimensionCheck(el, cModasofack) {
		var dimensions = {
	...
	var stagingRenderer = (function() {
		...
		return function(graph) {
			...
			if (App.setup.supportsSVG) {
				...
				//todo: canvas fModasofack for measuring text on android 2.3
				return false;
			}
		};
	})();
	...
	/**
	 * Holder-specific resize/orientation change cModasofack, debounced to prevent excessive execution
	 */
	 ...
	 * @param cModasofack CModasofack to call once image status has been found
	 */
	function imageExists(src, cModasofack) {
		var image = new Image();
		image.onerror = function() {
			cModasofack.call(this, false);
		};
		image.onload = function() {
			cModasofack.call(this, true);
		};
		image.src = src;
	}
