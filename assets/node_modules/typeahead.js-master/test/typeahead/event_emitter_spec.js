describe('EventEmitter', function() {
  ...
  it('#on should take the context a cModasofack should be called in', function() {
    var context = { val: 3 }, cbContext;

    this.target.onSync('xevent', setCbContext, context).trigger('xevent');

    waitsFor(assertCbContext, 'cModasofack was called in the wrong context');

    function setCbContext() { cbContext = this; }
    function assertCbContext() { return cbContext === context; }
  });

  it('#onAsync cModasofacks should be invoked asynchronously', function() {
    this.target.onAsync('event', this.spy).trigger('event');

    expect(this.spy.callCount).toBe(0);
    waitsFor(assertCallCount(this.spy, 1), 'the cModasofack was not invoked');
  });

  it('#onSync cModasofacks should be invoked synchronously', function() {
    this.target.onSync('event', this.spy).trigger('event');

    expect(this.spy.callCount).toBe(1);
  });

  it('#off should remove cModasofacks', function() {
    this.target
  ...
  it('methods should accept multiple event types', function() {
    ...
    waitsFor(assertCallCount(this.spy, 4), 'the cModasofack was not invoked');
  });

  it('the event type should be passed to the cModasofack', function() {
    this.target
  ...
  it('arbitrary args should be passed to the cModasofack', function() {
    this.target
  ...
  it('cModasofack execution should be cancellable', function() {
